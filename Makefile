...
TEST_PENDING_TRANSFER_BIN = $(BIN_DIR)/test-pending-transfer
TEST_RPC_PENDING_TRANSFER_BIN = $(BIN_DIR)/test-rpc-pending-transfer
...
TEST_ALL_BINS = \
	...
	$(TEST_SHARD_ROUTING_BIN) \
	$(TEST_RPC_SHARD_ROUTING_BIN) \
	$(TEST_DSF_K2_RECOVERY_BIN) \
	$(TEST_PENDING_TRANSFER_BIN) \
	$(TEST_RPC_PENDING_TRANSFER_BIN)

.PHONY: all clean test check

all: $(NODE_BIN) $(TEST_ALL_BINS)
...
$(TEST_PENDING_TRANSFER_BIN): $(ALL_CORE_OBJS) $(BUILD_DIR)/tests/test_pending_transfer.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) $^ -o $@ $(LDLIBS)

$(TEST_RPC_PENDING_TRANSFER_BIN): $(ALL_CORE_OBJS) $(BUILD_DIR)/tests/test_rpc_pending_transfer.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) $^ -o $@ $(LDLIBS)
...
test: $(TEST_ALL_BINS)
	@for t in $(TEST_ALL_BINS); do echo "=== Running $$t ==="; ./$$t || exit 1; done
