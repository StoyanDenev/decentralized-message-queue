...
#
# Asserts:
#   1. unchained init --profile tactical_test produces a valid config.
...
for n in 1 2 3; do
  $UNCHAINED init --data-dir $T/n$n --profile tactical_test 2>&1 | tail -1
done
...
EOF
$UNCHAINED genesis-tool build-sharded $T/gen.json
...
for n in 1 2 3; do
  port=$((7777 + (n-1)*10))
  rpc=$((7778 + (n-1)*10))
  $UNCHAINED start --data-dir $T/n$n --listen-port $port --rpc-port $rpc \
