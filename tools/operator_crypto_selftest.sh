...
# the determ binary — all 33 C99-family subcommands listed in `determ
...
Phase 2 (live, skipped with --static-only):
  ...
  directly, one subcommand at a time (the 33 C99-family subcommands in
  `determ help` order):
    ...
    test-bp-rangeproof-c99 test-bp-agg-rangeproof-c99 test-p256-balance-c99
    test-p256-confidential-tx-c99 test-ct-c99 test-rng-c99
...
# test-p256-balance-c99 / test-p256-confidential-tx-c99 the §3.19 inc.7-8
# P-256 confidential-tx balance + end-to-end composition).
