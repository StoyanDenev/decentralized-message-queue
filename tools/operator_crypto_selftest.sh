...
# the determ binary — all 32 C99-family subcommands listed in `determ
...
Phase 2 (live, skipped with --static-only):
  ...
  directly, one subcommand at a time (the 32 C99-family subcommands in
  `determ help` order):
    ...
    test-ff-pedersen-c99  test-ff-scalar-c99  test-ff-ipa-c99
...
# All 32 in-process C99-family subcommands, in `determ help` order
