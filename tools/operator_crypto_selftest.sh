...
# the determ binary — all 24 C99-family subcommands listed in `determ
...
Phase 2 (live, skipped with --static-only):
  ...
  `determ help` order):
    ...
    test-bp-rangeproof-c99 test-bp-agg-rangeproof-c99 test-ct-c99
...
# All 24 in-process C99-family subcommands, in `determ help` order
