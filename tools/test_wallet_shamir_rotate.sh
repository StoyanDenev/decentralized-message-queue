...
case "$UNAME_S" in
    ...
    *)
        ...
        echo "  SKIP: 0600 check (uname=$UNAME_S; POSIX-only assertion)"; skip_count=$((skip_count + 1))
        ;;
esac
...
if [ "$pass_count" -eq 0 ]; then
    echo "  FAIL: test_wallet_shamir_rotate — every section declined; nothing was asserted"; exit 1
