...
for _ in $(seq 1 50); do
  HEIGHT_PRE=$($UNCHAINED status --rpc-port 8771 2>/dev/null \
...
STAKE_PRE=$($UNCHAINED stake_info node1 --rpc-port 8771 2>/dev/null \
...
for attempt in $(seq 1 120); do
  sleep 0.5
  STAKE_POST=$($UNCHAINED stake_info node1 --rpc-port 8771 2>/dev/null \
                | python -c "import sys,json; print(json.load(sys.stdin).get('locked','-'))")
  HEIGHT_POST=$($UNCHAINED status --rpc-port 8771 2>/dev/null \
                 | python -c "import sys,json; print(json.load(sys.stdin)['height'])")
...
for ((i = HEIGHT_PRE; i <= HEIGHT_POST; i++)); do
  HAS_EV=$($UNCHAINED show-block $i --rpc-port 8771 2>/dev/null \
            | python -c "import sys,json
