...
for chain in beacon shard; do
  for n in 1 2; do
    $UNCHAINED init --data-dir $T/$chain/n$n --profile regional_test 2>&1 | tail -1
  done
done

echo
echo "=== 2. Generate peer-info entries (one creator pair per chain) ==="
$UNCHAINED genesis-tool peer-info node1 --data-dir $T/beacon/n1 --stake 1000 > $T/beacon_p1.json
$UNCHAINED genesis-tool peer-info node2 --data-dir $T/beacon/n2 --stake 1000 > $T/beacon_p2.json
$UNCHAINED genesis-tool peer-info node1 --data-dir $T/shard/n1 --stake 1000  > $T/shard_p1.json
$UNCHAINED genesis-tool peer-info node2 --data-dir $T/shard/n2 --stake 1000  > $T/shard_p2.json
...
EOF
$UNCHAINED genesis-tool build-sharded $T/beacon_gen.json
...
EOF
$UNCHAINED genesis-tool build-sharded $T/shard_gen.json
...
NODE_PIDS=("" "" "" "")
$UNCHAINED start --config $T/beacon/n1/config.json > $T/beacon/n1/log 2>&1 &
NODE_PIDS[0]=$!; sleep 0.3
$UNCHAINED start --config $T/beacon/n2/config.json > $T/beacon/n2/log 2>&1 &
NODE_PIDS[1]=$!; sleep 0.3
$UNCHAINED start --config $T/shard/n1/config.json > $T/shard/n1/log 2>&1 &
NODE_PIDS[2]=$!; sleep 0.3
$UNCHAINED start --config $T/shard/n2/config.json > $T/shard/n2/log 2>&1 &
