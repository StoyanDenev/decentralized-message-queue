...
# (gitignored; must match a sha256 in PINS below; fetch the immutable release:
#   curl -fsSL -o tools/tlc/tla2tools.jar \
#     https://github.com/tlaplus/tlaplus/releases/download/v1.7.4/tla2tools.jar ).
# Fails closed when java or the jar is absent: a model check that did not run
# is not a pass (THE SKIP CONVENTION, tools/run_all.sh). CI: tools/ci_local.sh --tla.
#
...
case "$JAR" in /*|[A-Za-z]:*) ;; *) JAR="$(pwd -W 2>/dev/null || pwd)/$JAR" ;; esac
JAVA="${DETERM_JAVA:-java}"
# Accepted jars, "version sha256". v1.7.4 is the latest immutable release (what
# CI fetches). v1.8.0 is TLA+'s rolling pre-release tag — its download changes —
# and this is the build the committed results were first generated with.
PINS="v1.7.4 936a262061c914694dfd669a543be24573c45d5aa0ff20a8b96b23d01e050e88
v1.8.0 237332bdcc79a35c7d26efa7b82c77c85c2744591c5598673a8a45085ff2a4fb"
...
if ! "$JAVA" -version >/dev/null 2>&1; then
  echo "  FAIL: test_tla_model_check (java not found — install a JRE 11+ or set DETERM_JAVA)"
  exit 1
fi
if [ ! -f "$JAR" ]; then
  echo "  FAIL: test_tla_model_check ($JAR absent — fetch it, see header)"
  exit 1
fi
JAR_SHA256=$( (sha256sum "$JAR" 2>/dev/null || shasum -a 256 "$JAR") | awk '{print $1}')
JAR_VERSION=$(printf '%s\n' "$PINS" | awk -v s="$JAR_SHA256" '$2 == s {print $1}')
if [ -z "$JAR_VERSION" ]; then
  echo "  FAIL: test_tla_model_check ($JAR sha256=$JAR_SHA256 matches no pin — refuse to run an unpinned model checker)"
  exit 1
fi
...
quarantine_reason() {
  local base="$1" q
  for q in ${QUARANTINE[@]+"${QUARANTINE[@]}"}; do   # bash < 4.4: empty array + set -u
...
"
  ...
  out=$(cd "$TLADIR" && timeout "$TIMEOUT_S" "$JAVA" -XX:+UseParallelGC \
        -cp "$JAR" tlc2.TLC -deadlock -config "$base.cfg" "$base.tla" 2>&1)
  local rc=$?
  t1=$(date +%s); dur=$((t1 - t0))
...
echo "=== TLC model check (tla2tools $JAR_VERSION, timeout ${TIMEOUT_S}s/config) ==="
if [ -n "$ONLY" ]; then
  run_one "$ONLY"
else
...
if [ "$WRITE" = "1" ] && [ -z "$ONLY" ]; then
  {
    ...
    echo
    echo "Generated by \`bash tools/test_tla_model_check.sh --write\` with tla2tools **$JAR_VERSION**"
    echo "(sha256 \`$JAR_SHA256\`), java: \`$("$JAVA" -version 2>&1 | grep -v '^Picked up' | head -1)\`,"
