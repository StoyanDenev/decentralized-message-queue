...
# each test's output for a "PASS:", "FAIL:" or "PLATFORM-SKIP:" line; a
# wrapper that exits non-zero without a FAIL: marker is also a failure.
...
#                         (no multi-node clusters). The set is the
#                         ONLY_PATTERN below — its single source; a stem
#                         with no tools/test_<stem>.sh fails the run.
...
for t in tools/test_*.sh; do
    ...
    if [ "${QUIET:-0}" = "1" ]; then
        ...
        OUT=$(bash "$t" 2>&1 | tee /dev/stderr; exit "${PIPESTATUS[0]}"); RC=$?
    fi
    ...
    if echo "$LAST" | grep -qE "^\s*FAIL:"; then
        ...
        # A wrapper that exits non-zero failed, whatever its tail says: under
        # `set -e` a wrapper aborts right after a run of PASS: lines, and the
        # marker scan alone scored that green.
        echo "  (exited $RC — counted as failure)"
        FAIL_COUNT=$((FAIL_COUNT + 1))
        FAILED_TESTS+=("$t (exit $RC)")
        if [ "${QUIET:-0}" = "1" ]; then
            echo "  ── output of the wrapper that exited non-zero (QUIET=1 suppressed the live tee) ──"
            echo "$OUT"
        fi
    elif echo "$LAST" | grep -qE "^[[:space:]]*PLATFORM-SKIP:[[:space:]]*[^[:space:]]" \
         && grep -qE '^[^#]*echo[^#]*PLATFORM-SKIP:' "$t"; then
        ...
        #
        # The cause is REQUIRED: a bare `PLATFORM-SKIP:` names nothing, matches
        # nothing here, and lands in the markerless branch below as a failure — a
        # skip nobody can audit is not a skip.
        #
        # AND the wrapper's OWN SOURCE must emit the marker. This closes a
        # fail-OPEN hole in the first shape of this branch: the outcome is scored
        # from CAPTURED OUTPUT, and text cannot say who printed it, so a marker
        # written by a CHILD process was read as the wrapper's own verdict.
        # Measured against the real runner: a wrapper that asserted nothing and
        # merely ran a child which printed the marker was scored PLATFORM-SKIP
        # with the suite exiting 0, where before this outcome existed it was a
        # hard failure. Requiring the stem in the cause does NOT fix it — a relay
        # names the stem just as easily. The source check does, and it composes
        # with test_gates_can_fail.sh's D5 ratchet, which pins WHICH wrappers may
        # contain that echo at all: the runner here refuses to score a wrapper
        # that does not emit it, and D5 turns RED when a third one starts to.
