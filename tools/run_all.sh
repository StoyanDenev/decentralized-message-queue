...
#                         make_contrib_commitment_distinct. Each runs in <5s with no
...
if [ "${FAST:-0}" = "1" ]; then
    ONLY_PATTERN='test_(atomic_scope|composable_batch|dapp_register|dapp_call|s018_json_validation|merkle|committee_selection|shard_routing|ed25519|sha256|anon_address|genesis_message|state_root_unit|block_rand|rate_limiter|block_digest|block_hash|binary_codec|wire_types|transaction|merge_event_codec|merge_event_bytes|consensus_msgs|tx_root|genesis|envelope|resolve_fork|shamir|random_state|snapshot_defense|encoding|chain_helpers|json_validate|block_roundtrip|config_roundtrip|tx_binary_codec|chain_append|state_types|validator_config|timing_profiles|params_constants|supply_invariant|enum_values|block_accessors|make_block_sig|domain_separation|tx_signing_bytes|make_genesis_block|pending_param_changes|merge_state|chain_apply_block|snapshot_roundtrip|state_proof_unit|abort_event_apply|equivocation_apply|unstake_deregister_apply|cross_shard_receipt_apply|param_change_apply|subsidy_distribution|merge_event_apply|cross_shard_outbound_apply|supply_lifecycle|dapp_state_transition|overflow_paths|state_root_namespaces|multi_tx_block|state_proof_namespaces|applied_receipt_restore|stake_accounting|fee_distribution_edge|equivocation_multi|cross_shard_multi_receipt|multi_block_chain|tx_edge_cases|snapshot_then_apply|genesis_with_region|anon_routing|merge_event_apply_edge|block_event_composition|nef_pool_drain|tx_payload_bounds|empty_block_apply|account_create_on_credit|randomized_delay|block_timestamp|node_registry|tx_replay_protection|chain_save_load|block_validator_basic|genesis_sharded|cross_shard_atomicity|chain_ctor_bootstrap|snapshot_version_rejection|config_defaults|required_block_sigs|config_load_save|block_from_json_minimal|config_permissive|chain_shard_routing_config|view_root|frost_types|make_contrib_commitment_distinct|rate_limiter_bucket|merkle_proof_tampering|protocol_version_pinning|binary_codec_roundtrip_exhaustive|time_monotonicity|chain_prev_hash_link|block_validator_extensive|state_root_determinism|tx_signing_determinism|merge_event_determinism|block_rand_distribution|config_determinism|hello_handshake_determinism|genesis_determinism|shard_routing_determinism)\.sh$'
