...
#                         consensus_msgs, tx_root, genesis. Each runs
...
if [ "${FAST:-0}" = "1" ]; then
    ONLY_PATTERN='test_(atomic_scope|composable_batch|dapp_register|dapp_call|s018_json_validation|merkle|committee_selection|shard_routing|ed25519|sha256|anon_address|genesis_message|state_root_unit|block_rand|rate_limiter|block_digest|block_hash|binary_codec|wire_types|transaction|merge_event_codec|consensus_msgs|tx_root|genesis)\.sh$'
