...
MUTANTS = [
    ...
    ('crypto-hmac-heap-free',
     ...
     '    uint8_t pad[64];\n    size_t i;\n    { extern void *malloc(size_t); extern void free(void *); free(malloc(1)); }\n\n    memset(k0, 0, sizeof k0);'),
    ('crypto-hmac512-key-block-edge',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/hmac.c',
     'if (keylen > sizeof k0) determ_sha512(key, keylen, k0);',
     'if (keylen >= sizeof k0) determ_sha512(key, keylen, k0);'),
    ('crypto-hmac512-heap-free',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/hmac.c',
     '    uint8_t inner[64];\n    determ_sha512_ctx ctx;\n    size_t i;\n',
     '    uint8_t inner[64];\n    determ_sha512_ctx ctx;\n    size_t i;\n    { extern void *malloc(size_t); extern void free(void *); free(malloc(1)); }\n'),
    ('crypto-sha512-heap-free',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
     '    memcpy(ctx->h, H0, sizeof H0);',
     '    { extern void *malloc(size_t); extern void free(void *); free(malloc(1)); }\n    memcpy(ctx->h, H0, sizeof H0);'),
    ('crypto-sha512-init-total',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
     '    ctx->total  = 0u;\n    ctx->buflen = 0u;\n}\n\nvoid determ_sha512_update',
     '    ctx->buflen = 0u;\n}\n\nvoid determ_sha512_update'),
    ('crypto-sha512-init-buflen',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
     '    ctx->total  = 0u;\n    ctx->buflen = 0u;\n}\n\nvoid determ_sha512_update',
     '    ctx->total  = 0u;\n}\n\nvoid determ_sha512_update'),
    ('crypto-sha512-partial-block',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
     'if (ctx->buflen == 128u) {',
     'if (ctx->buflen == 127u) {'),
    ('crypto-sha512-pad-boundary',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
     'size_t padlen = (rem < 112u) ? 128u : 256u;',
     'size_t padlen = (rem <= 112u) ? 128u : 256u;'),
    ('crypto-sha512-final-no-wipe',
     'test-c99-crypto-bounds',
     'src/crypto/sha2/sha512.c',
