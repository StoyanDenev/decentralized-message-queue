...
- `rp/` — relying-party / citizen verification SDK.
  - `rp/python/determ_rp/d5.py` — **SHIPPED** (extracted from the working D.5 build, SPEC §12 item 8):
    ...
    d5codec decode + lowest-hash `d5_draw` re-derivation) — never a false SELECTED. Dependency-free
    (stdlib only). Gated + falsified via the reference `tools/verify_d5rp.py` (which dogfoods it) +
    `tools/test_d5rp.sh`.
