...
- *K-of-K:* no separate finality layer. Fork freedom rests on committee intersection
  ...
  floor ([BoundedReorgDesign](../proofs/BoundedReorgDesign.md)), as a local acceptance
  rule, not a network-finality theorem (§8.2).
...
- *Open because:* no mechanism exists; it is a later simulation increment, after the
  single-shard gates of §8.4 (ADR-005 §3.5).
...
| Recovery from silence | Use K=2's selected shard-wide witnesses (every eligible member, the pair included; H1, H3), parent-bound failure certificates and local 3B deadlines. | Prove receiver agreement on attempt authority and progress; importing K-of-K's producer-only abort quorum would halt with one silent member at K=2 (two at larger K; H3, H4). |
...
and checkpoint settlement. "External" means not producer-only: every member of the
shard's eligible population, the pair included, may sign a failure statement, and a
certificate needs two thirds of the shard's stake (H1, H3). The joint-receipt rule does not acquire the union's
