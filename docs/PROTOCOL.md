...
**v2 items NOT yet covered (not yet shipped):** v2.7 F2 view reconciliation (full S-030 D2 closure; `docs/proofs/F2-SPEC.md` is the implementation spec), v2.8 post-quantum signature migration, v2.10 threshold randomness aggregation (`plan.md` A11 — **block-beacon application de-scoped; the v1 MPDH commit-reveal beacon is retained**, see `docs/proofs/V210-PhaseD-RandomnessWiring.md` §9; FROST DKG infra retained for other uses), v2.14 real OPAQUE wallet recovery (gates on Windows MSVC porting of upstream VLAs), v2.22-v2.24 confidential transactions + cross-chain bridge + audit hooks, v2.25-v2.26 distributed identity provider + on-chain key rotation. See `docs/V2-DESIGN.md` for the full design space.
