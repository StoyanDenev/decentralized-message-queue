...
| `submit_tx` → `{"status":"queued"}` (or "incumbent tx at (from, nonce) has equal-or-higher fee") | A daemon held the bytes in its mempool at that instant, and at that instant the sender could fund the transaction together with its other pending ones out of its balance at that daemon's head (S-079 affordability; an unfundable one is rejected with "mempool: unaffordable (S-079)", never queued). | Inclusion. The mempool has no TTL but drops txs on fee eviction, on build-time verifier rejection and — when a later head leaves the sender unable to fund it — on build-time affordability re-check or first at a cap eviction, silently (§10.2; `Node::tx_admit_locked`, `mempool_make_room_for`). `submit_tx` does not run the verifier's per-tx rules. |
...
`be8` = 8-byte big-endian, `be4` = 4-byte big-endian; SHA-256 builder appends multi-byte integers in big-endian (`crypto::SHA256Builder::append(uint64_t)` etc.). The Merkle tree itself is a balanced binary tree with SHA-256 inner nodes (`merkle_root` helper in `src/crypto/merkle.cpp`); leaf and inner hashes are domain-separated by a prefix byte — `0x00` for leaves, `0x01` for inner nodes. Empty slices contribute no leaves; if the entire leaf vector is empty the root is the empty-tree sentinel `Hash{}`.

**Leaf-count binding (S-040).** The committed root is not the bare inner root but a root-wrapper hash that binds the leaf count: `state_root = SHA256(0x02 ‖ be4(leaf_count) ‖ inner_root)`, where `inner_root` is the balanced-tree root over the sorted leaves and `0x02` is a third domain-separation prefix distinct from `0x00` (leaf) / `0x01` (inner). `merkle_verify` re-derives `inner_root` from the inclusion proof, then re-applies the wrapper using the **caller-supplied** `leaf_count` and compares to the committed root; a proof presenting a forged `leaf_count M ≠ N` yields `SHA256(0x02 ‖ M ‖ inner_root) ≠ SHA256(0x02 ‖ N ‖ inner_root) = state_root` and is rejected. This makes the genuine leaf count cryptographically binding rather than a caller-trust obligation.
...
Slot reserved for future use. v1.x validators reject any tx with this type with a "REGION_CHANGE tx type is reserved for future use" error. A future v2.X may use the slot for in-place validator region updates (since V-REG-1, 2026-09-15, a registered validator's region cannot be changed at all: REGISTER is create-only and DEREGISTER is terminal under the same name).
