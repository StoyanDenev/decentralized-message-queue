...
- For `UNSTAKE` (S-017 closure): validator + producer + chain layer all enforce `b.index >= chain.stake_unlock_height(tx.from)`. Pre-fix the chain layer was the only gate (with fee refund on too-early); post-fix the validator rejects too-early UNSTAKE at the block-validation layer, the producer skips it during `build_body` assembly, and the chain layer keeps the fee-refund as belt-and-suspenders. Successful UNSTAKE: stake `locked` drained to sender's balance; entry erased; nonce consumed.
