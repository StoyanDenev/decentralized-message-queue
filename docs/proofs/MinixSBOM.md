...
| **determ-crypto-c99** | Determ Contributors | in-tree (this repo) | `include/determ/crypto/**`, `src/crypto/**` | tracked in git; per-primitive dual-oracle + KAT (`docs/proofs/`, `tools/vectors/`) | Apache-2.0 (multi-licensed repo since v3.1 — the core/crypto tier is Apache-2.0, canonical text `LICENSES/Apache-2.0.txt` referenced by the root `LICENSE` per-tier map + `NOTICE`, with the per-component marker `src/crypto/LICENSE` binding the crypto core to that text per the `LICENSING.md` component map; per-file `SPDX-License-Identifier: Apache-2.0` on the `src/crypto` C++ wrapper `.cpp` + the newer C modules — the from-scratch C99 `.c` core is covered by the Apache core tier) | ALL production crypto (hash, Ed25519/X25519/P-256, AEAD, KDF, ML-DSA, entropy) | **IN** (static lib linked into `determ`, `determ-wallet`, `determ-light`, `determ-cryptotest`) | The C99 / from-scratch goal — **done** for the crypto TCB. This is the minix ideal (auditable, in-tree). |
...
oracle — so no shipped binary links OpenSSL, the §3 OUT-of-TCB claim); the
core/crypto tier is Apache-2.0 — the root `LICENSE` per-tier map declares the
Apache-2.0 core tier and its canonical text `LICENSES/Apache-2.0.txt` is present
(plus `NOTICE`) — and every `src/crypto` C++ wrapper `.cpp`
