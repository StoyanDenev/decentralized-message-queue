...
| 10 | `c:` | 6 A1 supply counters — five always-on (`genesis_total_`, `accumulated_subsidy_`, `accumulated_slashed_`, `accumulated_inbound_`, `accumulated_outbound_`) + `accumulated_shielded_` (§3.22, **conditional**: leaf emitted only when non-zero) | one leaf per always-on counter via `const_leaf("c:…")` (494-498); `c:accumulated_shielded` conditionally (502-503) | individual scalar fields (2212-2216); `accumulated_shielded` when non-zero (2219) | individual `snap.value("name", default)` reads (2439-2442; `accumulated_shielded` at 2443, defaulting to 0) + back-solve at 2628-2637 |
...
Σ_{d ∈ accounts_} balance[d]  +  Σ_{d ∈ stakes_} locked[d]
   == genesis_total_ + accumulated_subsidy_ + accumulated_inbound_
                     − accumulated_slashed_ − accumulated_outbound_ − accumulated_shielded_,
```

holds with **identical** left- and right-hand values on `C` and on `C₁ = R(serialize(C))`. Equivalently, the supply-bearing namespaces `a:` (account balances), `s:` (stake locked), and `c:` (the six counters) round-trip exactly, so the unitary-supply check (`Chain::live_total_supply() == Chain::expected_total()`, asserted at `chain.cpp:1866-1892`) yields the same verdict pre- and post-restore. (The sixth counter `accumulated_shielded_` is the v2.20 §3.22 confidential-pool term, `chain.hpp:898`; its `c:accumulated_shielded` state-root leaf and its snapshot JSON field are emitted only when non-zero, so a shield-free chain round-trips byte-identically to a pre-§3.22 one.)

**Proof.** The identity's left-hand side, `live_total_supply(X) = Σ balance + Σ locked`, is a fold over the `a:` namespace (`accounts_[d].balance`) and the `s:` namespace (`stakes_[d].locked`). The right-hand side, `expected_total(X)`, is the signed sum of the six `c:` counters. By SD-1's intermediate `≡_S` result (conditions 1–4), all three namespaces coincide between `C` and `C₁`:
...
- `c:` — the six counters coincide (T-S1 condition 4): the five always-on counters `genesis_total_`, `accumulated_subsidy_`, `accumulated_inbound_`, `accumulated_slashed_`, `accumulated_outbound_` are each persisted by `serialize_state` (`chain.cpp:2212-2216`) and re-read by `restore_from_snapshot` (`chain.cpp:2439-2442`), and the sixth, `accumulated_shielded_` (§3.22), is persisted conditionally when non-zero (`chain.cpp:2219`) and re-read defaulting to `0` when absent (`chain.cpp:2443`). [Edge case — legacy snapshots without `genesis_total`: the fieldless branch of `restore_from_snapshot` back-solves `genesis_total_ = live + deltas_neg − deltas_pos` (`chain.cpp:2628-2637`) so the identity is satisfied at restore by construction. The back-solve must be the EXACT inverse of `expected_total()`; §3.22 makes that a *six*-term sum (`chain.hpp:590-597`, adding the negative `− accumulated_shielded_`), so `deltas_neg = slashed + outbound + accumulated_shielded_` (the shielded counter is restored at `chain.cpp:2443`, before the back-solve). Omitting the shielded term would under-compute `genesis_total_` by exactly `accumulated_shielded_`, giving `expected_total = live − accumulated_shielded_ ≠ live` — a fail-closed reject of a *valid* snapshot (`require_supply_invariant` at `chain.cpp:2693`, else the first apply at `chain.cpp:1868`). Latent, not live: `serialize_state` writes `genesis_total` unconditionally (`chain.cpp:2212`) but `accumulated_shielded` only when non-zero (`chain.cpp:2219`), so an honest fieldless snapshot predates §3.22 ⇒ `accumulated_shielded_ == 0` ⇒ `+0`. For a *post-S-038* snapshot the field is always present, so the explicit value is used and the round trip is exact. Either way the RHS equals the LHS post-restore.]

Therefore both sides of the identity take the same numeric value on `C` and `C₁`, and in particular the equality holds on `C₁` iff it held on `C`. Since `C` is reachable, the identity held on `C` (it is an apply-path invariant, `AccountStateInvariants.md` T-A6), so it holds on `C₁`. The first post-restore block's apply-tail A1 assertion (`chain.cpp:1866-1892`) thus starts from a satisfied invariant — which is exactly why the A1 counters are persisted (the `serialize_state` comment at lines 2208-2211 states this: without them `expected_total()` would be 0 on a restored chain and the first apply would trip the invariant). ∎
...
**Code witness.** `chain.cpp:699` (`live_total_supply`), `expected_total` (the six-counter sum, `chain.hpp:590-597`), `chain.cpp:1866-1892` (the A1 assertion), `chain.cpp:2212-2219` (counter persistence, incl. the conditional `accumulated_shielded`), `chain.cpp:2439-2443` + the fieldless back-solve at `chain.cpp:2628-2637` (counter restore + legacy back-solve, the 6-term inverse incl. `accumulated_shielded_`). **Test witness (back-solve inverse).** `determ test-snapshot-genesis-backsolve` (`tools/test_snapshot_genesis_backsolve.sh`) hand-builds a fieldless snapshot with `accumulated_shielded_ > 0` and asserts the back-solved `genesis_total_` is A1-consistent (`expected_total == live`); falsify-on-mutant — deleting the `+ c.accumulated_shielded_` term fail-closed rejects that valid snapshot, while the shield-free control stays green.
...
**"Ten namespaces" is the originally-tabulated set, not the current total.** Since this proof was written, `build_state_leaves` has grown additional state-root namespaces beyond the ten enumerated in §2.1 — `t:` (shard_tip_records, D3.2), `cc:` (committee_checkpoints, D3.3a), `cn:` (shielded_pool, §3.22), `en:` (enote_commitments, NC-8), `ak:` (audit_keys, A2), `al:` (audit_log_count, A2), `nk:` (note_keys, NC-8 §5a) — for a current total of **17** distinct namespace prefixes (see `SnapshotEquivalence.md` §2, which now enumerates all of them with their `serialize_state`/`restore_from_snapshot` line pairs). Each is a conditionally-emitted (empty-when-unused) container that round-trips by the *identical* L-S0/L-S1 coverage argument this document composes, so **SD-1..SD-5 and SD-3's no-silent-drop guard extend to all 17 structurally**; the "ten" framing in §1–§4 and the §2.1 table is the set that existed when this proof was authored, not a bound on what `compute_state_root` covers. F1's `test-snapshot-full-determinism` empirically pins whatever subset its in-process fixture populates; the analytic statement is over the full shipped set.
