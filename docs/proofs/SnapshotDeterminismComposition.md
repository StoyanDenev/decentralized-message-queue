...
The proof is a composition, not a fresh mechanical argument: it threads `SnapshotEquivalence.md` (FA-Apply-2, the core serialize-restore identity), `AppliedReceiptRestore.md` (FA-Apply-12, the `i:` namespace), `MerkleTreeSoundness.md` MT-1/MT-2/MT-3 (the tree-over-a-leaf-set determinism + collision-resistance properties), `S033StateRootNamespaceCoverage.md` (the 10-namespace coverage completeness), and the paired S-037 / S-038 closures (the `d:` serialize-gap fix + the producer-side `body.state_root` wiring) into one all-namespace round-trip statement. The empirical pin is sibling F1's in-process unit `determ test-snapshot-full-determinism` (R40), which exercises every SD theorem on a fixture chain that populates all populatable namespaces.
...
2. **The `k:` scalar values include `merge_threshold_blocks_`, `revert_threshold_blocks_`, `merge_grace_blocks_`** as `state_root` leaves (lines 392-394) but `serialize_state` does **not** emit them as separate JSON fields. This is a *latent S-037-class gap for the `k:` merge-threshold sub-fields*: on restore these three default to the `GenesisConfig` defaults (`100 / 200 / 10`, `chain.hpp:592-594`). For a chain whose merge thresholds equal the defaults the round trip is exact; for a chain whose thresholds were set to non-default values, the restored `k:` leaves would differ and SD-2 would fail under the restore-side gate. This document **states the theorem over the populated set + structurally over all ten namespaces** (§5) and flags the merge-threshold sub-field gap as a limitation; F1's test exercises the namespaces it can populate in-process (see §4). The structural SD-3 result is what makes this gap *detectable* rather than silent — exactly the regression-guard property the brief asks for.
...
**Namespace population in F1's in-process test.** F1's test populates the namespaces reachable from an in-process fixture chain. If F1's report indicates a namespace it could not populate in-process (e.g. `m:` merge-state requires a live MERGE_EVENT apply, or `p:` pending-param-changes requires a staged PARAM_CHANGE), the SD theorems are still stated **structurally over all ten namespaces** via SD-3's sensitivity result + the per-namespace coverage of FA-Apply-2 (L-S0/L-S1) and FA-Apply-12 (the `i:` specialization). The empirical pin is over the populated set; the analytic statement is over the full set. The `k:` merge-threshold sub-fields (`merge_threshold_blocks_`, `revert_threshold_blocks_`, `merge_grace_blocks_`) are a known latent S-037-class gap (§2.1 fact 2): they contribute `k:`-leaves to `compute_state_root` but are not emitted as JSON fields, so a chain with non-default merge thresholds would fail SD-2 on restore. SD-3 is what makes this gap *detectable* — a test that sets non-default thresholds and exercises the round trip would catch it. It is recorded here as a limitation, not silently passed over.
...
| `docs/SECURITY.md` §S-012 / §S-033 / §S-037 / §S-038 | Snapshot bootstrap gate / Merkle state commitment / `d:` serialize-gap closure / producer-side `body.state_root` wiring. The S-037/S-038 narrative is the canonical SD-3 instance. |
