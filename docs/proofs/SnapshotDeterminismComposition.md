...
- `c:` — the six counters coincide (T-S1 condition 4): the five always-on counters `genesis_total_`, `accumulated_subsidy_`, `accumulated_inbound_`, `accumulated_slashed_`, `accumulated_outbound_` are each persisted by `serialize_state` (lines 2212-2216) and re-read by `restore_from_snapshot` (lines 2439-2442), and the sixth, `accumulated_shielded_` (§3.22), is persisted conditionally when non-zero (line 2219) and re-read defaulting to `0` when absent (line 2443). [Edge case — legacy snapshots without `genesis_total`: `restore_from_snapshot` back-solves `genesis_total_ = live + deltas_neg − deltas_pos` (lines 2628-2637) so the identity is satisfied at restore by construction. That back-solve omits `accumulated_shielded_`, which is sound because it runs only for pre-A1 / pre-§3.22 snapshots where `accumulated_shielded_` is `0`; every post-§3.22 snapshot always carries `genesis_total` (unconditional at line 2212) and takes the explicit-value branch. Either way the RHS equals the LHS post-restore.]
...
**Code witness.** `chain.cpp:699` (`live_total_supply`), `expected_total` (the six-counter sum, `chain.hpp:590-596`), `chain.cpp:1866-1888` (the A1 assertion), `chain.cpp:2212-2219` (counter persistence, incl. conditional `accumulated_shielded`), `chain.cpp:2439-2443` + `2628-2637` (counter restore + legacy back-solve).
