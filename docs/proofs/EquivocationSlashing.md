...
The two detection paths feed the **same** `EquivocationEvent` struct through the **same** V11 validator and the **same** apply path (per the digest-agnostic argument in §4: V11 only checks "two distinct hashes both signed by the same registered key" — it doesn't care whether those hashes are `block_digest`s or contrib commitments). An external implementer must wire both paths; missing either leaves an equivocation surface unslashable. PROTOCOL.md §6.1 also flags this two-source requirement.
