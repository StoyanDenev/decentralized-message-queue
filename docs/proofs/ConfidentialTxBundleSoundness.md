...
- **NC-4 — No consensus / wallet consumer; owner-gated PROFILE choice pending.** No Determ chain / ledger / wallet path builds or verifies a DCT1 bundle (inherited **CTB / NC-2**). Chain integration is a separate, consensus-critical, **owner-gated** step. (The MODERN-profile `Z_p*` mirror once contemplated here was removed 2026-07-07 — MODERN reuses the profile-agnostic P-256 stack; there is no separate `Z_p*` backend.) See [`ConfidentialTxIntegrationDesign.md`](ConfidentialTxIntegrationDesign.md) (a FUTURE-tier proposal, decides nothing).
...
- **L-3 — P-256 stack.** DCT1 is P-256, the profile-agnostic confidential-tx stack (used by both FIPS and MODERN). The `Z_p*` MODERN mirror once contemplated was removed 2026-07-07; there is no separate backend.
...
- **Non-claims (NC-1..NC-5).** No shielded-pool state model (commitment + nullifier sets, deposit/withdraw); does NOT prevent replay/double-spend (that is the nullifier set); amount privacy only (not sender/receiver/graph/fee/existence); no consensus/wallet consumer (owner-gated chain integration); not post-quantum.
