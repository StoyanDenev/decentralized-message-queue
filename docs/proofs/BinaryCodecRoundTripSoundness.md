...
> **⚠ D2 tail revision (2026-09-16, inc7c; inc7a/7b landed 2026-08-12).** Every one of the 19 MsgTypes is now a fixed binary frame: the six Block-carrying/CONTRIB types since inc7a/7b and the last two — HEADERS_RESPONSE (a page of DHF1 header records) and SNAPSHOT_RESPONSE (the DSN1 snapshot record verbatim) — since inc7c (PROTOCOL.md §9.1). The length-prefixed JSON fallback, `json_structural_precheck` and the WIRE-2 ceilings are deleted; an unknown type byte is rejected on both encode and decode. Statements below that count eleven fixed frames or eight JSON types, L-5, adversary (e) and the WIRE-2 references are historical and carry dated notes in place; the round-trip theorem (T-2) for the eight former JSON types is discharged per frame by `test-wire-payload-frames`, `test-headers-frame-codec` and `test-snapshot-response-frame-codec`.
...
**Setup.** Let a *body* `B ∈ {0,1}^{8n}` (0 ≤ n ≤ kMaxFrameBytes) denote the framing-stripped payload that `Peer::read_body` hands to `Message::deserialize` (`src/net/messages.cpp:15`). The framing layer (a 4-byte big-endian length prefix, `src/net/peer.cpp:40-60` read side) is outside the codec; `serialize_binary` re-prepends it (`messages.cpp:55-65`) and `read_body` strips it, so the codec operates purely on `B`.
...
Encoder side `binary_codec.cpp:315-394`; builders `messages.hpp:268-270, 282-284, 305-318`; consumer reads `gossip.cpp:265-313`.
...
| `EQUIVOCATION_EVIDENCE` (11) | `(equivocator: string, block_index: u64, kind: u8, index_a: u64, gen_a: u64, body_root_a: 32 B, sig_a: 64 B, index_b: u64, gen_b: u64, body_root_b: 32 B, sig_b: 64 B, shard_id: u32, beacon_anchor_height: u64)` — EQV-height-bind 2026-08-12 + gen-bind; fixed **245 B after the lp_str**, exact-length checked, decode fail-closes on `kind > 1` (layout restated 2026-09-14) | 246 + \|equivocator\| |

Encoder side `binary_codec.cpp:441-557`; builders `messages.hpp:224-226, 230-232, 233-243, 244-246`; layout comment `binary_codec.cpp:115-140`. For the remaining eight types `π` was, when this section was written, the full `payload` JSON value carried as a length-prefixed JSON payload. **That is historical (D2 inc7a/7b, 2026-08-12, and inc7c, 2026-09-16):** `BLOCK`, `BEACON_HEADER`, `SHARD_TIP`, `CROSS_SHARD_RECEIPT_BUNDLE` and `CHAIN_RESPONSE` carry `chain::Block::encode_frame` frames, `CONTRIB` its always-present field layout, `HEADERS_RESPONSE` the page of DHF1 header records and `SNAPSHOT_RESPONSE` the DSN1 record — 19 fixed frames of 19 (PROTOCOL.md §9.1). For each, `π` is the builder's DOM and the theorem gated by `test-wire-payload-frames` PF-1 / `test-headers-frame-codec` HF-1 / `test-snapshot-response-frame-codec` SR-1 is `dec(enc_B({type, π})).payload == π`, with PF-2 / HF-2 / SR-2 pinning the canonical fixed point `enc_B(dec(x)) == x`.
...
Determ peers exchange `Message` values over TCP. The transport (`Peer`) frames each body with a 4-byte big-endian length prefix (`src/net/peer.cpp:40-60` read side; `messages.cpp:55-65` write side). Inside that frame the *body* is exactly one format:
...
One later hardening layer sits *inside* `dec` and is part of its totality surface: the WIRE-1 pre-decode per-type cap (`messages.cpp:21-47` — the type is readable in the clear at offset 2, so the S-022 ceiling is applied before any payload work). A second one — the WIRE-2 structural ceiling (`json_structural_precheck`, invoked in `decode_binary` on the length-prefixed JSON payloads the non-fixed-frame types carried) — survived the D2 envelope strip, D2-inc6a/6b and inc7a/7b, and was **deleted in D2 inc7c (2026-09-16)** together with the last two JSON payloads and the fallback parse site: no type reaches `nlohmann::json::parse` from the wire any more. WIRE-1 is throw-on-violation, i.e. another instance of the T-4 return-or-throw contract; its DoS soundness is proved in `S022WireFormatCaps.md`, not here.
...
Total-or-throw in three stages: the T-1 magic reject (`messages.cpp:16-20`, the specific `"not a binary envelope"` string that the D2 negative gate pins), the WIRE-1 pre-decode per-type cap (`messages.cpp:39-47` — strictly accept-narrowing, the same cap `Peer::read_body` re-applies post-decode), then `decode_binary` (§3.3–§3.5). There is no JSON branch; `nlohmann::json::parse` is never reached from a non-`0xB1` body.
...
`enc_B(m)` (`binary_codec.cpp:309-333`) calls `put_envelope_header` (`binary_codec.cpp:291-296`) first on every path, pushing `kBinaryMagic = 0xB1` then `kBinaryVersion = 0x01` as the first two bytes, then `type` and the reserved `0x00`. Every branch (the 19 fixed frames; the `default` arm throws, so an unknown type never reaches the wire) appends after the header. Hence `enc_B(m)[0] = 0xB1`, `enc_B(m)[1] = 0x01`, and `|enc_B(m)| ≥ 4`. There is no other reachable wire encoder: `Peer::send` (`peer.cpp:119`) is the only wire write path and calls `serialize_binary()`, which defers to `encode_binary` (`messages.cpp:55-56`). □
...
**⚠ CORRECTED (D2-inc5 pre-verification).** An earlier revision of this lemma ended at the sentence above with **no precondition on `payload.size()`**, and that unqualified form was FALSE. `payload_len` is a `u16`, but the encoder's overflow section wrote `payload.size() - 32` bytes *unclamped* while declaring a value clamped to `0xFFFF`. For `payload.size() > 0xFFFF` the two disagreed: the decoder consumed `0xFFFF - 32` overflow bytes and then read `from`'s length prefix out of the **middle of the payload** — attacker-chosen bytes, with no error raised. At `payload.size() = 70000` that is 4,465 surplus bytes. So "every encoded field has a matching decode read" held only for `payload.size() ≤ 0xFFFF`, and the lemma as written claimed a theorem the code did not have.

The precondition is now discharged in the code rather than added to the lemma: `Transaction::encode_frame` **refuses** an oversized payload (`src/chain/block.cpp:188-189`, `"tx frame: payload exceeds u16 length"`), mirroring the `pq_auth` sibling immediately below it, so `payload.size() ≤ TX_FRAME_PAYLOAD_MAX` holds on every reachable encoder output and L-4 is unconditionally true of `dom(enc_B)`. `TX_FRAME_PAYLOAD_MAX` (`include/determ/chain/block.hpp:315`) is the u16 field's exact capacity, not a policy number. Gated by test-tx-binary-codec leg 11, whose third assertion is the property itself — *every payload size the encoder accepts decodes back to the same transaction* — so reverting the throw to the clamp reddens it (mutant-verified).

*Reachability, for the record.* The defect was NOT reachable through `COMPOSABLE_BATCH` as first supposed: inner frames must be `TRANSFER` with `payload ≤ 32` and `MAX_COMPOSABLE_INNER = 64`, capping a batch payload at 47,554 — below the threshold. The real route was the RPC ingress: `Node::rpc_submit_tx` → `Transaction::from_json` (no payload bound) → `Node::mempool_admit_check` (which had no payload byte cap) → `gossip_.broadcast` → `encode_frame`. A wire-received transaction could never trigger it, because `decode_frame` reads `payload_len` as a `u16` and so can never produce an oversized `payload` — the injection point was local submission only. Ingress now rejects it first (`src/node/node.cpp:2761`) so the submitter gets a diagnosable error; the encoder throw is the backstop that makes the ambiguity unrepresentable. Both layers are needed: `GossipNet::broadcast` swallows per-peer exceptions (`src/net/gossip.cpp:335`, `catch (...) {}`), so the encoder throw alone would have degraded to a silent non-propagation.

### Lemma L-5 (length-prefixed-JSON path is verbatim) — VOID as of D2 inc7c (2026-09-16)

*Historical.* For `m.type` outside the then-eleven fixed-frame types, `enc_B` computed `s = m.payload.dump()`, wrote `le_u32(|s|)` then `s` verbatim; `decode_binary` read `plen`, verified `4 + plen ≤ body_len`, ran the WIRE-2 structural pre-scan and parsed the window as JSON, and `nlohmann::json::parse(dump(v)) == v` gave `dec(enc_B(m)).payload == m.payload`. **No such path exists any more:** every type has a fixed frame, `encode_binary`'s and `decode_binary`'s `default` arms throw, and the round-trip obligation for the eight former JSON types is discharged per frame by `test-wire-payload-frames` PF-1/PF-2, `test-headers-frame-codec` HF-1/HF-2 and `test-snapshot-response-frame-codec` SR-1/SR-2 (builder-DOM equivalence + canonical fixed point). □

### Lemma L-6 (encoder/decoder field agreement for `HELLO`)

`encode_hello_frame` (`binary_codec.cpp:267-276`) writes `lp_str(domain)`, `port` (u16 LE), `role` (u8), `shard_id` (u32 LE), `wire_version` (u8), reading each from the payload via `.value()` with the documented default. `decode_hello_frame` (`binary_codec.cpp:278-299`) reads the same five fields at the same offsets in the same order and rebuilds a payload with all five keys explicit. For a payload carrying all five fields (the `make_hello` shape, `messages.hpp:198-217`), decode returns them exactly; for a payload omitting a field, the encoder writes the default and the decoder returns it explicitly — equal under the tolerant `.value()` read that defines `π(HELLO)`. The exact-consumption check makes the frame boundary unambiguous, so no field read can absorb bytes of another. □
...
Each encoder reads its source values from the payload via `.value()` with the same default the corresponding `messages.hpp` builder and `GossipNet::handle_message` case use, and each decoder rebuilds a payload with exactly those keys explicit. Hence for a payload carrying all of its type's fields — the builder shape (`messages.hpp:268-270, 282-284, 305-318`) — decode returns them exactly, and `dec(enc_B(m)).payload == m.payload` as JSON values; for a payload omitting a field, the encoder writes the default and the decoder returns it explicitly, which is `π`-equal under the tolerant `.value()` read that defines `π` for these types. Two boundary details make the agreement total rather than approximate:
...
Hence for a builder-shaped `m` (`messages.hpp:224-226, 230-232, 233-243, 244-246`), decode returns every field exactly, and `dec(enc_B(m)).payload == m.payload`. □
...
**(e) Oversized length-prefix in the length-prefixed-JSON path — CLOSED BY DELETION (D2 inc7c, 2026-09-16).** Adversary sets `plen` to a huge value to provoke a large allocation. Until inc7c: `decode_binary` rejected `4 + plen > body_len` *before* parsing, `body_len` was bounded by the WIRE-1 pre-decode per-type cap (`messages.cpp:39-47`) and the WIRE-2 structural ceiling bounded the DOM the parse could build (D2-inc6a/6b and inc7a/7b narrowed the surface to two types; the type byte at offset 2 is attacker-chosen, so WIRE-2 stayed load-bearing). Now no type reaches a JSON parse: the same attacker bytes are read as the fixed [from][height][count] prefix of a HEADERS_RESPONSE or the DSN1 magic of a SNAPSHOT_RESPONSE and rejected within the first field reads (`test-binary-codec` 8d/8e/8h), and every frame's own length and count fields are proven against the remaining bytes before any allocation (HF-3e/3f, SR-3d/3e).
...
**Finding F-5 (`Transaction::encode_frame` clamped a length it then over-wrote — RESOLVED).** Found during the inc5 pre-verification, by an adversarial pass whose brief was to REFUTE the finding. The encoder declared `payload_len` clamped to `0xFFFF` while writing the full `payload.size() - 32` overflow, so above 65,535 bytes encode and decode disagreed and the decoder reconstructed a **different transaction** — parsing `from`'s length prefix from attacker-chosen payload bytes — with no error on either side. Two sibling fields in the same function already threw rather than clamped (`put_lp_str`'s 255-byte bound, and `pq_auth exceeds u32 length`), so the clamp was the outlier, not the convention.

Three properties of this finding are worth carrying forward. **(a) It falsified a shipped B3 lemma** — L-4 asserted slot agreement with no size precondition (see the correction there). A proof claiming a theorem the code lacks is the sharpest violation of the provable-security doctrine, and it is exactly the class an adversarial re-read catches and a round-trip test does not: every gate in the tree passed, because none exercised a payload above 65 KB. **(b) The first reachability story was wrong.** `COMPOSABLE_BATCH` caps a batch payload at 47,554 (inner frames are `TRANSFER`-only with `payload ≤ 32`, `MAX_COMPOSABLE_INNER = 64`), below the threshold; the real route was the unbounded RPC ingress. A finding's *mechanism* being confirmed says nothing about its *reachability* — they need separate evidence. **(c) The disposition had to be derived against the send path, not just the codec.** `GossipNet::broadcast` swallows per-peer exceptions, so an encoder throw alone converts silent corruption into silent non-propagation; the ingress rejection is what makes the failure visible to the submitter. Fixed at both layers, gated by test-tx-binary-codec leg 11 (boundary control at exactly 65,535, the reject one byte over, and the round-trip-faithfulness property across eight payload sizes), mutant-verified by restoring the clamp.
...
What the two increments move is the *proof* surface: eleven of the 19 types became true fixed frames, so the T-2 case analysis had five cases instead of three, T-3's guard inventory has §3.6's and §3.7's rows, and a new theorem T-2b (encoding canonicality) states the property the migration is actually for — gated by L-9's sweep after F-4 showed the per-frame legs could not see it. WIRE-2 did **not** retire with them: the remaining 8 types still carried length-prefixed JSON payloads through `nlohmann::json::parse`. **D2 inc7a/7b (2026-08-12) then moved six of those (all delegating to `chain::Block::encode_frame`, or CONTRIB's own layout) and D2 inc7c (2026-09-16) the last two — HEADERS_RESPONSE as a page of DHF1 header records and SNAPSHOT_RESPONSE as the DSN1 record verbatim — after which `json_structural_precheck`, both ceilings and the fallback parse site were deleted; L-9's exact-length sweep now covers 19 frames (`cases.size() == 19`).**
...
- `src/net/messages.cpp:15-49` — the binary-only `Message::deserialize` (T-1 magic gate at 83-87, WIRE-1 pre-decode cap at 106-114, T-4 totality).
- `src/net/messages.cpp:55-65` — `Message::serialize_binary` (framing). (`json_structural_precheck` — WIRE-2 — lived at the top of this file until D2 inc7c deleted it, 2026-09-16.)
- `include/determ/net/messages.hpp:268-270, 282-284, 305-318` — the five request/status builders; `make_status_request` at 308-314 carries P-2's `nlohmann::json::object()` normalization.
...
- ~~`src/net/messages.cpp` — `json_structural_precheck`~~ (WIRE-2; survived D2, D2-inc6a/6b and inc7a/7b for the types that still carried length-prefixed JSON payloads; DELETED in D2 inc7c, 2026-09-16, with the last two).
- `src/net/messages.cpp:15-65` — `Message::deserialize` (`dec`) + `Message::serialize_binary` (T-1 gate, WIRE-1 cap, T-4 totality; framing).
- `include/determ/net/messages.hpp:84-89` — `kWireVersionBinary` (the single shipped version; advertisement-only, §2.1).
- `include/determ/net/messages.hpp:91-174` — `kMaxFrameBytes` + `max_message_bytes` (the S-022 surfaces T-4 composes with; all five D2-inc6a types **and** all four D2-inc6b types remain in the 1 MB default tier — inc6b changes no cap value).
...
- `include/determ/net/messages.hpp:224-226, 230-232, 233-243, 244-246` — `make_block_sig` / `make_abort_claim` / `make_abort_event` / `make_equivocation_evidence` (the builder shapes π uses for the four chatter frames).
- `include/determ/net/messages.hpp:176-196` — `Message` struct, codec declarations (the WIRE-2 ceilings that preceded them were deleted in D2 inc7c, 2026-09-16).
- `include/determ/net/messages.hpp:198-217` — `make_hello` (the five-field HELLO shape; π(HELLO) per §1).
- `include/determ/net/messages.hpp:268-270, 282-284, 305-318` — `make_snapshot_request` / `make_headers_request` / `make_get_chain` / `make_status_request` / `make_status_response` (the builder shapes π uses for the five frames; P-2's object normalization at 404-410).
