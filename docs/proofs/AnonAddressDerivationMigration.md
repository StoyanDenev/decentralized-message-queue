...
**Status:** **REOPENED 2026-07-04** (authority: Stoyan Denev; `DECISION-LOG.md 2026-07-04 R61`). The owner authorized **on-chain post-quantum signatures**, which introduce a second pubkey form (ML-DSA/Dilithium) and require the address-format + wire changes the 2026-07-03 freeze foreclosed. The freeze is therefore lifted; the §5 A-vs-B address-scheme analysis below is **live again** and must be re-decided before the PQ wire integration lands (with PQ forms now genuinely in scope, the §7.6.7 discriminator is no longer satisfied vacuously). ~~Superseded: **DECIDED — FROZEN AS-IS** (2026-07-03) — `"0x"+hex(pubkey)` permanent, Ed25519-only, discriminator vacuous, Option-A cancelled.~~ The prior freeze rationale stands as the decision record.
