...
**Status:** OPEN — design analysis pending decision-authority sign-off. This is a forward-looking options analysis, NOT a re-decision. It exists to give the project the information it needs to MAKE the §7.6.7 address-derivation decision before v1.1 mainnet, and to propose that the resolution be recorded in `DECISION-LOG.md` once made.
