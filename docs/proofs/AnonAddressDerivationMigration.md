...
**Status:** **DECIDED — FROZEN AS-IS** (2026-07-03, authority: Stoyan Denev; recorded in `DECISION-LOG.md 2026-07-03`). The current formula `"0x" + hex(pubkey)` is permanent for v1.1 genesis; anon addresses are Ed25519-only for the chain's lifetime; the §7.6.7 discriminator does NOT enter the address preimage (the requirement is satisfied vacuously — one pubkey form, no cross-form aliasing); the Option-A implementation bundle is cancelled. The analysis below is retained as the decision record's supporting document.
