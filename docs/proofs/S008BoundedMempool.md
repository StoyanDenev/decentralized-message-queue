...
**Correction 2026-09-16 (S-079, SECURITY.md; DECISION-LOG 2026-09-16 D19a item 2).** The mechanisms below were sound for what they claim (the capacity, quota and RBF invariants T-1..T-3 are unchanged) but they did NOT close the pool against a sender who cannot pay: `mempool_admit_check` asked nothing about funding, so 100 free-to-mint anonymous senders × 100 `amount = 0, fee = UINT64_MAX` transfers filled the cap with entries no block would ever apply, and the fee floor of (c) then rejected every representable fee for ever. Shipped since: (f) **affordability at the head** — a transaction is admitted only if `balance(from)` at the head covers its transparent debit (`Node::mempool_tx_cost`, what apply charges) plus the debits of its other resident transactions (`mempool_committed_from`); (c') the eviction victim in `mempool_make_room_for` is an UNAFFORDABLE resident first (re-checked against the current head, per sender in nonce order, `mempool_scan_locked`) and only then the minimum-fee entry, and the floor `min_fee` is taken over affordable residents only; and the build-time predicate (`Node::tx_admit_locked`) evicts a resident the head can no longer fund. The verbatim code listings in §2 are the pre-S-079 text and are kept as the record of what T-1..T-6 were proved against; the S-079 gate is `determ test-mempool-admit-affordability`. **Which theorems now rest on a changed rule, stated plainly and NOT re-derived here:** T-5 (no useful-tx loss under pressure) and the min-fee monotonicity argument beneath it describe the shipped code only under the added hypothesis "every resident is affordable at the head" — when one is not, the shipped victim is that resident whatever its fee (an unaffordable transaction is by construction not a useful one, so the claim T-5 exists to make is preserved, but its proof text is not the shipped selection function). T-1 (capacity), T-2 (nonce-gating), T-3 (RBF determinism), T-4 (memory bound) and T-6 (S-014 composition) are untouched.
