...
> **SHIPPED (A6, 2026-07-09, commit `9093189`) — as-built reconciliation.** The discriminator is implemented on `Block` as `uint8_t signature_form{0}` with enum constants `SIG_FORM_KK_ED25519=0` / `SIG_FORM_BLS12_381_AGGREGATE=1` / `SIG_FORM_MLDSA_KK=2` (`include/determ/chain/block.hpp`). Two deliberate as-built refinements of the sketch above, both in service of no-migrations byte-invariance:
> 1. **Zero-skip, not "emit unconditionally."** The byte is bound into `signing_bytes()` and BOTH block-digest sites (`producer.cpp::compute_block_digest`, `light/verify.cpp::light_compute_block_digest`) **only when non-zero** — the `partner_subset_hash`/`state_root` conditional-append pattern — and elided from `to_json()` at form 0. So every pre-A6 chain, golden, fixture, and snapshot is byte-identical (unlike the sketch's "emit unconditionally," which would have rewritten every golden). The security property the sketch wanted — two blocks differing only in `signature_form` have distinct hashes — holds for every non-zero form; form 0 is *by definition* the pre-discriminator encoding, so there is nothing to distinguish it from.
> 2. **Append position = tail, not "before `creator_block_sigs[]`."** The conditional append is the LAST digest term (field order `…partner_subset_hash, timestamp, signature_form`), which is what keeps (1) true. Dispatch is still gated on the decoded value at `BlockValidator::validate` (fail-closed on any non-zero form BEFORE any signature check). Pinned by `determ test-block-signature-form` (14 assertions) + the FB62 cross-binary digest-parity guard (16 tokens) + the live 3-node cluster.
