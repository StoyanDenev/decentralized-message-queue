...
- **`determ-wallet verify-batch`** (`cmd_verify_batch`, D2 R40, `wallet/main.cpp:11457`) — batch-verifies `N` signed envelopes, reporting per-envelope validity.
...
**Proof.** `verify-batch` (`cmd_verify_batch`, D2 R40, `wallet/main.cpp:11457`) is the batch analogue of `validate-tx` (§3.3): it parses the input as a JSON array and applies the single-envelope verify gate to each element, accumulating a per-envelope verdict array. The soundness reduction is in two parts.
...
**Note on `verify-batch` (landed, D2 R40).** `cmd_verify_batch` shipped this round at `wallet/main.cpp:11457`; BS-3 is stated against its contract — *"batch-verify `N` envelopes = `N` independent `validate-tx` operations"* — and the `validate-tx` ground truth (§3.3) it composes. The reduction is purely structural and does not depend on `verify-batch`'s internal loop mechanics beyond the per-element-independence that the implementation provides (each envelope's pubkey is recovered from its OWN `from` field; report-only by default, `--strict` exit 3 on any invalid envelope). Verified end-to-end by `tools/test_wallet_verify_batch.sh` (43 assertions).
...
- `tools/test_wallet_verify_batch.sh` (D2 R40, 43 assertions) — covers BS-3 (batch-verify of mixed valid/invalid envelopes; per-envelope verdict matches single `validate-tx`; `--strict` exit-3 gating vs report-only default).
...
| `verify-batch` command | `cmd_verify_batch` (D2 R40, `wallet/main.cpp:11457`) | **BS-3 core** — `N`-fold `validate-tx`; per-envelope verdict, no cross-envelope channel |
...
| Verify-batch regression | `tools/test_wallet_verify_batch.sh` (D2 R40, `wallet/main.cpp:11457`) | BS-3 (per-envelope verdict matches single `validate-tx`) |
...
**Implementations shipped.** `tx-batch-sign` (`cmd_tx_batch_sign`, `wallet/main.cpp:9609-10271`), `account-import-many` (`cmd_account_import_many`, `wallet/main.cpp:1847-2220`), and `verify-batch` (`cmd_verify_batch`, `wallet/main.cpp:11457`, D2 R40) are all live on this branch; BS-3 resolves to `cmd_verify_batch` + `tools/test_wallet_verify_batch.sh` (43 assertions).

**Regression tests passing.** `tools/test_wallet_tx_batch_sign.sh` (38 assertions) and `tools/test_wallet_account_import_many.sh` (30 assertions) exercise the structural claims end-to-end — independent per-tx validation, order preservation, byte-identical sign output, per-record fault isolation, the encrypted-keyfile round-trip, and the encrypted-output-non-determinism-by-design split. `tools/test_wallet_verify_batch.sh` (43 assertions, D2 R40) exercises BS-3 — per-envelope verdict parity with single `validate-tx`, `--strict` exit-3 gating, hash-substitution + tampered-body rejection.
...
- `wallet/main.cpp` — `cmd_tx_batch_sign` (`:9609-10271`), `cmd_account_import_many` (`:1847-2220`), the single-record `cmd_sign_anon_tx` (`:9110-9524`) / `cmd_validate_tx` (`:10511-…`) / `cmd_derive_tx_hash` (`:11170-11435`) primitives the batch ops replicate, and `cmd_verify_batch` (D2 R40, `wallet/main.cpp:11457`).
