...
**Setup.** Let `kMaxFrameBytes` denote the framing-layer ceiling (`include/determ/net/messages.hpp:117`):

```cpp
inline constexpr size_t kMaxFrameBytes = 16 * 1024 * 1024;   // 16 MB
```

Let `max_message_bytes : MsgType → size_t` denote the per-MsgType body-cap function defined at `include/determ/net/messages.hpp:146-174`. The function is a `switch` over the `MsgType` enum (values 0-18 at `include/determ/net/messages.hpp:13-82`) with three returnable values:
...
The default branch is **tight** (1 MB) by deliberate design — any new MsgType added without explicit categorisation inherits the strict cap rather than the permissive 16 MB ceiling. The `include/determ/net/messages.hpp:165-170` comment names this explicitly:
...
Let `M = (type, payload)` denote a `Message` (struct at `include/determ/net/messages.hpp:176-189`). Let `|body(M)|` denote the body-buffer length on the wire (the 4-byte-header-stripped payload, as observed at `Peer::read_body`'s `body_buf_.size()`).
...
1. **JSON envelope — CLOSED BY DELETION (D2, `ce31c6f`, 2026-07-31).** At round 12, `Message::deserialize`'s JSON branch called `json_structural_precheck(data, len)` before `json::parse` — allocation-free, single-pass, aborting at the first byte that exceeds `kMaxJsonDepth` or `kMaxJsonNodes`. The D2 strip deleted the branch (and `Message::serialize()`) outright: a non-0xB1 body is now rejected by the magic check (`src/net/messages.cpp:16-20`) with **no parse at all** — strictly stronger than the guarded parse it replaces.
2. **Binary envelope — CLOSED BY DELETION (D2 inc7c, 2026-09-16).** Until inc7c `decode_binary` called `json_structural_precheck(body + 4, plen)` before its payload `json::parse` for the types still on length-prefixed JSON (8 after D2-inc6a/6b, 2 after inc7a/7b), closing the path a hostile frame reached by claiming a 16 MB type at offset 2. That parse site no longer exists: every one of the 19 types dispatches to a fixed frame decoder (`decode_binary`'s switch) and the `default` arm throws `unknown MsgType ... no length-prefixed JSON fallback`. The bound is now structural — each frame decoder proves every count against the remaining bytes before it allocates (`bf_get_count` / `wf_get_count` / `SnRd::count`), so pre-dispatch work is linear in the body size.
...
Consequently (as of inc7c) no JSON DOM is built from the wire at all, and the per-connection repeat rate of a rejected frame is bounded by the TCP reconnect cost rather than by zero. **Residual (historical, retired 2026-09-16):** while the JSON payloads lived, T-6 bounded the DOM, not the *ratio* of DOM to input; a 16 MB frame within the ceilings still reached a measured 482 MB (25.5×) — versus 130 MB (8.1×) for the densest legitimate message. (The 525 MB / 41.4× figure that added the JSON-envelope `payload` deep copy retired earlier: D2 deleted that copy site.) The D2 tail binarized the remaining payloads (inc7a/7b, then inc7c), which is what eliminated it — see F-6 and the measured table in §2.3.
...
- **⚠ CORRECTED — `Message::deserialize` does NOT return `msg.type` cheaply.** This bullet previously read: *"Both paths return `msg.type` within `O(min(|body|, header_size))` work — well before the full payload is parsed."* **That premise was FALSE, and it was load-bearing for T-5.** The round-12 hostile-wire audit (wf_c277c6d1) measured the actual cost:
  ...
  **D2 note (2026-07-31, `ce31c6f`): the JSON path described above is deleted.** `Message::deserialize` is binary-only, so no shipped wire path carries a type readable only post-parse; the 831 MB measurement is a record of the deleted path. WIRE-2 survives because `decode_binary` still parses length-prefixed JSON payloads for the 8 remaining types (D2-inc6a `ad595bb` and D2-inc6b `e845b44` moved nine more types to fixed binary frames, 11 in total) — and the type at offset 2 remains attacker-chosen, so the binary half of this correction is still live.
...
1. **Per-connection flood with maximally-padded oversized messages of consensus-chatter types.** One TCP connection, sustained 16-MB-padded `MsgType::CONTRIB` or `MsgType::BLOCK_SIG` frames.
   ...
   **Current status: MITIGATED, not eliminated (T-2 + T-3 + T-4 + T-6).** The WIRE-2 structural ceiling (§2.3) now aborts the pathological shapes before the parser allocates — the 16 MB `'['` flood dies after 65 bytes — and WIRE-3 closes the connection on any parse failure, restoring the per-attempt reconnect cost. The D2 strip (`ce31c6f`) then deleted the JSON envelope the 831 MB (51.9×) figure was measured on. The residual was the in-ceiling DOM on the binary envelope's length-prefixed JSON payloads — a measured 482 MB (25.5×) from a 16 MB frame, reachable only by claiming a 16 MB type at offset 2; see F-6 and the table in §2.3. **Retired 2026-09-16 (D2 inc7c):** no length-prefixed JSON payload remains, so a 16 MB frame buys O(16 MB) of bounded frame decoding and nothing more. (2026-09-25, SECURITY.md S-119: for SNAPSHOT_RESPONSE that decoding rebuilds a full chain state, may recompute the state root and re-serializes a JSON DOM, all before HELLO and the rate limit — bounded, but a large constant that repeats on one connection.)
...
Per `include/determ/net/messages.hpp:146-174`:
...
The framing-layer outer ceiling at `include/determ/net/messages.hpp:117`:
...
Per `include/determ/net/messages.hpp:13-82`, the full MsgType enum is. (D2 note: every body on the wire is the 0xB1 binary envelope with the type byte at offset 2 — **11** types are fixed binary frames (HELLO + TRANSACTION per `ce31c6f` / `b29d422`; GET_CHAIN, STATUS_REQUEST, STATUS_RESPONSE, SNAPSHOT_REQUEST, HEADERS_REQUEST per D2-inc6a `ad595bb`; ABORT_CLAIM, BLOCK_SIG, EQUIVOCATION_EVIDENCE, ABORT_EVENT per D2-inc6b `e845b44`); the other **8** carry the JSON payload shape below length-prefixed inside the envelope. `S022WireFormatCapsCompleteness.md` §1 is the authoritative per-type record of that split.)
...
| 1 | `BLOCK` | The canonical `Block::encode_frame` container (D2-inc7a) | 4 MB |
| 2 | `TRANSACTION` | Fixed binary tx frame (128-byte core + trailer, incl. `pq_auth` per `b29d422`) | 1 MB (default) |
| 3 | `BLOCK_SIG` | Fixed binary frame (D2-inc6b): `BlockSigMsg` = block_index + signer + delay_output + dh_secret + ed_sig | 1 MB (default) |
| 4 | `CONTRIB` | Fixed binary frame (D2-inc7b): `ContribMsg` (Phase-1 commit + view-roots; ~few KB typical) | 1 MB (default) |
| 5 | `GET_CHAIN` | Fixed binary frame (D2-inc6a): `{from, count}` | 1 MB (default) |
| 6 | `CHAIN_RESPONSE` | Fixed binary frame (D2-inc7a): `[has_more][count][count x Block frame]` | 16 MB |
| 7 | `STATUS_REQUEST` | Fixed binary frame (D2-inc6a): zero-length | 1 MB (default) |
| 8 | `STATUS_RESPONSE` | Fixed binary frame (D2-inc6a): `{height, genesis}` | 1 MB (default) |
| 9 | `ABORT_CLAIM` | Fixed binary frame (D2-inc6b): the shared `chain::encode_abort_claims` blob carrying exactly one claim | 1 MB (default) |
| 10 | `ABORT_EVENT` | Fixed binary frame (D2-inc6b): `AbortEvent` with the K-1 claims blob last | 1 MB (default) |
| 11 | `EQUIVOCATION_EVIDENCE` | Fixed binary frame (D2-inc6b): two `(digest, sig)` pairs + equivocator + shard/anchor | 1 MB (default) |
...
| 15 | `SNAPSHOT_REQUEST` | Fixed binary frame (D2-inc6a): `{headers: N}` | 1 MB (default) |
| 16 | `SNAPSHOT_RESPONSE` | Serialized chain state (multi-MB at scale) | 16 MB |
| 17 | `HEADERS_REQUEST` | Fixed binary frame (D2-inc6a): `{from, count}` | 1 MB (default) |
...
The `MsgType` enum at `include/determ/net/messages.hpp:13-82` declares 19 variants with values 0 .. 18 (inclusive). The `switch` statement at `messages.hpp:146-174` enumerates:
...
1. `src/net/messages.cpp`, `Message::deserialize` — **the JSON branch this clause guarded is deleted (D2, `ce31c6f`).** The shipped function rejects any non-0xB1 body at the magic check (`messages.cpp:16-20`) and applies the WIRE-1 pre-decode cap (`messages.cpp:39-47`) before `decode_binary`; no parse of an unprefixed JSON document exists to guard. (As proved at round 12, the branch called the precheck on the line immediately preceding `nlohmann::json::parse(data, data + len)` — allocation-free, throwing from inside the scan loop — which is why the clause held then; deletion is strictly stronger.)
2. `src/net/binary_codec.cpp`, `decode_binary` — **the payload parse this clause guarded is deleted (D2 inc7c, 2026-09-16).** Until inc7c it called `json_structural_precheck(body + 4, plen)` immediately before its payload parse; this was the site that mattered for the attacker-chosen-type bypass, since `m.type` is read from `data[2]` and never gated the call, so a frame claiming a 16 MB type was scanned on exactly the same terms as any other. Now every type byte dispatches to a fixed frame decoder and the `default` arm throws; no parse exists to guard, and the per-frame count discipline (`wf_get_count` / `bf_get_count` / `SnRd::count`, each proving the count against the remaining bytes before any reserve) is the bound.
...
| `BLOCK_SIG` | D2-inc6b frame: exactly 137 + \|signer\| B (≤ 392 at the 255-byte lp-string cap) | ~150 bytes | ≥2600× |
...
| `ABORT_CLAIM` | D2-inc6b frame: exactly 109 + \|missing_creator\| + \|claimer\| B (≤ 619) | ~130 bytes | ≥1600× |
| `ABORT_EVENT` | D2-inc6b frame: 84 + \|aborting_node\| + claims blob; each claim 107 + \|missing_creator\| + \|claimer\| B | ~K × 171 bytes | ≥6× (K=1000, ~32-byte identifiers); ≈1.7× at the 255-byte cap |
| `EQUIVOCATION_EVIDENCE` | D2-inc6b frame: exactly 213 + \|equivocator\| B (≤ 468) | ~225 bytes | ≥2200× |
| `SNAPSHOT_REQUEST` | `{headers: N}` | ~30 bytes | ≥30000× |
| `HEADERS_REQUEST` | `{from, count}` | ~50 bytes | ≥20000× |

Even the worst case (`ABORT_EVENT` at K=1000) leaves ≥6× headroom at realistic identifier lengths, and ≈1.7× if every identifier were padded to the codec's 255-byte lp-string cap — still under the tier. Since D2-inc6b the claims blob is count-prefixed with a **u16**, so the frame layout alone would admit 65,535 claims (≈38.6 MB): the 1 MB tier cap, not the codec, is what bounds the claim list. The defaults are sound at present and remain sound under any reasonable K growth. **No action required.**
...
- **(D) Drop the `payload` deep copy.** `src/net/messages.cpp` did `m.payload = envelope["payload"]` with `envelope` still alive, the difference between the 25.5× and 41.4× rows. **Moot** — the copy site was deleted with the JSON envelope; `decode_binary` parses directly into `m.payload` (`src/net/binary_codec.cpp:695`).

**What this closure does NOT claim.** The binary path's residual is untouched and remains exactly as fact (2) of §2.3 states it: the type byte at offset 2 is **attacker-chosen**, so a hostile frame claiming `SNAPSHOT_RESPONSE` (16) or `CHAIN_RESPONSE` (6) still buys the 16 MB tier and lands in `decode_binary`'s length-prefixed JSON payload parse — the 8 remaining types (D2-inc6a `ad595bb` and D2-inc6b `e845b44` moved nine more types to fixed binary frames, 11 in total) still carry JSON there. That in-ceiling DOM (measured 482 MB / 25.5× at the worst admitted shape) is **bounded by WIRE-2** (`kMaxJsonDepth` / `kMaxJsonNodes`, applied pre-parse at `binary_codec.cpp:694`), not eliminated; it retires with the D2 tail (true binary frames per type), at which point WIRE-2 retires with it.

**Severity:** was Medium, owner-gated on a protocol decision. **Status: CLOSED-BY-DELETION (2026-07-31, `ce31c6f`)** — the JSON-envelope premise (post-parse type discovery + legitimate 16 MB JSON traffic) no longer exists; the binary-path in-ceiling residual persisted under WIRE-2 and was tracked to the D2 tail, not to an owner decision. **The D2 tail landed (inc7a/7b 2026-08-12, inc7c 2026-09-16): the residual is gone.** `decode_binary` has no JSON parse site — HEADERS_RESPONSE is a page of DHF1 header records and SNAPSHOT_RESPONSE is the DSN1 record verbatim, the `default` arm rejects an unknown type byte, and `json_structural_precheck` with both ceilings is deleted. What replaces WIRE-2 is not a ceiling but the per-frame count discipline (every count proven against the remaining bytes before allocation), gated in `test-headers-frame-codec`, `test-snapshot-response-frame-codec`, `test-wire-payload-frames` and the binary-only legs of `test-binary-codec`. The historical decision-relevant fact stands for the record: (C) was measured dead, and the pre-measurement version of this table got the exposure wrong in both directions.
...
**Finding F-10 (⚠ HIGH — the depth ceiling is ENVELOPE-RELATIVE, and WIRE-3 turns the resulting asymmetry into a permanent sync wedge). ✅ CLOSED — the root-cause fix was taken (owner-approved); see "Fix shipped" below.**
...
**Fix shipped (root cause, one line, consensus-byte-neutral).** `AbortEvent::from_json` now stores `canonical_abort_claims(...)` — the six consensus-bound fields rebuilt, exactly what `canonical_abort_claims_dump` already produced for the digest, refactored so the dump form *is* that value's `.dump()` and the stored bytes cannot drift from the hashed bytes. A stored claim is six scalars, so it contributes **zero** nesting: admitted depth is a property of the schema again, and the entire unknown-member channel (of which unbounded depth was one symptom) is closed at its source. Anchored in [AbortDigestCanonicalizationSoundness.md](AbortDigestCanonicalizationSoundness.md) §2.1 / ADC-7 / ADC-8.

**One deviation from the fix as originally recommended:** it does **not** reject a claim that fails to canonicalize — it keeps the verbatim value, as the digest path already did. Rejecting would be strictly worse *because of WIRE-3*: `from_json` throwing is a parse failure, and a parse failure now closes the peer, so a malformed gossiped claim would become a disconnect for no security gain. Soundness is unaffected: canonicalization is strictly **weaker** than per-claim validation (`at(k).get<T>()` versus `json_require<T>` — the same `get<T>` plus a `contains(k)` check, plus an exact hex-length check on the two hex fields), so *canonicalization throws ⟹ validation throws ⟹ no such block is ever accepted*. Nothing that survives validation can reach the fallback, and the gate pins that implication over five malformed shapes rather than asserting it in prose.

**Consensus-byte-neutral for EVERY input, not merely for honest ones** — the stronger statement the ingest application needs, since it changes the stored bytes for a *hostile* claim too, and those bytes feed the digest. The rebuild is idempotent (`canon∘canon = canon`), so a claim that canonicalizes hashes `canon(canon(v)) = canon(v)` — what it already hashed — and one that does not is stored unchanged. Every block's hash is identical before and after, and no accept/reject decision moves: no fork, no golden migration, nothing for the no-migrations constraint to bite on. Proof: ADC-7.
...
**Severity:** Low. **Status: ✅ CLOSED-BY-DELETION (D2 inc7c, 2026-09-16).** It was reported-not-fixed at the time, deliberately out of scope for a round scoped to the pre-auth daemon path. The finding's premise — `decode-wire`'s unguarded `json::parse(body + 4, body + 4 + plen)` — no longer exists: the light mirror decodes all 19 types as fixed frames (the two inc7c additions are `hpw_walk` for the HEADERS_RESPONSE page and `snw_walk` for the DSN1 record, each bounding every count against the remaining bytes before it walks), and its `lp_json` branch is deleted, so an lp-JSON artifact under any type is MALFORMED. The "same rule, two binaries" convention is restored at the frame level, gated by `tools/test_light_decode_wire.sh` legs 8-9.
...
**F-10 gated shipping and is now CLOSED.** F-1 through F-5 are advisory. F-6, F-7 and F-9 were open gaps that this change set did not make worse (F-6's last residual retired with D2 inc7c, 2026-09-16). **F-10 was a hazard this change set INTRODUCED** — it did not exist without the WIRE-2 depth ceiling, and WIRE-3 escalated its consequence. It was found by adversarial audit of the fix itself, not of the original code, which is the argument for auditing changes with the same lenses used on the code they harden. It was closed at the root (`claims_json` canonicalized on ingest) rather than by retuning the ceiling, because the accept/reject band is exactly 2 wide for *every* cap value. **F-7 remains an open gap; F-6's JSON-envelope premise is closed-by-deletion (D2) and its binary-path in-ceiling residual retired with D2 inc7c (2026-09-16)** — §2.4 family 1 keeps its historical MITIGATED reading with the dated retirement note.
...
| ~~**WIRE-2** (accept / string state / nodes / binary bypass)~~ — RETIRED 2026-09-16 (D2 inc7c) | formerly `test-binary-codec` (8d-8h) | The four WIRE-2 legs pinned the structural ceiling on the length-prefixed JSON payloads; the ceiling and the payloads are deleted | — |
| **BINARY-ONLY** (D2 inc7c) | `test-binary-codec` (8d-8h) | An lp-JSON SNAPSHOT_RESPONSE / HEADERS_RESPONSE body carrying a VALID document is **rejected** (and the same DOM round-trips as its frame); an unknown MsgType byte is rejected on decode by name and refused on encode; a 2 GB `json_len` prefix under SNAPSHOT_RESPONSE dies at the DSN1 magic after 8 bytes | Restoring the length-prefixed JSON fallback in `decode_binary` → **RED** |
| **Frame count bounds** (D2 inc7c) | `test-headers-frame-codec` (HF-3d/3e/3f), `test-snapshot-response-frame-codec` (SR-3d/3e) | HEADERS_RESPONSE: 256 records decode, 257 are rejected by the page cap before any record is parsed; a count-lie and a `frame_len` past the buffer are rejected before any allocation. SNAPSHOT_RESPONSE: an accounts count-lie is rejected before any entry is read; 257 tail headers backed by zero bytes are rejected by the cap before any frame | Skipping the cap / the byte proof / the `frame_len` bound → **RED** |
...
- `include/determ/net/messages.hpp:146-174` — `max_message_bytes(MsgType)` cap table (this proof's primary object).
...
- ~~`include/determ/net/messages.hpp` — `kMaxJsonDepth` / `kMaxJsonNodes` structural ceilings + the threat-model, sizing and soundness commentary (T-6)~~ — DELETED in D2 inc7c (2026-09-16); the surviving cap surface is `kMaxFrameBytes` (`messages.hpp:117`), `kHeadersPageMax` (`messages.hpp:123`) and `max_message_bytes` (`messages.hpp:146-174`).
- `src/net/messages.cpp` — the WIRE-1 pre-decode per-type cap and the D2 binary-only magic reject, both in `Message::deserialize` (T-6 clause 1 closed by deletion; the JSON-branch pre-parse call is gone with the branch). `json_structural_precheck` lived here until D2 inc7c deleted it (2026-09-16).
...
- `include/determ/net/messages.hpp:117` — `kMaxFrameBytes` framing-layer outer ceiling (16 MB).
- `include/determ/net/messages.hpp:146-174` — `max_message_bytes(MsgType)` per-type cap function (the proof's primary object).
- `include/determ/net/messages.hpp:176-189` — `Message` struct + `serialize_binary` / `deserialize` (D2: `Message::serialize`, the JSON envelope encoder, is deleted).
