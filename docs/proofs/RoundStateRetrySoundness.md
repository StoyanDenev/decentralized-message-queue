...
**Deterministic rejection is outside T-2 (S-050, 2026-07-15).** T-2's delivery argument assumes a re-delivered message is *admissible* at the receiver. A **round-state fork** breaks that: two abort quorums for the same round can form concurrently against different creators, the hash-chained abort tail then forks by adoption order, and — because each abort `event_hash` is folded into the committee seed — the diverged sides re-derive different committees and `delay_output`s. From then on every relayed `BlockSig` is dropped **deterministically** (`mismatched delay_output`), and relayed abort events fail the receiver's committee-view admission: re-delivery re-enters the same reject, so no retry cadence converges. Reproduced live on a clean network under CPU starvation (`test-fa-partition-virtual`, CI-contention round). The complementary mechanism is the **S-050 stall valve** (`Node::maybe_stall_reset_locked`): when round timers keep firing but **no block has been applied for a wall-clock window** (blocks are the only progress signal — abort transitions deliberately do not clear the counter, or abort-churn would suppress the valve), the node resets its round *scratch* state — including the abort tail — to the canonical post-head state, re-enters selection, and re-probes peers (`STATUS_REQUEST` + tolerance-0 sync). The valve preserves T-1's consensus-invisibility argument (blocks self-certify their abort events; scratch state is never load-bearing for validation) and makes the round-state-fork wedge non-absorbing: a reset node re-adopts one peer's tail in that peer's chain order via this document's relay. See `AdversarialTransportHarness.md` §3.4 for the empirical record.
...
- **Re-sync probing (narrowed 2026-07-15).** The catch-up `STATUS_REQUEST` is emitted once at boot grace, inside a one-shot grace timer — the S-047 retry is a *round-layer* delivery mechanism, not a *chain-sync* mechanism. The **S-050 stall valve** now adds a second, stall-triggered probe: a node whose round has stalled past the valve threshold re-broadcasts `STATUS_REQUEST` and syncs with tolerance 0, so a height-stranded node that still runs rounds self-heals. An **idle off-committee** node has no round timers, so for it recovery remains operational (restart / resync).
