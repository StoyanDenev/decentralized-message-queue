...
*Mitigation.* Mechanical refactor only. Production behavior is unchanged because `RealClock` and `AsioTransport` are pass-through wrappers. Existing 99 in-process unit tests + integration tests catch regressions immediately.
