...
**Status:** design spec + **IMPLEMENTED (increments 1-7, owner-authorized R61).** The self-contained simulation core (virtual clock + deterministic scheduler + virtual-network fault SEAM + byte-stable trace + safety/liveness property loop) ships under `sim/` with the `determ-dsf` runner and **41 baked scenarios** (incl. the §Q5 randomized generator's THREE templates — 6 reliable-broadcast variants + a self-test (inc-4), 6 single-value-flood agreement variants + a self-test (inc-6), and 6 monotone-ratchet variants + a self-test (inc-7)): 6 framework-demonstration (increment 1) + 14 adversarial (increments 2-3) covering the §Q7 families equivocation, cross-shard receipt conservation, selective-abort bias, BFT-escalation monotonicity, DKG threshold, F2 view-reconciliation, and partition quorum — each adversarial family paired with an `expect_violation` self-test proving its checker fires on a planted bug. **Increment 5 exposes the §Q5 generator on the command line: `--generate N --seed S [--template broadcast|agree]` registers N reproducible `gen_run_NN` variants drawn from S (count-parametric, generator-seed byte-stable + seed-varied), listable and runnable, each byte-identically replayable (§Q6) — the "CI runs N variants overnight; a failed variant's seed reproduces it bit-for-bit" contract. Increment 6 adds the SECOND generator template (single-value-flood agreement, a §Q7 equivocation/partition checker family) and increment 7 the THIRD (monotone ratchet / non-regression, a §Q7 BFT-escalation/commit-index checker family), both selected via `--template`.** Determinism is byte-identical per (scenario, seed); gated by `tools/test_dsf_{core,inc2,inc3,inc4,inc5,inc6,inc7}.sh`. **STILL FUTURE:** the §Q1/§Q2 production dependency-injection (threading `time::Clock` + `net::Transport` through real Node/Validator/Producer) so scenarios drive the actual consensus engine, and the four production invariants (FA1/A1/FA6/FA7) — the toy SimState stands in for now. Resolves the design for S-035 Option 2. **Recommended sequencing: ship before Phase A** (rather than as a Phase D prerequisite as previously planned) — subsumes A10 NH1 Stage 1 streams 1 + 2, provides Byzantine coverage for every Phase A through D item as it lands.
...
**IMPLEMENTED (increments 4-6).** `sim/generator.hpp` — `draw_params(SplitMix64&)` draws a fault profile (followers/latency/jitter/drop/dup) as a pure function of a generator seed; `register_generated_scenarios(out, gen_seed, count, name_prefix, with_selftest, tmpl)` emits `count` reproducible variants of a `GenTemplate`. **Three templates ship:** (a) `make_broadcast_variant()` — a reliable-broadcast template (leader re-broadcasts the issued total as an idempotent monotone `SET(total)`, follower keeps `count=max`) so every variant never overcounts and always converges; (b) `make_agreement_variant()` (increment 6) — a single-value-flood agreement template (leader floods one decision value V, follower latches the FIRST value first-write-wins) so every honest variant never splits (`agree_no_split`) and always decides (`agree_all_decided`); (c) `make_ratchet_variant()` (increment 7) — a monotone-ratchet template (leader ramps a ceiling and re-floods it, each follower keeps a monotone high-water mark and commits it) so every honest variant's committed value never regresses (`ratchet_no_regress`) and always advances to the ceiling (`ratchet_advanced`). Each template carries a self-test proving a generated fault profile surfaces a real bug: `gen_overcount_selftest` (non-idempotent apply under forced dup) / `gen_disagree_selftest` (a Byzantine-equivocating leader) / `gen_regress_selftest` (a raw-committing follower under a Byzantine decreasing leader → the committed value drops below its high-water). Increment 4 baked 6 `gen_broadcast_NN`; increment 6 baked 6 `gen_agree_NN`; increment 7 baked 6 `gen_ratchet_NN`; **increment 5 wires the CLI** — `determ-dsf --generate N --seed S [--template broadcast|agree|ratchet] [--list | --scenario gen_run_NN]` registers N `gen_run_NN` variants of the chosen template drawn from S (no self-test on this path). Two-seed model: fault PARAMETERS = f(generator seed); fault REALIZATION = f(run seed); the `--generate N --seed S` form collapses both to one seed for a single-command reproducible run. Gated by `tools/test_dsf_{inc4,inc5,inc6,inc7}.sh`. **The three templates are one code pattern (`GenTemplate` enum + `make_variant` dispatcher + a per-template planted-bug self-test twin) — the documented recipe for a 4th.**
