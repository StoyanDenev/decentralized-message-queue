...
**Status:** design spec + **IMPLEMENTED (increments 1-10, owner-authorized R61).** The self-contained simulation core (virtual clock + deterministic scheduler + virtual-network fault SEAM + byte-stable trace + safety/liveness property loop) ships under `sim/` with the `determ-dsf` runner and **62 baked scenarios** (incl. the §Q5 randomized generator's SIX templates — 6 reliable-broadcast variants + a self-test (inc-4), 6 single-value-flood agreement variants + a self-test (inc-6), 6 monotone-ratchet variants + a self-test (inc-7), 6 quorum/threshold-commit variants + a self-test (inc-8), 6 receipt-conservation variants + a self-test (inc-9), and 6 view-reconciliation variants + a self-test (inc-10)): 6 framework-demonstration (increment 1) + 14 adversarial (increments 2-3) covering the §Q7 families equivocation, cross-shard receipt conservation, selective-abort bias, BFT-escalation monotonicity, DKG threshold, F2 view-reconciliation, and partition quorum — each adversarial family paired with an `expect_violation` self-test proving its checker fires on a planted bug. **Increment 5 exposes the §Q5 generator on the command line: `--generate N --seed S [--template broadcast|agree|ratchet|quorum|conserve|recon]` registers N reproducible `gen_run_NN` variants drawn from S (count-parametric, generator-seed byte-stable + seed-varied), listable and runnable, each byte-identically replayable (§Q6) — the "CI runs N variants overnight; a failed variant's seed reproduces it bit-for-bit" contract, realized operationally by `tools/operator_dsf_sweep.sh`. Increment 6 adds the SECOND generator template (single-value-flood agreement, a §Q7 equivocation/partition checker family), increment 7 the THIRD (monotone ratchet / non-regression, a §Q7 BFT-escalation/commit-index checker family), increment 8 the FOURTH (quorum/threshold commit, a §Q7 DKG-threshold/BFT-pre-vote checker family), increment 9 the FIFTH (receipt conservation, the §Q7 FA7 no-double-credit checker family), and increment 10 the SIXTH (F2 view reconciliation, the §Q7 no-phantom-evidence checker family), all selected via `--template`.** Determinism is byte-identical per (scenario, seed); gated by `tools/test_dsf_{core,inc2,inc3,inc4,inc5,inc6,inc7,inc8,inc9,inc10}.sh`. **STILL FUTURE:** the §Q1/§Q2 production dependency-injection (threading `time::Clock` + `net::Transport` through real Node/Validator/Producer) so scenarios drive the actual consensus engine, and the four production invariants (FA1/A1/FA6/FA7) — the toy SimState stands in for now. Resolves the design for S-035 Option 2. **Recommended sequencing: ship before Phase A** (rather than as a Phase D prerequisite as previously planned) — subsumes A10 NH1 Stage 1 streams 1 + 2, provides Byzantine coverage for every Phase A through D item as it lands.
