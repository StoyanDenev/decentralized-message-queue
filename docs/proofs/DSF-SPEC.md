...
**Status:** design spec + **IMPLEMENTED (increments 1-5, owner-authorized R61).** The self-contained simulation core (virtual clock + deterministic scheduler + virtual-network fault SEAM + byte-stable trace + safety/liveness property loop) ships under `sim/` with the `determ-dsf` runner and **27 baked scenarios** (incl. the §Q5 randomized generator's 6 reproducible fault-profile variants + a self-test, inc-4): 6 framework-demonstration (increment 1) + 14 adversarial (increments 2-3) covering the §Q7 families equivocation, cross-shard receipt conservation, selective-abort bias, BFT-escalation monotonicity, DKG threshold, F2 view-reconciliation, and partition quorum — each adversarial family paired with an `expect_violation` self-test proving its checker fires on a planted bug. **Increment 5 exposes the §Q5 generator on the command line: `--generate N --seed S` registers N reproducible `gen_run_NN` reliable-broadcast variants drawn from S (count-parametric, generator-seed byte-stable + seed-varied), listable and runnable, each byte-identically replayable (§Q6) — the "CI runs N variants overnight; a failed variant's seed reproduces it bit-for-bit" contract.** Determinism is byte-identical per (scenario, seed); gated by `tools/test_dsf_{core,inc2,inc3,inc4,inc5}.sh`. **STILL FUTURE:** the §Q1/§Q2 production dependency-injection (threading `time::Clock` + `net::Transport` through real Node/Validator/Producer) so scenarios drive the actual consensus engine, and the four production invariants (FA1/A1/FA6/FA7) — the toy SimState stands in for now. Resolves the design for S-035 Option 2. **Recommended sequencing: ship before Phase A** (rather than as a Phase D prerequisite as previously planned) — subsumes A10 NH1 Stage 1 streams 1 + 2, provides Byzantine coverage for every Phase A through D item as it lands.
...
**IMPLEMENTED (increments 4-5).** `sim/generator.hpp` — `draw_params(SplitMix64&)` draws a fault profile (followers/latency/jitter/drop/dup) as a pure function of a generator seed; `make_broadcast_variant()` is a reliable-broadcast template (leader re-broadcasts the issued total as an idempotent monotone `SET(total)`, follower keeps `count=max`) so every generated variant never overcounts and always converges under any drawn profile; `register_generated_scenarios(out, gen_seed, count, name_prefix, with_selftest)` emits `count` reproducible variants. Increment 4 baked 6 `gen_broadcast_NN` variants + a `gen_overcount_selftest`; **increment 5 wires the CLI** — `determ-dsf --generate N --seed S [--list | --scenario gen_run_NN]` registers N `gen_run_NN` variants drawn from S (no self-test on this path). Two-seed model: fault PARAMETERS = f(generator seed); fault REALIZATION = f(run seed); the `--generate N --seed S` form collapses both to one seed for a single-command reproducible run. Gated by `tools/test_dsf_{inc4,inc5}.sh`.
