...
**Status:** design spec + **IMPLEMENTED (increments 1-4, owner-authorized R61).** The self-contained simulation core (virtual clock + deterministic scheduler + virtual-network fault SEAM + byte-stable trace + safety/liveness property loop) ships under `sim/` with the `determ-dsf` runner and **27 scenarios** (incl. the §Q5 randomized generator's 6 reproducible fault-profile variants + a self-test, inc-4): 6 framework-demonstration (increment 1) + 14 adversarial (increments 2-3) covering the §Q7 families equivocation, cross-shard receipt conservation, selective-abort bias, BFT-escalation monotonicity, DKG threshold, F2 view-reconciliation, and partition quorum — each adversarial family paired with an `expect_violation` self-test proving its checker fires on a planted bug. Determinism is byte-identical per (scenario, seed); gated by `tools/test_dsf_{core,inc2,inc3,inc4}.sh`. **STILL FUTURE:** the §Q1/§Q2 production dependency-injection (threading `time::Clock` + `net::Transport` through real Node/Validator/Producer) so scenarios drive the actual consensus engine, and the four production invariants (FA1/A1/FA6/FA7) — the toy SimState stands in for now. Resolves the design for S-035 Option 2. **Recommended sequencing: ship before Phase A** (rather than as a Phase D prerequisite as previously planned) — subsumes A10 NH1 Stage 1 streams 1 + 2, provides Byzantine coverage for every Phase A through D item as it lands.
