...
**Status: increments 1-11 SHIPPED.** This is
...
frame-duplication fault + dup phase), increment 8 (the per-step FA CHECKERS —
`FaStepMonitor` riding every drain-boundary predicate of both harnesses, with
expect-violation self-tests), increment 9 (fault-delivery WITNESSES —
`fault_counts()`, every fault phase proves its fault fired, RED-on-mutant
executed — plus the composed loss+dup phase), and increment 10 (per-link
CONSTANT latency — the REORDER dimension, network-held deliveries fired via
`GlobalScheduler::attach_network`), and increment 11 (the CROSS-TOOLCHAIN
schedule-signature diff — `--signature` emission +
`tools/operator_fa_determinism_diff.sh`; the MSVC and GCC binaries produce
BYTE-IDENTICAL signatures for both harnesses) are SHIPPED** (pure-std test
backend,
...
| 9 | **Fault-delivery witnesses** — **SHIPPED**: per-link `n_dropped`/`n_blocked`/`n_dup` counters summed by `VirtualNetwork::fault_counts()`; every fault phase gates its delta > 0 (a silently no-opped fault can no longer green a phase vacuously — a review probe demonstrated the vacuity first) and replay pins the EXACT terminal counts; falsify-on-mutant executed (all three faults zeroed → exactly the witness check RED). Plus the composed loss+dup phase pinning the shared-rng interleaved draw stream, the `agree_upto` vacuity guard, and the max-height dup baseline. | mutant-RED + restored-GREEN + both platforms + FAST 240/0 ✓ |
| 10 | **Per-link latency / REORDER** — **SHIPPED**: `VirtualNetwork::set_latency(min,max)` assigns each link a CONSTANT latency derived from its immutable pseudo-port id via splitmix (deliberately NOT a fault-rng draw — the drop/dup streams are byte-identical with and without latency). Constant-per-link is the honest TCP model: per-connection FIFO preserved, REORDER across heterogeneous links. A latencied frame's write completes immediately; its delivery is a NETWORK-held pending entry ordered by (arrival, seq) — never a loop timer (loop objects may die before pairs; the entry pins the Pair) — fired by `GlobalScheduler::attach_network`'s extended event selection (earliest of loop timers ∪ pending arrivals, deliveries win ties, trace tag `N@ms`). Lock discipline: write path holds `Pair::mu` → takes network `mu_` (allowed); `deliver_next` pops under `mu_` and RELEASES it before `Pair::mu` (the `Peer::do_write` ABBA lesson applied at design time). A frame arriving after the destination closed is discarded ("packet after FIN"). Deterministic-drive-only knob, ENFORCED not just documented (review): the write path gates on `drive_attached()` — with no scheduler attached, latency falls through to immediate delivery, so an unattached network can never silently blackhole frames. Review hardening: `flush_pending()` + a ready-work-only settle run BEFORE `set_latency(0,0)` (a stranded pending frame delivered after newer immediate frames on the same link would violate per-connection FIFO; no phase ends with in-flight state), and a HETEROGENEITY witness (`latency_diversity() >= 2` hard-gated — a constant-latency regression cannot fake the reorder claim). `test-fa-adversarial-deterministic` Phase 6: +2 NEW blocks under heterogeneous 5-30 ms latency interleaved with the round timers, settled agreement, the delayed-frames witness in-phase, exact delayed count pinned in replay; falsify-on-mutant executed (zeroed `latency_for` → exactly the Phase-6 check RED). | both platforms + witness RED-on-mutant + byte-neutrality re-runs (net-virtual/crash/multiloop) + FAST 240/0 ✓ |

| 11 | **Cross-toolchain schedule-signature diff** — **SHIPPED**: both deterministic subcommands accept `--signature` and print one machine-parseable line (`SIGNATURE v1 <kind> trace=<sha256 of the concatenated action traces> faults=<exact delivery counts> n0..n4=<full head_hash>:<full state_root>`, + kill height for the crash harness); zero output change without the flag. `tools/operator_fa_determinism_diff.sh` runs BOTH platform binaries (SKIP-clean below two; sole normalization = the MSVC text-mode trailing CR) and byte-compares per subcommand — mismatch = `CROSS-TOOLCHAIN DIVERGENCE` exit 1; gated by `tools/test_operator_fa_determinism_diff.sh` (6 stub-based contract assertions, real binaries never invoked; both tools deliberately NOT in FAST). **Evidence at ship: MSVC and GCC signatures are BYTE-IDENTICAL for both harnesses** — the full adversarial schedule (loss/partition/heal/valve/dup/composed/latency) and the crash/failover/rejoin schedule, including every fault count and all ten terminal head/state-root pairs, are cross-toolchain deterministic (the FA-level extension of the golden-vector cross-toolchain gate that once caught a real `state_root` fork). | tool PASS on the real MSVC+GCC pair under WSL interop + gate 6/6 + FAST 240/0 ✓ |
