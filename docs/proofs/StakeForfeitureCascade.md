...
**Code witness.** `src/chain/chain.cpp:1813–1825` (Phase 4 equivocation forfeit loop — reads `sit->second.locked` only, never `unlock_height`); `src/chain/chain.cpp:1266–1270` (REGISTER's `unlock_height = UINT64_MAX` init) + the DEREGISTER `unlock_height` write that established the pre-`B_e` state — preserved across `B_e` because Phase 4 never reads or writes it; `src/chain/chain.cpp:1864–1888` (apply-tail A1 fold).

**Gate (executed).** The anti-dodge property T-C1 rests on — a `staked-pending-unlock` (finite-`unlock_height`) equivocator forfeits its ENTIRE locked stake — is now driven falsify-on-mutant by `determ test-equivocation-multi` scenario 4 (`src/main.cpp`, "Pre-deactivated equivocator"): alice DEREGISTERs at block 1 (setting `unlock_height > 2`) then equivocates at block 2 (strictly inside the deferred-unlock window), and the test asserts `c.stake("alice") == 0` **and** `c.accumulated_slashed() == 500` (the genesis stake fully forfeited + folded into A1). Before this, scenario 4 asserted ONLY the registry override (`inactive_from == 3`); a **surgical mutant** that makes the forfeit `unlock_height`-sensitive (skip the slash when `b.index < unlock_height`, leaving registry deactivation intact) dodged forfeiture yet passed every scenario — the whole `unstake_delay` slashing-window design's anti-dodge property was unproven-by-gate. *Falsify-on-mutant:* that mutant flips the two new stake assertions RED while `inactive_from == 3` stays green (registry deactivation is a separate write). The check sits in the apply path (past `check_block_sigs`), so only the in-process apply-driver reaches it. (Round-10 apply-path invariant audit `wf_a941ce55`.)
