...
**Code witness.** `src/node/validator.cpp:728-736` (the gate; the `b.index < unlock` comparison is line 730).

**Test witness (direct falsify-on-mutant).** `determ test-unstake-deregister-apply` (`tools/test_unstake_deregister_apply.sh`) now drives `BlockValidator::check_transactions` in isolation through the `check_transactions_for_test` const-forwarder seam. The leg builds a genesis creator with a *real* Ed25519 keypair (so the tx signature verifies — `check_transactions` checks it at `validator.cpp:684`) whose genesis stake is locked (`stakes_[d].unlock_height == UINT64_MAX`), hand-assembles a signed, otherwise-well-formed UNSTAKE at `b.index = 1 ≪ unlock_height`, and asserts `check_transactions` rejects it with the specific `"UNSTAKE before unlock_height"` diagnostic. A positive control runs the identical tx at `b.index == unlock_height` (`UINT64_MAX`), which clears the gate (`b.index < unlock` is false) — isolating the comparison at `validator.cpp:730` as the sole cause of the reject. **Falsify-on-mutant:** neutering `validator.cpp:730` to `if(false)` (or deleting the `if (tx.type == UNSTAKE)` block) lets the otherwise-valid tx pass, flipping the negative leg RED while the control stays green. This closes the round-3 proof-claim gate-gap (`wf_97a30e14`, the same class as the `check_cumulative_rand` gap closed in `1b53344`): before this leg the reject string was asserted in **no** test — the earlier reliance on `test_governance_param_change.sh` + code inspection did not *falsify* the validator gate, because the apply-time refund (T-3) kept the suite green under an accept-widening mutant. The producer gate T-1 (`producer.cpp`, same-shape check against the same `chain.stake_unlock_height` getter) is covered by inspection from the same fixture. Structural alignment with the apply gate remains verified via the shared `chain.stake_unlock_height(tx.from)` dependency.
