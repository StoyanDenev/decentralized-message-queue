...
> **⚠ SUPERSEDED — THE CURVE DECISION (§2) AND D1 (§8) ARE RESOLVED; secp256k1 REJECTED.** This document was written while the confidential-tx curve was an OPEN choice between P-256 (Option A) and secp256k1 (Option B); it repeatedly frames secp256k1 as "the plan of record" and P-256 as "a deviation from Stoyan's choice." **That framing is stale and inverted.** On **2026-07-07** the owner **REJECTED secp256k1** (a Koblitz curve) — see `DECISION-LOG.md` 2026-07-07 (D3) and `CRYPTO-C99-SPEC.md` profile table (*"NIST P-256 (profile-agnostic; secp256k1 rejected 2026-07-07)"*). The confidential-tx / Bulletproofs / Pedersen / range-proof path IS **NIST P-256** (the shipped `src/crypto/pedersen/` §3.19 stack + the §3.22 profile-agnostic shielded pool) — i.e. **Option A is the decision.** Therefore: **Option B (build a secp256k1 range-proof library via libsecp256k1-zkp) is MOOT** — secp256k1 was NEVER built (no `src/crypto/secp256k1/` exists), and P-256 is NOT a deviation, it IS the owner's choice. CT over P-256 is consequently **FIPS-profile-AVAILABLE** (not MODERN-only). `ristretto255` was likewise eliminated entirely (CRYPTO-C99-SPEC.md §Q2) and is not in the stack. Read every "Option B", "secp256k1 plan of record", "curve deliberately not decided", and FIPS-unavailability passage below as **historical design context, not current direction.** The other owner decisions (D2 adopt-at-all, D3 state-model, D4 rollout, D5 default, D6 CT-hardening) remain genuinely open; only the curve (D1) is closed.
...
- **The chain-level Bulletproofs plan of record WAS secp256k1** — but on 2026-07-07 the owner REJECTED secp256k1 (a Koblitz curve) and resolved this decision to **NIST P-256** (Option A below); see `DECISION-LOG.md` 2026-07-07 (D3). The CRYPTO-C99-SPEC.md quotes that follow are the now-SUPERSEDED record of the abandoned secp256k1 plan (several still stale in that file), retained here only as history:
...
- **FIPS-profile compatible.** P-256 is FIPS 186-5 validated; secp256k1 is not on NIST's list. Confidential transactions over P-256 (Pedersen + Bulletproofs, §3.19 / the §3.22 shielded pool) ARE deployable in FIPS profiles — built on FIPS-approved primitives (P-256 + SHA-256), though the range-proof construction is not itself a NIST-standardized / FIPS-validated algorithm (a per-op-CMVP deployment treats it as out-of-module). The spec's earlier "UNAVAILABLE in FIPS" recording assumed secp256k1; that has been corrected to AVAILABLE (P-256).
...
- **Transition.** Because it is opt-in and additive, there is no flag-day. A genesis/param gate (`confidential_enabled`) can turn the feature on for a deployment. (Curve now resolved to **NIST P-256** — secp256k1 rejected 2026-07-07, `DECISION-LOG.md` — so CT is **FIPS-profile-available** and is NOT gated off on curve grounds; the old "secp256k1 ⇒ MODERN-only" conditional no longer applies.)
...
| `docs/proofs/CRYPTO-C99-SPEC.md` §2.Q1/Q2 (lines 40-79), §3.7 (line 616), the profile table (lines 481-487), §3.19 (lines 1157-1279) | The curve history: §3.7 records secp256k1 as **DE-SCOPED (2026-07-03)** and the profile table records **secp256k1 rejected 2026-07-07 → NIST P-256** as the resolved prime-order/Bulletproofs curve (so CT over P-256 is **FIPS-AVAILABLE**); §3.19 is the shipped P-256 range-proof library entry. §2. |
