...
**Companion documents.**
- `StateRootAnchorSoundness.md` (SR-1..SR-5; the §3.3 transitive-forward binding; `committee_bound_state_root` at `trustless_read.cpp:333-411`) — the **online sibling**. The bundle's anchor binding (OSB-3) is SR-1's interior-regime statement evaluated over bundle bytes; the bundle's "head not bundleable" disposition is SR-1's head-regime fail-closed boundary (`StateRootAnchorSoundness.md §6.3`) moved to **export time**.
...
The bundle's anchor binding (OSB-3) is SR-1's **interior-regime** statement evaluated over bundle bytes: it reuses `committee_bound_state_root` (`trustless_read.cpp:333-411`) — the exact helper SR-1 §3.4 / §6.4 documents as the mechanized binding — except the FULL anchor block, the successor header, and the proof come from the bundle, not live RPC. The "head not bundleable" disposition (OSB-F2) is SR-1's **head-regime** fail-closed boundary (`§6.3`), moved to EXPORT time: the head has no committee-signed successor, so `committee_bound_state_root` throws during export and no head bundle is written. The pre-S-033 rejection (OSB-F3) is SR-5's vacuous-binding honesty applied at gate (7).
...
| OSB-3 | online binding helper reused | `light/trustless_read.cpp:333-411` | `committee_bound_state_root` — same logic, live RPC. |
