...
For `submit_equivocation` (the sixth method — since D4, 2026-09-16, it mutates no L1 state at all), the apply-time replay defense is now trivial rather than idempotent-by-accounting: `Chain::apply_transactions` reads NOTHING from `b.equivocation_events`, so re-applying the same `EquivocationEvent` any number of times changes no leaf of the state root (`EquivocationSlashingApply.md` T-E0). The pre-D4 T-E3 argument ("the second apply finds `locked == 0`") is historical. What replay still costs is block bytes and two Ed25519 verifies per included event at V11 — a DoS surface with no remaining limiter, recorded as sequence step 3b (the per-block cap + in-block duplicate rejection) and NOT closed here.

So the end-to-end replay statement is: even under `A_outside` with HMAC replay, and under `A_inside` with the attacker computing fresh valid HMACs over a stale transaction, the apply-layer gate at `chain.cpp:739` (for tx-based methods) or, for evidence submission, the fact that apply reads nothing from the event at all (D4) provides the second-line defense. The HMAC layer is not required to bind a nonce because the apply layer already does — and the apply layer is the consensus-binding correctness gate (per `RpcInputValidationDefense.md` T-5).
...
The cross-reference is: `rpc_submit_equivocation` (one of the six state-mutating RPC methods enumerated in T-3) accepts an `EquivocationEvent` constructed by an external party (e.g., a watchdog that observed equivocation off-chain and wants to submit evidence). The RPC layer's auth gate (T-1) ensures only authenticated callers can submit; and the apply layer is state-neutral on the event (FA-Apply-10 T-E0, D4), so re-application is a no-op by construction. The residual is bytes, not state: an unbounded re-submission is the step-3b DoS item.
...
- **`docs/proofs/EquivocationSlashingApply.md`** — FA-Apply-10 (the apply layer for equivocation evidence). T-E0 (apply is state-neutral on the event, D4) cited as the replay-defense backstop for `submit_equivocation` in T-2 here; the pre-D4 T-E3 idempotence argument is historical.
