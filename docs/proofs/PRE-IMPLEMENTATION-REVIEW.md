...
- [ ] **C99-11: Dual-profile bundling accepted.** `CryptoProfile { MODERN, FIPS }` bundled into `TimingProfile` rather than orthogonal genesis field. `cluster` + `tactical` bundle FIPS (AES-256-GCM, PBKDF2, NIST P-256 + P-256 Bulletproofs); `web` + `regional` + `global` bundle MODERN (XChaCha20-Poly1305, Argon2id, big-prime Z_p* Bulletproofs). Notes that FIPS-profile confidential-tx (the §3.22 P-256 shielded pool) is built on FIPS-approved primitives but its Bulletproofs construction is not itself a FIPS-validated algorithm (per-op-CMVP: out-of-module). Crypto is a posture, not a code switch. Alternative: orthogonal profile axis (5 timing × 2 crypto = 10 combinations).
