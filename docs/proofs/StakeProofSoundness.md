...
### 4.3 Theorem SP-3 (`leaf_count` caller-trust boundary; single-envelope sourcing)

**Statement.** SP-2's soundness is stated with `n` = the *genuine* leaf count of the tree `R` commits to. Because `merkle_verify` does **not** bind `leaf_count` into any leaf or inner hash (the **S-040** caller-trust invariant; `MerkleTreeSoundness.md` §6.2), the soundness of the `s:` read additionally requires that the light client source `leaf_count` from the *same committee-anchored origin* as `state_root`. The shipped `stake-trustless` flow discharges this obligation: `state_root` and `leaf_count` arrive in one `rpc_state_proof` reply, and that reply's `state_root` is committee-anchored by SP-1 in the same invocation. Hence SP-3 holds operationally and the split-source attack is unreachable.
...
**Why the shipped flow is safe.** The danger is exclusively the *split-source* case: proof + `leaf_count` from one untrusted channel, `state_root` from a different (trusted) channel, with the two inconsistent. In `stake-trustless`:

1. `Node::rpc_state_proof` returns `state_root`, `leaf_count`, `target_index`, `value_hash`, and `proof` as fields of **one** JSON object (`node.cpp:3325-3335`), computed atomically under the daemon's `state_mutex_` read lock (`node.cpp:3289`) over a single consistent state. The `leaf_count` is `chain_.state_proof(...).leaf_count = build_state_leaves().size()` (`chain.cpp:456`) — the genuine count for the very tree whose root is `compute_state_root()` in the same reply (`node.cpp:3333`).
2. The light client anchors *that reply's* `state_root` to a committee-signed header (SP-1) in the same invocation, then verifies the `s:` proof against the anchored root using *that reply's* `leaf_count` (`verify.cpp:308-335`). There is no second channel; the `(state_root, leaf_count)` pair is consistent-by-construction because the daemon computed both from one locked state snapshot, and the anchoring step rejects any `state_root` the committee did not sign.

If a malicious daemon serves an inconsistent `(state_root, leaf_count)` — e.g., the genuine `leaf_count` but a tampered `state_root` — the tampered root fails SP-1's committee anchoring (the proof's `state_root` won't match a committee-signed header, throwing at `trustless_read.cpp:269-275`). If it serves a tampered `leaf_count` with the genuine `state_root`, then by SP-2's exact-consume + range gates the wrong walk shape generically fails root-equality against the genuine `R` for the genuine `s:` leaf; and even in the degenerate same-walk-shape coincidence of S-040, the *value* the verifier accepts is still the genuine `s:` leaf at that position under the genuine root — the daemon gains no ability to substitute a *wrong* `(locked, unlock_height)`, only (at most) to mislabel which sorted slot it occupies, which the `key_s(D)`-binding of SP-2 still ties to `D`. The operational obligation is therefore met: **source `leaf_count` from the same committee-anchored envelope as `state_root`** (`MerkleTreeSoundness.md` §6.2; `docs/SECURITY.md` §S-040), which the single-reply shape enforces structurally.

**Status.** S-040 is a registered **Low/Op** caller-trust item, orthogonal to SP-2's soundness (which holds with the genuine `leaf_count`). The structural fix — domain-separating `leaf_count` into the leaf hash — breaks v1 wire compatibility and is tracked for a future v2.x flag-day (`MerkleTreeSoundness.md` §6.2). SP-3 records that the shipped `stake-trustless` flow does not split the source and is therefore unaffected.

**Concrete-security bound.** SP-3 adds no independent term beyond SP-1 + SP-2 under single-envelope sourcing; a split-source integrator would expose the S-040 surface, which SP-3 directs them to avoid.   ∎
...
SP-2 *is* MT-4 applied at the `s:` leaf; it asserts no new cryptographic claim. MT-1 (determinism) guarantees the `s:` leaf's position and the root are reproducible across honest nodes; MT-2 (domain separation) underwrites the cross-namespace-swap rejection that stops an `a:` balance leaf from masquerading as an `s:` stake leaf; MT-3 (collision-resistance inheritance) is the divergence-detection that makes any wrong committed stake state produce a different root. SP-3 is the `stake-trustless`-specific statement of MT-4's §6.2 (S-040) operational obligation.
...
### 6.3 S-040 leaf_count caller-trust (SP-3)

Recorded in §4.3: sound under the shipped single-envelope sourcing; a future split-source integrator must source `leaf_count` from the committee-anchored envelope. Low/Op; structural fix tracked for a v2.x wire-breaking flag-day.
...
| `tools/test_merkle_proof_tampering.sh` + `determ test-merkle-proof-tampering` (15 scenarios) | SP-2 tamper rejection; SP-3 / S-040 `leaf_count` limitation (#12). |
...
- **Theorems.** SP-1 (committee-signed `state_root` binds the `s:` leaf — reduces to A1 forgery on `h+1` or A2 collision at `h`, via SR-1); SP-2 (`s:` Merkle inclusion soundness — MT-1/MT-3/MT-4 at the `s:` leaf); SP-3 (`leaf_count` caller-trust boundary — S-040, discharged by single-envelope sourcing). Corollary SP-E composes them with T-L1 + T-L2 to the same `≤ 2⁻⁹²` end-to-end bound as the balance read.
- **Composes with.** `MerkleTreeSoundness.md` (MT-1/MT-3/MT-4 + §6.2 S-040), `StateRootAnchorSoundness.md` (SR-1/SR-2/SR-3), `S033StateRootNamespaceCoverage.md` (T-1/T-2/T-3/T-4 + §2.1 `s:` row), `LightClientThreatModel.md` (T-L1/T-L2/T-L3/T-L4 + `A_daemon` + L-6), `BlockchainStateIntegrity.md` (S-021 + S-033 + S-038 prerequisite).
- **Known limitations.** No non-membership for unstaked domains (§6.1); `unlock_height = UINT64_MAX` is a committed value not an absent-sentinel (§6.2); S-040 `leaf_count` caller-trust, mitigated by single-envelope sourcing (§6.3). None undermine the per-invocation soundness claim.
...
- `docs/proofs/MerkleTreeSoundness.md` — MT-1 (determinism), MT-2 (domain separation), MT-3 (collision-resistance inheritance), MT-4 (inclusion-proof soundness — SP-2 core), §6.2 (S-040 — SP-3).
...
- `docs/SECURITY.md §S-040` — `leaf_count` caller-trust invariant (SP-3).
