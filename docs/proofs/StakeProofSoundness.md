...
- **Stale-state lies across invocations, multi-peer redundancy, persistence, transport encryption, RPC auth.** All inherited verbatim from `LightClientThreatModel.md` §6.1-§6.7. Within one invocation the S-042 anchoring (SP-1 §4.1.1) is sound; cross-invocation head-regression is operator-visible but not auto-detected.
...
Crucially, the daemon sources `state_root`, `leaf_count`, `target_index`, `value_hash`, and the sibling vector from **one** `rpc_state_proof` reply (`node.cpp:3443-3453` returns them in a single JSON object), and the light client verifies the proof's `state_root` against a committee-signed successor in the *same* invocation (`committee_bound_state_root`, `light/trustless_read.cpp:335-437`). Since **S-040 is CLOSED** — `leaf_count` is bound into the committed root via the root-wrapper `H(0x02 ‖ be_u32(leaf_count) ‖ inner_root)` — a forged `leaf_count` is now rejected cryptographically regardless of source; the single-envelope shape remains the implementation reality but is no longer a soundness precondition. SP-3 formalizes this enforced binding.
...
2. Light client runs `stake-trustless --domain D`. Internally it anchors genesis (T-L1), walks + committee-verifies headers (T-L2), and in the S-042 binding (`committee_bound_state_root`, `light/trustless_read.cpp:335-437`, called at `light/main.cpp:2039-2041`) fetches the FULL block at `proof_height − 1`, recomputes its `block_hash`, committee-sig-verifies the successor header at `proof_height`, requires `successor.prev_hash == recomputed`, and matches the attested `state_root` to the proof's claimed root.
...
**Head-block boundary.** If the operator queries a stake at the chain *head*, that block's `state_root` has no signed successor yet, so it is committee-certified only once a successor is produced. The shipped flow handles this inside `committee_bound_state_root` (the S-042 binding, `light/trustless_read.cpp:335-437`): the anchor is `proof_height − 1` — the head — and the helper either fails closed when its successor is absent (`:388-401`) or, under the opt-in `--wait <seconds>`, polls for the successor then binds the ALREADY-HELD proof (`:383-387`; soundness-neutral per `WaitHoldAndWaitSoundness.md` FB64). Full nodes enforce the head's root meanwhile via the apply-layer S-033 gate (`StateRootAnchorSoundness.md` §3.4 + §6.3). SP-2 below proves Merkle-path soundness *given* such an `R`; SP-1 is the committee-binding of `R`.
...
**Cleartext cross-check (SHIPPED, mirroring L-4 — no longer optional).** `read_stake_trustless` performs BOTH binds: the step-3a **key-bind** (`proof.key_bytes == "s:" + D` byte-for-byte, `light/main.cpp:1985-1998` — the `NegativeVerdictSoundness.md` F-6 closure; without it a daemon could serve a valid proof for a DIFFERENT `s:` leaf and lie consistently in the cleartext, which the value-hash check alone cannot catch) and the **cleartext hash-bind**: it fetches the daemon's `stake_info` cleartext (`light/main.cpp:2061`) and recomputes `H(u64\_be(\text{locked}_A) \| u64\_be(\text{unlock}_A))` to compare against the proof's verified `value_hash`, then a daemon lying about either scalar while serving an honest proof must find `(\text{locked}_A, \text{unlock}_A) ≠ (\text{locked}_T, \text{unlock}_T)` with colliding value-hashes — a SHA-256 collision, `≤ 2⁻¹²⁸` (A2). This binds the human-readable pair to the committed leaf identically to the balance read's L-4; whether the sibling command ships this cross-check or decodes the pair from a second proof field, the value-hash is the load-bearing binding and SP-2 holds on the proof alone.

**Gate (executed).** This cross-check is now enforced by `tools/test_light_stake_tamper.sh` (register claim **SP-2**, `ProofClaimGateTraceability.md` §3i — the `s:`-namespace sibling of CR-2/RP-3/SU-2, and the last of the light-client value-hash cleartext cross-checks). A lying-daemon `tools/rpc_tamper_proxy.py` flips the `stake_info` `locked` (then `unlock_height`) cleartext while forwarding an honest `s:` state-proof; the client rejects with exit 1 and the SPECIFIC `TAMPERED — daemon's stake_info reply` detail (not a SECURITY/key-bind message — every throw exits 1, so the detail is the discriminator). *Falsify-on-mutant:* neutering the compare at `light/main.cpp:2395` (`→ if (false)`) flips both tamper legs to a false `verified=true`/exit 0 — the client certifies the daemon's forged `(locked, unlock_height)` as committee-verified — while a pass-through control stays green.
...
**Why a forged `leaf_count` is rejected.** `merkle_verify` consumes the caller-supplied `leaf_count` to drive the number of levels and the per-level duplication parity (`merkle.cpp:140-155`) **and** to re-apply the root-wrapper `H(0x02 ‖ be_u32(leaf_count) ‖ inner_root)` before comparing to the committed root. Two distinct `(target_index, leaf_count)` pairs that yield the *same inner walk shape* used to verify identically against the bare inner root (the pre-S-040 hazard — `determ test-merkle-proof-tampering` scenario #12 originally pinned that a claimed `leaf_count = 8` against a genuinely-5-leaf tree verified as `true`). Under the closed design the committed root commits to `leaf_count`, so a claimed `M ≠ n` re-wraps the inner root with the wrong `be_u32(M)` and fails root-equality unless the attacker exhibits a SHA-256 collision on the wrapper input (A2, `≤ 2⁻¹²⁸`). Lock-in scenario #12 was inverted accordingly: it now asserts a forged `leaf_count` is **REJECTED**.

**Consequence for the `stake-trustless` flow.** `Node::rpc_state_proof` still returns `state_root`, `leaf_count`, `target_index`, `value_hash`, and `proof` as fields of **one** JSON object (`node.cpp:3443-3453`), computed atomically under the daemon's `state_mutex_` read lock (`node.cpp:3365`) over a single consistent state; `leaf_count` is `chain_.state_proof(...).leaf_count = build_state_leaves().size()` (`chain.cpp:456`) — the genuine count for the very tree whose root is `compute_state_root()` in the same reply (`node.cpp:3451-3452`). That single-envelope shape remains the implementation reality, but it is no longer load-bearing for soundness:

1. If a malicious daemon serves the genuine `leaf_count` but a tampered `state_root`, the tampered root fails SP-1's committee anchoring (the attested root from `committee_bound_state_root` won't match it, throwing at `light/main.cpp:2042-2047`).
...
SP-1 *is* SR-1 applied at the height the `s:` proof is anchored at, plus SR-2 (genesis-binding, no floating header) + SR-3 (height-binding) inherited for the walk. The transitive-forward `state_root(h) ∈ signing_bytes(h) ∈ block_hash(h) = prev_hash(h+1) ∈ digest(h+1)` chain is the SR-series mechanism; SP-1 names the `s:` read as a consumer of it, exactly as `LightClientThreatModel.md` T-L4 does for the `a:` read (both now route through `committee_bound_state_root`, `trustless_read.cpp:335-437`).
...
A domain with no `stakes_` entry produces no `s:` leaf, so `state_proof("s", D)` returns `{"error":"not_found"}` (`node.cpp:3436`; `chain.cpp:449`). The light client cannot cryptographically conclude "`D` has zero stake" — only "this daemon claims `D` is absent." An honest operator querying an unstaked domain sees the error and must treat it as non-authoritative (a Byzantine daemon could withhold a genuine entry; multi-peer cross-check, out of scope, is the only mitigation). This is the `MerkleTreeSoundness.md` MT-5 / `S033StateRootNamespaceCoverage.md` §6.6 non-membership boundary, inherited.
...
|---|---|---|---|
| SP-1 | committee-anchor (S-042) in the read flow | `light/trustless_read.cpp:335-437` (`committee_bound_state_root`); called from `light/main.cpp:2039-2041` after the stale-height gate `:2014-2021` | Anchor the `s:` proof's `state_root` to a committee-signed SUCCESSOR (full-block recompute + `prev_hash` bind, SR-1); head fails closed / holds-and-waits (FB64). |
| SP-1 | `verify_block_sigs` | `light/verify.cpp:235-328` | Per-block Ed25519 K-of-K committee-sig verify over `light_compute_block_digest`. |
| SP-1 | `verify_headers` | `light/verify.cpp:135-233` | prev_hash continuity walk from genesis (SR-2 genesis-binding). |
| SP-1 | `anchor_genesis` | `light/trustless_read.cpp:55-82` | T-L1 genesis pin (`compute_genesis_hash` byte-compare). |
| SP-2 | `verify_state_proof` | `light/verify.cpp:330-394` | Parse `s:` proof JSON, delegate to `merkle_verify`; optional `--state-root` override. |
| SP-2 | `merkle_verify` | `src/crypto/merkle.cpp:131-162` | Recompute root from `s:` leaf + siblings; range/underflow/exact-consume gates. |
...
- **Implementation.** The `s:` namespace is already served by `Node::rpc_state_proof` (`node.cpp:3390`) and committed by `build_state_leaves` (`chain.cpp:291-297`); the `determ-light stake-trustless` composite command is sibling-agent work this round, structurally identical to `read_account_trustless` with `namespace="s"`.
