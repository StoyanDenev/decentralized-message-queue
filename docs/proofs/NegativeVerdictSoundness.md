...
| `verify-receipt-inclusion` | `i:` leaf `(key_i(S,H), SHA256(0x01))` | daemon `state_proof` reply `not_found` (`light/main.cpp:3223-3226`) | state-proof |
| `verify-merge-state` | `m:` leaf `(key_m(D), value_hash)` | daemon `state_proof` reply `not_found` (`light/main.cpp:3538-3543`) | state-proof |
| `verify-param-change` | `p:` leaf `(key_p(E,j), value_hash)` | daemon `state_proof` reply `not_found` (`light/main.cpp:3878-3883`) | state-proof |
| `verify-account` | `a:` leaf `(key_a(addr), SHA256(u64_be(balance) ‖ u64_be(next_nonce)))` | daemon `state_proof` reply `not_found` → `NOT-CREATED` (`light/main.cpp:5199-5205`) | state-proof |

**Scope note (same-shape negatives not yet given per-command rows).** Four further subcommands emit a state-proof-regime negative of exactly this shape: `verify-registrant` (`r:`, `not_found`→`NOT-INCLUDED`, `light/main.cpp:4455-4459` — it adds a self-consistency cross-check against the `account` RPC's registry cleartext immediately after, which catches a *self-contradicting* daemon but not a *consistently-lying* one, so the (H-neg) footing is unchanged), `verify-dapp-registration` (`d:`, `:4806-4810`), `verify-unstake-eligibility` (`s:`-absence → "sound NO-STAKE", `:2405-2431`), and `verify-abort-record` (`b:` → `NOT-RECORDED`, `:2163-2194` — shipped already carrying the NV-2/NV-3 daemon-asserted framing and the F-2 `negative_footing` field). NV-2, NV-3, NV-4, and NV-5 are namespace-agnostic — *every* `not_found`→negative branch over the sorted-leaves tree carries the same (H-neg) footing — so the analysis transfers verbatim; the per-command rows (and their NV-5a canonical-key legs) are a tracked follow-up extension. The five-row table above is the set analyzed in detail.
...
`A_daemon` instead replies `{"error":"not_found"}`. The verifier's code (`light/main.cpp:3223` for `i:`, `:3538` for `m:`, `:3878` for `p:`, `:5199` for `a:`) reaches the branch `if (err == "not_found")` and sets `verdict = NOT_INCLUDED` (`NOT_CREATED` for `a:`). No genesis-anchor, header-sig, prev_hash, `key_bytes`, or `value_hash` check has any purchase here: those checks gate the *positive* path (binding a served proof to the intended leaf) and are simply not exercised when the daemon serves no proof at all. `verify-account` is no exception despite its richer positive path (it additionally hash-binds the daemon's `account` cleartext to the proof's `value_hash`, `:5236-5264`): the entire bind-and-verify chain lives in the `else` arm of the error check (`:5211-5331`) and is bypassed wholesale by a `not_found` reply.
...
**Remark (source-comment vs. proof).** The `i:`/`m:`/`p:` command headers now label the `not_found`→`NOT-INCLUDED` outcome "a DAEMON-ASSERTED negative — sound only under (H-neg)" (`light/main.cpp:3116-3121`, `:3413-3417`, `:3748-3752` — the shipped F-1 refinement), though their inline branch comments still open with "a genuine absence (`not_found` for our exact key → a sound NOT-INCLUDED)" (`:3216-3218`, `:3531-3533`, `:3871-3873`). `verify-account` still carries the pre-refinement framing throughout: "a **sound** state_proof `not_found` at the verified head → NOT-CREATED … its balance is a TRUE zero, not a daemon-fabricated one" (`:5072-5074`), "NOT-CREATED is a VERIFIED negative anchored to the committee-signed state_root" (`:5087-5089`), "a sound NOT-CREATED" (`:5192-5194`), and the *emitted detail string itself* repeats "its balance is a TRUE zero, not a daemon-fabricated one" (`:5201-5205`). NV-2 sharpens all of these identically: the negative is *sound relative to an honest-daemon assumption*, and **unsound** against `A_daemon`. (What IS committee-anchored in `verify-account`'s negative path is the *context* — genesis pin + verified header chain + a non-empty head state_root, `:5168-5186` — none of which constrains the daemon's freedom to answer `not_found`.) The verdict is correct *information* when the daemon is honest, but it is not a *proof*. The block-body `NOT-INCLUDED` (NV-1) is the only negative that is a proof. §11 records this as finding F-1, with the shipped/remaining wording refinements.
...
- State-proof (`i:`/`m:`/`p:`/`a:`): `APPLIED`/`INCLUDED`/`EXISTS` requires a served Merkle proof with `key_bytes == local canonical key` **and** `value_hash == expected marker` (for `a:`, the recomputed `SHA256(u64_be(balance) ‖ u64_be(next_nonce))` over the daemon's `account` cleartext) **and** `merkle_verify(...) == R` (`light/main.cpp:3232-...` for `i:`, `:3549-...` for `m:`, `:3889-...` for `p:`, `:5211-5331` for `a:`). The `not_found` branch (`:3223`, `:3538`, `:3878`, `:5199`) is *mutually exclusive* with this branch (the outer `if (proof.contains("error"))` short-circuits). A daemon that returns `not_found` has, by construction, **not** supplied a proof, so it cannot reach the positive branch in the same call. For `a:` in particular, a forged `NOT-CREATED` cannot fabricate an `EXISTS`-with-some-balance: `balance`/`next_nonce` are populated **only** inside the fully-bound positive branch (`:5326-5330`), never from the negative one.
...
| NV-2 (`i:` forgeable negative) | `light/main.cpp:3223-3226` (`not_found`→`NOT_INCLUDED` branch); `:3232-...` (disjoint positive branch) | `not_found` reachable without any binding check. |
| NV-2 (`m:` forgeable negative) | `light/main.cpp:3538-3543` (`not_found`→`NOT_INCLUDED`); `:3549-...` (disjoint positive branch) | Same shape; `m:` additionally mutable. |
| NV-2 (`p:` forgeable negative) | `light/main.cpp:3878-3883` (`not_found`→`NOT_INCLUDED`); `:3889-...` (disjoint positive branch) | Same shape; `p:` param-change staging. |
| NV-2 (`a:` forgeable negative) | `light/main.cpp:5199-5205` (`not_found`→`NOT_CREATED`); `:5211-5331` (disjoint positive branch: key-bytes bind `:5214-5227`, cleartext value-hash bind `:5236-5264`, committee-anchored root `:5289-5316`, `merkle_verify` `:5321`) | Same shape; negative label `NOT-CREATED`; positive additionally hash-binds the `account` cleartext. |
...
| NV-4 (no negative→positive) | `light/main.cpp:3219` / `:3534` / `:3874` / `:5195` outer `if (error)` short-circuit; `light/verify_tx_inclusion.cpp` `INCLUDED` only after gate | Positive and negative branches mutually exclusive. |
| NV-5 (fail-closed on other errors) | `light/main.cpp:3227-3231` / `:3544-3548` / `:3884-3888` / `:5206-5210` (`else` → `UNVERIFIABLE`) | Only literal `not_found` yields the negative. |
| NV-5a (canonical-key reachability) | `light/main.cpp:3204-3209` (`i:` body), `:3520-3525` (`m:` body), `:3858-3865` (`p:` body), `:5128-5154` + `:5189-5190` (`a:` canonical-address derivation + query) | Query key built locally; daemon has no say. `a:` additionally case-normalizes (S-028). |
| Corollary NV-6.1 (`i:` append-only) | `light/main.cpp:3106-3111` (append-only note); `m:` mutability `:3404-3408` | Append-only rescues the *positive*, not the negative. |
