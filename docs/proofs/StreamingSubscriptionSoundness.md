...
**Behavioral gate (the takeover-after-auth ordering).** `tools/test_rpc_dapp_subscribe_auth.sh` (Windows-standalone, live single node, `M=K=1`, HMAC auth ENABLED) pins the load-bearing half of SS-5 — *no unauthenticated frame path* — end to end: with auth on, an UNauthenticated `dapp_subscribe` for an *unknown* domain returns `auth_required` (the S-001 gate fires **before** the domain is even inspected, i.e. before the streaming-takeover branch), while the SAME request AUTHENTICATED reaches the handler (`invalid_arg: unknown DApp`) — proving the takeover is strictly downstream of the auth verify and not a reject-everything tautology. Falsify-on-mutant via a rebuilt `determ.exe`: hoisting the `dapp_subscribe` branch above `verify_auth` in `handle_session` flips the unauthenticated leg to `unknown DApp` (the handler ran pre-auth) → RED (3/0 → 2/1); the status-only `test_rpc_hmac_auth.sh` cannot catch this because `status` stays auth-gated under the hoist. Recorded in `ProofClaimGateTraceability.md` §3ad (register row #14).
