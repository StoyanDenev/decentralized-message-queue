...
**Reclassified as.** Chain-aware DApp registered via v2.18 DAPP_REGISTER. K DApp instances run BY committee members as the paper's mutual-distrust IdP (academia.edu/80188125); **threshold-OPAQUE** authentication (OPAQUE for SRP + a t-of-n, unordered threshold OPRF) with the paper's hash-challenge-response RP token (`v2.25-DSSO-DAPP-SPEC.md`) — no threshold signature, no FROST (removed from the chain path per DECISION-LOG.md 2026-06-07).
...
- P-256 RFC 9497 OPRF/VOPRF stack (✅ shipped; `CRYPTO-C99-SPEC.md` §3.9b) — the threshold-OPAQUE T-OPRF substrate
- v2.26 on-chain key rotation (in Theme 9 review-track; v1.0 critical path)
- The paper's hash-challenge-response token over the PAKE-co-generated keys — no committee signature, no FROST (`v2.25-DSSO-DAPP-SPEC.md` §5). The DAPP_REGISTER/DAPP_CALL transport is already shipped.
...
> **SCHEDULED 2026-08-13 as D2's ENDGAME** (owner; `DECISION-LOG.md` 2026-08-13, correction (i)). This item is no longer unscheduled: it **slots between D2 step 3 (keyfiles) and D2 step 4 (parser deletion)** — after D2 finishes editing `src/main.cpp` (honouring the merge-war constraint stated under *Risk* below), and before the JSON parsers are deleted, so step 4 faces a far smaller JSON surface. Re-measured at `53a849d`: `src/main.cpp` is **64,683 lines**; **237 of its 292 `cmd == "…"` handlers are `test-*`/`selftest-*`**, spanning lines **6,955–64,364**; and **1,151 of the file's 1,513 `json`-bearing lines (76%) sit inside that test region** — so the extraction removes roughly three quarters of `src/main.cpp`'s JSON surface as a side effect. (D2 step numbering is the five-step authorization in `DECISION-LOG.md` 2026-07-28; `CLAUDE.md` CURRENT FRONT compresses it to three, where its step 2 is that entry's step 4.) The figures in the body below are the original 2026-08-11 measurement, retained as the historical basis.
...
**Change.** None required — this is exactly the **D2 migration, already the ACTIVE front**. Recorded here only so the simplification register is complete and nobody opens a duplicate track. Deletion of both parsers is D2 step 4 (`DECISION-LOG.md` 2026-07-28 five-step authorization; = `CLAUDE.md` CURRENT FRONT's compressed step 2). Per `DECISION-LOG.md` 2026-08-13 correction (i), **§12.1 lands immediately before that step**, so the deletion faces a `src/main.cpp` with ~76% less JSON in it.
...
> **⚠ CORRECTED + ESCALATED 2026-08-13** (owner; `DECISION-LOG.md` 2026-08-13, directive 4). Two things changed about this item; read them before acting on the body below.
>
> **(1) The precondition this item names is now ANSWERED — in the direction it feared.** The body says *"the claim holds only while no profile value is embedded in authenticated bytes."* **It is embedded.** `crypto_profile` is mixed into the genesis hash when non-default — `make_genesis_block` appends the domain-tagged marker `DTM-genesis-crypto-profile-v1` plus the enum value to the `cumulative_rand` builder (`src/chain/genesis.cpp:837-839`), and `compute_genesis_hash` is `make_genesis_block(cfg).compute_hash()` (`src/chain/genesis.cpp:891-894`). It also round-trips through both containers — emitted by `to_json` when non-default (`:107-108`), read back by `from_json` (`:303-304`), written as a `u8` by the `DGC1` binary encoder (`:497`) — and **decode is FAIL-CLOSED: an unknown value is rejected outright** (`:571-574`). So the `CryptoProfile` value set is genesis-frozen surface. Adding a value later is still additive for a NEW chain at its own genesis; it is not retrofittable to an existing one, and an older binary rejects a value it does not know. The item is therefore **on the `CLAUDE.md` PRE-GENESIS BACKLOG** as of 2026-08-13, where it had zero prior mentions.
>
> **(2) The named subjects do not exist.** `tactical_civilian` and `cluster_civilian` are **TIMING-profile presets, not `CryptoProfile` values**, and they were **never implemented**: `grep` over `src/ include/ light/ wallet/` finds zero occurrences, and `git log -S tactical_civilian -- include src` finds no commit that ever added them. They were decided in `DECISION-LOG.md` 2026-05-24 (C99-11 mid-review revise) and the decision was never executed. The proposed change below ("defer the two civilian variants") is therefore a **no-op against shipped code**. The shipped preset set is `PROFILE_CLUSTER` / `PROFILE_WEB` / `PROFILE_REGIONAL` / `PROFILE_GLOBAL` / `PROFILE_TACTICAL` plus six `*_TEST` mirrors (`include/determ/chain/params.hpp:195-322`); each carries a `CryptoProfile` field, and that field is the only part reaching the genesis hash. The enum itself is `CryptoProfile { MODERN = 0, FIPS = 1 }` (`include/determ/chain/params.hpp:122-125`).
>
> **What remains actionable** is therefore the *enum value set*, not the two phantom presets: decide it before genesis, since it is hash-bound and fail-closed on decode. The verification-matrix argument in the body still stands on its own terms for the shipped preset count.
...
*Authority: measured and drafted by Claude Fable at the owner's direction, 2026-08-11. §12.1 was the recommended first move and is now SCHEDULED as D2's endgame (owner, 2026-08-13 — see its banner); §12.5 is corrected and escalated onto the `CLAUDE.md` PRE-GENESIS BACKLOG (same directive). §12.2 and §12.3 remain unexecuted and still need owner scheduling; §12.4 needs no action (it is D2 itself).*
