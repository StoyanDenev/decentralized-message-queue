...
FA6 + FA-Apply-10  — RECORDS the equivocator(s) responsible for the multi-candidate
                     scenario. Since D4 (2026-09-16) apply reads nothing from the
                     event: the record is committed, nothing is punished, and the
                     L2 consumer (D22) does not exist yet.
```

Hence the BFT-escalation path composes cleanly on the consensus side: V8 admits, `resolve_fork` selects, FA6 attributes. The single converged chain is the resolve_fork-winner's chain. **Restated 2026-09-17:** the third step used to read "FA6 punishes"; it does not, and `BFTSafety.md` T-5.1-R records that a B1 violation has no recovery. Convergence is unaffected — it is T-1/T-2, not FA6, that makes the chain single. ∎
...
**NO CLOSURE — re-derived 2026-09-17 (step 3c). Level 3 is grindable, cheaply, by two independent parties, and the economic argument that used to cover it no longer exists.**

The original closure argued that each grind iteration requires re-signing by every committee member, that a member signing `M > 1` distinct digests at one height is provably equivocating, and that the resulting full stake forfeiture makes grinding economically infeasible. **All three legs fail.** Taking them in the order they fail:

**(i) The signer-side refutation (recorded 2026-09-14; DECISION-LOG 2026-08-13 `7570989`; CLAUDE.md doctrine "never seed anything security-relevant from a block hash").** `Block::compute_hash` hashes `signing_bytes()` and then APPENDS `creator_block_sigs`. Ed25519 verification checks only pk-y canonicality, `S < L` and the group equation — RFC 8032's deterministic nonce is a signer-side convention no verifier can check. So the LAST committee member to broadcast its `BlockSigMsg` enumerates unboundedly many VALID signatures over the **same** `compute_block_digest`, each yielding a different block hash, at ~one Ed25519 sign per trial. **No re-signing by anyone else, no second digest, and no equivocation** — the digest is fixed, so V11 has nothing to see and H2 is not violated. Legs 1 and 2 of the original closure are simply false.

**(ii) The relayer-side refutation (NEW, 2026-09-16; recorded in the S-102 adjudication entry of DECISION-LOG, "Noticed and deliberately left alone" item 1).** Grinding does not even require a committee key. `initial_state` is appended to `Block::signing_bytes` for EVERY block, is **outside** `compute_block_digest` (`src/node/producer.cpp::compute_block_digest_body` — the field list does not contain it), is read by `Chain::apply_transactions` only at `index == 0`, is carried by the BLOCK frame codec at any index, and is **not mentioned anywhere in `src/node/validator.cpp`** — no rule reads it. Therefore an unauthenticated RELAYER takes a finalized block `B`, appends a junk allocation entry to `initial_state`, and re-rolls `compute_hash` at **one SHA-256 per trial**, with the digest unchanged (so every K-of-K signature still verifies), the validator unchanged (nothing checks the field) and the apply outcome unchanged (the field is ignored above genesis). The same holds for a wrong non-zero `state_root`, likewise outside the digest. **The block hash is relayer-malleable, not merely signer-malleable** — CLAUDE.md records only the signer case — so Level 3 is grindable by any party on the network at hashing cost, not signing cost.

**(iii) The economic leg is gone regardless.** "Plus the slashing risk (full stake forfeiture per FA-Apply-10)" is void since 2026-09-16: `Chain::apply_transactions` reads nothing from `b.equivocation_events` (owner decision D4; `EquivocationSlashingApply.md` T-E0). Even in the counterfactual where grinding DID require a committee member to double-sign, the cost of doing so is now zero. The `(K − 1) · min_stake` figure is not a smaller number; it is not a number at all.

**What survives, and it is the only thing that ever mattered.** Level 3 is a **Schelling point for determinism**, not a defence. T-1 (deterministic tiebreak) and T-2 (confluence) hold unconditionally: every honest node ranks the same pair identically and converges on the same head, whoever ground the hash. The lexicographic-smallest rule needs no unpredictability to do that job. Two further containments, both structural rather than economic:

- **Level 3 only fires after Levels 1 and 2 tie.** A grinder must already match the honest block's signature count and abort count. An outside relayer achieves that trivially — by relaying a *mutation of the honest block itself*, which has the identical signature set — so this containment is weak against (ii) and only bites against a genuinely competing block.
- **The prize is one block of tip divergence.** The next height's committee derives from `cumulative_rand`'s commit-reveal, every input of which is digest-covered or commit-pinned; the block hash seeds nothing. That is the doctrine invariant, and it is what keeps this from being a consensus break.

**The hole, stated plainly.** A relayer can decide which of two same-digest twins every node adopts, repeatedly and at negligible cost, and each adoption costs every node a revert + apply and a round reset — a liveness-griefing surface with no L1 price. The fixes are accept rules (reject a non-empty `initial_state` on `index > 0`; refuse to replace a non-zero-`state_root` head with a zero-root same-digest twin), they are pre-genesis and therefore free, and they are **NOT implemented**: they were recorded in the S-102 adjudication as belonging to their own increment with its own review. Until one of them lands, F-1 below is OPEN, not closed. **Nothing in this document may be cited as bounding the cost of hash grinding.**
...
**Closure status — OPEN (re-derived 2026-09-17).** The former status read "economically infeasible at any chain with meaningful `min_stake`". Per §5.3 as re-derived, that is wrong three times over: grinding needs no re-signing by anyone (the trailing signer re-rolls its own signature over a fixed digest), it needs no committee key at all (the relayer path through `initial_state` / `state_root`, both outside the digest and unread by the validator), and the forfeiture that priced it was removed on 2026-09-16 (D4). **Grinding the Level-3 tiebreak costs one hash per trial and is available to any peer on the network.**

**Residual risk.** Independent of `min_stake` and of the inclusion model: the cost is hashing, not capital. The impact is bounded by what the block hash feeds — a one-block tip choice and nothing security-relevant (committee selection and the subsidy lottery route through `cumulative_rand`'s commit-reveal; CLAUDE.md doctrine). The realistic harm is liveness griefing: repeated forced adoptions of same-digest twins, each costing every node a revert + apply + round reset. Determinism (T-1) and confluence (T-2) hold unconditionally and are unaffected. **The fix is an accept rule and is not written**; see §5.3.
