...
> **⚠ 2026-09-14 — this closure is REFUTED first-order (DECISION-LOG 2026-08-13 `7570989`; CLAUDE.md doctrine "never seed anything security-relevant from a block hash").** `Block::compute_hash` hashes `signing_bytes()` and then APPENDS `creator_block_sigs`, and Ed25519 verification cannot check RFC 8032's deterministic nonce, so the LAST member to broadcast its BlockSig can enumerate unboundedly many VALID signatures over the SAME digest, each yielding a different block hash, at ~one Ed25519 sign per trial — no re-signing by the other members, no second digest, no equivocation. Level 3 is therefore grindable by one committee member at negligible cost; the economic argument below does not apply. This is harmless today only because nothing security-relevant (committee selection, subsidy lottery) is seeded from the block hash — keep it that way.
