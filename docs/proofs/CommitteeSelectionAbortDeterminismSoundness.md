...
- **The grinding *payoff* bound.** Whether abort-grinding *helps* the adversary (it does not, beyond a uniform redraw; since D13, 2026-09-16, at NO stake cost — an abort deducts nothing, so the cost of a grind step is bandwidth plus the abort's own suspension window on the accused, not on the grinder) is `Censorship.md` §4 Step 4 + `BFTProposerElectionSoundness.md` PE-3 + `AbortEventApply.md` (FA-Apply-11). T-4 below states only the determinism half (grinding is shared), and cross-references those documents for the payoff half.
...
- That each induced Round-1 abort carried a *slashing cost* (proportional stake forfeiture) was `AbortEventApply.md` (FA-Apply-11) T-A1 — RETIRED by D13 (2026-09-16): a Phase-1 `AbortEvent` records the S-032 suspension and deducts nothing. The grinder therefore pays no stake per grind step. This does NOT affect T-4, which is a determinism statement and consumes no cost term; it removes a term from the *payoff* bound delegated above, and the honest restatement of that bound is `S010S011SybilEconomics.md` §6.7.
...
| `A_grind` (steer committee) | Grind aborts to land a favorable committee | T-4: grinding is a *shared* transition (favorable identically for everyone, so no fork); the *payoff* bound (uniform redraw, bounded rounds — the T-A1 stake cost term is GONE since D13) is delegated to `Censorship.md` §4 + `Liveness.md` §6 + `S010S011SybilEconomics.md` §6.7. |
