...
| `k:` | chain.cpp:1597-1608 (thirteen scalars) | every name in §2.1's `k:` list |
| `k:c:` | chain.cpp:1614-1618 (five counters) | every name in §2.1's `k:c:` list |
...
2. A new namespace addition (e.g., a `t:` namespace for some new per-tx-id state) requires updating PROTOCOL.md §4.1.1 + `build_state_leaves` + `test_state_root_namespaces.sh` + `serialize_state` + `restore_from_snapshot` in tandem. The S-037 lesson stands: any one of these touched without the others creates a coverage gap.

**Closure mechanism:** the pattern is now established (S-037 + S-038), and the four-surface threading is the discipline. A future audit checklist item should explicitly enumerate the surfaces.
