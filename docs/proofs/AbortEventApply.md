...
Per `include/determ/chain/chain.hpp:252–255 + 847`:
...
*Proof sketch.* By inspection of `chain.cpp:1782–1797`. The gate at line 1783 admits the round-1 case. Line 1786 ensures the abort_records snapshot is captured (A9 atomic-apply pattern; rollback-safety). Lines 1787–1789 increment the cache: `ar.count++`, `ar.last_block = b.index`. Line 1791 looks up the stake entry; line 1792 takes the `stakes_.end()` defensive `continue` for absent stake (handled by T-A5). Line 1793 computes `deduct = std::min(suspension_slash_, locked)` — this is the floor-at-zero arithmetic without branching: when `locked < suspension_slash_`, `min` returns `locked` (full deduction); when `locked ≥ suspension_slash_`, `min` returns `suspension_slash_`. Line 1794 ensures the stakes snapshot is captured. Line 1795 debits: `sit->second.locked -= deduct`. Line 1796 accumulates: `block_slashed += deduct`, which later feeds `accumulated_slashed_ += block_slashed` at line 1864. The A1 closure at `chain.cpp:1866–1888` confirms the slash arithmetic is supply-conserving: the slashed amount becomes unspendable (no balance credit, no fee redistribution) and the bookkeeping accumulator captures the loss. ∎

**Code witness.** `src/chain/chain.cpp:1782–1797` (the AbortEvent slash branch); `src/chain/chain.cpp:1864 / 1868` (A1 closure path); `include/determ/chain/params.hpp:79` (`SUSPENSION_SLASH` build-time default); `include/determ/chain/chain.hpp:849` (`suspension_slash_` instance state).
...
*Proof sketch.* By inspection of `chain.cpp:1782–1797`. The AbortEvent loop body's write set comprises (a) `abort_records_[d]` (lines 1787–1789), (b) `stakes_[d].locked` (line 1795), (c) `block_slashed` (line 1796, a per-block accumulator), and (d) the snapshot-capture lambdas (`__ensure_abort_records`, `__ensure_stakes`, no `__ensure_registrants` call). `registrants_` is never accessed inside the AbortEvent branch — neither read nor write. This is in stark contrast to the EquivocationEvent branch at `chain.cpp:1820–1824` which DOES set `registrants_[d].inactive_from = b.index + 1` (the full FA6 deregistration). The asymmetry is the design's "guilt vs. unavailability" primitive: an aborting validator is presumed available-but-late (their stake is the price of late delivery), not Byzantine (which is what the equivocation deregistration handles). The aborting validator's recovery path is straightforward: continue running, gather their next-round committee selection, and contribute normally — no fresh REGISTER needed. ∎

**Code witness.** `src/chain/chain.cpp:1782–1797` (the AbortEvent branch with no `registrants_` write); compare with `src/chain/chain.cpp:1820–1824` (the EquivocationEvent branch that DOES deregister).
...
*Proof sketch.* By inspection of `chain.cpp:1784–1796`. Lines 1784–1789 unconditionally update `abort_records_[d]` (the cache update runs before the stake lookup). At line 1791 the stake lookup `auto sit = stakes_.find(d)` returns `stakes_.end()` for case (a), triggering the `continue` at line 1792 — exit the iteration with no stake mutation. For case (b) where the entry exists but `locked == 0`, line 1793 computes `deduct = std::min(suspension_slash_, 0) = 0`; line 1795 writes `locked -= 0` (no-op); line 1796 writes `block_slashed += 0` (no-op). In both subcases the cache write at lines 1787–1789 persists, the stake deduction is zero, and the A1 contribution from this event is zero. The decoupling is intentional: the cache exists to track validator behavior independent of the economic channel, which is what makes DOMAIN_INCLUSION mode coherent for permissioned-consortium deployments. ∎

**Code witness.** `src/chain/chain.cpp:1791–1792` (defensive `stakes_.end()` check); `src/chain/chain.cpp:1793` (the `std::min` floor at 0 via the locked operand); `src/chain/chain.cpp:1784–1789` (the cache write that survives the stake-lookup skip).
...
**Code witness.** `src/chain/chain.cpp:1793` (the `std::min(suspension_slash_, locked)` flooring); `src/chain/chain.cpp:1795–1796` (the deduct + accumulator).
...
**Code witness.** `src/chain/chain.cpp:1793` (deduct computation); `src/chain/chain.cpp:1795–1796` (the stake debit + accumulator); `src/chain/chain.cpp:1864` (`accumulated_slashed_ += block_slashed`); `src/chain/chain.cpp:1868` (the A1 closure assertion).
...
*Proof sketch.* By inspection of `chain.cpp:1782–1797`. The loop body is a deterministic function of (a) the AbortEvent fields (`round`, `aborting_node`), (b) the current `stakes_[d]` value, (c) the current `abort_records_[d]` value, and (d) the chain instance state (`suspension_slash_`). No I/O, no randomness, no system-clock reads, no map iteration that depends on iteration order beyond what the vector's index provides. The std::map writes (`abort_records_[d]`, `stakes_[d]`) are deterministic in C++ for the same key sequence and value writes. The `std::min(suspension_slash_, locked)` arithmetic is pure (integer comparison). The accumulator increment at line 1796 is sequential. Re-running the same loop on identical input produces byte-identical post-state — this is what makes snapshot replay (T-S2 of FA-Apply-2) work for abort_records, and what makes cross-shard apply equivalence work in EXTENDED mode (the `b:` namespace contributes to the state_root, so any non-determinism would surface as a state_root mismatch on snapshot restore). ∎

**Code witness.** `src/chain/chain.cpp:1782–1797` (the deterministic loop body); `include/determ/chain/chain.hpp:311` (the `b:` state-root namespace, which makes abort_records part of the S-033 commitment — any non-determinism would be caught by the state_root gate).
...
| Apply branch | `chain.cpp:1782–1797` | `chain.cpp:1813–1825` |
...
| `include/determ/chain/block.hpp:319–332` | `AbortEvent` struct. |
| `include/determ/chain/chain.hpp:252–258` | `AbortRecord` struct + `abort_records()` getter. |
| `include/determ/chain/chain.hpp:311` | `b:` state-root namespace key encoding. |
| `include/determ/chain/chain.hpp:847` | `abort_records_` field. |
| `include/determ/chain/chain.hpp:849` | `suspension_slash_` instance state (A5 PARAM_CHANGE mutable). |
| `include/determ/chain/params.hpp:79` | `SUSPENSION_SLASH` build-time default (= 10). |
| `src/chain/chain.cpp:1782–1797` | The AbortEvent apply branch (T-A1 through T-A8). |
| `src/chain/chain.cpp:1813–1825` | The EquivocationEvent apply branch (contrasting FA6 channel; §3 comparison). |
| `src/chain/chain.cpp:1864` | `accumulated_slashed_ += block_slashed` per-block composition. |
| `src/chain/chain.cpp:1868` | A1 unitary-balance closure assertion. |
...
- **T-A1** (Phase-1 abort slashing) closed via the `std::min(suspension_slash_, locked)` deduct + `block_slashed += deduct` accumulator at `chain.cpp:1793–1796`; regression `test_abort_event_apply.sh` "Phase-1 slashing" scenario.
- **T-A2** (Phase-2 abort no-slash) closed via the `if (ae.round != 1) continue;` gate at `chain.cpp:1783`; regression `test_abort_event_apply.sh` "Phase-2 no-slashing" scenario.
- **T-A3** (S-032 abort_records cache update) closed via the `ar.count++; ar.last_block = b.index;` writes at `chain.cpp:1787–1789`; regression `test_abort_event_apply.sh` "S-032 abort_records cache" scenario.
...
- **T-A5** (aborts on zero-stake DOMAIN_INCLUSION mode) closed via the `stakes_.find / continue` defense + the cache-write-before-stake-lookup sequencing at `chain.cpp:1784–1792`; regression `test_abort_event_apply.sh` "Aborted-without-stake" scenario.
- **T-A6** (stake exhaustion) closed via the `std::min` flooring at `chain.cpp:1793` + the cache-keeps-tracking property at lines 1787–1789; regression `test_abort_event_apply.sh` "Stake exhaustion" scenario (51 aborts on stake=500, slash=10).
- **T-A7** (A1 invariance per Phase-1 abort) closed via the `block_slashed += deduct` per-event composition + the chain-tail `accumulated_slashed_ += block_slashed` + the A1 closure at `chain.cpp:1864 / 1868`; regression `test_abort_event_apply.sh` "A1 invariant" scenario.
...
The proof's foundation rests on a small set of code primitives: the round-1 gate at `chain.cpp:1783`, the `std::min(suspension_slash_, locked)` flooring at line 1793, the cache-write-before-stake-lookup sequencing at lines 1784–1791, and the per-block `block_slashed` accumulator that feeds the chain-wide A1 invariant. The breadth of consequences — eight theorems, a documented asymmetry vs. equivocation full-forfeit, an explicit DOMAIN_INCLUSION mode story, and a regression scenario covering each — is testimony to how few primitives the chain needs to express the proportional-penalty design.
