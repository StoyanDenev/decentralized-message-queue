...
The `event_hash` feeds the committee re-derivation fold (§1.3). **Correction 2026-09-15 (S-074):** until that date the validator folded the value as given — it was the assembler's choice, not "the chain's deterministic fingerprint" — so whoever assembled the event chose the post-abort committee. Since S-074 the event's `timestamp` MUST equal the parent block's and `event_hash` MUST equal `chain::canonical_abort_event_hash(event, predecessor, parent)` = `SHA256("DTM-ABORT-ID-v1" ‖ 0 ‖ committee_seed ‖ block_index ‖ round ‖ aborting_node)` for the first event at the height and `SHA256("DTM-ABORT-ID-v1" ‖ 1 ‖ prev.event_hash ‖ round ‖ aborting_node)` thereafter, where `committee_seed` is the §1.3 `prev_rand` every verifier already derives — the timestamp is not an input, because the parent committee chooses the block timestamp inside the ±30 s window; step 4b below recomputes both, so the fold now consumes a value every node derives. The `claims` list is the **certificate** — the `M−1` peer attestations that authorize the slash named by `aborting_node`.
...
Per `src/node/validator.cpp:243–386`, `check_abort_certs(b, chain, registry)` performs, for each `ae ∈ b.abort_events` in iteration order:
...
2. **Membership check** (lines 322–326). Reject unless `ae.aborting_node ∈ domains_at_event`.
3. **Quorum-count check** (lines 340–342). Reject unless `ae.claims.size()` is **exactly** `chain::abort_claim_quorum(domains_at_event.size())` = `max(2, M−1)` (S-044 F-a — the same floor the producer/gossip formation uses; at `M = 2` the quorum is unsatisfiable by construction, so no `K=2` block can ever carry an abort event).
...
4b. **Canonical identity (S-074, 2026-09-15; lines 370–380).** Reject unless `ae.timestamp == chain.head().timestamp` ("timestamp != parent block timestamp (S-074)") and `ae.event_hash == canonical_abort_event_hash(ae, i == 0 ? nullptr : &b.abort_events[i−1], prev_rand, b.index)` ("event_hash not canonical (S-074)"). The same recomputation gates gossip adoption (`Node::on_abort_event`, before the claim signatures, together with an accused-in-current-committee check mirroring step 2), the beacon's shard-tip verification (`verify_shard_tip_committee_sig_root`, hence also the beacon's `check_shardtip_witnesses` accept rule) and the light auditor's witness re-verification, and is what the assembler (`Node::on_abort_claim`) emits. Gates: `test-abort-cert-validation` (chosen hash, chosen timestamp, chained control, chained-as-first) and `test-abort-event-canonical` (formula pinned independently of the helper; a chosen hash demonstrably seats a different committee; replay against an excluded member dropped; a non-member assembles the canonical event).
5. **Advance fold** (lines 382–383). `excluded.insert(ae.aborting_node)` and `rand := SHA256(rand ‖ ae.event_hash)`, so the next event's committee re-derivation sees the same exclusion + remix the producer used.
...
Per `validator.cpp:257–262` (with the per-event fold at line 383), the seed the validator feeds to `select_m_creators` is derived identically to `check_creator_selection` (V3):
...
**Code witness.** `src/node/validator.cpp:243–386` (the pure verification function); `include/determ/chain/chain.hpp` (`resolve_epoch_rand`, `eligible_in_region`, `shards_absorbed_by` — all deterministic over restored state).
...
- Reading `check_abort_certs` (`validator.cpp:243–386`) to confirm all five steps (re-derivation, membership, exact-count, per-claim binding + sig verify, fold) fire before `{true, ""}` is returned.
