...
**Relationship to `read_account_trustless` (the per-height primitive).** The shipped single-point read at `light/trustless_read.cpp:188-350` reads the account state at the daemon's *current verified head* (`verify_chain_to_head` walks to the daemon's tip, then the race-window dispatch in §4.4.1 of `LightClientThreatModel.md` re-anchors the proof to a committee-signed header at the proof's height). For a *historical* read at a fixed sampled height `h < head`, `account-history` (D3 R40) must request the state-proof bound to height `h` specifically and anchor it to `header[h]`'s committee-signed `state_root`, rather than to the head. This is an extension of the single-point flow's structure, not a change to its trust model:
...
- **T-L1 (genesis-anchored chain identity).** `anchor_genesis` recomputes `compute_genesis_hash(genesis_O)` locally and cross-checks the daemon's block-0 hash byte-for-byte (`light/trustless_read.cpp:52-79`). Under A2, a daemon serving a chain whose genesis ≠ the operator's pinned `genesis.json` cannot pass this gate (`LightClientThreatModel.md` T-L1). In `account-history` this gate runs **once** at startup; every per-height read inherits the anchored chain identity.

- **T-L2 (head trust via committee signatures).** `verify_block_sigs` (`light/verify.cpp:190-283`) requires every entry of `header[h].creators` to be in the committee map and `valid ≥ required` Ed25519 verifications over `light_compute_block_digest(header[h])` at the K-of-K (MD) or `⌈2k/3⌉` (BFT) threshold. Under A1, a daemon cannot present a `header[h]` the committee did not sign (`LightClientThreatModel.md` T-L2). Crucially, `state_root` is part of `header[h]`'s content bound into its `block_hash` chain (`S033StateRootNamespaceCoverage.md`; `LightClientThreatModel.md` §5.2 note), so a committee-signed `header[h]` pins a *specific* `state_root` value at `h`.

- **T-L3 / MT-4 (state-proof correctness).** `verify_state_proof` (`light/verify.cpp:285-349`) delegates to `crypto::merkle_verify` (`src/crypto/merkle.cpp:113-141`). `MerkleTreeSoundness.md` **MT-4 (inclusion-proof soundness, §4)** proves: fix a leaf set `Λ` with root `r = MR(Λ)` and `n = |Λ|`; for any `(key⋆, value⋆, target_index⋆, proof⋆)` with `target_index⋆ < n`, a passing `merkle_verify` against `r` implies genuine membership with probability `≥ 1 − 2⁻¹²⁸` under A2. Applied to `account-history`: a state-proof for `D`'s `a:`-namespace leaf that verifies against `header[h].state_root` proves `D`'s leaf value is genuinely committed under that root — i.e., is `D`'s on-chain account state at `h`. (`MerkleTreeSoundness.md` §6.2 / S-040 records the operational obligation that `leaf_count` be sourced from the same anchor as `state_root`; here both come from the committee-signed `header[h]`.)

- **T-L4 + race-window (balance/nonce composition).** `read_account_trustless` (`LightClientThreatModel.md` §4.4 / T-L4) chains genesis-anchor → header-walk + per-block sig verify → state-proof fetch → self-anchored proof verify → race-window dispatch (anchor the proof's `state_root` to a committee-signed header) → cleartext cross-check (`SHA256(balance_be ‖ nonce_be) == proof.value_hash`, Lemma L-4). The **race-window mitigation** (`LightClientThreatModel.md` §4.4.1) handles the case where the chain advanced during the round-trip by re-anchoring the proof to `header[proof_height − 1]`'s committee-signed `state_root` and walking the prev_hash chain from the previously-verified head. For `account-history`'s historical read at a *fixed* `h`, the analogous anchoring is to `header[h]` directly (the sampled height is the target, not a moving head); the structural binding — proof's root ⇔ committee-signed header's `state_root` ⇔ prev_hash linkage to genesis — is identical.
...
**Composition statement.** AH-1 is, structurally, **T-L4 (`LightClientThreatModel.md`) applied at a fixed historical height `h`**: T-L2 (committee-signed `header[h]`) + MT-4 / T-L3 (state-proof verifies against `header[h].state_root`) + T-L1 (genesis anchor, once). The single-point read's race-window dispatch (`LightClientThreatModel.md` §4.4.1) is specialized: instead of re-anchoring a moving head, `account-history` anchors directly to the sampled `header[h]`. No new cryptographic claim beyond T-L4 is asserted per point.
...
Each per-height AH-1 Gate 3 invokes `merkle_verify(R_h, key_bytes_D, value_hash, target_index, leaf_count, sibs)` anchored against the committee-signed `R_h`. `MerkleTreeSoundness.md` **MT-4 (§4, inclusion-proof soundness)** is the exact theorem that licenses "a passing verification implies genuine membership under A2." The operational obligation MT-4 carries — that `leaf_count` be sourced from the same anchor as `state_root` (`MerkleTreeSoundness.md` §6.2 / S-040) — is discharged here because both `R_h` and the proof's `leaf_count` are anchored to the same committee-signed `header[h]` (the proof's `leaf_count` is cross-checked implicitly by the verification against `R_h`; a wrong `leaf_count` yields a different recomputed root). AH-1 inherits MT-4's `log₂(leaf_count) · 2⁻¹²⁸` per-attempt bound for the inclusion gate.
...
The per-height committee-sig verification (AH-1 Gate 1) uses the committee map seeded from genesis `initial_creators` via `build_genesis_committee` (`light/trustless_read.cpp:43-50`) — the **static** `K_0`. If the active committee **rotates** within `[H1, H2]` via on-chain `REGISTER` / `DEREGISTER`, per-height verification must use the committee valid AT that height. The honest status:

- **`read_account_trustless` (and hence `account-history` built on it) tracks only `K_0`.** `verify_chain_to_head` builds `committee_json` once from the genesis seed and never mutates it across the walk (`light/trustless_read.cpp:106-113`, invoked once; the per-block `verify_block_sigs` call uses the same object every iteration). `verify_block_sigs` demands every entry of a header's `creators` be in `K_0` (`light/verify.cpp:223-229`: `"creator '<domain>' is not in the supplied committee"`).
...
|---|---|---|
| AH-1 / AH-2 (composition) | `light/account_history.cpp` (D3 R40) | Per-sampled-height trustless read + trajectory assembly. Composes `read_account_trustless`'s gates across `Σ`. Fail-closed per height (AH-L5). Spec-level here; cite D3's function names once landed. |
| AH-1 (per-height read template) | `read_account_trustless` | `light/trustless_read.cpp:188-350` | The single-point read AH-1 specializes to a fixed historical `h` (AH-L1). |
| AH-1 Gate 1 / §6.2 (T-L2) | `verify_block_sigs` | `light/verify.cpp:190-283` | `creators ⊆ K_0` membership + Ed25519 threshold verify over `light_compute_block_digest`; per `header[h]`. |
| AH-1 Gate 1 (digest binding) | `light_compute_block_digest` | `light/verify.cpp:47-61` | Byte-for-byte copy of `producer.cpp::compute_block_digest`; binds `header[h]` content (incl. fields chaining to `state_root`) to the signed digest. |
| AH-1 Gate 2 / AH-2 (continuity) | `verify_headers` | `light/verify.cpp:104-188` | prev_hash linkage of `header[h]` to the genesis-anchored prefix. |
| AH-1 Gate 2 / AH-L3 (genesis anchor) | `anchor_genesis` | `light/trustless_read.cpp:52-79` | Once at startup; `compute_genesis_hash(genesis_O)` vs daemon block 0. |
| AH-1 Gate 3 (MT-4 / T-L3) | `verify_state_proof` → `merkle_verify` | `light/verify.cpp:285-349` → `src/crypto/merkle.cpp:113-141` | State-proof verified against committee-signed `R_h := header[h].state_root`. |
| AH-L4 (cleartext cross-check) | value-hash recompute | `light/trustless_read.cpp:323-343` | `SHA256(balance_be ‖ nonce_be) == proof.value_hash`, per height (if performed). |
| §6.2 (committee seed) | `build_genesis_committee` | `light/trustless_read.cpp:43-50` | Seeds `K_0` from genesis `initial_creators` only — the static-committee fact §6.2 turns on. |
...
| [MerkleTreeSoundness.md](MerkleTreeSoundness.md) | MT-4 (§4, inclusion-proof soundness) is the cryptographic core of AH-1 Gate 3 (the per-height state-proof step); §6.2/S-040 `leaf_count`-sourcing obligation discharged by anchoring both `R_h` and `leaf_count` to the committee-signed `header[h]`. |
...
- **Implementation.** `account-history` shipping this round (sibling D3, R40), in parallel with this proof. `light/account_history.cpp`; test `tools/test_light_account_history.sh`. The proof cites the per-height composition at the spec level (per-sampled-height trustless read reusing `read_account_trustless`'s gates, assembled into a trajectory); function-name citations to be tightened once D3's code lands and is threaded.
