...
**Implementation-status note (S-043 — the gossip half shipped broken, now fixed).** T-3 as stated covers the *block-validation* recompute (`check_creator_tx_commitments`), which shipped correctly with `f99eeb8`. The SAME commitment is also recomputed at gossip pre-admission — `Node::on_contrib` verifies each arriving `ContribMsg.ed_sig` before pooling it — and THAT recompute shipped with the pre-feature 7-argument call (defaulted `proposer_time = 0`), so every production contrib (non-zero committed time) failed pre-admission sig-verify at every peer and no multi-node cluster could mint a block (total liveness outage; zero safety impact — reject-everything direction). Registered + fixed as **`docs/SECURITY.md §S-043`** (fix `992cf2a`: `on_contrib`'s recompute and the S-006 evidence recompute now re-bind the message's `proposer_time`; commitment format / wire bytes / signer behavior unchanged). The wire+verify chain is regression-locked by `determ test-contrib-wire-verify` (9 assertions incl. the pre-fix 7-arg recompute being REJECTED). Lesson recorded there: a commitment formula with a signer half and a verifier half needs a wire-roundtrip-verify pin per message type at ship time — validator-path coverage does not substitute for the gossip receiver's recompute.
...
collision — the same A2 step, one hash layer further out. The domain-separation argument above is
strengthened, not weakened: `"DTM-TS-v1"` remains distinct from the F2 `"DTM-F2-v1"` roots, and the two
new outer tags are distinct from both and from each other. The digest VALUES all changed (pre-genesis,
free); no theorem statement here depends on a specific value.
...
**Proof.** On a reconciled block, the digest BODY appends `b.timestamp` as the last field (order `inbound, eq, abort, partner_subset_hash, timestamp`; `src/node/producer.cpp:820 compute_block_digest_body`). The committee's Phase-2 signatures `b.creator_block_sigs` are over `digest(b)` (`ConsensusPhaseStructureSoundness.md` T-4; `producer.cpp::make_block_sig`).
