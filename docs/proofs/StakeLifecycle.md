...
with no other state mutation. The A1 unitary-supply invariant holds — STAKE moves `amount` from balance to stake and the `tx.fee` portion enters the per-block `total_fees` accumulator (later distributed to creators per FA11), so total supply is conserved at the apply-tail A1 evaluation (`chain.cpp:1866-1892`).

*Proof sketch.* By inspection of `chain.cpp:1317–1336`. The nonce gate at line 970 admits the tx (hypothesis). The payload-size check at line 1318 admits a `continue` on length mismatch — under the 8-byte hypothesis the branch proceeds. `amount` is decoded at lines 1319–1321 (little-endian 8-byte field). `cost = amount + tx.fee` is computed with the S-049 overflow-checked `checked_add_u64` at lines 1325–1326 (skip-on-overflow); balance check `sender.balance < cost` at line 1327 fails the hypothesis, so the apply proceeds. `sender.balance -= cost` at line 1328 debits the source (single atomic write); `stakes_[tx.from].locked += amount` at line 1330 credits the stake map (single atomic write). `total_fees += tx.fee` (S-049-checked) at line 1331 captures the fee for later distribution. `sender.next_nonce++` at line 1334 bumps the nonce. The four deltas in the statement are exactly the four state writes; no other field is touched. The A1 closure follows from I-5 + I-6 of FA-Apply: `cost = amount + tx.fee` is split exactly between the `Δstakes_[d].locked` term and the `total_fees` accumulator, and the apply-tail check at `chain.cpp:1866-1892` confirms `live_total_supply` (which sums balances AND stakes) is invariant under this transition. ∎

**Code witness.** `src/chain/chain.cpp:1317–1336` (STAKE branch); `src/chain/chain.cpp:1866-1892` (A1 closure assertion); `include/determ/chain/chain.hpp:23–30` (`StakeEntry` struct).
...
with no other state mutation. The A1 unitary-supply invariant holds: `amount` moves from `stakes_[d].locked` to `accounts_[d].balance` (an intra-domain transfer), and the `tx.fee` enters `total_fees` for distribution (same channel as STAKE's fee). Total supply is invariant; the apply-tail A1 check at `chain.cpp:1866-1892` succeeds.
...
**Code witness.** `src/chain/chain.cpp:1354–1358` (the unstake success path); `src/chain/chain.cpp:1866-1892` (A1 closure).
...
| `src/chain/chain.cpp:1866-1892` | A1 unitary-balance closure assertion. |
