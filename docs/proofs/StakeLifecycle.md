...
Three transactions in this proof use the `charge_fee` lambda (`chain.cpp:954–963`) or an inline equivalent: DEREGISTER (line 1299), UNSTAKE (line 1343), and the implicit fee path inside STAKE (line 1331 — fees folded into the `cost = amount + fee` debit at line 1328). The fee is consumed before any state mutation that could fail. For STAKE, the `cost` includes the stake amount so insufficient-balance is a single check at line 1327 with `continue` semantics — no partial debit, no fee charged; the `cost = amount + tx.fee` addition is itself S-049 overflow-checked via `checked_add_u64` (line 1326, skip-on-overflow). For DEREGISTER, the fee is consumed unconditionally on entry (line 1299) and the rest of the branch runs even on the defensive "no registry entry" path at line 1301. For UNSTAKE, the fee is consumed at line 1343 before the unlock-height / locked-amount checks; on failure of any of those checks the fee is explicitly refunded via `sender.balance += tx.fee; total_fees -= tx.fee;` at lines 1349–1350 before the nonce bump and `break`.
...
with no other state mutation. The A1 unitary-supply invariant holds — STAKE moves `amount` from balance to stake and the `tx.fee` portion enters the per-block `total_fees` accumulator (later distributed to creators per FA11), so total supply is conserved at the apply-tail A1 evaluation (`chain.cpp:1866-1888`).

*Proof sketch.* By inspection of `chain.cpp:1317–1336`. The nonce gate at line 970 admits the tx (hypothesis). The payload-size check at line 1318 admits a `continue` on length mismatch — under the 8-byte hypothesis the branch proceeds. `amount` is decoded at lines 1319–1321 (little-endian 8-byte field). `cost = amount + tx.fee` is computed with the S-049 overflow-checked `checked_add_u64` at lines 1325–1326 (skip-on-overflow); balance check `sender.balance < cost` at line 1327 fails the hypothesis, so the apply proceeds. `sender.balance -= cost` at line 1328 debits the source (single atomic write); `stakes_[tx.from].locked += amount` at line 1330 credits the stake map (single atomic write). `total_fees += tx.fee` (S-049-checked) at line 1331 captures the fee for later distribution. `sender.next_nonce++` at line 1334 bumps the nonce. The four deltas in the statement are exactly the four state writes; no other field is touched. The A1 closure follows from I-5 + I-6 of FA-Apply: `cost = amount + tx.fee` is split exactly between the `Δstakes_[d].locked` term and the `total_fees` accumulator, and the apply-tail check at `chain.cpp:1866-1888` confirms `live_total_supply` (which sums balances AND stakes) is invariant under this transition. ∎

**Code witness.** `src/chain/chain.cpp:1317–1336` (STAKE branch); `src/chain/chain.cpp:1866-1888` (A1 closure assertion); `include/determ/chain/chain.hpp:23–30` (`StakeEntry` struct).
...
*Proof sketch.* By inspection of `chain.cpp:1327`: `if (sender.balance < cost) continue;`. Under the hypothesis `balance < cost`, the `continue` branch is taken before any state mutation. The subsequent lines (1328–1334) — balance debit, stake credit, fee accounting, nonce bump — are unreached. The validator's V15 should have rejected the tx upstream; the apply-time `continue` is the safety net (see I-1 of FA-Apply for the same pattern across all debit channels). Critically, the nonce is NOT bumped: a STAKE on an empty balance does not consume the nonce slot, so the user can retry the same `(from, nonce)` after a balance top-up — no penalty for a too-eager STAKE. ∎

**Code witness.** `src/chain/chain.cpp:1327` (the gate); `src/chain/chain.cpp:970` (upstream nonce gate, which precedes this and is unaffected by the balance check).
...
Δregistrants_[d].inactive_from = (b.index + 1) + δ_reg − UINT64_MAX_as_NULL
                              ...
                              and δ_reg = derive_registration_delay(b.cumulative_rand, tx.hash)
...
*Proof sketch.* By inspection of `chain.cpp:1298–1315`. The nonce gate at line 970 admits the tx (hypothesis). `charge_fee(sender, tx.fee)` at line 1299 debits the fee (succeeds under the balance hypothesis). The registry lookup at line 1300 finds the entry (hypothesis). `inactive_from = height + derive_registration_delay(b.cumulative_rand, tx.hash)` at line 1303 — the `derive_registration_delay` helper (now header-only inline in `include/determ/chain/registration_delay.hpp`, so the light client's registry tracking computes identical activation heights by construction, S-043 discipline) returns `1 + (v % REGISTRATION_DELAY_WINDOW_BLOCKS)` where `v` is the leading 8 bytes of `SHA256(tx.hash ‖ b.cumulative_rand)` — so `inactive_from ∈ [b.index + 1, b.index + REGISTRATION_DELAY_WINDOW]` with a value the deregistering operator cannot pre-pick (the binding through `b.cumulative_rand`, which is committee-randomness-derived per F0 §1.3 V9, prevents operator-side bias on the deactivation height). Line 1305 writes the registry's `inactive_from`. Lines 1307–1311 conditionally write `stakes_[d].unlock_height = inactive_from + unstake_delay_` — the deferred-unlock-height. `sender.next_nonce++` at line 1313 bumps the nonce. `stakes_[d].locked` appears nowhere in the branch's write set — the stake is not released by DEREGISTER. The slashing-window safety claim follows: any equivocation or suspension event with `evidence_height ∈ [b.index + 1, inactive_from + unstake_delay_]` lands while `stakes_[d].locked > 0`, so the slash mechanism (suspension `chain.cpp:1782–1797`, equivocation forfeit `1813–1825`) can still take effect on the staked value. ∎

**Code witness.** `src/chain/chain.cpp:1298–1315` (DEREGISTER branch); `include/determ/chain/registration_delay.hpp` (`derive_registration_delay` helper); `include/determ/chain/chain.hpp` (`unstake_delay_` field); `include/determ/node/registry.hpp:15` (`REGISTRATION_DELAY_WINDOW` constant = 10).

**Test witness.** `tools/test_unstake_deregister_apply.sh` (`determ test-unstake-deregister-apply`) — the DEREGISTER scenario asserts `inactive_from = height + derive_registration_delay > height` and `unlock_height = inactive_from + unstake_delay`. Three assertions in the DEREGISTER scenario block; the `set_unstake_delay(1)` call in the test bypasses the production 1000-block window to make the test fast.
...
*Proof sketch.* By inspection of `chain.cpp:1338–1359`. The nonce gate at line 970 admits the tx (hypothesis). The payload-size check at line 1339 admits (hypothesis). `amount` is decoded at lines 1340–1342. `charge_fee` at line 1343 succeeds (balance hypothesis); state at this point shows `sender.balance -= tx.fee; total_fees += tx.fee;`. The conditional at lines 1345–1346 is the unlock-failure check: `(sit == stakes_.end()) OR (sit->second.locked < amount) OR (height < sit->second.unlock_height)` — under the hypothesis at least one of (a), (b), (c) holds, so the branch enters the refund body at lines 1347–1352. Line 1349 refunds the fee: `sender.balance += tx.fee` (un-doing the `charge_fee` debit). Line 1350 reverses the `total_fees += tx.fee` from `charge_fee` so the per-block fee accumulator is also restored. Line 1351 bumps the nonce (the nonce slot IS consumed — see Discussion §3 below for the rationale). Line 1352 `break`s out of the switch. `stakes_[d]` is untouched. The net deltas are: balance unchanged (+fee on refund cancels −fee from charge_fee), `total_fees` unchanged (same cancellation), `stakes_[d].locked` unchanged (no write reaches line 1355), `next_nonce` bumped by exactly one. The branch's only persistent state mutation is the nonce bump. ∎

**Code witness.** `src/chain/chain.cpp:1343–1352` (the charge / refund / bump pattern); `src/chain/chain.cpp:954–963` (`charge_fee` lambda); `src/chain/chain.cpp:1349–1350` (the refund itself — load-bearing for the UX guarantee).
...
with no other state mutation. The A1 unitary-supply invariant holds: `amount` moves from `stakes_[d].locked` to `accounts_[d].balance` (an intra-domain transfer), and the `tx.fee` enters `total_fees` for distribution (same channel as STAKE's fee). Total supply is invariant; the apply-tail A1 check at `chain.cpp:1866-1888` succeeds.

*Proof sketch.* By inspection of `chain.cpp:1338–1358`. The nonce gate at line 970 admits the tx (hypothesis). Payload size at line 1339, amount decode at lines 1340–1342. `charge_fee` at line 1343 succeeds (balance hypothesis): `sender.balance -= tx.fee; total_fees += tx.fee;`. The conditional at lines 1345–1346 is FALSE under the hypothesis (stake entry exists, `locked ≥ amount`, `height ≥ unlock_height`), so the refund branch at lines 1347–1352 is skipped. The apply proceeds to line 1354 `__ensure_stakes()` (no-op if `stakes_` already exists, which it must by `sit != stakes_.end()` from line 1344), line 1355 `sit->second.locked -= amount` debits the stake, line 1356 `sender.balance += amount` credits the balance, line 1357 bumps the nonce. The four deltas are exactly the four state writes. A1 closure: the `amount` round-trip from stake to balance is captured in I-5 of FA-Apply (the UNSTAKE post-unlock credit channel + the STAKE locked-balance debit channel), and `tx.fee` enters `total_fees` exactly as for STAKE. ∎

**Code witness.** `src/chain/chain.cpp:1354–1358` (the unstake success path); `src/chain/chain.cpp:1866-1888` (A1 closure).
...
*Proof sketch.* In the `staked-active` state, `state.stakes_[d].unlock_height == UINT64_MAX` (initialized to that value at REGISTER per `chain.cpp:1266–1270`, never modified except by DEREGISTER which has not yet fired by hypothesis). The unlock-failure condition at `chain.cpp:1346` evaluates `height < UINT64_MAX`, which is `true` for any `b.index ≤ UINT64_MAX − 1` — i.e., every reachable block height. The refund branch at lines 1347–1352 fires, producing the T-K4 outcome. The operator must DEREGISTER first to enter the `staked-pending-unlock` state (T-K3), then wait until `b.index ≥ stakes_[d].unlock_height` to satisfy T-K5's success precondition. The sentinel-based gating is what makes "UNSTAKE before DEREGISTER" structurally a no-op rather than a special-cased error: the unlock-height check is the unified gate. ∎

**Code witness.** `src/chain/chain.cpp:1266–1270` (REGISTER initializes `stakes_[d].unlock_height = UINT64_MAX`); `src/chain/chain.cpp:1346` (the unified unlock-height gate); `include/determ/chain/chain.hpp:29` (`unlock_height{UINT64_MAX}` struct default).
...
*Proof sketch.* By inspection of the three branches at `chain.cpp:1298–1359`. Each branch resolves a single `sender` reference at line 966 (`accounts_[tx.from]`), a single `stakes_[tx.from]` access (line 1330 for STAKE, line 1310 for DEREGISTER, line 1344 for UNSTAKE), and at most a single `registrants_[tx.from]` lookup (line 1300 for DEREGISTER, not touched by STAKE or UNSTAKE). Every map access is keyed by `tx.from`; no second-key access (`stakes_[other_domain]`, `accounts_[other_domain]`) appears anywhere in the three branches' body. `std::map` access semantics give the standard guarantee: reading or writing `m[k1]` does not modify `m[k2]` for `k1 ≠ k2` (the underlying red-black tree is structurally shared only by lookup; `operator[]` allocates a node for `k1` if missing without disturbing any `k2` node). Therefore the independence claim follows directly from the per-`tx.from` keying of every access. ∎

**Code witness.** `src/chain/chain.cpp:1317–1336` (STAKE — all accesses keyed by `tx.from`); `src/chain/chain.cpp:1298–1315` (DEREGISTER — same); `src/chain/chain.cpp:1338–1359` (UNSTAKE — same); `include/determ/chain/chain.hpp` (`stakes_` map declaration).
...
The fee-refund branch at `chain.cpp:1349–1350` is a deliberate UX choice. The honest user's only way to know whether `b.index ≥ stakes_[d].unlock_height` at the moment a transaction lands is to consult the chain's current head height at the moment of submission and add a margin for tx propagation + block-production delay. If the operator misjudges that margin (network congestion, peer-list freshness, abort-induced block delay), their UNSTAKE arrives one block early and would — without the refund — burn `tx.fee` for an apply that produced no state mutation. The refund neutralizes the cost: the misclocked tx is a no-op modulo the nonce slot.

**Analogue with `NonceMonotonicity.md` T-N2 (future-nonce silent skip).** A future-nonce tx (`tx.nonce > sender.next_nonce`) is silently dropped at the nonce gate `chain.cpp:970` before `charge_fee` runs. The user pays no fee — same UX outcome as T-K4. The two misclock surfaces (nonce vs unlock-height) are symmetric in their UX guarantee: misjudging either side of the chain's state machine produces a no-op tx, not a fee burn.

The asymmetry is structural: T-N2's silent skip is "before charge_fee," T-K4's silent skip is "after charge_fee with refund." Both routes achieve zero net fee debit. The difference is consumed nonce slot: T-N2 does NOT consume the nonce (the gate fires before any per-type branch), while T-K4 DOES consume the nonce (the gate that fails is the unlock-height check inside the UNSTAKE branch, after the nonce-strict-equality gate has already passed at line 970). The T-K4 nonce consumption is intentional: a non-consuming variant would let an attacker who observed the unlock_height computation re-submit a stream of UNSTAKE-too-early txs at every fresh height to spam the producer's tx-inclusion queue without ever paying a fee. The nonce consumption forces the attacker to either (a) wait for the unlock to actually occur and have their UNSTAKE succeed, or (b) pay the gossip/inclusion cost without making progress on stake — neither of which is amplified.
...
| `include/determ/chain/chain.hpp:850` | `unstake_delay_` field (instance-state mutable via A5 PARAM_CHANGE). |
| `include/determ/chain/params.hpp:23` | `UNSTAKE_DELAY` build-time default (= 1000 blocks). |
| `include/determ/node/registry.hpp:15` | `REGISTRATION_DELAY_WINDOW` (= 10 blocks; bounds `derive_registration_delay` output). |
| `include/determ/chain/registration_delay.hpp` | `derive_registration_delay` helper (header-only inline; randomized cooldown, S-043 one-formula). Called at `chain.cpp:1260` (REGISTER) / `1303` (DEREGISTER). |
| `src/chain/chain.cpp:1266–1270` | REGISTER's initialization of `stakes_[d].unlock_height = UINT64_MAX`. |
| `src/chain/chain.cpp:1298–1315` | DEREGISTER apply branch (T-K3). |
| `src/chain/chain.cpp:1317–1336` | STAKE apply branch (T-K1, T-K2; S-049 `checked_add_u64` cost at `1326`). |
| `src/chain/chain.cpp:1338–1359` | UNSTAKE apply branch (T-K4, T-K5, T-K6). |
| `src/chain/chain.cpp:1782–1825` | Suspension (`1782–1797`) + equivocation forfeit (`1813–1825`) slash branches (FA6 scope; §4 intersection). |
| `src/chain/chain.cpp:1866–1888` | A1 unitary-balance closure assertion. |
...
- **T-K1** (STAKE locks balance) closed via the STAKE branch at `chain.cpp:1317–1336` + I-3 (balance → locked channel) + I-6 A1 closure; regression `test_stake_accounting.sh`.
- **T-K2** (STAKE-on-empty silent skip) closed via the `if (sender.balance < cost) continue;` gate at `chain.cpp:1327`; structurally identical to the TRANSFER negative case in `test_tx_edge_cases.sh`.
- **T-K3** (DEREGISTER deferred-unlock) closed via the `inactive_from = height + derive_registration_delay` + `unlock_height = inactive_from + unstake_delay_` writes at `chain.cpp:1303 / 1310`; regression `test_unstake_deregister_apply.sh` DEREGISTER scenario.
- **T-K4** (UNSTAKE pre-unlock refund) closed via the refund branch at `chain.cpp:1349–1350` (the load-bearing UX guarantee); regression `test_unstake_deregister_apply.sh` "UNSTAKE too-early" + "UNSTAKE insufficient locked".
- **T-K5** (UNSTAKE post-unlock success) closed via the success path at `chain.cpp:1354–1358` + A1 closure; regression `test_unstake_deregister_apply.sh` "UNSTAKE success after unlock".
- **T-K6** (pre-DEREGISTER UNSTAKE silent skip) closed via the `UINT64_MAX` sentinel default at `StakeEntry` initialization + the unified unlock-height gate at `chain.cpp:1346`; regression `test_unstake_deregister_apply.sh` "UNSTAKE too-early" (covers the sentinel case).
...
The proof's foundation rests on a small set of code primitives: the `charge_fee` lambda, the `derive_registration_delay` helper, the `UINT64_MAX` sentinel on `StakeEntry.unlock_height`, and the strict-equality nonce gate from FA-Apply-3. The breadth of consequences — seven theorems, a fully-pinned three-state machine, a fee-refund UX contract, and a documented slashing-window intersection — is testimony to how few primitives the chain actually needs to express the lifecycle.
