...
**Why this escapes the `S030-D2-Analysis.md` §2 trap.** The §2 fatality applies to digesting each member's *raw local* view, which diverges under gossip-async. Here the digested value is `vr` over the *post-reconciliation* admitted set, which by T-4 is a deterministic function of the K *committed* views — every honest member computes the same admitted set (sites 1+3) hence the same `vr`, so the K-of-K gather converges. The digest append is safe *because* it sits downstream of the intersection reconciliation, not upstream of it. This is the structural distinction the equivocation/abort dimensions do not yet satisfy (they have committed view-roots carried but no union-reconciliation wired into `build_body`/`compute_block_digest`), which is why the present closure is inbound-only.

---

## 5. Scope and non-closure of the evidence dimensions

This binding closes the S-030 D2 removal gap for the **inbound** dimension only. The equivocation and abort dimensions remain open with respect to the *digest*:

- `Block` carries `creator_view_eq_roots` and `creator_view_abort_roots` (signature-bound via the Phase-1 commits / `creator_ed_sigs` in the digest), but there are **no** `creator_view_eq_lists` / `creator_view_abort_lists` per-creator carried lists, `build_body` does **not** `reconcile_union` `equivocation_events` / `abort_events`, and `compute_block_digest` does **not** append a root over them.
- Consequently the §2 precondition (a deterministic committee-wide reconciled set computed before digesting) is **not** met for evidence; a naive `compute_block_digest` append over `equivocation_events` / `abort_events` would reintroduce the gossip-async stall `S030-D2-Analysis.md` §2 warns about. Closing those dimensions requires first wiring the carry+union-reconcile pattern (the union analog of sites 1+3) so the digested evidence set is the committee-wide `reconcile_union` of *committed* views — only then is an evidence digest append safe.
- The evidence dimensions are additionally backstopped at the **apply layer** (S-033 `state_root` + S-038 producer wiring; `EquivocationSlashingApply.md` / `AbortEventApply.md`), so a divergence is caught at the next block's `prev_hash`/`state_root` chain even without consensus-layer digest binding. The removal-gap closed here is the strictly stronger *signature-layer* guarantee for the inbound slice.

This scoping is consistent with `F2-SPEC.md` §Q1: intersection-reconciled inbound receipts admit a safe pre-digest canonicalization (delivered by `a727cb2`); union-reconciled evidence is the remaining work.
...
This proof does not change any code; it consolidates the argument that the inbound dimension of the S-030 D2 removal gap is closed at the signature layer — a relayer can no longer strip an admitted inbound receipt after K-of-K signing without breaking K-of-K verification (T-3), the change is byte-identical on all non-cross-shard blocks (T-2), the binding is order-independent (T-1), and the bound set is the deterministic committee-wide intersection rather than the producer's local timing (T-4). The equivocation/abort dimensions remain open at the digest layer (§5) and are backstopped at the apply layer pending the union analog of the carry+reconcile+digest pattern.
...
- `docs/proofs/EquivocationSlashingApply.md` (FA-Apply-10) + `docs/proofs/AbortEventApply.md` (FA-Apply-11) — the apply-layer backstop for the still-open evidence dimensions (§5).
- `docs/proofs/F2-SPEC.md` §Q1 — union-for-evidence / intersection-for-receipts asymmetry that scopes this proof to the inbound dimension.
