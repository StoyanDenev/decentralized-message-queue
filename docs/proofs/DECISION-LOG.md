...
**Status:** SHIPPED. `dapps/dsso/assertion.{h,c}` on the `determ-dsso` target; gate `determ-dsso selftest-assertion` (`tools/test_dsso_assertion_module.sh`, FAST, 61 assertions); spec §5 rewritten; `determ test-dsso-assertion` RETIRED. Ledger row **S-106** closed. No L1 accept rule, wire format, apply path or digest is touched — DSSO is off-chain and links `determ-crypto-c99` alone.
...
**Documentation.** New untiered `docs/proofs/DssoAuthenticationAssurance.md` (the claim, the two factors and why their categories differ, the state machine, the limiter's bound, the code loci, the gate, and a blunt "what is NOT claimed" section), indexed in `docs/proofs/README.md`. `v2.25-DSSO-DAPP-SPEC.md` §6 (the attempt-limiting note, now implemented, plus a new §6b stating that §3-§5 is one factor) and §8 (recovery, and the verified `ROTATE_KEY` status) point at what is now built. `docs/SECURITY.md` gains row **S-106** (✅ Mitigated, High) with the scope of the closure stated exactly, the summary's High mitigated-in-session cell moves 24 → 25 and the total 57 → 58, and the test table gains the `determ-dsso selftest-authn` row.
