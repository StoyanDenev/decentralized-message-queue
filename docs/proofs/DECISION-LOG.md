...
## 2026-09-17 — the outbox strace leg is repaired: it could not fail on an architecture without the 2-argument `rename`/`link` syscalls

**Status:** LANDED. Test-and-documentation increment. Zero change to any shipped source file.

**Problem this change solves.** `tools/test_light_outbox.sh` section C is the power-loss half of OB-1: it straces `outbox enqueue` and asserts that an fsync sits between the temp-file write and the publish, and another between the publish and the acknowledgement write. It selected syscalls with `-e trace=fsync,fdatasync,rename,link,write,openat` and located the publish with `(link|rename)` followed by the 2-argument form. Both are x86-64 spellings. The aarch64 kernel ABI has neither a `rename` nor a `link` syscall: glibc's `link()` issues `linkat(AT_FDCWD, ..., AT_FDCWD, ..., 0)` and `rename()` issues `renameat(AT_FDCWD, ..., AT_FDCWD, ...)`. On that architecture the publish was filtered out of the trace before the parse ever ran, so the leg could not pass. It surfaced as the single RED of an otherwise green FAST suite on the first non-x86 build of this tree (Linux aarch64, GCC 11.4.0, 2026-09-17: **319 passed, 1 failed**; `test_light_outbox` 39 of 40 assertions PASS). Reproduced in isolation before the test was touched: a five-line C program that fsyncs a temp file and publishes it shows **zero** `rename`/`link` lines under the test's exact filter on aarch64, and `renameat(AT_FDCWD, "a.tmp", AT_FDCWD, "a.msg") = 0` and `linkat(AT_FDCWD, "b.tmp", AT_FDCWD, "b.msg", 0) = 0` under a full trace, against `rename("a.tmp", "a.msg")` on x86-64. The section-C path publishes with `::link` (`durable_write_new`), so `linkat` is the spelling that actually mattered.

**Severity, stated plainly: no claim was left unsupported.** OB-1's durability contract is independently gated by section B, whose crash-point injection (`DETERM_LIGHT_OUTBOX_CRASH_POINT`) is platform-neutral and passed on aarch64. What was missing is the syscall-order evidence, on architectures this project had never built for. No triage row is opened: this is a coverage gap in a second, redundant leg of a gate, not a defect in shipped behaviour, and the reasoning is written here so the omission is not silent.

**The change (test and documentation only), in four parts.** (1) The strace selection gains `renameat,renameat2,linkat`, and the whole set now goes through the capability probe as well as the binary: an strace that does not know one of these names exits 1 **without creating its `-o` file**, which would turn a platform that should run the leg into a hard RED with a Python traceback and no explanation. Probing the set could itself have cost coverage — a pre-4.10 strace ran the old leg fine and would now SKIP — so the probe DEGRADES rather than skips: it drops `renameat2`, the only name such an strace lacks and one the code under test never issues on an architecture this project builds for, and skips only if the reduced set is still rejected. The SKIP line names which of the four causes fired. Both branches were exercised with a shim `strace` earlier on `PATH`: one that rejects `renameat2` leaves the leg running with all three assertions green, and one that also rejects the `*at` names produces the SKIP with its cause named. A `[ -s "$TR" ]` check reports a missing trace as itself rather than as a parse failure. (2) The publish is matched in both spellings, and **only a call that returned 0** is taken as the publish: the create-new path tries `::link` and falls back to `::rename` on a filesystem without hard links, and a libc may try `renameat2(..., RENAME_NOREPLACE)` first, so a failed attempt must not open the post-publish window. (3) `.*` inside the quoted operands is tightened to `[^"]*` in both the publish and the temp-open patterns so a match cannot span a quote, and `-f` is dropped: nothing in `cmd_enqueue` forks, and a second traced task could only contribute fsyncs the tracee did not earn. (4) Two parser self-checks are added, each removing the fsyncs of ONE window and requiring the same parse to answer RED with the other window's fsyncs still in place.

**What the two self-checks are, and what they are not.** They are self-checks on the parser, not statements about the product, and they are not mutants of the shipped code. The first draft of this increment removed EVERY fsync line and required RED; the independent review showed that to be a tautology (the same predicate filters the input and then finds nothing, so the answer is RED for any input at all, including an empty trace) and it was replaced. The one-window form is falsifiable, and was falsified: see P1 and P2 below.

**Gate.** `tools/test_light_outbox.sh` (FAST). The FILE's assertion count goes **40 to 42**, the two added ones being the parser self-checks in section C (the file scores 39 wherever section C SKIPs, which is every non-Linux platform AND any Linux box without strace, without permission to ptrace — a container under the default seccomp profile is the common case — or with an strace that rejects a name the leg needs). FAST membership and the suite count are unchanged at 320 tests: this increment adds no wrapper and retires none.

**Mutants.** **M9** (the file fsync removed before the publish, the mutant the 2026-09-16 record names): section C assertion 1 RED **on x86-64 and on aarch64** against the repaired leg. **M9b**, a new sibling (the directory fsync removed between the publish and the acknowledgement): RED on both. **P1** (the `tmp < i < pub` conjunct deleted from the parser): turns self-check 2 RED and nothing else. **P2** (the `pub < i < ack` conjunct deleted): turns self-check 3 RED and nothing else. Each product mutant was applied alone to `light/outbox.cpp`, `determ-light` was rebuilt and the build confirmed to succeed before any verdict was read, the gate was run, then the source was restored and rebuilt clean. Separately, the `= 0` requirement was demonstrated on a constructed trace in which a failed `linkat` precedes the real `rename` and no fsync follows the real publish: without the requirement the parse answers `ok` (a false pass, taking the failed attempt as the publish), with it `bad`.

**Independent adversarial review before commit** returned **NOT SAFE AS WRITTEN** on the first draft: 2 blockers and 9 should-fixes. Both blockers are fixed above — the unprobed `renameat2` name (with the measured evidence that strace exits 1 and writes no `-o` file), and the tautological negative control together with the three sentences that had booked it as evidence. Seven should-fixes are folded in: the `= 0` requirement, the comments that named `rename` where the path uses `::link`, the untightened temp-open pattern, `-f`, the in-test mutant index, the platform-qualified assertion count, and the Verification line. Two are recorded rather than fixed, below.

**Riders.** One inline fact correction in an untiered document: `docs/proofs/DurableOutboxSoundness.md` OB-1's gate cell now names the spellings the leg matches, the returned-0 requirement, the two parser self-checks and the two limitations below, and the falsify-on-mutant record carries M9's 2026-09-17 re-run on both architectures and the new M9b. No proof is re-derived; no other document changes.

**Two limitations of this leg, now written into the gate cell rather than left implied.** It checks that SOME fsync falls in each window, not the identity of the two descriptors, so an fsync on an unrelated descriptor would satisfy a window. And the acknowledgement is observed where stdio flushes it (the `write(1, ...)` from the exit-time flush of `std::cout`), not where it is printed, so `publish < ack` here is weaker than it reads; section B is what actually gates the acknowledgement against the publish. Binding the descriptors is a further increment with its own mutants.

**Noticed and deliberately left alone, with the locus.** `tools/test_wallet_shamir_rotate.sh` section 28 (the output-file permission check) is an assertion that cannot fail: both branches print `PASS` and increment `pass_count`, so a rotated key file written world-readable scores PASS. It is the same defect class as the one this increment exists to close and it is security-relevant, but it is a different gate on a different claim and needs its own mutant, so it lands as its own increment rather than as a rider here. The correct pattern is already in the tree at `tools/test_wallet_cold_sign.sh` section 20, which asserts hard on POSIX and SKIPs elsewhere.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration. No `src/`, `include/`, `light/`, `wallet/` or `dapps/` source file: `light/outbox.cpp` is byte-identical to its state before this entry, the mutants having been applied and restored. No triage row is opened or closed and no summary cell moves. The C leg still runs on Linux only: on macOS, where `full_fsync` uses `F_FULLFSYNC` rather than `fsync(2)`, there is no strace and the leg SKIPs, and this increment does not change that.

**Stated limitation of the run this came from.** The second-platform run was made in the Cowork Linux VM on the owner's machine (Linux aarch64, GCC 11.4.0, CMake 4.4.3), NOT under Apple's toolchain on macOS. `src/main.cpp` could not be compiled there at the project's `-O3`: `cc1plus` was OOM-killed at about 2.3 GB resident at `-O3` and at `-O2` alike on that 3.9 GB VM, so `CMakeFiles/determ.dir/src/main.cpp.o` was produced at `-O1` with otherwise identical flags while the remaining six targets built at `-O3` unchanged. That is a faithful compile of the same source by the same compiler at a lower optimisation level; it is not a substitute for an `-O3` run, and no `-O3`-specific codegen claim rests on it. The macOS / Apple-clang run remains outstanding. The aarch64 evidence in this entry is that one gate and its two PRODUCT mutants, M9 and M9b, not a full suite pass; P1 and P2 edit the embedded parser and are architecture-independent by construction, and were run on x86-64 only: the full aarch64 `ci_local` is run after this lands.

**Verification:** `tools/ci_local.sh` on **Linux x86_64** — build 7 targets, **FAST 320 passed / 0 failed**, 16 doc guards green. `tools/test_light_outbox.sh` alone: 42 pass / 0 fail on x86_64 and 42 pass / 0 fail on aarch64.

**Authority:** the standing verification doctrine in `CLAUDE.md`, under which `tools/ci_local.sh` is the second-platform gate and a gate that cannot fail on a platform is not a gate there. No owner decision is required or implied: this increment changes no shipped behaviour and no recorded decision. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-17 — the shamir-rotate output-permission gate could not fail; and the patch set is now green on three toolchains

**Status:** LANDED. Test-and-documentation increment. Zero change to any shipped source file.

**Problem this change solves.** `tools/test_wallet_shamir_rotate.sh` section 28 checked the mode of the rotated share file and then scored `PASS` whatever it found: the `if` arm printed `output file perms are 600` and the `else` arm printed `output file perms reported '$PERMS' (best-effort; non-fatal)`, both incrementing `pass_count`. The file it is checking carries Shamir shares of a live secret, and `wallet/main.cpp`'s `write_bytes_file_0600` exists to narrow exactly that file to owner-only. So the one gate on that property could not fail. Found while reviewing the increment that landed earlier today for the same defect class in `tools/test_light_outbox.sh`, and recorded there as noticed-and-left-alone.

**Reproduced before it was touched, at the layer where the rule lives.** `write_bytes_file_0600`'s `std::filesystem::permissions(... owner_read | owner_write ..., replace, ...)` was mutated to add `group_read | others_read`, `determ-wallet` was rebuilt, and the mutated binary was confirmed to write mode **644** for a real `shamir-split --out`. The unmodified gate then ran against it and reported `PASS: output file perms reported '644' (best-effort; non-fatal)`, **52 pass / 0 fail, exit 0**. A world-readable file of Shamir shares was green.

**The change.** Section 28 now takes the shape of its correct sibling, `tools/test_wallet_cold_sign.sh` section 20: a `case` on `uname -s` that asserts hard on Linux, Darwin and the BSDs with `assert_eq "$PERMS" "600"`, and on anything else prints an explicit `SKIP: 0600 check (uname=...; POSIX-only assertion)` rather than a pass. One deliberate difference from the sibling: the `stat` chain ends in `|| echo "unreadable"`. Under the `set -e` in force at that point, a system where neither `stat -c` nor `stat -f` works aborts the whole script at the assignment — exit 1 with no assertion line and no summary — so the condition is named and the assertion fails on it instead.

**Gate, and what verifies it.** `tools/test_wallet_shamir_rotate.sh`, **52 assertions, unchanged in count** (the old code incremented `pass_count` on both arms, so the number is the same and only its meaning changed). **This test is NOT a FAST member**: `tools/ci_local.sh` does not run it, so a green ci_local is not evidence about this change and is not offered as such below. It was run directly, on Linux x86_64.

**Mutants.** **W1**, the product mutant above (`write_bytes_file_0600` writes 0644): against the repaired gate it is RED — `FAIL: rotated shares file mode is 0600 / expected: 600 / got: 644`, 51 pass / 1 fail, exit 1 — where against the old gate it was green. **W2**, a shim `stat` on `PATH` that fails only for the rotated file, standing in for a platform where the mode cannot be read: RED with `got: unreadable`, where without the `|| echo` the script instead aborted at the assignment with no assertion and no summary (observed: the log ends at the section header). The product source was restored and `determ-wallet` rebuilt clean after W1; the build was confirmed to succeed before either verdict was read, and the session's mutant harness (an audit-directory script, not a program in this tree) refuses to report a verdict if the mutation did not actually reach the source — it did so once, silently building an unmutated binary, and was hardened before any verdict was taken from it.

**The three-platform result, and a correction to the entry above.** The 2026-09-17 entry on the outbox strace leg closes with *"The macOS / Apple-clang run remains outstanding."* That is no longer true and this entry is the correction, the log being append-only. The owner ran `tools/ci_local.sh` on the Mac: **PASS on Darwin arm64 — FAST 320 passed / 0 failed, 16 doc guards green.** The patch set therefore stands on three toolchains and two architectures: Linux x86_64 with GCC, Linux aarch64 with GCC 11.4.0, and Darwin arm64 with Apple clang. Two qualifications belong with that. First, the aarch64 result is no longer a single Linux VM's word: ARM is now covered by two operating systems and two compilers. Second, the strace leg repaired in the entry above did **not** run on the Darwin pass — macOS has no strace, so section C takes the SKIP branch and `test_light_outbox` scores 39 of its 42 assertions there. On Darwin, OB-1's durability rests on section B's crash-point injection, which did run and did pass. That matters because macOS is the one platform where `full_fsync` takes a different path entirely, `F_FULLFSYNC` rather than `fsync(2)`, and no syscall-order gate anywhere observes that path.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no migration. No `src/`, `include/`, `light/`, `wallet/` or `dapps/` source file: `wallet/main.cpp` is byte-identical to its state before this entry, the mutant having been applied and restored. No triage row is opened or closed and no summary cell moves: the shipped behaviour was already correct — `write_bytes_file_0600` did and does set 0600 — and what was missing was the gate that could catch it changing. `docs/proofs/WalletRecoveryFlows.md` §6.1(g) already states the mitigation; it is not restated here, but note that until today no falsifiable gate stood behind it on this flow.

**Outstanding on this change, stated because it is exactly the standard this increment exists to raise.** The Darwin and BSD arm of the repaired assertion has never been executed anywhere. Before the change, whatever `stat -f "%Lp"` printed on a Mac was reported and passed; it is now load-bearing, and neither this test nor the sibling it copies is a FAST member, so the owner's Darwin `ci_local` run did not execute either of them. `600` is the expected form and a wrong one fails loudly rather than silently, but expectation is the standard being replaced here. One direct run of `bash tools/test_wallet_shamir_rotate.sh` on macOS settles it.

**Noticed and deliberately left alone, with the locus.** (1) `tools/test_wallet_cold_sign.sh` section 20, the sibling this change copies, has the same latent `set -e` abort on an unreadable mode: correct in its assertion, silent in that one failure path. (1b) `write_bytes_file_0600` (`wallet/main.cpp:239-247`) DISCARDS the chmod result — `std::filesystem::permissions(..., perm_ec); (void)perm_ec; return true;` — so on a filesystem or platform where narrowing the mode fails, the file stays at the umask mode and the command still reports success. It has eleven call sites, every one of them writing key material; exactly two are now gated, this one and `tools/test_wallet_cold_sign.sh` section 20 over cold-sign. `keyfile-create` (`wallet/main.cpp:3448`) is the next one worth a gate. Recorded for the owner: making a failed narrowing fatal is a change to shipped behaviour and belongs in its own increment. (1c) The repo now carries two incompatible SKIP conventions, and this change entrenches one of them: `tools/test_light_outbox.sh` section C skips WITHOUT incrementing `pass_count` (hence the 42 and 39 in the entry above), while this section and its sibling increment on skip (hence "52, unchanged"). Both are internally consistent; together they mean a summary's "N pass" does not mean the same thing in two adjacent files. A `skip_count` in the summary is the fix and it is a repo-wide convention change, not this increment's business. (2) `docs/proofs/WalletRecoveryFlows.md` §7 records `tools/test_wallet_shamir_rotate.sh` as **33 assertions** where the file has **52**; pre-existing drift that this change does not create and does not correct, since the count is unchanged by it. (3) `tools/test_fa_liveness_virtual`, a FAST member, bounds itself with real `steady_clock` deadlines and `sleep_for` polling rather than the virtual clock its name promises, so its verdict depends on machine load: it went RED once on the loaded aarch64 box and green on a re-run and twice standalone, with identical output. That is a gate whose result is not a function of the code alone, and closing it means changing a consensus liveness test, so it is recorded for the owner rather than fixed in passing.

**Verification:** `tools/test_wallet_shamir_rotate.sh` directly on Linux x86_64 — 52 pass / 0 fail clean, 51 pass / 1 fail under each of W1 and W2. `tools/ci_local.sh` on Linux x86_64 — build 7 targets, FAST 320 passed / 0 failed, 16 doc guards green — run to confirm this change breaks nothing else, NOT as evidence for the gate itself, which ci_local does not execute.

**Authority:** the standing verification doctrine in `CLAUDE.md`, under which a gate that cannot fail is not a gate. No owner decision is required or implied: this increment changes no shipped behaviour and no recorded decision. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-17 — the wallet wrote every secret to disk world-readable first and narrowed afterwards; the window is closed on POSIX and a failed narrowing is no longer silent

**Status:** LANDED. Wallet-only increment (`wallet/main.cpp`), one new FAST gate, one new ledger row. No consensus, apply, wire, digest or genesis surface.

**Problem this change solves — two defects, both reproduced before anything was touched.**

**P-1, the window.** `write_bytes_file_0600` is the helper through which ten wallet commands put key material on disk. It created the file with `std::ofstream`, which opens at 0666 masked by the umask (0644 under the usual `umask 022`), wrote the complete secret into it, closed it, and only then called `std::filesystem::permissions(..., 0600)`. `strace` of a plain successful `shamir-split --out`, nothing injected and nothing mutated:
...
Measured directly with an `LD_PRELOAD` observer on the narrowing call: *at narrowing time the file is mode 0644 with 60 bytes already written* — the whole share set. Any local process that can `open(2)` the path during that window reads the key material. Eight of the ten helper callers write PLAINTEXT key material (`shamir-split`, `shamir-rotate`, `account-create-batch`, `account-derive-batch`, `account-import`, `account-export`, `keyfile-recover`, `account-recover`; `keyfile-create` and `keyfile-reencrypt` write the encrypted DNK1 envelope). The final mode is 0600, so nothing that looks afterwards can see it: `keystore-audit`, the `mode_not_0600` hazard tag and the section 28 assertion landed earlier the same day all observe a correct file and report nothing. This is not exotic — it happens on every platform, every filesystem, every successful run.

**P-2, the silence.** The same function called `permissions(..., perm_ec)` and then `(void)perm_ec; return true;`. Under an `LD_PRELOAD` shim making `chmod`/`fchmod`/`fchmodat` return `EPERM` — standing in for a filesystem or platform where narrowing cannot happen (FAT/exFAT, some network and FUSE mounts, a path owned by another user) — `shamir-split --out`, `shamir-rotate --json` and `keyfile-create --json` each exited **0**, left the file at **644**, printed **nothing** on stderr, and reported success in their `--json` summary. The daemon binary already does the right thing here (`src/main.cpp::cmd_account_create`, plaintext `--out` branch: name the file, print `perm_ec.message()`, tell the operator to verify manually) — but note it is itself a P-1 instance, so it is a model for the error handling only, not for the ordering.

**The change.** In `write_bytes_file_0600`, on POSIX only: `::open(path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_NOFOLLOW, 0600)` **and** `::fchmod(fd, 0600)` before the first write. BOTH are needed and this is the measured part: `O_CREAT | O_TRUNC` with a mode does **not** narrow an existing file. Against the mutant that keeps the create mode and drops the `fchmod`, on a fresh create the observer reports `mode 0600 with 60 bytes already written`, and on a PRE-EXISTING 0644 target the same binary reports `mode 0644 with 60 bytes already written` and still ends at 0600. So the create mode alone leaves the entire window on every overwrite — the `--force` path and any second run — while a fresh-create test stays green. `fchmod` on the descriptor closes it on both paths with no TOCTOU. `O_NOFOLLOW` is deliberate and independent: without it a pre-existing symlink at the path is followed and the secret is written into whatever file it names, and the call sites' `fs::exists` checks are TOCTOU-racy, not a substitute. `O_CLOEXEC` keeps the descriptor out of any child.

On **Windows nothing is closed and the increment does not pretend otherwise**: the Windows branch keeps the previous `ofstream` code verbatim, because `_S_IREAD` / `_S_IWRITE` drive only `FILE_ATTRIBUTE_READONLY` and the effective ACL comes by inheritance from the parent directory — already recorded in `docs/proofs/S005PassphraseKeyfile.md` F-4. The code comment and the ledger row say so in those words.

For P-2 the trailing `permissions` call is KEPT as the belt-and-braces step, its `error_code` is now CHECKED, and a failure emits the daemon's diagnostic (file, `perm_ec.message()`, `Verify manually (chmod 0600 / icacls).`) and is reported back to the caller through a new defaulted `bool* perms_narrowed_out`. A failing `fchmod` is reported the same way. The write still SUCCEEDS, the exit code is unchanged and the file is NOT deleted: destroying a `keyfile-recover` result the operator may be unable to regenerate is a worse outcome than a wide file they were told about. Seven commands that already print a `--json` summary on the write path now add `"perms_narrowed": false` to it when the narrowing failed — `shamir-rotate`, `keyfile-create`, `keyfile-reencrypt`, `keyfile-recover`, `account-recover`, and the two sites below. The field is emitted **only on failure**, so every success summary is byte-identical to what it was and no consumer, golden or documented shape changes; its absence carries no new promise, and the loud stderr diagnostic accompanies every `false`.

Two independent sites are routed through the hardened helper, both near drop-ins that already built a single buffer, with the `determ_secure_zero` moved after the call: `backup-create --shares-out` (the DSS1 Shamir share-set) and `keyfile-decrypt --out` (the decrypted plaintext node key). One Windows-only consequence, stated rather than hidden: the helper writes in binary mode, so `keyfile-decrypt --out` now emits LF where the old text-mode stream emitted CRLF on Windows; on POSIX the bytes are identical, and the daemon's JSON parser accepts both. The in-code "matches `save_node_key` byte-for-byte" note is corrected in place to say it is a POSIX statement.

**A correction owed to this log, which is append-only.** The 2026-09-17 entry on the shamir-rotate gate says, under "noticed and deliberately left alone", that `write_bytes_file_0600` *"has eleven call sites, every one of them writing key material"*. It has **ten** callers. **Eleven** was the count of `std::filesystem::permissions` sites in `wallet/main.cpp`, of which exactly **one** was inside the helper; the other ten were independent code each with its own `ofstream` → write → `permissions` → `(void)perm_ec` sequence, and they are **not** all key material — four of them write signed transactions (`cold-sign`, `sign-anon-tx`, `tx-batch-sign`, `param-change-build`). The true surface was twenty output paths, not ten, which is why a helper-only change would have fixed one narrowing site of eleven. After this increment there are nine `permissions` sites: one in the helper and eight independent.

**The gate.** New wrapper `tools/test_wallet_out_perms.sh`, **18 assertions**, added to the FAST `ONLY_PATTERN` in `tools/run_all.sh` — FAST membership is not automatic and without that line `ci_local` would not run it, exactly as it does not run `tools/test_wallet_shamir_rotate.sh`.

*Leg A, ordering (Linux, strace), against a **PRE-EXISTING 0644 target***, which is the path where the defect survives a create-mode-only fix; a gate that only tested a fresh create would test the wrong thing. The assertion is: among the mode-setting calls affecting that file between the creating open and the first `write` to that descriptor, **at least one exists and the last one sets 0600**. It does **not** assert on the mode argument of the `openat`: measured, `strace` prints `openat(..., O_CREAT|O_TRUNC, 0600) = 3` on a file that stayed 0644, so that assertion would gate the source text of the call rather than the property. It does not assert "no write precedes the openat" either — a write to fd N cannot precede the openat that returns N, which is a tautology. Path-based calls (`chmod` / `fchmodat` / `fchmodat2` naming the target) count alongside descriptor-based ones, or the widen-between mutant below would pass. The creating open is selected by `O_CREAT` **and** a mode **and** a return >= 0, since the target path also appears in non-creating opens and a failed open creates nothing. The trace set is behind a capability probe of the SET as well as the binary (the pattern landed the same day in `tools/test_light_outbox.sh`), includes `fchmodat2` (glibc >= 2.39 / Linux >= 6.6) and degrades without it, and SKIPs **by name** when strace, ptrace or the trace set is unavailable. A parser self-check removes the pre-write mode-setting lines and requires the same parse to answer RED.

*Leg B, fault injection (anywhere `LD_PRELOAD` and a C compiler work).* The wrapper generates and builds a shim making `chmod`/`fchmod`/`fchmodat` return `EPERM`, probes with a compiled program that `LD_PRELOAD` actually intercepts (it does not under macOS SIP, and there is no such mechanism on Windows) and SKIPs by name otherwise, then runs `shamir-rotate`, `backup-create` and `keyfile-decrypt` against pre-existing 0644 targets under it and asserts BOTH halves: the harm is real (the file is left at **644**) AND the command says so — the pre-write diagnostic, the final-narrowing diagnostic, the verify-manually line, and `"perms_narrowed": false` in the `--json` summary. A fourth block asserts the success path is unchanged: 0600, empty stderr, and **no** `perms_narrowed` field.

A SKIP prints `SKIP:` with the cause and does **not** increment the pass count.

**Mutants**, each applied ALONE, rebuilt, and restored + rebuilt clean afterwards. The harness refuses to report a verdict unless a marker it injects is present in the source AND the build log shows `wallet/main.cpp` recompiled and `determ-wallet` relinked — a harness in this project silently built an unmutated binary once.

| id | mutation | verdict |
|---|---|---|
| M1 | restore the `std::ofstream` create (the original shape) | RED leg A + leg B (pre-write diagnostic) |
| M2 | keep the create mode, DROP the `fchmod` | RED leg A + leg B (pre-write diagnostic) |
| M3 | restore `(void)perm_ec` | RED leg B (final-narrowing diagnostic) |
| M4 | `fchmod` moved to AFTER the write | RED leg A |
| M5 | create 0600, `chmod 0666`, write, `chmod 0600` | RED leg A |

M5 is the one that matters most: it ends at mode 0600, so it satisfies the section 28 assertion in `tools/test_wallet_shamir_rotate.sh` and leaves the window fully open. All five leave that gate GREEN, which is the measured statement that the final-mode assertion cannot see any of this. M2 is green on a fresh create and RED on the pre-existing target, which is why leg A uses the latter.

**Riders: none.** No unrelated code improved, no formatting sweep, no other command touched.

**What does NOT change.** No consensus accept rule, apply path, wire format, digest, genesis or state layout; no migration; no `src/`, `include/`, `light/` or `dapps/` file. No exit code changes on any path — a failed narrowing still exits 0. No file is deleted on a failed narrowing (considered and rejected: a `keyfile-recover` result may be unregenerable). No new CLI flag. Every success-path `--json` summary is byte-identical. `tools/test_wallet_shamir_rotate.sh` section 28 stays green: the final mode is unchanged by this work.

**Noticed and deliberately left alone, with the locus.** (1) `wallet/main.cpp:2279` — `account-import-many` writes one plaintext DAK1 keyfile PER RECORD inside a loop with a `determ_secure_zero` between records, so it needs an open-restricted-then-stream shape rather than a buffer swap; it is the named follow-up and it is plaintext key material. (2) The other seven independent `permissions` sites keep the old `ofstream` → narrow → `(void)perm_ec` sequence: `wallet/main.cpp:2311` (`account-import-many` summary), `:3215` (`backup-create --envelopes-out`, AEAD-wrapped), `:4207` (`keyfile-rotate`, whose staged tmp file is also created by `ofstream` at the umask mode and narrowed only after the atomic rename — so the wide file exists under the FINAL name), and `:9776`, `:10161`, `:10778`, `:23541` (`cold-sign`, `sign-anon-tx`, `tx-batch-sign`, `param-change-build` — signed transactions, not keys). (3) `wallet/main.cpp:19026` and `:19493` — two bare `::chmod(out_path, 0600)` calls that ignore their return; both write audit/history exports (account identifiers, not keys) and both carry a comment recording that a failed chmod deliberately does not fail the export. Prior decision, lower sensitivity, untouched. (4) The write-and-rename shape (create the temp at 0600, fsync, rename over the target) is the better end state and would also make the overwrite atomic; `keyfile-rotate` already has the rename half. Not bundled here. (5) Windows, per the above. (6) `docs/CLI-REFERENCE.md` documents the success-path `--json` shapes of the seven affected commands and is UNCHANGED, because the failure-only field falsifies none of those sentences; documenting `perms_narrowed` there is a seven-row edit to a CRLF file and is its own increment. (7) `docs/proofs/WalletRecoveryFlows.md` section 8 cites `wallet/main.cpp:639-919`, `:1607-1773`, `:3239-3483` and `:3553-3940` for four `cmd_*` functions; those ranges were already stale before this change and this change shifts them further. In-bounds line drift, which `tools/test_doc_citation_bounds.sh` deliberately does not check; not re-derived here. (8) `docs/SECURITY.md` records S-091 — `save_node_key` writes the node identity seed with a bare `ofstream` and **no** permission call at all (`src/crypto/keys.cpp::save_node_key`) — as the same class at its limit. It stays OPEN and is not touched.

**Ledger.** New row **S-109** (`⚠ OPEN`, Medium) in `docs/SECURITY.md` covering P-1 and P-2 across the wallet output surface, stating what this increment mitigates and what it does not, and cross-referencing S-091. The summary Medium cell moves 10 to 11 and the Open total 33 to 34; `tools/test_security_ledger_coherence.sh` green. `docs/proofs/WalletRecoveryFlows.md` section 6.1(g) is corrected inline — the ORDERING was the substance of that row and it was wrong — and section 7 gains the new gate's row.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 321 passed / 0 failed** (320 before; this increment adds one FAST wrapper), 16 doc guards green. `tools/test_wallet_out_perms.sh` alone: 18 pass / 0 fail, 0 sections skipped on this box. Each of M1..M5 RED against the REBUILT binary on the leg named above, restored and rebuilt clean afterwards.

**Authority:** the standing verification doctrine in `CLAUDE.md` (provable security B3; assert at the layer where the rule lives; smallest increment that keeps the tree green AND truthful) and the node-/client-local backlog authorized under D19a, item (11) of the 2026-09-16 implementation sequence. No consensus decision is touched and none is implied. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only; nothing above it was edited, and the numeric correction it carries is stated rather than applied in place.
## 2026-09-17 — the node identity seed was written world-readable; the permissions half of S-091 closes, the plaintext half does not

**Status:** LANDED. Shipped-source change in `src/crypto/keys.cpp` (POSIX only) plus a new FAST gate. **S-091 stays ⚠ OPEN** and no summary cell in `docs/SECURITY.md` moves: what closed is the file's permissions and the visibility of a failure, not the encryption the row exists for.

**Problem this change solves.** `crypto::save_node_key` writes the node identity's 32-byte RFC 8032 SEED — the key that stakes, signs blocks and IS the node's consensus identity — as plaintext hex in JSON, and until today it wrote it through a bare `std::ofstream`. `std::ofstream` creates at `0666 & ~umask`, so the file landed at **0644** under the default umask 022 and at **0666** under umask 000, and nothing narrowed it afterwards: unlike the wallet's `write_bytes_file_0600`, which at least chmods after the write, this path made **no permission call at all**. The containing directory came from `fs::create_directories` at `0777 & ~umask`, i.e. **0755**. The callers, confirmed by reading them rather than assuming: `determ init` (`src/main.cpp` `cmd_init`, the `if (fs::exists(kpath))` / else keygen block) and `determ start` on first run (`cmd_start`, the `if (!fs::exists(cfg.key_path))` block). The brief's line numbers for both were correct. Every other call site in the tree is inside a `test-*` selftest block.

**Reproduced first, executed, before anything was touched.** Against the binary built from HEAD on Linux x86_64 (transcript in the session's audit directory): `determ init --data-dir D` under umask 022 produced `D` at **755** and `D/node_key.json` at **644**, containing `"priv_seed": "<64 hex>"` in clear; `su -s /bin/sh nobody -c 'grep priv_seed …'` **printed the seed**. The same command under umask 000 produced **777** and **666**. Under `strace`, the only line touching the key was `openat(AT_FDCWD, ".../node_key.json", O_WRONLY|O_CREAT|O_TRUNC, 0666) = 3` and the count of mode-setting calls on descriptor 3 was **zero**. That trace is also the concrete instance of today's wave lesson: strace printed the mode argument `0666` for a file that ended up 0644, so the printed mode says nothing about what the kernel used.

**The change (`src/crypto/keys.cpp::save_node_key`, POSIX arm only).** The file is created with `::open(path, O_WRONLY|O_CREAT|O_TRUNC|O_CLOEXEC|O_NOFOLLOW, 0600)` and then narrowed with `::fchmod(fd, 0600)` **before the first byte is written**, the body being written with a `::write` loop that handles `EINTR` and short writes. Both the create mode and the `fchmod` are load-bearing and neither implies the other: measured, `O_CREAT|O_TRUNC` with a mode does **not** narrow a file that already exists, so the create mode alone leaves the window open on every rewrite, while a `::chmod` by path after the write is both a TOCTOU window and a period in which the seed is on disk world-readable. `O_NOFOLLOW` matters because without it a symlink planted at the path is followed, which redirects the seed into a file of the attacker's choosing and aims the narrowing at the wrong inode. A directory that **this call creates** is narrowed to 0700 before the file exists inside it, on that directory's own descriptor (`::open(parent, O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW)` then `::fchmod(dfd, 0700)`) rather than by path, for the same anti-TOCTOU reason. The Windows arm is byte-for-byte what it was.

**Decision 1 — only directories this call created are narrowed.** `fs::create_directories` is called on a path the operator may already own and use for other things: the data dir also holds `config.json`, `chain.json` and exports, and an operator may have deliberately made it group-readable for a monitoring account. Clamping an existing directory to 0700 from inside a keyfile writer is an invisible side effect on state this function does not own, and it can break a deliberate deployment. A directory that did not exist one syscall ago has no such claims on it and its entire purpose at that moment is to hold a node identity, so 0700 there is both safe and strictly better. The two-argument `fs::create_directories(p, ec)` reports whether a directory was actually created; the throwing one-argument form the old code used discarded exactly that fact. **The consequence is stated rather than hidden:** `determ init` creates its data dir **itself**, before calling `save_node_key`, so that directory stays at 0755. The key file inside it is 0600, so the seed is not exposed, but the directory listing is. Narrowing it belongs to `cmd_init` and is a separate increment; it is recorded below.

**Decision 2 — a failed narrowing throws, it does not warn.** `save_node_key` already throws on a write failure, so throwing is the established shape of this function and both callers already handle it (`cmd_start` catches and exits 1). Beyond precedent the choice is not close. A node identity is written **once** — both shipped callers skip keygen when the file exists — and then read on every start for the life of the deployment, so a stderr warning is seen at most once, in a provisioning shell that is usually non-interactive, and never again, while the exposure is permanent. Unlike a wallet recovery transcript, a node identity **can be regenerated at no cost**: refusing costs the operator one re-run after fixing the filesystem, where continuing costs a world-readable identity that stakes and signs blocks. The repo already fails closed on the sibling case — `generate_node_key` throws when the CSPRNG fails rather than writing a weak seed. And the refusal is reachable only at CREATION time, so no provisioned node can be bricked by it on restart.

**Decision 3 — `load_node_key` is unchanged and does NOT warn about a wide mode.** The container is the same JSON it always was, so an existing 0644 `node_key.json` still loads byte-identically and there is no migration; that is asserted, not assumed. It does not warn, for three reasons. The advice a warning could give is wrong: once a 0644 seed has existed on a shared host the remedy is ROTATION, not `chmod`, and telling the operator to chmod invites exactly the false comfort of tightening a leaked file. It is `Node`'s constructor path (`src/node/node.cpp`, `key_ = crypto::load_node_key(cfg_.key_path)`), so it runs on every node start and inside every in-process cluster fixture that constructs Nodes — the line would print into the FAST suite's output repeatedly and become noise. And it is behaviour change on the happy path of every already-provisioned node, which is a rider on a permissions increment. The honest place for it is the D2 src-side keyfile increment, which rewrites this container anyway and can offer a real migration.

**Windows: nothing is closed there and nothing is claimed.** `_S_IREAD | _S_IWRITE` on `_open` drives only `FILE_ATTRIBUTE_READONLY` — it is not a mode and grants nobody anything; the file's ACL arrives by inheritance from the parent directory, and `std::filesystem::permissions` does not rewrite it either. `docs/proofs/S005PassphraseKeyfile.md` F-4 records exactly this for the wallet's keyfiles ("the actual NTFS ACL after the call is operator-environment-dependent"). Calling either here would look like a fix and change nothing measurable, so the Windows writer is left exactly as it was, including its TEXT-mode `ofstream` whose LF-to-CRLF translation is part of the existing on-disk bytes. The exposure stands on Windows and the gate SKIPs there **by name** rather than banking a pass it did not earn.

**Gate.** New FAST member `tools/test_node_key_perms.sh`, three legs, **7 wrapper assertions over 18 in-process ones**, all asserting OUTCOMES on the filesystem and never source text. Leg A is the in-process property test `determ test-node-key-perms` (18 assertions, 7 scenarios): a path whose parent does not exist yields a 0600 file in a 0700 directory under a fixed umask 022 where the defaults would be 0644 and 0755 — the umask is fixed deliberately, because under umask 077 a plain `ofstream` already yields 0600 and every mutant would survive; a parent the operator already owns is left exactly at 0755; an ALREADY-EXISTING 0666 key file is narrowed to 0600, with a fixture control proving the target really was 0666 first; a symlink planted at the path is refused and its target is untouched; the on-disk JSON is byte-identical to the pre-change writer and an existing 0644 file still loads with no migration and no mode rewrite; and an injected failure of either narrowing throws, leaving no seed in the unprotectable file. Leg B is the outcome of the shipped `determ init` under umask 022 **and** under umask 000 — a mode that is only right under a lucky umask is not a mitigation — with a control assertion that the file under test really does contain `priv_seed`. Leg C is the syscall-order property no in-process check can make: under `strace`, scoped to the one descriptor the key was opened on and bounded by its `close` so descriptor reuse cannot be misread, **among the mode-setting calls on that descriptor before its first write, at least one exists and the last one sets 0600**. Leg C deliberately does NOT assert on the mode argument of the `openat`, for the reason the reproduction above demonstrates. It sits behind a capability probe — strace present, Linux, ptrace permitted, and the trace SET accepted, since an strace that does not know a name in the set exits 1 without creating its `-o` file — in the shape `tools/test_light_outbox.sh` section C uses, with `fchmodat2` in the set because a newer glibc can route `fchmod(2)` through it, degrading to the set without it on an strace that does not know the name. Leg C also carries a self-check on its own PARSER, not on the product: the same parse over the same trace with the `fchmod` lines stripped must NOT answer "closed", or the assertion above would be vacuous.

**Mutants — five, each alone, each against the REBUILT binary, source restored and rebuilt clean afterwards.** Every mutant injected a unique marker string and the harness refused to report a verdict unless that marker was found **both** in the patched source and, by `strings`, in the LINKED BINARY; a harness that silently builds unmutated code turns a green gate into a false kill, which happened to a harness earlier in this project. **M1** restores the pre-fix `std::ofstream` create: RED, 3 pass / 4 fail — leg A's file-mode, rewrite, symlink and injection assertions, leg B at both umasks (644 and 666), and leg C. **M2** keeps the `O_CREAT` mode and drops only the `fchmod`, leaving the injection arm intact: RED, 5 pass / 2 fail — killed **only** by the existing-file property and by leg C, exactly as predicted, since a fresh file still lands at 0600 and leg B alone would have passed it. **M3** leaves the created directory at 0755: RED on leg A's directory assertion. **M4** swallows both narrowing failures: RED on all four failure-reporting assertions. **M5** creates 0600, widens to 0666, writes the seed, then narrows back to 0600 — it ends at the right mode and passes every mode assertion in legs A and B, and is killed by leg C alone, which reports `open 600,666`. M5 is the mutant that a weaker ordering assertion (or any final-state assertion) would let through, and it is why leg C is written as "the LAST mode-setting call before the first write" rather than "some call sets 0600". Additionally, and unplanned, the whole pre-fix `save_node_key` was restored and the gate run against it: **3 pass / 4 fail**, 11 of the 18 in-process assertions RED.

**Riders: none.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, no on-disk format change — the JSON container is asserted byte-identical. `load_node_key` is unchanged in behaviour. The only other shipped-source edit is the new `test-node-key-perms` selftest block plus its usage line in `src/main.cpp`, and two guarded POSIX includes.

**What remains OPEN in S-091 after this, by name.** The seed is still **PLAINTEXT hex inside JSON**, where the wallet ships the DWE2 Argon2id + AES-256-GCM envelope and the light client its binary DAK1/DNK1 containers; that is the D2 remainder "node_key.json stays src-owned JSON" (D2-DEFERRED(src)), it is owner-gated, and it is why the row stays ⚠ OPEN and the ledger summary does not move. Also open: nothing at all is mitigated on Windows; `load_node_key` does not warn about an existing wide mode; the `determ init` data dir stays 0755; an ancestor directory that `save_node_key` created on the way to the leaf is not narrowed (only the leaf is), which is harmless while the leaf is 0700 but is worth knowing; `O_NOFOLLOW` guards only the FINAL path component, so an attacker who controls an ancestor directory can still redirect through a symlinked component; `O_NOFOLLOW` does not stop a HARD link either, so an attacker with write access to the containing directory can still aim the write at an inode of their choosing (the narrowing then fails with EPERM and throws whenever that inode is not ours, and `O_EXCL` — which would close it outright — is not used because the rewrite-narrows-an-existing-file property this increment gates requires the rewrite path to work); a pre-existing `node_key.json` is still truncated by `O_TRUNC` before anything is checked, exactly as the old writer did, so a failed narrowing on a path that already held a key destroys it — unreachable from either shipped caller, both of which skip when the file exists; and the seed still passes through a `std::string` and an `nlohmann::json` that are not zeroized, exactly as before, which the D2 container increment is the place to fix.

**On the test-only fault injection.** `DETERM_NODE_KEY_INJECT=fchmod|dirchmod` makes the corresponding narrowing report failure. It exists because a failed narrowing cannot be produced from outside the process on the platforms this is gated on — the test runs as the file's owner and an owner's `fchmod` on a local filesystem does not fail — so without it the "report the failure" rule would have had no gate that can go RED, i.e. no gate. It follows the shape `light/outbox.cpp` already uses (`DETERM_LIGHT_OUTBOX_INJECT`), it is one `getenv` when unset, and it can only ever turn a success into a REPORTED FAILURE, never the reverse, so it cannot be used to weaken a permission.

**Noticed and deliberately left alone, with the locus.** (1) `src/main.cpp` `cmd_init` creates the data dir with `fs::create_directories(data_dir)` at 0755 and never narrows it; the seed inside is 0600 after this change, but the directory listing is world-readable. Narrowing it is a change to a directory that also holds `config.json`, `chain.json` and exports and affects roughly thirty in-tree fixtures, so it is its own increment. (2) `wallet/main.cpp`'s `write_bytes_file_0600` still DISCARDS the chmod error — `(void)perm_ec; return true;` — across eleven call sites, all writing key material; it also chmods by PATH after the write, the TOCTOU shape this increment removed from the node key. The same descriptor-based treatment belongs there and is a separate increment (already recorded in the 2026-09-17 shamir-rotate entry as item 1b). (3) `src/main.cpp`'s `cmd_account_create` plaintext-file path (`determ account create --out`) writes the account privkey with `std::ofstream` and then `std::filesystem::permissions(...)` — the same create-wide-then-narrow window this change closed for the node key, on a file that also holds a private key. (4) `docs/SECURITY.md` states the FAST tier is "299 wrappers" in two places where it is 320 before this change; pre-existing drift this increment did not create and does not correct, since correcting it means editing two large shared paragraphs that another track may be touching today. (5) The repo carries two incompatible SKIP conventions (skip without incrementing `pass_count`, as `test_light_outbox` does and this gate does, versus skip as a banked pass); this gate entrenches the first.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 321 passed / 0 failed** (320 before, plus `test_node_key_perms`), 16 doc guards green. `tools/test_node_key_perms.sh` directly: 7 pass / 0 fail clean; RED under each of M1–M5 with the per-mutant counts above; 3 pass / 4 fail against the fully restored pre-fix `save_node_key`. Every mutant verdict was read only after the build was confirmed to succeed AND the marker was confirmed present in the linked binary.

**Authority:** D19a (the node-/client-local backlog, `docs/SECURITY.md` S-091 "authorized 2026-09-16 (D19a)"), for the PERMISSIONS half only. The encryption half is D2 src-side and owner-gated; it is not taken here and S-091 is not closed. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only per convention and carries no tier marker in its body.
---

## 2026-09-17 — determ-light secret seeds can be supplied off the command line (S-110); the raw flags stay, and now say so

**Status:** LANDED. Client-tooling increment, ADDITIVE ONLY. No consensus accept rule, no apply path, no wire format, no digest, no migration.

**Problem this change solves, reproduced before it was touched.** Every `determ-light` subcommand that takes a private seed took it as a command-line ARGUMENT: `pq-sign-tx --mldsa-seed <hex32> [--ed-seed <hex32>]`, `pq-address --mldsa-seed <hex32>`, `pq-transfer --mldsa-seed <hex32>`, and `build-shield` / `build-unshield --blind-seed <hex>`. A running process's argument vector has three readers the operator did not choose. `/proc/<pid>/cmdline` is world-readable for the lifetime of the command; `ps -ww -o args=` prints the same line to every user on the host; and the operator's interactive shell appends the command verbatim to the history file, where it outlives the process. For an ML-DSA or Ed25519 seed the hex string IS the private key. The reproduction (executed, transcript in the audit dir, not an argument from reading the source) started a `pq-sign-tx` whose `--out` was a FIFO so it blocked in `open(2)` after signing, read the seed out of `/proc/<pid>/cmdline` and out of `ps` from a second process, showed the same seed landing in a `$HISTFILE`, and then rebuilt a valid `PQ_TRANSFER` from the leaked seed — the reader signs as that identity.

**The change.** Every one of those flags gained a `--*-seed-from <source>` twin — `--mldsa-seed-from`, `--ed-seed-from`, `--blind-seed-from` — taking `file:<path>` (first line, whitespace-trimmed), `env:<NAME>`, or `prompt` (stdin, terminal echo disabled). This deliberately MIRRORS `determ-wallet keyfile-create --passphrase-from` (`wallet/main.cpp::passphrase_from_source`): same three source forms, same diagnostic wording, same exit code 1 on a bad source. That helper is in the wallet binary's own translation unit and `determ-light` is a separate CMake target that does not link it, so the parsing is re-implemented in the new `light/seed_source.cpp` rather than dragging `wallet/main.cpp` across a binary boundary. One deliberate divergence: the wallet strips only CR/LF because a passphrase may contain spaces; a hex seed cannot, so the seed reader trims ASCII whitespace at both ends. Passing both forms of one seed is refused rather than silently preferring one. Resolved seed material — the hex string, the decoded 32 bytes, the blinding vector — is `determ_secure_zero`d by a scope guard on every exit path including the throwing ones, matching `light/keyfile.cpp`'s existing discipline; `light/` had no zeroing on this path before.

**What does NOT change, stated plainly because the ledger row depends on it.** The raw `--mldsa-seed` / `--ed-seed` / `--blind-seed` flags still exist and still work: same accepted inputs, same output bytes, same exit codes. Removing them is an owner decision, not a track's — `tools/test_light_pq_sign.sh`, `tools/test_pq_transfer_e2e.sh` and `tools/test_light_ct_tx.sh` all drive them, and those three are the regression lock that the old form is unchanged, so they were deliberately NOT converted. An operator who keeps typing the raw flag keeps the whole exposure. The only behavioural change on that path is a named `WARNING[seed-on-command-line]` line on stderr naming the process table, the shell history file and the `-from` alternative — a warning, not a refusal, exit code untouched. So S-110 is recorded `🟠 Partially mitigated`, not closed. Two smaller diagnostic changes ride on the same path and are stated rather than hidden: a malformed hex seed now names the offending flag (`--mldsa-seed is not valid hex (odd hex length)`) where it previously surfaced the bare `from_hex` message, and `build-shield` / `build-unshield` now decode the blinding hex before loading the keyfile, so when BOTH are bad the hex error is reported first. Exit code 1 in every one of those cases, before and after.

**Gate.** `tools/test_light_seed_source.sh`, new, added to the FAST `ONLY_PATTERN` alternation in `tools/run_all.sh` (membership there is explicit, not automatic); FAST rises 320 -> 321. 28 assertions in five groups, each asserted at the layer where its rule lives. (A) `--mldsa-seed-from file:` produces BYTE-IDENTICAL output to `--mldsa-seed` with the same seed for `pq-address`, `pq-transfer` and hybrid `pq-sign-tx`, and the `-from`-signed tx verifies through `pq-verify-tx` — the equivalence that makes the new form usable. (B) `env:` and `prompt` agree with the raw form, `env:` reads the variable it was NAMED and not another, and a missing file, an empty file, an unset variable, an unknown source scheme, both forms at once and an odd-length or short hex seed are each refused with a named diagnostic and exit 1 — on the raw flag as well as the `-from` one. (C) the raw form emits the named warning and still exits 0 with correct output; the `-from` form writes nothing to stderr; the hybrid `--ed-seed` gets its own warning naming `--ed-seed-from`. (D) the property itself, read from the kernel and not grepped out of the source: with `--mldsa-seed-from` the seed is ABSENT from the running child's `/proc/<pid>/cmdline`, and the control shows the raw flag DOES put it there in the same window. Both children are pinned in a deterministic window by a FIFO and the wait is on `/proc/<pid>/comm`, which no argv edit can forge; the section prints `SKIP:` by name where `/proc/self/cmdline` does not exist rather than banking a pass. (E) `--blind-seed-from` round-trips byte-identically for `build-shield` / `build-unshield` and the raw form warns.

**Mutants, each alone, each RED against the REBUILT binary, source restored and rebuilt clean afterwards.** M1 `-from` silently falls back to treating its argument as raw hex — RED on A1/A2/A3/A4, B1..B5, B7..B9, C3, D1, D3, E1, E3, E4, E5. M2 the warning is dropped on the raw form — RED on C1, C4, E2. M3 a short or odd-length hex seed is accepted (zero-padded) — RED on B6, B7, B11, B12. M4 `env:` reads a fixed variable instead of the one named — RED on B3. M5 the `-from` path writes the resolved seed back into the process's own argument block — RED on D1. The harness REFUSES a verdict unless the injected marker is found in the source, `cmake --build` exits 0, AND the `determ-light` mtime strictly advances; it aborts if the baseline or the restored tree is not green.

**A measured lesson about the /proc assertion, recorded because it nearly banked a false pass.** The first version of group D pinned the child by making the `file:` SOURCE a FIFO, so the snapshot of `/proc/<pid>/cmdline` was taken while the process was still blocked reading the seed — i.e. BEFORE the seed existed in the process at all. Against that window M5 stayed GREEN: an argv write-back that happens after the read is invisible to a sample taken before it. The gate now takes the `-from` window strictly AFTER the signature (ordinary file as the source; the FIFO is `--out`, which `pq-sign-tx` opens only once the envelope is built), and M5 is RED. The general form of the lesson: for a "secret is absent from X" property, the sample must be taken at or after the last moment the code could have put it there.

**Riders.** None. No file outside `light/`, `CMakeLists.txt`, `tools/run_all.sh`, the new wrapper and the three doc/ledger loci is touched.

**Noticed and deliberately left alone, with the locus.** (1) `determ-wallet` takes private key material on the command line in nine places and is NOT changed here — `cmd_keyfile_create --priv` (`wallet/main.cpp:3304`), `cmd_account_import --priv` (`:1633`), `cmd_message_sign --priv` (`:6103`), `cmd_account_derive_batch --seed` (`:1383`), `cmd_create_recovery --seed` (`:5884`), `cmd_shamir_split_raw --secret` (`:431`), `cmd_backup_create --secret` (`:2845`), `cmd_merkle_proof --key` (`:14524`) and `cmd_rpc_auth --secret` (`:23851`). Ironically `keyfile-create` is the very command whose `--passphrase-from` this increment mirrors: it takes the passphrase safely and the Ed25519 seed unsafely, in the same argument list. (2) The `determ` daemon does the same in six places — `cmd_account_create --passphrase` (`src/main.cpp:5857`), `cmd_account_decrypt --passphrase` (`:5969`) and the `--priv` of `cmd_submit_param_change` (`:6133`), `cmd_submit_merge_event` (`:6263`), `cmd_submit_dapp_register` (`:6346`) and `cmd_submit_dapp_call` (`:6472`). (3) `determ-light verify-selection-offline --seed-hex` (`light/main.cpp:10746`) is the PUBLIC `cumulative_rand` beacon seed, authenticated by `verify-rand`, not a secret — left alone on purpose. (4) `file:` does not check the seed file's permissions, and `env:` is still readable at `/proc/<pid>/environ` by the same UID; both match the wallet's `--passphrase-from` exactly and are documented in `light/seed_source.hpp` rather than silently diverged from. (5) `docs/SECURITY.md` §6 still says the FAST tier holds "299 wrappers"; that sentence was already false at 320 before this change and is not this increment's to re-derive. (6) The three existing tests now print the new warning into the suite log; that is the intended effect of using the raw flag and was not suppressed.

**Verification:** `tools/ci_local.sh --jobs 1` on **Linux x86_64** — configure + build **7 targets**, **FAST 321 passed / 0 failed** (320 before; +1 is `tools/test_light_seed_source.sh`), **16 doc guards green**. Gate alone: 28 pass / 0 fail; 27 pass / 1+ fail under each of M1..M5.

**Authority:** the standing doctrine in `CLAUDE.md` — provable security (B3), new behaviour ships with a falsify-on-mutant gate at the layer where the rule lives, smallest increment that keeps the tree green AND truthful. No owner decision is required or implied by an additive flag; the one decision this increment explicitly does NOT take is removing the raw `--*-seed` flags, which is the owner's.

---

## 2026-09-17 — integration pass over the three permissions increments: a daemon abort introduced by one of them, and three corrections this log owes itself

**Status:** LANDED. Fixes a regression introduced earlier in this same wave, hardens two of the wave's own gates, opens S-111, and corrects three false statements in the three entries above.

**Problem this change solves — 1: `determ init` aborted and then silently reported success over an empty node identity.** The entry above that closes the permissions half of S-091 made `save_node_key` THROW when it cannot narrow the seed file to 0600. Its stated justification says "both callers already handle it (`cmd_start` catches and exits 1)". `cmd_init` does NOT. And the throw fires after `O_TRUNC` has already created the file, so a zero-byte `node_key.json` is left behind. Reproduced, executed, before this fix: `DETERM_NODE_KEY_INJECT=fchmod determ init --data-dir D` terminated with `terminate called after throwing an instance of 'std::runtime_error'` and **exit 134**, i.e. SIGABRT and the terminate handler's text rather than the diagnostic that was carefully written. The retry the diagnostic asks for then hit `cmd_init`'s `if (fs::exists(kpath))` guard, printed **"Key already exists (skipping keygen)"**, exited **0**, and told the operator to run `determ start` — which failed with `parse error at line 1, column 1: attempting to parse an empty input`, naming neither the key file nor the permission problem. A crash followed by a false success followed by an unrelated-looking fatal, on the identity a node stakes and signs with. This was introduced by this wave and would have shipped.

**The fix.** `::unlink(path)` before every POSIX throw in `save_node_key` that fires after the file exists — the fchmod-failure throw, the write-failure throw and the close-failure throw — so nothing is left for an exists-guard to misread. `cmd_init`'s keygen is wrapped in the try/catch shape `cmd_start` has always had, so the refusal exits 1 with its own text. After the fix, measured: exit **1**, the real diagnostic on stderr, **no leftover file**, and the retry genuinely regenerates a 165-byte 0600 key that `determ start` loads.

**Problem 2: the gate could not see it.** The selftest's assertion was `slurp(p).find("priv_seed") == npos` — "no seed was written to the unprotectable file" — which a zero-byte leftover satisfies exactly. It now also asserts `!fs::exists(p)`, the form the sibling directory-failure case two blocks below was already using. Mutant N1 (drop the `unlink`, rebuild): **RED** on that assertion, GREEN before it existed.

**Problem 3: one of the wave's two strace legs could not see a path-based widen.** `tools/test_node_key_perms.sh` leg C matched only descriptor-first-argument calls (`fchmod(fd, …)`), while its sibling `tools/test_wallet_out_perms.sh` leg A counts path-based `chmod`/`fchmodat` on the target as well and says why. Demonstrated on the exact adversarial trace — create 0600, `fchmod(fd,0600)`, `chmod(path,0666)`, write, `chmod(path,0600)`, final mode 0600 so every mode assertion in the tree passes — the descriptor-only parser answers **closed** and the corrected one answers **open**. Leg C now counts both, and its own non-vacuity self-check strips both spellings rather than only one. Mutant N2 (a path-based widen inside `save_node_key`, rebuilt) turns leg C RED with `parse said: open 600,666`.

**Problem 4: one of the wave's gates could report PASS having asserted nothing.** `tools/test_wallet_out_perms.sh` printed `PASS:` and exited 0 when the wallet binary was absent, and its verdict was `fail_count == 0` alone — so on a box with neither strace/ptrace nor a C compiler both sections SKIP and `0 pass / 0 fail` would have printed PASS. In the gate whose own increment exists because a permission check could not fail, that is not acceptable. It now fails closed on a missing binary and refuses to report success when `pass_count` is zero. Mutant N3 (point `DETERM_WALLET_BIN` at a nonexistent path): **RED**, exit 1, where it previously printed PASS.

**Problem 5: a spin in one of the two hand-rolled write loops.** `src/crypto/keys.cpp` looped forever on a `write()` returning 0 for a positive count; `wallet/main.cpp`'s sibling writer, written the same day by a different track, guards exactly that case and says so in a comment. Impossible on a regular file, and a hang on the secret path is worse than an error, so the guard is now in both. This is the first visible cost of two independent hand-rolled restricted writers with nothing shared between them; a `write_all` primitive in `include/` is the obvious remedy and is NOT done here.

**Corrections this log owes itself, since it is append-only.**
(a) The S-091 entry's item (2) says `wallet/main.cpp`'s `write_bytes_file_0600` "still DISCARDS the chmod error — `(void)perm_ec; return true;` — across eleven call sites, all writing key material", chmods "by PATH after the write", and that fixing it "is a separate increment". Every clause was already false when that entry landed, because the wallet increment is its parent commit: `perm_ec` is checked, the narrowing is `::fchmod` on the descriptor BEFORE the first write, there are twelve callers not eleven, and four of the independent sites write signed transactions rather than key material. The entry two above it states the corrected counts and is the one to read.
(b) All three entries of this wave, and all three commit messages, record **"FAST 321 passed / 0 failed (320 before)"**. That is true of each increment measured alone in its own worktree. In the integrated linear history the counts are 320 → 321 → 322 → 323, and HEAD is **323**.
(c) The wallet entry's "what does not change" says "no exit code changes". `O_NOFOLLOW` on the output path means a symlinked `--out` (including `--out /dev/stdout`) now fails with `ELOOP` and exit 1 where it previously succeeded. That is the flag's purpose and no test or document used a symlinked output, but it IS an exit-code change and the entry should have said so. The sibling increment made the same change and did document it.

**S-111 opened.** `determ account create --out`, the daemon's plaintext branch, is the same P-1 defect on plaintext key material in the shipped daemon, and it is the one site of its class that S-109, S-110 and S-091 all leave uncovered. Reproduced: the creating `openat` takes mode 0666, the JSON carrying `"privkey"` in clear is written, and the narrowing to 0600 comes last; final mode 600, so nothing after the fact can see it. Its P-2 half is already correct there — it is the site the wallet increment copied its diagnostic from. Not fixed here; this wave was scoped to three other files, and an increment that fixes it should carry its own gate.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration. The on-disk `node_key.json` format is byte-identical and `load_node_key` is untouched; an existing 0644 key still loads. No wallet or `determ-light` source changes in this entry.

**Noticed and deliberately left alone, with the locus.** The three new gates entrench three different missing-binary conventions — `tools/test_wallet_out_perms.sh` now fails closed, `tools/test_node_key_perms.sh` fails through `tools/common.sh`, `tools/test_light_seed_source.sh` exits 0 with no marker — and the repo already had two incompatible SKIP conventions before today; a `skip_count` in the suite summary is the fix and it is a repo-wide convention change. `tools/test_light_seed_source.sh`'s `/proc/<pid>/cmdline` sample is bounded above by a FIFO but has no lower bound on the child's progress, so its "strictly after the signature" claim is stronger than the code; it held on every observed run and the mutant that would exercise it writes argv early. `docs/SECURITY.md` still describes the FAST tier as "299 wrappers" in two places where it is 323. `src/crypto/keys.cpp`'s `blob` still holds the seed in an unzeroized `std::string`. The `determ init` data directory is still created 0755.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 323 passed / 0 failed**, 16 doc guards green. Mutants N1, N2 and N3 each RED against the rebuilt binary and restored clean afterwards; the leg C parser change additionally demonstrated offline on the adversarial trace above.

**Authority:** the standing verification doctrine in `CLAUDE.md`, and the independent adversarial review of the integrated diff that returned NOT SAFE AS WRITTEN on the two blockers fixed here. No owner decision is required: this changes no recorded decision, and the one behaviour change it makes — `determ init` exiting 1 instead of aborting — is the behaviour the increment above it intended and documented. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only per convention and carries no tier marker in its body.
## 2026-09-18 — the daemon's `account create --out` wrote the private key into a world-readable file and narrowed it afterwards; the POSIX window closes on both branches, and a selftest that banked a pass on Windows stops doing so

**Status:** LANDED. Two changes in `src/main.cpp`; one new gate (`tools/test_account_create_perms.sh` plus its in-process `determ test-account-create-perms`) added to the FAST alternation in `tools/run_all.sh`; one row in `docs/SECURITY.md` moved to partially mitigated; two rows in `docs/CLI-REFERENCE.md`; and one header sentence in `tools/test_node_key_perms.sh` corrected. No consensus, apply, wire, digest, genesis or state surface; no migration; no new dependency.

**Problem 1 — the window, reproduced before anything was touched.** `determ account create --out F` is the DAEMON binary's key-generation command. Both of its file branches wrote through a bare `std::ofstream`, which creates at `0666 & ~umask` — 0644 under the usual umask 022 — filled the file with the complete secret, and only then called `std::filesystem::permissions(..., 0600)`. `strace` of a plain successful run against the binary built from HEAD, umask 022, nothing injected and nothing mutated:
...
The 243 bytes are the JSON carrying the fresh 32-byte Ed25519 seed as `"privkey"` in clear. Measured directly with an `LD_PRELOAD` observer on the narrowing call: *at narrowing time the file is mode 0644 with all 243 bytes already written*. With the narrowing denied by a second shim, in a traversable directory, `su -s /bin/sh nobody -c 'grep privkey …'` **printed the key**. The FINAL mode is 0600, so `keystore-audit`, the `mode_not_0600` hazard tag and every after-the-fact check in the tree see a correct file — which is why this survived S-004, S-109, S-110 and S-091. The `--passphrase` branch a few dozen lines below had the identical ordering defect on its AES-256-GCM container. This is the same P-1 defect S-109 records for the wallet, in the shipped daemon, on plaintext key material; it was opened as S-111 by the 2026-09-17 integration pass and left unfixed there.

**Problem 2 — a selftest that banked a pass having asserted nothing.** `determ test-node-key-perms` printed its Windows `SKIP:` line and then, immediately after it, the terminal marker `PASS: node-key-perms all assertions` and returned 0. `tools/test_node_key_perms.sh` leg A greps for exactly that marker, and legs B and C already skip on Windows, so on that platform the wrapper's verdict was 1 pass / 0 fail — green forever, whatever `save_node_key` did. The wrapper's own header claimed "nothing is asserted and nothing is claimed" there, which was true of legs B and C and false of leg A. A gate that cannot fail on a platform is the defect two of the 2026-09-17 increments existed to remove.

**The change (1) — one restricted writer for both branches of `cmd_account_create`.** `write_account_file_0600(out_path, data)` creates with `::open(path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_NOFOLLOW, 0600)` **and** `::fchmod(fd, 0600)` before the first byte, then writes with a loop that handles `EINTR` and short writes and refuses to spin on `write() == 0`. Both calls are load-bearing and neither implies the other, which is the measured part: `O_CREAT | O_TRUNC` with a mode does **not** narrow a file that already exists, so the create mode alone leaves the entire window open on every overwrite while a fresh-create test stays green. `::fchmod` on the descriptor closes the create path and the overwrite path with nothing to race. `O_NOFOLLOW` is independent: without it a symlink planted at the path is followed, which both writes the key into whatever file it names and aims the narrowing at the wrong inode. `O_CLOEXEC` keeps the descriptor out of any child. Both branches are byte-for-byte identical on the wire — the plaintext JSON and the `DETERM-ACCOUNT-V1 <addr>` + blob container are the same bytes as before, and `account decrypt` reads them unchanged.

**On Windows nothing is closed and the increment does not pretend otherwise.** `_S_IREAD | _S_IWRITE` on `_open` drives only `FILE_ATTRIBUTE_READONLY`, and the effective ACL arrives by inheritance from the parent directory — recorded in `docs/proofs/S005PassphraseKeyfile.md` F-4. The Windows arm therefore keeps the previous `std::ofstream` **text-mode** behaviour verbatim, CRLF translation included, so its output stays byte-identical there too; the sibling wallet increment switched to binary mode and changed its Windows bytes, and that is deliberately not repeated. The code comment, the usage text, the ledger row and this entry all say the window is closed on POSIX only.

**P-2, the reporting, was already correct at this site and is UNCHANGED — deliberately.** This is the one place in the tree that checked `perm_ec` and printed a named warning; `wallet/main.cpp::write_bytes_file_0600` copied its diagnostic from here. The trailing `std::filesystem::permissions` call, its `error_code` check and both lines of the diagnostic are the previous ones verbatim. A narrowing that fails before the write is reported with the wording the wallet's sibling writer uses and **the write continues**: exit codes do not change. REFUSING there instead — the shape `src/crypto/keys.cpp::save_node_key` uses for the node identity seed — was written, gated and then **removed from this increment before commit**. It is defensible (a freshly generated key costs nothing to regenerate), but it is an exit-code change on a shipped command, it is separable from the write window this increment closes, and it needed a `getenv` fault-injection hook on the shipped key-write path to be gated in process. CLAUDE.md records that two verified slashing cores were sunk by riders, one of them an exit-code change. It is named as the S-111 remainder instead.

**The ONE exit-code change, stated rather than hidden.** `O_NOFOLLOW` makes `--out` naming a SYMLINK (including `--out /dev/stdout`) fail `ELOOP` with exit 1 on both branches, where it previously followed the link and succeeded. Nothing in the tree used a symlinked `--out`. The `determ` usage text and the `docs/CLI-REFERENCE.md` row for `account create` now say so; the wallet increment made the same change without documenting it and had to be corrected the next day.

**The change (2) — the platform-skip arm banks no pass.** `test-node-key-perms`' Windows arm prints a `SKIP:` marker **instead of** the terminal PASS marker and returns 1, and the new `test-account-create-perms` does the same, so no caller can read success out of either. On a platform where the property cannot be observed the honest answer is "no result", and between silently answering yes and loudly answering no the repo fails closed — which is what `tools/test_wallet_out_perms.sh` already does. `DETERM_TEST_FORCE_PLATFORM_SKIP` makes that arm reachable on POSIX, inside the `test-*` blocks only, because a box that cannot build for Windows could otherwise check this property only by reading the source.

**The gate.** New in-process `determ test-account-create-perms` (**22 assertions**) plus the new wrapper `tools/test_account_create_perms.sh` (**29 assertions**), added to the FAST `ONLY_PATTERN` alternation in `tools/run_all.sh` — FAST membership is an explicit alternation, not automatic, and without that edit `ci_local` would never run it.

*In process,* asserted as outcomes on the filesystem, on the command's exit code, on its diagnostics and on the bytes of the container, never as source text: a fresh target is 0600 and the container, the success line and the empty stderr are unchanged, with a control that the file really holds the key and that the key derives the address in it; a **PRE-EXISTING 0644** target is narrowed to 0600 on **both** branches — the case the `O_CREAT` mode argument provably cannot reach — and `O_TRUNC` really replaced the stale bytes; the envelope still decrypts with the address as AAD; a symlink planted at the path is refused by both branches and its target is untouched; and under umask 000 both branches still land 0600, so the mode is chosen and not inherited. Deliberately NOT in process: the syscall ORDER, which a process cannot observe about itself, and a narrowing that genuinely FAILS, which this process cannot produce — it owns the file it writes and an owner's `fchmod` does not fail. Faking the second with an injection hook would have put test-only code on the shipped key-write path for a property this increment does not change.

*Leg B of the wrapper, ordering (Linux, strace), against a **PRE-EXISTING 0644 target** on both branches*: among the mode-setting calls affecting that file between the creating open and the first `write` to that descriptor, at least one EXISTS and the LAST one sets 0600. It does **not** assert on the mode argument of the `openat` — strace prints `openat(..., O_CREAT, 0600)` whether or not the kernel used it, and the pre-fix trace above prints 0666 for a file that ended at 0644. Path-based calls (`chmod` / `fchmodat` / `fchmodat2` naming the target) are counted alongside descriptor-based `fchmod`, and the non-vacuity self-check strips BOTH spellings; a descriptor-only parser reports `create 0600; fchmod(fd,0600); chmod(path,0666); write; chmod(path,0600)` as closed with the window fully open. The trace set is behind a capability probe of the SET as well as the binary, includes `fchmodat2` and degrades without it.

*Leg C, fault injection*: an `LD_PRELOAD` shim makes `chmod` / `fchmod` / `fchmodat` return `EPERM`, and the shipped command runs under it against a **pre-existing 0644 target** on both branches — with the fix in place a FRESH create under that shim still lands 0600 from the create mode, so a fresh-create leg could not show the harm at all. It asserts BOTH halves: the harm is real (the file is left at **644** with `"privkey"` in it) AND both named diagnostics appear — the pre-write one and the `perm_ec` one — plus the verify-manually line, plus that the exit code did **not** change. A fourth block asserts the success path is unchanged: 0600, empty stderr, exit 0.

*Leg D*: neither this gate's nor `test-node-key-perms`' platform-skip arm prints a terminal PASS marker, each names itself as a SKIP, and each exits non-zero.

Every leg SKIPs BY NAME where its capability is absent, a SKIP does not increment the pass count, the wrapper fails closed when the binary is missing, and the verdict requires `pass_count > 0` as well as `fail_count == 0`.

**Mutants**, each applied ALONE, rebuilt, then restored and rebuilt clean. The harness REFUSES a verdict unless a marker it injects is present in the SOURCE, the build SUCCEEDED, and the marker is present in the RELINKED BINARY. The first run of this harness REFUSED its first verdict for exactly that reason — the marker was a local `const char*` that `-O3` discarded, so it was present in the source and absent from the binary — and the marker was changed to a file-scope `extern "C"` definition before any mutant was judged. That is the check working, not a formality.

| id | mutation | verdict |
|---|---|---|
| M1 | restore the pre-fix `std::ofstream` create-then-narrow on both branches | RED — in-process (symlink refusal) AND leg B (ordering, both branches) AND leg C (the pre-write diagnostic) |
| M2 | keep the 0600 create mode, DROP the `::fchmod` before the write | RED — leg B on both branches (`got: none`) AND leg C; **green on every in-process mode assertion**, which is why leg B runs against a pre-existing target |
| M3 | `::fchmod` moved to AFTER the write (final mode still 0600) | RED — leg B on both branches (`got: none`) |
| M4 | create 0600, `fchmod(fd,0600)`, widen the PATH to 0666, write, narrow back by path (final mode 0600) | RED — leg B on both branches (`got: 666`) |
| M5 | drop the `perm_ec` check and its named warning | RED — leg C (the final-narrowing diagnostic and the verify-manually line, on both branches) |
| M6 | restore the Windows-banked `PASS: node-key-perms all assertions` in the skip arm | RED — leg D (all three assertions for `test-node-key-perms`) |

M4 is the one that matters most: it ends at mode 0600, so every final-mode assertion in the tree is green on it, and only the ordering leg that counts path-based calls kills it. M1 and M2 are green on every in-process MODE assertion too, because the trailing `permissions` call fixes the final mode in both — the measured statement that no after-the-fact check can see this defect.

**Riders: none.** No unrelated code improved, no formatting sweep, no other command touched, and the refusal-on-unprotectable policy described above was deliberately backed out rather than bundled.

**What does NOT change.** No consensus accept rule, apply path, wire format, digest, genesis field, state layout or migration. No `wallet/`, `light/`, `dapps/`, `include/` or `sim/` file. The on-disk containers of both branches are byte-identical, so `account decrypt` and every existing keyfile still work. Exit codes are unchanged everywhere except the symlinked `--out` named above. No new CLI flag. `tools/test_node_key_perms.sh` keeps all three of its legs and its Linux verdict is unchanged; on Windows it now goes RED instead of falsely green, and only its header sentence was edited to say so.

**Noticed and deliberately left alone, with the locus.** **(1) The S-111 remainder, which is why the row is PARTIALLY mitigated and not closed.** (a) Windows: the `#ifdef _WIN32` arm of `write_account_file_0600` (`src/main.cpp:5915`) is the previous `std::ofstream` verbatim and the window there is exactly as wide as it was. (b) When the narrowing genuinely fails the command still writes the plaintext key into a file it could not protect and exits 0 with two warnings; refusing instead is the separable increment described above. **(2) Every remaining path in `src/` that puts secret material on disk, enumerated rather than assumed.** There are exactly two file-writing sites for key material in `src/`, and after this increment both are the same shape: `src/main.cpp` `write_account_file_0600` (this one) and `src/crypto/keys.cpp` `save_node_key` (POSIX-fixed 2026-09-17). `save_node_key`'s Windows arm (`src/crypto/keys.cpp:238`) is unfixed for the same reason as (1a), and the seed it writes is still PLAINTEXT hex inside JSON — S-091 stays OPEN for the encryption, and this increment does not touch it. No other `src/` or `include/` path writes key material: the remaining `std::ofstream` sites in `src/main.cpp` are `chain-export --out` (`:3439`, ledger JSON), `snapshot-fetch --out` (`:4812`, a DSN1 snapshot), the three genesis `.hash` companions (`:5517`, `:5795`, `:5833`) and the `ct-timing-probe` CSV (`:16692`), and outside `main.cpp` they are `src/node/node.cpp:126`, `:5148`, `src/chain/chain.cpp:3348` and `src/chain/genesis.cpp:715` — chain, config and snapshot state, none of it secret. Verified by reading each, not by pattern. **(3) Secret material that reaches a shell history rather than a file, unfixed in this binary.** `determ send_anon`, `submit-param-change`, `submit-merge-event`, `submit-dapp-register`, `submit-dapp-call` and `account address` all take the 32-byte seed as a hex ARGV argument (`src/main.cpp:6195`, `:6276`, `:6409`, `:6491`, `:6620`, `:6170`), so it lands in `/proc/<pid>/cmdline` and in the shell history file. That is the S-110 class, which was closed for `determ-light` only; the daemon has no `--*-seed-from` equivalent. Out of scope here and named so it is not silently dropped. **(4) The key is never zeroized in this command.** `cmd_account_create` holds it in `priv_hex` (`src/main.cpp:6053`), in the `nlohmann::json` object built from it, and in the `std::string` handed to the writer; none is wiped, and `src/crypto/keys.cpp`'s `blob` has the same problem (already recorded 2026-09-17). Wiping needs the JSON path replaced, which is the D2 keyfile work. **(5) The THIRD hand-rolled restricted-write loop.** `wallet/main.cpp::write_bytes_file_0600` and `src/crypto/keys.cpp::save_node_key` are the other two, and they had already drifted from each other on the `write() == 0` case within a day of being written. This increment chose to write the third rather than extract a shared primitive, and says so rather than hiding it: extracting one into `include/determ/` and migrating only these two call sites would leave a header with one consumer and three shapes still in the tree, so the useful version of that change migrates all three — it spans three binaries and two other tracks' files in this wave. Named follow-up: `include/determ/util/restricted_write.hpp` plus all three call sites, as one increment after this wave integrates. **(6) The strace-ordering parser is now in three copies** (`tools/test_node_key_perms.sh` leg C, `tools/test_wallet_out_perms.sh` leg A, `tools/test_account_create_perms.sh` leg B), with the same drift risk; the three are currently in agreement, including on counting both mode-setting spellings. **(7) `O_NOFOLLOW` protects the FINAL path component only.** An attacker who controls a parent directory of `--out` can still redirect the write; the same is true of the two sibling writers, and defending it needs an `O_PATH` directory walk. Not this increment. **(8) `tools/test_node_key_perms.sh` leg A has no skip arm of its own,** so on Windows that wrapper is now RED rather than falsely green. Only its header sentence was edited, to say exactly that; giving leg A a recorded skip belongs to whoever owns `tools/` next, because `tools/*.sh` is another track's in this wave. **(9) `docs/SECURITY.md` still describes the FAST tier as "299 wrappers" in two places,** noticed and left alone on 2026-09-17 and left alone again here: this increment makes the true number 324, a second track in this wave is adding to the same alternation, and a count that two tracks race on belongs in the integration pass, not in either track. **(10) `determ init` still creates its data directory at 0755** (recorded 2026-09-17, unchanged).

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets (`determ`, `determ-wallet`, `determ-light`, `determ-cryptotest`, `determ-dsf`, `d5rp`, `determ-dsso`), **FAST 324 passed / 0 failed** (323 at this branch's base; this increment adds exactly one FAST wrapper), 16 doc guards green. `tools/test_account_create_perms.sh` alone: 29 pass / 0 fail, 0 sections skipped on this box; `determ test-account-create-perms` alone: 22 assertions. Each of M1..M6 RED against the REBUILT binary on the leg named above, with the marker verified present in that binary, and the source restored and rebuilt clean afterwards.

**Authority:** the standing verification doctrine in `CLAUDE.md` (provable security B3; assert at the layer where the rule lives; smallest increment that keeps the tree green AND truthful; no riders), and the node-/client-local backlog authorized under D19a, item (11) of the 2026-09-16 implementation sequence, which is where S-111 sits. No consensus decision is touched and none is implied. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-17 — the two S-109 follow-ups: the per-record bulk-import loop, and an atomic publish that published a world-readable file

**Status:** LANDED. Closes the two `wallet/main.cpp` sites S-109 named as its follow-ups. S-109 stays **⚠ OPEN**: Windows is untouched, and the independent permission sites the row names are untouched.

**Problem this change solves — 1: `account-import-many` put every imported account's private seed on disk world-readable, once per record.** It is the bulk-onboarding command — its own header names a 50,000-account custodial migration — and it was the last wallet site writing PLAINTEXT key material on the pre-2026-09-17 shape: `std::ofstream` → write → `std::filesystem::permissions` → `(void)perm_ec`, inside the per-record loop. Reproduced before anything was changed, from a binary rebuilt at the parent commit; plain successful run, `umask 022`, nothing injected and nothing mutated. Under `strace`, for record **2** (not the first, and its target already on disk at 0644):

```
openat(AT_FDCWD, ".../rec2.keyfile", O_WRONLY|O_CREAT|O_TRUNC, 0666) = 3
write(3, "DAK1KK\320\254\3354\213\374\3532\314\213\243\2432\236\225\322\226\0`IP\233a;\324\315"..., 68) = 68
fchmodat(AT_FDCWD, ".../rec2.keyfile", 0600) = 0
```

and bytes 36..68 of that file are **byte-equal** to that record's `privkey_hex`. Measured a second way, with a passive `LD_PRELOAD` observer that forwards every call to libc unchanged and only records the state of the file at the instant the narrowing is attempted:

```
FCHMODAT .../rec1.keyfile want=0600 was=0644 size=68
FCHMODAT .../rec2.keyfile want=0600 was=0644 size=68
FCHMODAT .../rec3.keyfile want=0600 was=0644 size=68
```

`was=0644 size=68` — world-readable AND already holding the complete container, for **every** record. That matters: a gate that only inspects the first record leaves every later iteration ungated, and at N=50,000 the window opens 50,000 times. The P-2 half was live here too: under a shim returning EPERM from every chmod/fchmod/fchmodat, the command left three 0644 key files, exited **0**, wrote **zero bytes** to stderr and recorded `"status": "ok"` for each.

**Problem 2: `keyfile-rotate` did the atomic-publish dance correctly and published a world-readable file anyway.** It stages to `<out>_tmp.bin`, `fsync`s, and `fs::rename`s over the target — but the staging file was created by `std::ofstream` at the umask mode and narrowed only AFTER the rename. **The mode travels with the inode**, so the wide file existed under the FINAL name and the narrowing that follows closes a window already published through. Measured with the same observer wrapping `rename(2)` and reading the destination's mode the instant the call returns, on a `--out` that pre-existed at 0644:

```
RENAME <tmp> -> <out> srcmode=0644 size=171 rc=0 destmode_after=0644
FCHMODAT <out>        want=0600 was=0644 size=171
```

The staging file is always newly created, so there is no "only on overwrite" escape: this one is unconditional. The content is AEAD-wrapped (DNK1 + DWE2), so its severity sits below the plaintext sites, but it was the one write-and-rename shape in the file and the ordering defect was intact. Its P-2 half was live as well: with narrowing denied the rotation left a 0644 keyfile, printed nothing and reported `"rotated": true`.

**The change.** Both sites are routed through the file's existing restricted-write primitive, `write_bytes_file_0600` — `::open(O_WRONLY|O_CREAT|O_TRUNC|O_CLOEXEC|O_NOFOLLOW, 0600)` AND `::fchmod(fd, 0600)` before the first write, both required because the create mode applies only when the file is created. **No second or third copy of that primitive was written**, which is the specific thing the wave's lesson 17 asks about; nothing here is streamed, so an open-restricted-then-stream writer would have been a new primitive for no gain. The helper gained exactly one parameter, `std::string* err_out`, defaulting to `nullptr`: when non-null the HARD-FAILURE reason goes there instead of stderr. That exists for one reason — `account-import-many` is a fault-tolerant loop whose documented contract is to put a failure in THAT record's summary entry and keep importing, and a stderr-only diagnostic loses the per-record attribution at N=50k. The open / `fchmod` / write / narrow sequence and every guarantee are identical either way; the other eleven callers pass `nullptr` and are byte-for-byte unchanged. The P-2 **warning** still goes to stderr whatever `err_out` says: it describes a file that WAS written and that the operator must go and fix by hand.

For `account-import-many`, both the encrypted and the plaintext branch call the helper per record, and the zeroization stayed exactly where it was — `determ_secure_zero(bytes.data(), …)` immediately after the call, inside the same iteration — because each record is its own file and its buffer must not outlive the iteration that produced it. A record whose keyfile could not be narrowed carries a failure-only `"perms_narrowed": false`, so a fully successful summary is byte-identical to the old shape and the determinism comparison in `tools/test_wallet_account_import_many.sh` keeps holding.

For `keyfile-rotate`, the staging write goes through the helper and the fsync and the atomic rename that follow are untouched — they are the reason that site was otherwise the best-shaped writer in the file. The helper does NOT fsync and was deliberately not taught to: pushing an fsync into a primitive eleven other callers share is a rider. The post-rename `permissions()` call is kept — belt-and-braces on POSIX, the ONLY narrowing on Windows — and its `error_code` is now CHECKED, feeding the same `perms_narrowed` field.

**The gate.** `tools/test_wallet_out_perms.sh` was **EXTENDED, not duplicated**: it already owned this surface and already carried the ordering leg and the EPERM-shim leg for this binary, and it is already in the FAST `ONLY_PATTERN`, so the FAST total is unchanged. **18 → 44 assertions, 0 sections skipped** on this box, 3.7 s.

New section C, under `strace`, runs against **PRE-EXISTING 0644 targets** for both sites, because `open(O_CREAT|O_TRUNC, 0600)` applies its mode only when it creates the file — a fresh-create test cannot tell a create-mode-only fix from a correct one. For `account-import-many` it asserts records **2 and 3**, so a fix that restricts only the first file of the loop goes RED. For `keyfile-rotate` the subject of the assertion is the **staged temp**, because the mode travels with the inode through `rename(2)`: the file published under the final name IS that inode. The property asserted, over the trace, is that among the mode-setting calls affecting the temp between its creating open and the first write to that descriptor at least one EXISTS and the LAST one sets 0600, that nothing between that first write and the rename sets anything but 0600, and that the rename to the final name actually happened — otherwise the first three clauses would describe a file nobody published. Path-based `chmod` / `fchmodat` / `fchmodat2` naming the file count alongside descriptor-based `fchmod`, or a create-0600 → widen-by-path → write → narrow-back mutant reads as closed with the window fully open and satisfies every final-mode assertion in the tree. The leg does NOT assert on the `openat`'s mode argument: `strace` prints that argument whether or not the kernel used it. A parser self-check re-runs the same parse over a trace with the pre-write mode-setting lines removed — **both** spellings stripped — and requires the answer `none`.

Sections B5/B6 add the reporting legs under the shim the wrapper already builds and capability-probes. For `account-import-many` the harm is asserted where it is real: records 2 and 3 are left at **644** while record 1, freshly created, still reaches 0600 from the `open(2)` mode — so the 644 is a statement about the overwrite path and not about a broken create — together with the per-record stderr diagnostic naming a NON-first record and `perms_narrowed: false` on every ok record of the summary. For `keyfile-rotate` the honest assertion is different and is written as such: with every chmod denied the published file is **0600**, because the staging temp is CREATED restricted and the shim cannot touch an `open(2)` mode — at the parent commit this same command left it at **644** with an empty stderr and exit 0, which is the discriminator. Its two diagnostics are distinguished by the PATH they name, fixed-string, so the post-rename one (the `(void)perm_ec` this increment removes) cannot be satisfied by the staging one. Success paths are asserted unchanged in both: 0600, empty stderr, no `perms_narrowed` field anywhere.

**Mutants — each applied ALONE to the verified source, RED against the REBUILT binary, restored and rebuilt clean afterwards.** The harness refuses a verdict unless the mutation text is in the source AND an injected external-linkage marker is in the source AND `cmake --build` exited 0 AND that marker is found by `strings` in the RELINKED binary.

| id | mutation | goes RED on |
|----|----------|-------------|
| M1 | restore the `ofstream` create in `account-import-many` (both branches, narrow after, discard `perm_ec`) | C rec2, C rec3, B5 rec1 mode, B5 stderr, B5 summary field |
| M2 | restore the `ofstream` create of the `keyfile-rotate` staging temp | C rotate ordering, B6 published mode, B6 staging stderr |
| M3 | drop the `::fchmod(fd, 0600)` and keep only the `O_CREAT 0600` mode | section A, C rec2, C rec3, C rotate ordering, B shamir stderr, B5 stderr, B6 staging stderr |
| M4 | in `account-import-many`, restrict only the FIRST record of the loop | C rec2, C rec3, B5 stderr, B5 summary field |
| M5 | in `keyfile-rotate`, go back to discarding the post-rename `error_code` | B6 post-rename stderr |
| M6 | widen the file BY PATH between the restricted create and the first write, narrow again at the end so the final mode is still 0600 | section A, C rec2, C rec3, C rotate ordering |

M3 is the measured lesson restated: it leaves the final mode correct on every fresh create and is caught only by the pre-existing-0644 legs. M6 is the one that kills a descriptor-only parser.

**Riders.** None. The files touched are `wallet/main.cpp`, `tools/test_wallet_out_perms.sh`, the S-109 row in `docs/SECURITY.md`, one falsified sentence in `docs/proofs/WalletRecoveryFlows.md`, and this entry. No consensus accept rule, apply path, wire format, digest, genesis field or migration; no on-disk format change — DAK1 and DNK1 are byte-identical and every existing keyfile still loads; `tools/run_all.sh` is NOT touched, because the wrapper's stem was already in the FAST alternation.

**One behaviour change that is not a bug fix, stated because the previous wave failed to state its own.** The helper opens with `O_NOFOLLOW`, so a per-record target that is a **symlink** now fails with `ELOOP` where `account-import-many` previously wrote through it. Measured both ways on the same input: at the parent commit a symlink planted in `--out-dir` sent 68 bytes of a customer's plaintext seed to the link's target and that record still reported `"status": "ok"` with exit 0; now the record reports `"status": "error"` with the `ELOOP` reason in its `reason` field and the batch continues. **The command's exit code does not change** — `account-import-many` is per-record fault-tolerant and returns 0 for a record-level failure — so the change is visible in the summary, not in `$?`. `keyfile-rotate`'s staging path gains the same protection; there it is unreachable in practice because the stale temp is unlinked first, and it was measured to be: a symlink planted at `<out>_tmp.bin` is removed, the rotation succeeds and exits 0. Nothing in this increment adds a `throw`, so the wave's lesson 12 has no caller audit to report beyond that.

**What does NOT change, and the decision behind it.** The **six** remaining independent `std::filesystem::permissions` sites in `wallet/main.cpp` that still discard `perm_ec` are deliberately left alone: `account-import-many --summary` (`wallet/main.cpp:2319`) is an address-to-path map, `backup-create --envelopes-out` (`:3223`) is AEAD-wrapped, and `cold-sign` (`:9801`), `sign-anon-tx` (`:10186`), `tx-batch-sign` (`:10803`) and `param-change-build` (`:23566`) write signed transactions whose entire purpose is to be broadcast. Fixing the P-2 REPORTING on all six is mechanical, but it is a rider across six commands and four `--json` contracts on a change whose subject is key material, and this repo's own record — three slashing attempts sunk by bundling — says no. Fixing the P-1 ORDERING on all six is **not** mechanical: all six write through a TEXT-mode `std::ofstream`, while the hardened helper is binary-mode on both branches, so routing them through it would silently change their on-disk bytes on Windows — today `\n` is translated to CRLF on those six files, afterwards it would not be — in files downstream tooling parses, on a platform this branch cannot execute. That is its own increment, with its own gate and its own Windows verification. The two bare `::chmod(out_path, 0600)` audit/history exports (`:19051`, `:19518`) keep their recorded best-effort decision. **Windows is not fixed and nothing here claims it is**: `_S_IREAD|_S_IWRITE` drives only FILE_ATTRIBUTE_READONLY and the effective ACL comes by inheritance from the parent directory (`docs/proofs/S005PassphraseKeyfile.md` F-4), so the Windows branch of the helper keeps the previous `ofstream` behaviour rather than implying a fix. S-109 therefore stays OPEN and its summary cells do not move.

**Noticed and deliberately left alone, with the locus.** (1) Three wallet outputs narrow **nothing at all** — not even after the fact — and S-109 had not named them before this entry: `create-recovery` writes the DRS1 container of password-wrapped share envelopes with a bare `ofstream` and no permission call (`wallet/main.cpp:6051`), and `encrypt-message` (`:7098`) and `decrypt-message` (`:7264`) write through a SECOND unhardened byte-writer, `write_file_bytes` (`:6917`), which has no permission call either — and `decrypt-message --out` is the recovered PLAINTEXT, which is the whole point of the command. They are named in the S-109 row as of this entry and are not fixed here: each is a different content class with its own `--json` contract, and the increment that fixes them should migrate `write_file_bytes` onto the one primitive rather than add a third. (2) `keyfile-rotate`'s header comment still says the command stages to `<out>_tmp.json` in two places (`wallet/main.cpp:3818` and `:3868`) where the code has used `_tmp.bin` for some time; only the one clause this change falsified — step 10 of the Flow list — was corrected, because the rest is unrelated staleness. (3) When the staging narrowing fails but the post-rename one succeeds, `keyfile-rotate` reports `perms_narrowed: false` over a file that is in fact 0600. That is the helper's existing semantics for its other eleven callers — the two failures are OR-ed — and it errs toward telling the operator to check; making it observational would mean `stat`ing the result, which is a different design. (4) `account-import-many` per-record failures still write nothing to stderr, exactly as before; the reason lives in the summary, and an operator who passes no `--summary` sees only the aggregate counts. (5) The `fs::exists(out_path) && !force` precondition remains TOCTOU-racy against the helper's open; `O_NOFOLLOW` removes the symlink flavour of that race but not the general one. (6) A failed write still leaves a truncated or empty file behind, because `O_TRUNC` runs before it — unchanged from the previous shape, and unlike the node-key case there is no `fs::exists()` provisioning check anywhere that would read the leftover as success.

**Verification:** `tools/ci_local.sh` on **Linux x86_64** — build **7 targets**, **FAST 323 passed / 0 failed**, **16 doc guards green**. The FAST count is unchanged on purpose: an existing wrapper was extended rather than a fourth one added for the same surface. The gate alone, from the worktree root against the freshly built `determ-wallet`: **44 pass / 0 fail / 0 skipped sections**; against a binary rebuilt at the parent commit, **10 of those 44 RED**. Mutants M1..M6 each RED against the rebuilt binary, restored and rebuilt clean afterwards, with the restored source's checksum matching the pre-mutation one and the gate green again.

**Authority:** the standing doctrine in `CLAUDE.md` — provable security (B3), new behaviour ships with a falsify-on-mutant gate at the layer where the rule lives, smallest increment that keeps the tree green AND truthful, no riders. The follow-up scope is the one S-109 itself names. No owner decision is required or implied: no recorded decision changes, no format changes, and the judgement calls taken here — leaving the six non-key-material permission sites and the three unnarrowed outputs for their own increments — are stated above and in the S-109 row rather than left to inference. Recorded by Claude (Cowork session), 2026-09-17. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — the suite was swept for gates that cannot fail: four more found and fixed, the sweep made a repeatable gate, and skips finally counted in the summary

**Status:** LANDED. Fixes four wrappers that could report success having verified nothing, adds a self-testing ratchet so the class cannot regrow silently, adds ADDITIVE skip accounting to `tools/run_all.sh`, corrects two stale documentation figures, and hands the next increment a file:line work list for the C++ selftests and for two wrappers that are RED today. **No ledger id is allocated** — see "Ledger" below.

**Problem this change solves.** Three increments landed in the two days before this one whose entire subject was a gate that could not fail: a durability leg invisible on every non-x86 architecture, a key-permission check whose two branches both printed `PASS`, and an integration pass that found a wrapper printing `PASS` when the binary it tests was absent. Each was found BY ACCIDENT, one at a time, by a reviewer looking at something else. Nobody had swept the suite. A previous reviewer's block-structure detector (`/root/audit/outbox-arm/review/sweep2.py`) reported zero remaining shell `if`/`case` blocks in which every arm passes — a true result, and a FLOOR rather than a ceiling: by its author's own statement it saw only `if`/`case` structure in `tools/*.sh`, and did not see terminal verdict logic, skip accounting, or anything inside an embedded heredoc.

**What was swept, and what was found.** All **731** `tools/*.sh` (of which **570** are `tools/test_*.sh` wrappers; **324** are FAST members), plus — READ-ONLY — the in-process `test-*` blocks of `src/main.cpp`, `wallet/main.cpp` and `light/main.cpp`. Five shapes were searched for: a terminal verdict on `fail_count == 0` with no floor on the assertions actually made; a `SKIP` arm that banks a pass; a multi-arm `if`/`case` in which every arm passes and none can fail; an assertion inside an embedded python heredoc whose comparison can only ever be true or whose failure is swallowed; and a wrapper that prints a `PASS:` marker and exits 0 when the thing under test is absent. Four live instances were fixed here:

1. `tools/test_wallet_cold_sign.sh:289` — the non-POSIX arm of the output-mode check printed `SKIP:` and then did `pass_count=$((pass_count + 1))`. The same shape as the `shamir-rotate` section 28 defect fixed the day before, in the adjacent file, unnoticed. It now increments `skip_count`.
2. `tools/test_wallet_shamir_rotate.sh:488` — the identical spelling, in the very wrapper whose section 28 was fixed on 2026-09-17; the fix landed on the `if` and left the `case` two hundred lines below it alone.
3. `tools/test_light_keybind_surface.sh:159` and `tools/test_light_resume_monotonicity_guard.sh:70` (the loci AFTER the fix; each previously) printed `SKIP: … (source-light checkout)` followed by a terminal `PASS:` and `exit 0` when the source it guards is absent. **Both are among `ci_local`'s sixteen doc guards**, so this was a hole in the verdict itself, not merely in the full suite. There is no such thing as a "source-light checkout": `light/` is tracked, so an absent target is a broken tree, not a platform. Both now fail closed.
4. `tools/test_light_negative_footing.sh:74` and `tools/test_light_wait_surface.sh:233` (again the post-fix loci) — the same bail-out over a tracked source, in two wrappers outside `ci_local`. Both now fail closed; `test_light_negative_footing.sh` additionally gained the `pass_count == 0` floor, since its live leg declines on every box.

Both `tools/test_wallet_cold_sign.sh` and `tools/test_wallet_shamir_rotate.sh` also gained the assertion floor: their verdict was `fail_count == 0` alone, so a box on which every section declined would have printed `PASS` at `0 pass / 0 fail`.

**The convention, written down once.** `tools/common.sh` now carries THE SKIP CONVENTION in full, with the three spellings the repo was simultaneously using on 2026-09-17 named as the reason: a declined check prints `SKIP: ` and names the cause, never touches `pass_count`, is reported next to it, and `pass_count == 0` is a hard FAIL. A binary the wrapper needs, absent, is a FAILURE and not a skip. `dt_skip` is the shared implementation; the eight wrappers with their own `skip()` keep it, since it is this shape.

**Skip accounting in the suite summary — REPORTED, NOT GATED.** `tools/run_all.sh` now prints, next to `RUN` / `PASS` / `FAIL`, how many of the PASSING wrappers declined at least one section and which, and how many of those printed no assertion-level `PASS:` line at all — i.e. banked green having checked nothing. **No verdict and no exit code changes**, deliberately: two sibling tracks are adding gates against the current semantics in this same wave, and the FAST count is a number many documents cite. Turning a skip into a failure means, in order: giving every one of the 161 wrappers whose verdict is still `fail_count == 0` an assertion floor, then deciding per wrapper whether a declined section is a platform (report) or a missing tool (fail), then re-tiering whatever goes red. That is a suite-wide migration and it is not this increment. The runner also now falsifies its own arithmetic before reporting — `PASS + FAIL == RUN`, `SKIP <= PASS`, asserted-nothing `<= SKIP` — because a summary a reader cannot verify is the same defect class as a gate that cannot fail.

**The gate.** `tools/test_gates_can_fail.sh` (FAST member; shell + python, no `determ` binary, offline, never skips). **28 assertions** in five legs. **A** drives the REAL `tools/run_all.sh` over five synthetic wrappers in a throwaway tree — clean, declining, declining-and-asserting-nothing, failing, markerless — and pins every column and the exit code. **B** four mutants of that accounting, each of which must turn A RED. **C** a positive control: deleting the one `SKIP:` line from a fixture moves the column 2 → 1 and moves no verdict. **D** the ratchet over the real `tools/*.sh`: D1 no `if`/`case` in which every arm banks a pass and none can fail (zero), D2 no `SKIP` that banks a pass in the same statement (zero), D3 an EXACT allowlist of the five wrappers that still print `PASS:` and exit 0 on a cannot-run guard — so a sixth is RED and a fixed one must be struck off — and D4 zero python heredocs inside a `tools/test_*.sh` gate carrying an always-true comparison or a bare `except: pass`. **E** an inline SELFTEST on every invocation: the checkers are fed the two 2026-09-17 defects verbatim and both heredoc shapes and must flag each; the corrected shapes, and must flag none; and a WRONG-but-falsifiable predicate, which they must NOT flag.

**What the checker can and cannot see.** It can see structure: which branches bank a pass, which bank a failure, whether a bail-out precedes the assertions, whether a heredoc comparison has two identical operands or a bare `except: pass`. It CANNOT see (and the file says so, because the alternative is a guard that lies): (i) a WRONG PREDICATE — both remaining holes of 2026-09-17 were assertions that ran, could fail, and tested the wrong thing, and nothing structural separates those from correct ones; measured, the checkers return zero on `tools/test_node_key_perms.sh` as it stood at `a152664`; (ii) embedded awk, perl or sed, and the 37 single-quoted `python -c` one-liners — D4 parses python heredocs only; (iii) a leg that is falsifiable but has never executed anywhere, e.g. the Darwin/BSD arms of several mode assertions; (iv) any C++ selftest; (v) the 161 wrappers (18 of them FAST members) whose verdict is still `fail_count == 0` with no assertion floor — surfaced per run by the new summary columns rather than gated.

**Mutants.** Each planted ONE AT A TIME into a REAL wrapper in the worktree; the harness (`/root/audit/x3/mutants.sh`) REFUSES to report a verdict unless it has confirmed by grepping an injected marker that the mutation reached the file on disk, and re-runs the gate after every restore. M1 both arms of the `cold-sign` mode check bank a pass → D1 RED. M2 the `shamir-rotate` skip arm banks a pass again → D2 RED. M3 `tools/test_wallet_out_perms.sh` prints `PASS:` and exits 0 on a missing binary → D3 RED. M4 an always-true comparison inside `tools/test_node_key_perms.sh`'s trace-parsing heredoc → D4 RED. M5 a bare `except: pass` in the same heredoc → D4 RED. M6, the harness's own falsification, writes its mutation to a file the harness does not grep: **REFUSED, no verdict taken**. M7 removes the floor restored in (1): the repo gate stays GREEN by design (D does not ratchet that shape) and the audit-dir sweep reports it — recorded so the boundary is a decision on the record, not an unnoticed hole. Leg B's four mutants of `run_all.sh`'s own arithmetic use the same marker refusal. Negative direction, run in the same harness: with the tree restored, D1–D4 are all GREEN and the corrected skip arm at `tools/test_wallet_cold_sign.sh:289` is flagged by none of them.

**The detector was falsified before it was trusted.** The sweep tool is `/root/audit/x3/detect_cantfail.py`; the fixture is `/root/audit/x3/fixture/` (one planted instance of each of eleven shapes plus six correct controls — four shell, two C++) and the transcript is `/root/audit/x3/selftest_detector.log`, 18/18. Running it found TWO REAL DEFECTS IN THE DETECTOR, both of which had made it report zero on every tree it had ever been run over: its C1 check searched FORWARD from the end of a helper's signature for an opening brace and so analysed the NEXT function's body; and its H4 check required `not can_raise` while `can_raise` was set true by the very `.exit()` call H4's own precondition demands, so H4 could not fire on any input. A third defect was found by the mutant harness rather than the fixture: D3 required `exit 0` on a line of its own, so `exit 0  # comment` was invisible, and mutant M3 stayed GREEN until that was relaxed. This is the whole point of the increment: a detector nobody has falsified is worth nothing, and two of this one's five checks were silently dead.

**Reported, NOT fixed — the C++ selftests.** `src/main.cpp`, `wallet/main.cpp` and `light/main.cpp` belong to other tracks this wave. Swept read-only; every instance, with the locus:
- `src/main.cpp:22618-22628` — `test-node-key-perms`, the `#ifdef _WIN32` arm prints `SKIP: POSIX file modes do not exist here…` and then the terminal `PASS: node-key-perms all assertions` and returns 0. `tools/test_node_key_perms.sh` greps that terminal marker, so on Windows the gate scores GREEN having asserted nothing. The comment immediately above it, at `src/main.cpp:22614`, claims "No pass is banked on a branch that checked nothing (wave doctrine rule 5)" — the terminal marker IS the pass the wrapper banks, so the comment is false as well as the behaviour. This is the platform-skip instance.
- `src/main.cpp:22557` — `check(compute_result == compute_result, "compute_abort_hash deterministic (re-check)")`: tautological, can never fail.
- `src/main.cpp:22559` — `check(chain_result == chain_result, "chain_abort_hash deterministic (re-check)")`: the same. The comment above both, at `src/main.cpp:22550-22556`, says the intent was to check the two hashes DIFFER ("a sanity check that they're not just aliases"); as written neither determinism nor distinctness is asserted.
- Reviewed and NOT defects, listed so the next increment does not re-litigate them: `src/main.cpp:25382`, `:33239` and `:33633` are `check(true, …)` in the house idiom — reaching the line is the assertion and the failure mode is a crash, hang or throw, with a falsifiable sibling assertion beside each.
- `wallet/main.cpp` (5 `test-*` blocks, 3 `check(` calls) and `light/main.cpp` (13 blocks, 49 calls) produced NO findings of any shape.

**Two wrappers are RED at this commit, and were RED before it.** Both are outside FAST and outside `ci_local`'s sixteen guards, so `ci_local` is green with them red; both only became visible because fixing their vacuous bail-outs made them actually run here. `tools/test_light_wait_surface.sh` reports `committee_bound_state_root: opens=14 good=0 bad=14` and `helper calls: opens=8 good=4 bad=4` — its own text calls that an S-042 head-read REGRESSION. `tools/test_light_negative_footing.sh` reports 9 `daemon_asserted` footings where its contract expects 7, and 12 emissions where it expects 10. Either `light/` drifted from the contract or the contracts drifted from `light/`; adjudicating that requires reading `light/`, which belongs to another track this wave, and correcting either side would change what those assertions MEAN, which this increment is scoped not to do. Handed over as the first thing to look at.

**Documentation.** `docs/SECURITY.md` described the FAST tier as "299 wrappers" in two places (§6 prose and the option-1 row of the unit-test table); it was 299 in 2026-08 and the figure had never moved. Both now read **324**, DERIVED and stated as derived: the `ONLY_PATTERN` alternation in `tools/run_all.sh` carries 325 stems, one of which (`dsso_assertion`) has no wrapper file, and the pattern matches 324 of the 570 `tools/test_*.sh` on disk. `docs/proofs/WalletRecoveryFlows.md`'s regression-script table carried six assertion counts that were each between 10 and 30 short (`shamir-rotate` 33 → 52, `keyfile-recover` "30+" → 60, `account-recover` 32 → 57, `account-import` 26 → 58, `keyfile-decrypt` unstated → 52, `keyfile-info` unstated → 40); each is now the number the wrapper printed when run on Linux x86_64 on 2026-09-18, and each says so. Checked and deliberately NOT changed: every other FAST or assertion count in the tree (`CLAUDE.md:14`, `:293`, `:342`; `docs/proofs/AbortDigestCanonicalizationSoundness.md:154`; `docs/proofs/ProofClaimGateTraceability.md:828`; `docs/proofs/BoundedReorgDesign.md`, `BoundedReorgSoundness.md`, `ShardTipMergeDesign.md`, `ClockInjectionSeam.md`; `docs/SECURITY.md:1479-1485`) is a commit-anchored past-tense measurement — "FAST was N/0 at <commit>" — which a later count does not falsify. `docs/UNIT-TESTS.md:79` and `docs/SECURITY.md:1237` say "155+ subcommands" where `src/main.cpp` now dispatches 251; a lower bound that is still true, left alone rather than re-derived.

**Ledger.** **S-112 is NOT allocated.** Everything fixed here is a verification-coverage defect — a gate that could report success having verified nothing — and not a defect in shipped behaviour; the 2026-09-17 precedent recorded exactly that class in this log only. The one candidate for a shipped-code id is the `test_light_wait_surface` S-042 finding above, and it is NOT adjudicated: it may equally be guard drift, and opening a ledger row on an unread diagnosis would be worse than leaving the finding named here with its locus.

**Riders.** None. Outside `tools/`, only the two documentation loci above are touched. No source file, no `CMakeLists.txt`, no wire format, no binary.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, no binary. No assertion anywhere changes what it MEANS — only whether a run that asserted nothing can report success. No pass/fail verdict and no exit code in `tools/run_all.sh` changes, and no wrapper that was green is red because of this change. `docs/CLI-REFERENCE.md` gets no row: the new wrapper adds no CLI surface, matching `tools/test_cluster_output_discipline.sh` and `tools/test_doc_citation_bounds.sh`.

**Noticed and deliberately left alone, with the locus.** (1) The 161 wrappers whose terminal verdict is `fail_count == 0` with no assertion floor, 18 of them FAST members — the full list is `/root/audit/x3/findings_all.json`, kind `P1`; giving each a floor is a suite-wide migration and the new summary columns make each instance visible when it actually bites. (2) Five wrappers still print a `PASS:` marker and exit 0 on a cannot-run guard and are pinned in D3's exact allowlist: `tools/test_cluster_output_discipline.sh:174` (a known FALSE POSITIVE of the heuristic — that line is its SELFTEST terminal, and it is listed because the allowlist is the checker's output, not a judgement), `tools/test_light_block_verify.sh:35`, `tools/test_light_fetch_validators.sh:26`, `tools/test_light_verify_chain_file.sh:35` and `tools/test_light_verify_unstake_eligibility.sh:155`. Each bails on an absent BINARY or an absent multi-node cluster, which is an environment rather than a broken tree — unlike the four fixed above, which bailed on an absent TRACKED SOURCE. (3) Seven `tools/operator_*.sh` heredocs carry a bare `except: pass`: `tools/operator_anon_address_usage.sh:551` and `:552`, `tools/operator_dapp_topic_audit.sh:179`, `tools/operator_genesis_inspect.sh:259`, `tools/operator_peer_topology.sh:297`, `tools/operator_rpc_auth_probe.sh:278`, `tools/operator_rpc_method_surface.sh:306`. These are operator REPORTS, not gates — none produces a suite verdict — so D4 is scoped to `tools/test_*.sh` and they are named here instead. (4) `tools/test_gates_can_fail.sh` excludes ITSELF from the D1–D4 sweep, because it carries verbatim copies of the 2026-09-17 defects as leg-E fixtures and would otherwise report its own fixtures as tree defects; leg E is what stands behind it, and its verdict carries the `pass_count` floor. (5) Seven wrappers already used the correct `skip()` shape before this change and were left exactly as they were.

**Verification:** `tools/ci_local.sh` from the worktree root on Linux x86_64, `-j1`, `CCACHE_DIR=/root/.ccache` — configure + build **7 targets**, **FAST 324 passed / 0 failed** (323 at this branch's base; +1 is `tools/test_gates_can_fail.sh`, added to the FAST `ONLY_PATTERN`), **16 doc guards green**. The gate alone: 28 pass / 0 fail / 0 skip. Mutants M1–M5 each RED with the mutation confirmed present in the file, M6 REFUSED as designed, M7 as described, the tree restored and re-verified green after each. Detector self-test 18/18 against the synthetic fixture.

**Authority:** the standing doctrine in `CLAUDE.md` — provable security (B3): no gate without a mutant that turns it RED, green is not proof, smallest increment that keeps the tree green AND truthful. The 2026-09-17 entries above it establish the precedent this one follows: a gate that cannot fail is recorded here and not in the ledger. No owner decision is required or implied; the decision this increment explicitly does NOT take is making a declined section fail the suite, which changes the meaning of the published FAST count and belongs to whoever owns that number. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — integration pass over the three gate-and-permissions increments: a FAST gate that went RED on Windows, two counts the merge falsified, and the adjudication of two long-RED wrappers

**Status:** LANDED. Fixes three defects that exist only in the combination of the three increments above, and puts on the record an adjudication that has been outstanding since one of them made two dormant wrappers run again.

**Problem 1 — a FAST gate went RED on Windows, and the increment that did it reported the opposite.** The S-111 increment stopped `test-node-key-perms`' Windows arm from banking a pass: it now prints a `SKIP:` marker instead of the terminal `PASS: node-key-perms all assertions` and exits non-zero. That was correct. But `tools/test_node_key_perms.sh` leg A greps for exactly that PASS marker and had no way to read a SKIP, so it scored a FAIL — and that wrapper is a FAST member, while `.github/workflows/ci.yml` carries a `windows-2022` job running `bash tools/ci_local.sh`. The increment's own report says "On Linux (this box and CI) nothing changes"; the CI matrix is not Linux-only, and `tools/ci_local.sh`'s own header calls the MSVC/Windows box the primary one. Leg A now records the platform SKIP by name through `dt_skip`, and the wrapper gained the `pass_count > 0` floor it was missing, so a platform where every section skips fails closed instead of printing PASS. Verified both ways on this box: Linux unchanged at 7 pass / 0 fail / 0 skipped, and with the in-process arm forced to its skip path leg A records `SKIP: section A …` rather than a failure.

**Stated with it, because it is worse than what this wave broke:** `tools/test_wallet_out_perms.sh` — also a FAST member — **was already RED on Windows before this wave**. Both of its sections are Linux/`LD_PRELOAD`-gated and it carries a `pass_count == 0` floor, so on `windows-2022` it exits 1. That is not a defect introduced here and it is not fixed here; it means the Windows job has been failing since that gate landed, and nobody noticed, which is its own finding. Whoever next owns `tools/` should give both wrappers' remaining legs named skips, or the Windows job an explicit expectation.

**Problem 2 — the merge manufactured a false measurement.** `docs/proofs/WalletRecoveryFlows.md` records `tools/test_wallet_out_perms.sh` as "18 assertions (measured 2026-09-18 on Linux x86_64 — this one was already right) … Both legs SKIP by name". The measurement was honest when taken: the sweep increment measured the wrapper, and the wallet increment then extended that same wrapper from 18 to 44 assertions and from two sections to three, in the same wave, in a file the sweep did not touch again. Neither track could see it. Corrected to 44 in three sections, with the reason recorded in the row rather than a second bare number. This is the defect class the sweep increment exists to remove, produced by the integration of that very increment, which is worth saying plainly.

**Problem 3 — my own integration edit left a stale denominator.** Correcting the FAST count to **325** (two tracks each added one wrapper to a base of 323) left the sentence reading "matches 325 of the **570** `tools/test_*.sh` on disk"; there are **571**, the S-111 increment having added one. The sentence advertises itself as DERIVED. None of the sixteen doc guards derives that count, so nothing would have caught it. Both numbers are now measured against the tree as it stands.

**The adjudication that was outstanding: two long-RED wrappers are GUARD DRIFT, not a regression in `light/`.** The sweep increment fixed four wrappers that had been bailing out vacuously, and two of them then ran for the first time in a long while and came up RED — `tools/test_light_wait_surface.sh`, whose own text calls its finding an "S-042 head-read REGRESSION" (`committee_bound_state_root: opens=14 good=0 bad=14`), and `tools/test_light_negative_footing.sh` (9 `daemon_asserted` footings against a hard-coded 7, 12 emissions against 10). Both are outside FAST and outside the sixteen guards, so `ci_local` is green with them red. **Adjudicated by re-running each wrapper's own extraction against `light/`: there is no regression.** The wait-surface guard requires the wait variable to be the LAST argument of the call it inspects; all fourteen "bad" call sites do forward it, and `light/trustless_read.hpp`'s signature gained `expected_k` and `bft_enabled` AFTER the guard was written, so the wait argument is simply no longer last. The negative-footing guard's substantive legs all pass — `ungated=0`, `mismatched=0`, correctly gated and correctly labelled — and exactly three assertions fail, every one a hard-coded census pin that was never updated when two commands gained correctly-gated footings. **No S-row is opened**: nothing in the shipped client is wrong. Repairing the two contracts changes what those assertions MEAN, which is its own increment with its own review, and it is the next thing worth doing in `tools/`. Not established without a cluster: whether the two new footings are the semantically right class, and the live behavioural leg.

**Corrections this append-only log owes itself.** (a) The sweep increment's entry says the FAST tier is now **324** and that both `docs/SECURITY.md` sentences "now read 324"; integrated with the S-111 increment the count is **325**, the alternation carries **326** stems, and there are **571** wrappers on disk. All four figures in that sentence are superseded; the two documentation sentences read 325 as of this entry. (b) The wallet increment's entry is dated **2026-09-17** and sits between two 2026-09-18 entries; its commit is dated 2026-09-18 and the log's order is otherwise monotone. (c) The S-111 increment's new gate asserts `RC == 0` on the unprotectable-file path, which PINS the residual it documents — a future increment that makes that path refuse must edit the gate, not only the code.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration. No C++ source: this entry changes one wrapper's skip accounting and two documentation numbers. No triage row opens, closes or changes status, and no summary cell moves.

**Noticed and deliberately left alone, with the locus.** The wallet increment's extension to `tools/test_wallet_out_perms.sh` created the tree's first and only `if`/`case` block longer than the ratchet's 120-line analysis cap (153 lines); its first arm calls `skip()`, so the ratchet would not have flagged it either way, but the gate's "zero findings" now covers 5021 of 5022 lines rather than all of them, and no track's report says so. The `Partially mitigated` summary total is 6 against 5 triage rows because S-036 has a summary mention and no triage row — pre-existing, and the coherence guard checks only the Open and Reopened rows. There are now THREE hand-rolled restricted-write loops and THREE copies of the strace-ordering parser; the write loops agree on `write() == 0` since yesterday but the wallet one alone does not `unlink` on a write or close failure, so a truncated key container survives under the final name — bounded, because it is 0600 by then, and named as part of the `include/determ/util/restricted_write.hpp` follow-up.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 325 passed / 0 failed**, 0 wrappers declined a section, 16 doc guards green.

**Authority:** the standing verification doctrine in `CLAUDE.md`, and the independent adversarial review of the integrated diff, which returned NOT SAFE AS WRITTEN on the three problems fixed here and established the adjudication recorded above. No owner decision is required or implied. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — the macOS run was RED on one wrapper, and it was the gate's fault: a Linux-only gate now asserts the outcome everywhere POSIX modes exist

**Status:** LANDED. Fixes a gate that could not run on the owner's own third platform, found by the owner running it.

**Problem.** `tools/ci_local.sh` on Darwin arm64 came back **RUN 325 / PASS 324 / FAIL 1**, the one failure being `tools/test_wallet_out_perms.sh`: `0 pass / 0 fail / 3 skipped section(s)` → `FAIL: every section skipped; nothing was asserted`. All three of its sections need Linux — two need `strace`, one needs `LD_PRELOAD` to intercept `chmod` (SIP blocks that on macOS) — so on Darwin the wrapper declined everything and the `pass_count > 0` floor, added the day before precisely so a run that asserted nothing could not report success, correctly called that a failure. **The floor is right and the wrapper was wrong.** A gate that cannot observe its property on a platform must not claim the product is broken there; and the entry above this one predicted exactly this for the `windows-2022` CI job, named it as pre-existing, and did not draw the obvious conclusion that macOS is in the same position. The owner's run drew it.

**What was actually missing: the outcome is observable on macOS; only the window is not.** `stat` works there and POSIX modes exist. What needs `strace` is the ORDER (was the file 0600 before the secret was written) and what needs `LD_PRELOAD` is the REPORTING of a failed narrowing. The mode each output ends at — the contract `write_bytes_file_0600` states — is observable on every POSIX platform, and the wrapper was not asserting it anywhere.

**The change.** A fourth section, D, gated on `uname` for the POSIX family and needing neither strace nor a compiler. It asserts the final mode of six outputs across the four commands this wrapper covers: a fresh `shamir-split --out`; `shamir-rotate` over a PRE-EXISTING 0644 target; **each of the three records** of `account-import-many`, since the defect that increment fixed was per-record and a fix that restricts only the first must fail; and the atomically published `keyfile-rotate` target, also over a pre-existing 0644 file. The terminal line now reports how many assertions were checked and how many sections declined, and says in the wrapper's own output that what declined is not asserted.

**What section D does NOT gate, written into the section so a green is not misread.** It asserts the outcome and not the window, and not the reporting. A mutant that restores the old create-then-narrow shape leaves the final mode 0600 and is GREEN in section D — which is why sections A, B and C exist and why this one does not replace them. On a platform without strace this gate now covers the contract, not the window S-109 was opened for. That distinction is the whole reason the S-109 window was invisible to every after-the-fact check in the first place, and it would be dishonest to let a Darwin green read as more than it is.

**Falsified before it was trusted.** Mutant D1 — the helper's trailing narrowing widened to 0644 — rebuilt, run with `strace` and the C compiler hidden so section D was the only judge: **five of the six assertions RED**, `1 pass / 5 fail`. The one that stayed green is `keyfile-rotate`, whose post-rename narrowing is its own call site rather than the helper's, which is evidence the six assertions are six properties and not one property in six sets of clothes. Source restored and rebuilt clean; the harness refused a verdict on an earlier attempt whose anchor no longer matched the file, rather than reporting a green mutant.

**The ratchet caught my own first attempt at this, and I fixed the wrapper rather than the gate.** My initial summary branched — `if skip_count > 0` print one PASS line `else` print another — and `tools/test_gates_can_fail.sh` D1 went RED on it: `tools/test_wallet_out_perms.sh:641 | if [ "$skip_count" -gt 0 ]`, an if/case in which every arm banks a pass and no arm can fail. The intent was to vary the wording of the terminal marker, not to make a verdict, but the shape is indistinguishable from the defect the ratchet exists to forbid, and a rule that is kept and then special-cased for its author is worthless. The summary is now one unconditional terminal marker carrying both counts. This is the ratchet doing on new code, the day after it landed, exactly what it was built for.

**What is still RED, and where.** On **Windows** this wrapper remains red: there is no POSIX mode to assert, so section D declines there too and the wrapper has nothing left. That is not fixable inside this file — the property genuinely does not exist on that platform, and the honest outcome is a wrapper-level SKIP, which `tools/run_all.sh` has no notion of: its per-wrapper outcome is PASS or FAIL, a missing marker counts as FAIL, and its self-checked arithmetic asserts `PASS + FAIL == RUN`. Adding a third outcome means changing that invariant and the "FAST N passed" figure many documents cite, which is a deliberate repo-wide change and not a rider on a macOS fix. `tools/test_node_key_perms.sh` is in the same position on Windows. Both are named here as the next increment in `tools/`, with the design: a third outcome, counted separately, with the arithmetic invariant widened to `PASS + FAIL + SKIPPED == RUN` and the suite still exiting non-zero only on FAIL.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no migration, and no C++ source at all: this entry changes one test wrapper. No triage row opens, closes or changes status; no summary cell moves. FAST stays at 325 wrappers.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, FAST 325 passed / 0 failed, 16 doc guards green; `tools/test_wallet_out_perms.sh` 50 assertions on Linux and 6 with strace and the compiler hidden, both green, and 1 pass / 5 fail under mutant D1 in the second configuration.

**Authority:** the standing verification doctrine in `CLAUDE.md`. The defect was reported by the owner's own Darwin arm64 run of `tools/ci_local.sh`, which is the second-platform gate working as designed. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.
---

## 2026-09-18 — three hand-rolled restricted writers had drifted twice in two days; they become ONE primitive, every deliberate divergence stays a parameter, and the one divergence that was drift is fixed and gated

**Status:** LANDED. One new header (`include/determ/util/restricted_write.hpp`), three call sites migrated (`wallet/main.cpp::write_bytes_file_0600`, `src/crypto/keys.cpp::save_node_key`, `src/main.cpp::write_account_file_0600`), four assertions added to an existing in-process selftest. **No new FAST wrapper and no `tools/` file touched** — the FAST count is unchanged. No consensus, apply, wire, digest, genesis or state surface; no migration; no new dependency; no CLI flag; no exit-code change anywhere.

**Problem this change solves.** Three implementations of "put a secret on disk in a file that is 0600 before its first byte" were written independently on 2026-09-17 and 2026-09-18 by three different agents, and they drifted **twice inside those two days, measurably**. First: two of them disagreed on `write() == 0` — the wallet's guarded it, `save_node_key`'s would have spun forever — caught by review and fixed on 2026-09-18. Second, and still live at this branch's base: **only the wallet writer omitted `::unlink` on the write- and close-failure paths**, so a truncated key container survived under the final name, where `src/crypto/keys.cpp` removes it and says why at the locus ("never leave a partial identity behind", "the contents are not trustworthy") and `src/main.cpp` removes it and says why ("what is on disk is a FRAGMENT of a private key under a name an operator reads as a provisioned account"). The third copy was written **deliberately**, and its own entry (2026-09-18, `account create --out`) says so and names this extraction as the follow-up: *"Named follow-up: `include/determ/util/restricted_write.hpp` plus all three call sites, as one increment after this wave integrates."* This is that increment.

**The wallet defect, REPRODUCED against the binary built from HEAD before anything was touched** (transcript in the session's audit directory). An `LD_PRELOAD` shim that makes `write(2)` return 0 for every descriptor above stderr — the condition the guard in all three writers exists for, and one the kernel will not produce on a regular file:

```
$ LD_PRELOAD=zerowrite.so determ-wallet shamir-split --secret <32 hex> --threshold 2 --shares 3 --out $T/s.json
shamir-split: write failed: /tmp/.../s.json: wrote 0 bytes
rc=1
$ stat -c '%a %s %n' $T/s.json
600 0 /tmp/.../s.json
$ LD_PRELOAD=zerowrite.so determ-wallet account-import --priv <32 hex> --out $T/k.dak1
account-import: write failed: /tmp/.../k.dak1: wrote 0 bytes
rc=1
600 0 /tmp/.../k.dak1
```

The command exits 1 and says so, and a **zero-byte key container survives under the final name** — which the `--force` guard on the next attempt then refuses to overwrite. The same probe against the daemon (`account create --out`) and against `save_node_key` (`determ init`) leaves nothing behind, because those two already unlink.

**The difference table, which is the durable part of this increment.** Before a line of the primitive was written, every behavioural difference between the three was enumerated and each one classified as POLICY (a deliberate choice of that call site, which the primitive must keep parameterised) or DRIFT (three copies disagreeing for no stated reason, which the primitive resolves). The full table is in the session's audit directory; its conclusions:

*Identical in all three, so the primitive simply owns them:* the open flags and create mode (`O_WRONLY|O_CREAT|O_TRUNC|O_CLOEXEC|O_NOFOLLOW`, `0600`); `O_NOFOLLOW`; `::fchmod(fd, 0600)` on the descriptor before the first byte; EINTR (three spellings, one semantics); short writes; the refusal to spin on `write() == 0`; a checked `::close`; and the absence of an `fsync` — `keyfile-rotate` re-opens and fsyncs its staging file itself "rather than pushing an fsync into a primitive ten other callers share", and that recorded reason now applies to the shared primitive verbatim.

*POLICY, kept and parameterised at the call site:* **(1)** what a failed pre-write narrowing does — the wallet and the daemon warn and CONTINUE (a `keyfile-recover` result may be unregenerable; refusing in the daemon is an exit-code change named as the S-111 remainder), `save_node_key` REFUSES (a node identity is regenerable at no cost and is read for the life of the deployment). `on_narrow_failure = Continue | Refuse`. **(2)** whether the trailing belt-and-braces `std::filesystem::permissions` call happens at all — the wallet and the daemon keep it and CHECK its `error_code`, `save_node_key` has none and deliberately so. `final_narrow`. **(3)** Windows text vs binary mode, which is about ON-DISK BYTES: the wallet switched to binary knowingly and changed its Windows output, and both `src/` sites deliberately did not repeat that. `windows_binary`. **(4)** the reporting shape — stderr with a `<cmd_label>:` prefix plus an optional `err_out` and a `perms_narrowed_out` flag; unprefixed stderr; or a throw. **Every diagnostic string, and every throw, stays at its call site and is byte-identical**; the primitive returns a status and the caller renders it, with the formula `msg + (err ? ": " + strerror(err) : "")` reproducing each site's previous POSIX **and** Windows text verbatim. **(5)** the test-only `DETERM_NODE_KEY_INJECT` hook: the `getenv` and the decision to honour it stay in `src/crypto/keys.cpp` where they were justified; the primitive only carries the errno in, and the other two sites pass 0. **(6)** creating and narrowing the parent DIRECTORY, which only `save_node_key` does and only for a directory it created itself — that stays entirely in `keys.cpp`.

*DRIFT, resolved:* **(a)** the wallet's missing `::unlink` after a failed write or close. The wallet's own comment justifying "the file is NOT deleted" is about the **failed-narrowing** path, where the file is COMPLETE and merely too wide — a good argument, kept unchanged. It says nothing about a failed write, where `O_TRUNC` has already run and what survives is a fragment that every container the wallet writes (DAK1 / DSS1 / DNK1 / DBE1 / DWE2) rejects on its own magic and length checks. Both siblings state the opposite rule at the locus, the wave doctrine's "never leave a file behind on a refusal path" is binding on all three, and the independent review of this wave reached the same verdict. Resolved: the primitive removes the file on every failure path that has already truncated it. **(b)** `save_node_key`'s Windows arm never checked its stream after close (`f << blob;` and the stream destructor swallows a failed flush), so a truncated node identity was reported as a success; both siblings check it, the 2026-09-17 entry's words are "left exactly as it was" rather than "designed", and its Windows reasoning is entirely about `_S_IREAD`/`_S_IWRITE` and NTFS ACLs, i.e. about permissions and not about error checking. The primitive checks it for all three.

**Header-only, and that was checked against `CMakeLists.txt` rather than assumed.** The consumers are two separate `add_executable` targets — `determ` (which compiles both `src/main.cpp` and `src/crypto/keys.cpp`) and `determ-wallet`. The only library between them is `determ-crypto-c99`, whose source list is exclusively `.c` under the C99 / Minix-portable discipline, so a C++ translation unit cannot be added there. Everything in the header is `inline`, and it reaches `determ-wallet` because `determ-crypto-c99` exports `include` as a PUBLIC include directory — which is already how `wallet/main.cpp` includes `<determ/crypto/ed25519/ed25519.h>`.

**The gate.** The three wrappers that pin these three sites — `tools/test_wallet_out_perms.sh` (50 assertions), `tools/test_node_key_perms.sh` (7 wrapper over 18 in-process) and `tools/test_account_create_perms.sh` (29 wrapper over what were 22 in-process) — are the safety net and **not one line of any of them was edited**; `tools/` belongs to other tracks in this wave. All three stay green with their counts unchanged.

The one behaviour that CHANGED needed its own assertion, and it went where it could go without touching `tools/`: **four new assertions inside `determ test-account-create-perms`** (`src/main.cpp`, 22 to 26), which `tools/test_account_create_perms.sh` leg A already runs by grepping its terminal marker. They assert the unified rule directly on the primitive — a write that cannot complete leaves NOTHING behind — against a **real** `::write` failure, produced by lowering `RLIMIT_FSIZE` below the payload with `SIGXFSZ` ignored across the window and the limit restored immediately, and **not** by an injection hook on the shipped write path. A control asserts that the identical call without the limit DOES create the file at 0600, so "absent" is the removal and not a failed open, and a second control asserts the limit really was lowered, so the leg cannot pass vacuously. Because the rule now lives once, in one header compiled into both binaries, that assertion covers all three call sites.

**Mutants — seven, each applied ALONE to the header, REBUILT, then restored and rebuilt clean.** Each injects a unique marker as a function-local `static const char* volatile` (a `const char*` local was discarded by `-O3` for an earlier harness in this project), and the harness REFUSES to report a verdict unless the marker is present in the SOURCE, the build SUCCEEDED, **and** the marker is present in BOTH relinked binaries.

| id | mutation | `test_wallet_out_perms` | `test_node_key_perms` | `test_account_create_perms` | bounded `write()==0` probe |
|---|---|---|---|---|---|
| M1 | restore the pre-fix `std::ofstream` create (create at the umask mode, write the secret, narrow afterwards if at all) | **RED** 41/9 | **RED** 3/4 | **RED** 24/5 | spins (an artifact of the ofstream retry loop, not the guard) |
| M2 | keep the 0600 create mode, DROP the `::fchmod` before the write | **RED** 43/7 | **RED** 5/2 | **RED** 25/4 | guarded |
| M3 | `::fchmod` moved to AFTER the write (final mode still 0600) | **RED** 43/7 | **RED** 6/1 | **RED** 25/4 | guarded |
| M4 | create 0600, `fchmod(fd,0600)`, widen the PATH to 0666, write, narrow back by path (final mode 0600) | **RED** 46/4 | **RED** 6/1 | **RED** 27/2 | guarded |
| M5 | remove the `write() == 0` guard | green 50/0 | green 7/0 | green 29/0 | **KILLED** — all three binaries spin and are killed by `timeout`, where the unmutated build exits 1 with each site's own diagnostic |
| M6 | leave the truncated file behind after a failed write (the wallet's pre-2026-09-18 shape) | green 50/0 | green 7/0 | **RED** 28/1 | guarded |
| M7 | leave the empty, `O_TRUNC`'d file behind on a refusal | green 50/0 | **RED** 6/1 | green 29/0 | guarded |

M1..M4 are the window mutants and each is RED on **all three** gates, which is the measured statement that the primitive really is shared: one header edit reddens the wallet, the node key and the daemon at once. M4 is the one that matters most — it ends at mode 0600, so every final-mode assertion in the tree is green on it and only the ordering legs that count path-based calls kill it. M5 is the one **no gate can catch**, by construction: `write(2)` does not return 0 for a positive count on a regular file, so the condition the guard exists for cannot be produced by any of the three gates. The bounded probe produces it with an `LD_PRELOAD` shim that returns 0 for every descriptor above stderr and runs each of the three commands under `timeout 10`: unmutated they exit 1 with `wrote 0 bytes` / `Input/output error` and leave nothing on disk; under M5 all three hang and are killed at 10s. M6 and M7 are the two halves of the one behaviour this increment changed, and each is RED on exactly one gate — M6 on the four new in-process assertions, M7 on `test-node-key-perms`' existing "no file is left behind at all" assertion under `DETERM_NODE_KEY_INJECT=fchmod`. Both of M6's controls stay green, so the kill is specific and not a collapse of the section.

**Riders: none.** No unrelated code improved, no formatting sweep, no other command touched, no new CLI surface, no `tools/` file edited, no FAST wrapper added.

**What does NOT change.** No consensus accept rule, apply path, wire format, digest, genesis field, state layout or migration. No `light/`, `dapps/`, `sim/` or `sdk/` file. Every diagnostic string at every site is byte-identical, verified by the three gates' `grep`-for-exact-text legs. No exit code changes on any path. The on-disk bytes of every output are unchanged on POSIX and on Windows, text-mode translation included — the wallet keeps binary, both `src/` sites keep text. The three reporting policies (warn / warn plus a `--json` flag / throw) are untouched, including `save_node_key`'s refusal and the daemon's deliberate non-refusal. `determ test-node-key-perms` keeps its 18 in-process assertions; `tools/run_all.sh`'s FAST alternation is not touched.

**Ledger: no new S-id, and that is an argument, not an omission.** The wallet's missing `unlink` is a genuine, reproduced, shipped-code defect, and one was reserved for it. It is recorded in `docs/SECURITY.md` as a third mitigation pass on the existing **S-109** row rather than as a row of its own, because: by the time a write can fail the file is already 0600, so no confidentiality boundary is crossed and this is not a permissions exposure; the harm is operational and bounded — exit code 1, a named diagnostic, a per-record `status: error` in the bulk loop, and a fragment every decoder rejects; `keyfile-rotate`, the one site that could publish it, already removed its own staging file; and the defect is CLOSED by the same commit that reports it, so a new row would be opened and closed in one change while moving two summary cells for something no reader ever sees as open. The precedent in this same wave is the `write() == 0` divergence — same three writers, same class, same severity profile — which was recorded in this log and not given an id. Summary cells therefore do not move and `tools/test_security_ledger_coherence.sh` is green.

**Noticed and deliberately left alone, with the locus.** **(1)** The Windows close check in **(b)** above is the one change in this increment with **no mutant**: it is unobservable on every platform this box can run a gate on, all three gates SKIP by name on Windows, and it changes no passing path (a close that succeeds behaves identically). It is stated here rather than buried; the alternative — a `check_close` policy field defaulting to true with `keys.cpp` passing false — was rejected because re-encoding an un-argued accident as a parameter of the shared primitive is exactly what this increment exists to stop. **(2)** `::unlink(path)` after a failed write re-resolves the name, so an attacker who already has write access to the containing directory could swap in another name between the `close` and the `unlink` and have that removed instead. `unlink(2)` does not follow symlinks, the attacker must already control the directory, and both `src/` sites have had this exact call since 2026-09-17/18; closing it needs an `O_PATH` directory walk, which is the same work item as **(3)**. **(3)** `O_NOFOLLOW` guards the FINAL path component only; an attacker controlling a parent directory of the output can still redirect the write, at all three sites. Already recorded 2026-09-18; unchanged. **(4)** The **strace-ordering parser is still in three copies** (`tools/test_node_key_perms.sh` leg C, `tools/test_wallet_out_perms.sh` leg A, `tools/test_account_create_perms.sh` leg B), with the same drift risk this increment removed from the product code. They are currently in agreement, including on counting both mode-setting spellings. `tools/` is another track's in this wave, so unifying them is not this increment — and it is the next instance of the pattern this entry is about. **(5)** Six independent `std::filesystem::permissions` sites and three unnarrowed outputs in `wallet/main.cpp` still do not go through any restricted writer (enumerated with their loci in the S-109 row); routing them through the primitive is now a much smaller change than it was, but it changes their Windows bytes and is a rider across six commands and four `--json` contracts. **(6)** The secret still passes through a `std::string` and an `nlohmann::json` that are not zeroized at `save_node_key` and `cmd_account_create`; the primitive does not zeroize the caller's buffer either, deliberately — the buffer belongs to the caller and two of the three already wipe theirs. Unchanged, and it is the D2 container increment's work. **(7)** `determ init` still creates its data directory at 0755 (recorded 2026-09-17). **(8)** `docs/SECURITY.md` still describes the FAST tier as "299 wrappers" in two places where it is 325; noticed and left alone twice before, and left alone again for the same reason — it is a shared paragraph two tracks may be editing today and it belongs in the integration pass.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 325 passed / 0 failed** (325 at this branch's base; this increment adds no wrapper, so the count is unchanged), 16 doc guards green. `tools/test_wallet_out_perms.sh` alone: 50 pass / 0 fail, 0 sections skipped; `tools/test_node_key_perms.sh` alone: 7 pass / 0 fail; `tools/test_account_create_perms.sh` alone: 29 pass / 0 fail; `determ test-account-create-perms` alone: 26 assertions. Each mutant's verdict was read only after the build was confirmed to succeed AND the marker was confirmed present in both relinked binaries, and the header was restored and rebuilt clean afterwards with a check that neither binary still carries a mutant marker.

**Authority:** the standing verification doctrine in `CLAUDE.md` (provable security B3; assert at the layer where the rule lives; smallest increment that keeps the tree green AND truthful; no riders), the node-/client-local backlog authorized under D19a as item (11) of the 2026-09-16 implementation sequence, and the explicit follow-up named by the 2026-09-18 `account create --out` entry in this log. No consensus decision is touched and none is implied. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — the suite gains a third per-wrapper outcome: a gate whose property does not exist on a platform now skips ENTIRELY there, counted in its own column, and the Windows CI job stops failing a product that is not broken

**Status:** LANDED. Verification accounting only; no shipped behaviour changes and no C++ source is touched. This is the increment the entry above this one named as next, with the design it specified.

**Problem.** `tools/run_all.sh` knew exactly two per-wrapper outcomes. It read the last ten lines of each wrapper's output for a `FAIL:` marker (first, fail-closed), then a `PASS:` marker, counted a wrapper that printed neither as a failure, and self-checked `PASS + FAIL == RUN`. There was therefore no way to express the one honest outcome for a platform where a wrapper's property **does not exist**. Two FAST members are in exactly that position on Windows: `tools/test_wallet_out_perms.sh` gates the mode a wallet key file ends at (S-109 P-1 ordering under `strace`, P-2 reporting under `LD_PRELOAD`, and since yesterday the final mode under `stat`), and `tools/test_node_key_perms.sh` gates the same for the node identity seed (S-091). Windows has no POSIX file mode at all, so every section of both declines, and each wrapper's `pass_count > 0` floor — which is correct, and exists precisely so a run that asserted nothing cannot report success — turns that into a FAILURE. `.github/workflows/ci.yml` carries a `windows-2022` job running `bash tools/ci_local.sh`, so the Windows job has been RED for a product defect that does not exist on that platform. **Reproduced before anything was changed**, against the HEAD tree (2a72a67) exported with `git archive`, on a simulated Windows environment — `uname -s` reporting `MINGW64_NT-10.0-22621`, `strace` and every C compiler removed from `PATH`, and `DETERM_TEST_FORCE_PLATFORM_SKIP=1` so the in-process arm takes its `_WIN32` path: `tools/test_wallet_out_perms.sh` reported `0 pass / 0 fail / 4 skipped section(s)` and exited 1, `tools/test_node_key_perms.sh` `0 pass / 0 fail / 1 skipped section(s)` and exited 1, and `tools/run_all.sh` over the pair reported `RUN 2 / PASS 0 / FAIL 2` and exited 1. This is a SIMULATION on Linux, not a run on Windows; it is stated that way rather than as a measurement of the real CI box.

**Note what this is NOT.** A wrapper that declined ONE section and still asserted something already worked and is reported in the SKIP column, which is additive over the passing wrappers. This is the different case: a wrapper that can assert nothing at all here because the subject of its assertion is absent.

**The change — a third outcome, `PLATFORM-SKIP:`.** A wrapper signals it with a terminal marker of its own, distinct from `PASS:` and `FAIL:`, naming the cause after the colon, and exits 0. The runner detects it between the two existing greps — after `FAIL:` so a wrapper that somehow prints both is still fail-closed, before `PASS:` so the marker decides — and the detection is spelled with POSIX `[[:space:]]` classes rather than `\s`, so `tools/test_cluster_output_discipline.sh`'s D5 fixed-string search still pins the two original greps and their order, and pins the greps rather than a comment quoting them. Four properties are load-bearing:

- **The cause is required.** The grep demands a non-blank character after the colon, so a bare `PLATFORM-SKIP:` is not a marker, falls through to the markerless branch and counts as a failure. A skip nobody can audit is not a skip.
- **A wrapper that claims the marker while printing assertion-level `PASS:` lines is a FAILURE, not a skip.** It asserted something and one of its two statements is false; the runner cannot tell which, so it fails closed. This was an open question in the brief and the answer is yes, it should be detectable: without it the new outcome is simply a fresh way to go quietly green, and the check costs one grep over output the runner already holds.
- **The arithmetic is WIDENED, not dropped:** `PASS + FAIL + PLATFORM-SKIP == RUN`, still a hard FAIL on mismatch. A third outcome that did not have to balance would be a hole big enough to lose a wrapper in.
- **The suite still exits non-zero ONLY when `FAIL > 0`.**

The summary prints the column unconditionally (so a reader and `ci_local` can always find it) and lists the skipped wrappers by name under a heading that says the properties are not gated here. `tools/ci_local.sh` now tees the suite's output, reads the `PASS` and `PLATFORM-SKIP` columns back out of it, fails closed if either cannot be read, and ends with `PASS: ci-local build + FAST (N wrappers asserted, M skipped entirely) + guards green` — one unconditional terminal marker carrying both figures, never a per-arm wording, because an `if`/`else` in which both arms print a PASS line is the shape the ratchet forbade the day before yesterday.

**The two wrappers, and the boundary, which is the whole point.** Each gained a gate near the top — after its missing-binary check, so a broken build still fails closed — that emits the marker on `MINGW*|MSYS*|CYGWIN*` and nothing else. It is a PURE `uname` test. A POSIX box that merely LACKS A TOOL does not reach it: the property is real there, the legs that can run still run, the legs that cannot print a named `SKIP:`, and if that leaves nothing asserted the floor fails closed exactly as before. An UNRECOGNISED `uname` also fails closed rather than claiming the property is absent. Both halves were measured, not argued: on Linux with `strace` and every C compiler hidden, `tools/test_wallet_out_perms.sh` still reports 6 assertions and `tools/test_node_key_perms.sh` 5, both green, with ZERO `PLATFORM-SKIP` markers printed; under a `uname` of `SunOS` both report `0 pass / 0 fail`, exit 1 and print zero `PLATFORM-SKIP` markers.

**What the suite's numbers now mean, per platform.** The FAST tier's MEMBERSHIP is 325 wrappers and is unchanged — the `ONLY_PATTERN` alternation carries 326 stems, `dsso_assertion` has no wrapper file, and the pattern matches 325 of the 571 `tools/test_*.sh` on disk, all three figures re-derived against the tree as it stands. Membership is no longer the same figure as the number that ASSERTED. On **Linux and macOS**: `PLATFORM-SKIP: 0` and all **325 assert**. On **Windows**: `PLATFORM-SKIP: 2` and **323 assert**. Both sentences in `docs/SECURITY.md` that cite the figure now say which is which, inline.

**What is still NOT gated on Windows after this change, said plainly because a tidier summary must not hide it.** The two wrappers skipping entirely there means their properties have NO coverage on that platform, and this change buys none: the at-rest permissions of every wallet key output (S-109 P-1 and P-2 — the ordering, the reporting and the final mode) and of the node identity seed written by `determ init` (S-091) are unchecked on Windows, as they were before, with the difference that the summary now names them instead of reporting a product failure. What actually protects those files there is the containing directory's NTFS ACL, and **nothing in this suite checks an ACL at all** — there is no assertion anywhere in `tools/` about inherited Windows permissions. No S-row is opened for that: both S-091 and S-109 are already OPEN and both already record that the mitigation is POSIX-only, so this is a stated limit of an open row, not a new finding. A Windows-side ACL gate is a real increment and nobody has written it.

**The gate.** `tools/test_gates_can_fail.sh`, which already drove the REAL `tools/run_all.sh` over synthetic wrappers and pinned every summary column and the exit code, was extended rather than duplicated: 48 assertions before this change, **52 now**, all green. Leg A's throwaway tree grew from five fixtures to eight — an honest platform-skip (its own column, no pass, no failure, named in the not-gated list and absent from the failure list), a LIAR that claims the marker while printing assertion-level `PASS:` lines (a failure, named with its reason), and one whose marker names no cause (a failure, `(no marker)`) — and leg A pins `RUN 8 / PASS 3 / FAIL 4 / PLATFORM-SKIP 1`, the widened identity, and exit code 1. A new leg A2 runs the same runner over a tree with the four failing fixtures removed and pins `RUN 4 / PASS 3 / FAIL 0 / PLATFORM-SKIP 1` with **exit code 0**: a wrapper that skipped entirely does not fail the suite. Leg C gained a second positive control — giving the causeless fixture a cause, and changing nothing else, moves it out of FAIL and into PLATFORM-SKIP. A new ratchet D5 pins the set of wrappers that EMIT the marker to EXACTLY those two, because claiming it takes a wrapper out of the judged population altogether and that must be a reviewed act rather than a quiet spread; leg E6 proves D5's detector is live by feeding it a file that emits the marker and one that only names it in a comment. Leg D's ratchet over the real `tools/*.sh` stayed green throughout — D1 was consulted before the shapes were written (every new branch is a single-arm `if`/`case`, so no block has every arm banking a pass) and nothing in the gate was relaxed to accommodate this change.

**Mutants — nine, each alone, each verified to have REACHED the copied runner before its verdict was read.** M1-M4 are the pre-existing arithmetic mutants and still turn the self-check RED. The five new ones are the interesting half, because **every one of them leaves `PASS + FAIL + PLATFORM-SKIP == RUN` perfectly satisfied**: the widened identity alone cannot see any of them, and they are caught only because leg A pins the VALUE of every column and the exit code. P1 counts a skipped-entirely wrapper as a pass (`8 4 4 0`); P2 counts it as a failure (`8 3 5 0`); P3 drops it from RUN instead, so the arithmetic silently balances (`7 3 4 0`); P4 lets the suite exit 0 with four real failures present (exit code 0 against the pinned 1); P5 lets a wrapper claim the marker while printing assertion-level `PASS:` lines (`8 3 3 2`). Each moves the pinned tuple `8 3 4 1 1 0` and therefore turns leg A RED. Two further mutants were run against `tools/ci_local.sh`: CI1 removed the `PLATFORM-SKIP` column from the summary — caught by `tools/test_gates_can_fail.sh` before `ci_local`'s own parse was reached, so the FAST suite went RED and `ci_local` exited 1 — and CI2 pointed `ci_local`'s parse at a column name that does not exist, which reached the new fail-closed branch: `FAIL: ci-local could not read the FAST suite's PASS / PLATFORM-SKIP columns`, exit 1. Both sources were restored and verified byte-identical by checksum before the final run.

**Riders: none.** No new wrapper, no new FAST member, no change to the `ONLY_PATTERN` alternation, no S-id taken. Doctrine lesson 20 asked to say so before writing a fourth copy of anything: this increment writes NO new copy of any primitive — no restricted-write loop, no strace parser, no assertion helper. The one new shell function is `pskip_emitters` in the gate, three lines, used twice.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, and no C++ source at all. No triage row opens, closes or changes status and no summary cell moves. The FAST membership stays 325. Every wrapper that passed before still passes with the same counts on Linux: `tools/test_wallet_out_perms.sh` 50 assertions in four sections and `tools/test_node_key_perms.sh` 7, both measured after the change. The runner's existing SKIP and asserted-nothing columns are untouched, and so is its rule of reading outcomes from markers rather than exit codes.

**Noticed and deliberately left alone, with the locus.** (a) `tools/run_all.sh:233` decides the new outcome from the MARKER only, exactly as it already did for `PASS:` and `FAIL:` — the runner has never read a wrapper's exit code, so a wrapper could print the marker and exit non-zero and still be scored a skip. Pre-existing property of the runner, not introduced here, and not narrowed here because doing so would re-tier every existing wrapper. (b) `tools/test_node_key_perms.sh:95` and `tools/test_wallet_out_perms.sh:130` are keyed on `uname` alone; a POSIX filesystem that genuinely cannot carry modes (FAT on Linux) is NOT covered by this outcome and would still be a declined section or a fail-closed floor, which is the conservative direction. (c) `docs/SECURITY.md:1128`, the test-table row for the in-process `determ test-node-key-perms`, describes that subcommand's own Windows SKIP and remains accurate; it was not extended to describe the new wrapper-level outcome. (d) `docs/proofs/WalletRecoveryFlows.md:331` read "44 assertions in three sections" — falsified by the commit BEFORE this one, which added section D and took the wrapper to 50 in four; the same sentence's platform clause is falsified by this change, so both were corrected together in that one sentence rather than left half-true. (e) `tools/test_gates_can_fail.sh` still records that 161 wrappers in this tree carry a `fail_count == 0` verdict with no assertion floor, 18 of them FAST members; that migration is untouched. (f) The D3 allowlist's known false positive on `tools/test_cluster_output_discipline.sh` is unchanged. (g) The reproduction of the Windows failure is a SIMULATION on Linux; no run on a real Windows box was performed by this increment, and the `windows-2022` job's return to green is therefore a prediction from that simulation, not a measurement.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **FAST 325 passed / 0 failed, 0 wrappers skipped entirely**, 0 wrappers declined a section, 16 doc guards green. The zero in the new column is itself the assertion that this change is invisible on this platform. `tools/test_gates_can_fail.sh` 52 assertions, nine runner mutants RED; the two `ci_local` mutants RED; the Windows simulation moves the pair from `RUN 2 / PASS 0 / FAIL 2`, exit 1, to `RUN 2 / PASS 0 / FAIL 0 / PLATFORM-SKIP 2`, exit 0.

**Authority:** the standing verification doctrine in `CLAUDE.md`, and the closing paragraph of the entry above this one, which specified this design — a third outcome, counted separately, the invariant widened to `PASS + FAIL + SKIPPED == RUN`, and the suite still exiting non-zero only on FAIL — and stated why it was not bundled into that fix. No owner decision is required or implied, and no S-id is taken: this is verification accounting, not shipped behaviour, the class the 2026-09-17 precedent recorded here only. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — the two long-RED `light/` wrappers were repaired, and repairing them found a real `--wait` gap the old guards could never have seen

**Status:** LANDED. Acts on the guard-drift adjudication recorded two entries above. Changes two test wrappers and no product code. Opens no ledger row, and reports one product finding that needs one.

**Problem.** `tools/test_light_wait_surface.sh` and `tools/test_light_negative_footing.sh` had been RED for weeks against a `light/` that was correct. Both sit outside FAST and outside `ci_local`'s sixteen doc guards, so `ci_local` was green with them red, which is exactly how they rotted; they became visible only when the sweep increment removed their vacuous bail-outs. Both failures had the same shape — **an assertion written as a PROXY for the property, true only on the day it was written.** The wait-surface guard asserted that a binding call's argument list CLOSES with a wait variable, as a stand-in for "the wait lands in the wait parameter"; `committee_bound_state_root` and `verify_state_root_at` later gained `expected_k` and `bft_enabled` AFTER the wait parameter, so every correct call stopped closing with it and the guard reported `opens=14 good=0 bad=14` and called it an "S-042 head-read REGRESSION". The negative-footing contract pinned hard-coded censuses — exactly 2 `cryptographic`, 7 `daemon_asserted`, 1 initializer-list, 10 total — as a stand-in for "every negative-verdict surface carries a correctly-classed footing"; `verify-notekey` (`nk:`) and `verify-enote-inclusion` (`en:`) later landed with correctly gated and correctly labelled footings and the pins read 9 against 7 and 12 against 10.

**The adjudication was re-established here, independently, not taken on trust.** All fourteen `committee_bound_state_root` call sites in `light/main.cpp` pass a wait variable at argument 4, which is exactly where `light/trustless_read.hpp:315` declares `max_wait_seconds`; all four `verify_state_root_at` calls pass it at argument 5, where `light/verify_state_root.hpp:116` declares it. **And the class of the two new footings — the thing the review said it could not establish — was established from the source.** Both `cmd_verify_notekey` and `cmd_verify_enote_inclusion` set their negative verdict inside an `err == "not_found"` branch of the `state_proof` RPC reply, which is the definition of the DAEMON_ASSERTED class; `cmd_verify_notekey`'s own comment says so ("Catches only a self-contradicting daemon; a consistent liar still forges the negative ((H-neg))"). `daemon_asserted` is the right class for both. No S-row for either wrapper: nothing in the shipped client was wrong about what they were asserting.

**The change: both assertions now derive what they used to pin.** The wait guard parses each helper's DECLARATION, finds the parameter named `wait` / `wait_seconds` / `max_wait` / `max_wait_seconds`, and asserts every call passes a wait expression AT THAT INDEX — the signature is the source of truth for which slot is the wait slot. The HELPER SET is derived too: the four named helpers plus every function in `light/` that takes a wait parameter and reaches a binding call, which today adds `light/account_history.cpp::verify_header_state_root_at` and `light/outbox.cpp::verify_and_bind`, and without which a new intermediate route could be introduced and its callers drop the wait while every named helper still checked out. Coverage went from `light/main.cpp` alone (14 calls examined, all four helpers) to all of `light/` (22 calls in the command layer, 11 in the routes and the other CLI entry points, six helpers). The negative-footing contract replaces its four censuses with COMPLETENESS — the set of functions that name a negative verdict in real code and emit json must equal the set that emits `negative_footing`, two code-derived sets with no number between them — and with a CLASS derived from how each negative was established: the contract collects every assignment made inside a `not_found` branch of the enclosing command and requires `absence-derived <=> "daemon_asserted"`.

**Each assertion is stronger, and here is the accounting, because "repaired by weakening" is the failure mode this was scoped to avoid.** (a) Wait slot: was "the wait variable is the LAST argument", now "a wait expression is at the argument index the declaration names". Stronger — the old form passed a call that put the wait into a trailing NON-wait slot, and had no view of the declaration at all, so it could not see the S-042 fix being reverted at the API. (b) Wait coverage: was 14 call sites in one file, now 33 across the whole directory with two DERIVED carve-outs (a file with no wait identifier and no `--wait` literal has nothing to forward) rather than a hand-maintained line-numbered quarantine that rots on any edit. Strictly stronger. (c) Footing census: was four exact counts, now set-equality plus a floor. Stronger where it matters — the old counts were preserved by a paired cross-site swap and by dropping one footing while adding another, both of which completeness and the derived class now catch — and weaker in exactly one respect, which is stated rather than hidden: deleting a whole command no longer turns the contract red, because both of its sets shrink together. The `MIN_CMDS = 11` floor is what stops that becoming vacuous, and it is a LOWER bound, so additions never trip it. (d) Footing value: was pinned to the GATE SHAPE by a hard-coded map asserting that a bare `verdict == InclusionVerdict::NOT_INCLUDED` means daemon_asserted. That map was the same proxy defect one level up — `verify-tx-inclusion` uses the SAME enumerator for a CRYPTOGRAPHIC negative — and it is replaced by the derivation. Strictly stronger. (e) One pin is kept DELIBERATELY and is asymmetric: a THIRD `cryptographic` footing is RED by design. A cryptographic footing claims soundness against a fully Byzantine daemon; the contract recognizes exactly the two derivations `NegativeVerdictSoundness.md` proves and refuses to bless a third. Adding a `daemon_asserted` footing — the common case, the one that rotted — needs no edit; adding a cryptographic one needs a human and a proof.

**What the repaired guards are robust to, and what they are not, said plainly in each wrapper's own output.** Robust to: parameters added, removed or reordered around the wait; the wait parameter moving to any position; new commands with correctly classed footings; renamed locals; multi-line calls; CRLF; and helper names, enumerator names and `not_found` markers appearing in comments or string literals, which are blanked before anything is matched. NOT robust to, each failing LOUD rather than silently green: a forwarded wait shadowed by a hard-coded local (a dataflow property; `tools/test_light_wait_flag.sh` and the cluster leg are what cover it); a wait parameter renamed outside the recognized set, which is indistinguishable from reverting the S-042 fix and is treated as such; an absence classifier written without the token `not_found`; and, for the footing contract, the value a BUILT binary actually emits, since it is a source contract and its live behavioural leg is a documented CI/WSL2 skip.

**The repair found a real gap, and it is a product finding, not a guard finding.** Extending the wait guard past `light/main.cpp` surfaced `light/outbox_cli.cpp:206`: `cmd_enqueue` calls `read_account_trustless` for its nonce hint WITHOUT forwarding a wait, while `outbox`'s shared `parse_args` accepts `--wait` for every verb (`light/outbox_cli.cpp:66`) and `outbox reconcile` does forward it (`:324`). So **`outbox enqueue --wait N` is accepted and is silently a no-op** — the exact S-042 symptom the flag exists to remove, and per `WaitHoldAndWaitSoundness.md` WH-4 the always-head readers fail closed on EVERY live current-state read without a wait, so the hint is not merely sometimes unavailable. It is NOT a soundness defect: WH-SN says `--wait` never changes WHICH value a verdict carries, and the hint is only floored into `max(local, hint)`, so the failure mode is the conservative fallback and, on a fresh outbox, the "cannot reserve a nonce offline" refusal. It is outside `WaitHoldAndWaitSoundness.md` §4.6 WH-6, whose enumeration covers `light/main.cpp` commands and does not mention the `outbox` verbs at all — so WH-6's coverage claim is narrower than its statement ("every trust-minimized reader ... forwards `--wait`") reads. **This needs a ledger row and a fix in `light/`, neither of which is in this increment's ownership.** The guard carries it as a single named KNOWN GAP that it prints on every run so it cannot rot back into invisibility, and the list is PINNED both ways: a second route-layer drop is RED, and a gap that gets FIXED is RED until its entry is deleted, so a standing permission cannot outlive its cause. Two other non-forwarding call sites were examined and are correctly excused by the derived carve-out — `light/verify_state_bundle.cpp:147` (`export-state-bundle` parses no `--wait` and `ExportStateBundleOptions` has no wait field) and `light/outbox_selftest.cpp:499` (`selftest-outbox-core` drives an in-memory `FixtureRpc`, so there is no head to race).

**Falsified before either was trusted, on scratch copies of `light/` — 26 cases, 0 unexpected.** The wrappers take a `LIGHT_DIR` so the transcripts are runs of the PRODUCTION path against a mutated tree, not of a private copy of the checking logic, and both harnesses REFUSE a verdict when a mutation did not actually reach the source (verified by breaking a mutant's pattern on purpose and watching the harness decline). Nineteen mutants RED: the wait argument omitted; a literal in the wait slot; the wait forwarded into the WRONG slot (invisible to the old position pin); the wait parameter removed from the signature; a forwarding route dropping it; the derived carve-out self-invalidating when a wait appears in an excused file; the known gap closed with its entry left standing; a new intermediate route whose caller drops the wait (invisible to a fixed helper list); the help surface gutted; a new negative-verdict command with no footing; a command dropping its footing; an absence-derived negative mislabelled cryptographic; the PAIRED CROSS-SITE SWAP that preserves every count; the unstake proven-zero and absence arms swapped; a footing emitted unconditionally; a gate replaced by a comment; both F-5 legs; the init-list emission leaving its NOT-RECORDED object; the whole F-2 surface deleted; and a new `NOT_` verdict enumerator appearing unadjudicated. **Three cases had to stay GREEN, and they are the point of the repair**: the signature gaining ANOTHER parameter after the wait — the exact change that turned the old pin red; the wait parameter MOVING to argument 3 with declaration, definition and every call updated together, where the derived slot follows it; and a NEW command landing with a correct `daemon_asserted` footing, the change that turned the old censuses red. Both wrappers also carry a `SELFTEST=1` mode that reruns the whole matrix in-tree.

**Riders: none.** No product code, no other wrapper, no document. Two files changed.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, no C++ source. FAST stays at 325 wrappers and the alternation is untouched: **neither wrapper is in FAST, so `ci_local` still does not run either of them**, and the verdict below is unchanged by this entry. Both now belong in FAST on the merits — each is pure source analysis needing no binary, no build and no cluster, and each runs in well under a second — but `tools/run_all.sh` is owned by another track this wave, so adding their stems to the alternation is recommended and not done here.

**Noticed and deliberately left alone, with the locus.** (a) Three documents still record the superseded censuses and are falsified by `light/` rather than by this change: `docs/proofs/NegativeVerdictSoundness.md:225` ("**8 × `daemon_asserted`** … 10 emissions total"; it is now 10 and 12), `docs/proofs/README.md:292` (the wait guard as "A: 9 … B: 6 …"; it is now 22 in the command layer and 11 in the routes, and the legs are differently shaped), and `docs/proofs/AbortRecordProofSoundness.md:188` ("the family's ten"). All three are in another track's ownership this wave and none is load-bearing for a proof step. (b) `WaitHoldAndWaitSoundness.md` §4.6 WH-6 states "every trust-minimized reader whose verdict rests on a `committee_bound_state_root` binding forwards `--wait`" and then enumerates `light/main.cpp` only; the `outbox` verbs are binding consumers and are absent from the table, which is how the `outbox enqueue` gap stayed unnamed. (c) Three further `NOT_`/`NO_` verdict enumerators exist in `light/` outside the F-2 family — `CommitteeVerdict::NOT_IN_COMMITTEE`, `EquivVerdict::NOT_EQUIVOCATION`, `SelectionVerdict::NOT_SELECTED`. Whether any owes an F-2 footing is NOT adjudicated here: it needs those three commands read against `NegativeVerdictSoundness.md`, which is its own increment. The contract records them as unadjudicated and asserts only that the set has not GROWN, so a fourth is RED rather than silently inherited; `Bind::NOT_INCLUDED` was examined and IS established out of family (a file-local return code in `light/outbox.cpp` that reaches no `--json` field). (d) Both wrappers now need `python3` for their source analysis and FAIL CLOSED without it, as `tools/common.sh` and 175 other tools already assume it.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, FAST 325 passed / 0 failed, 0 wrappers declined a section, 16 doc guards green — unchanged, since neither wrapper is in it. Run directly: `tools/test_light_wait_surface.sh` 11 assertions green (6 declaration-derived wait slots, 2 call-site legs, the known-gap pin, the help floor, non-vacuity), `tools/test_light_negative_footing.sh` 9 pass / 0 fail / 1 skip; `SELFTEST=1` green on both; the external matrix 26/26 as expected against scratch copies of `light/`.

**Authority:** the guard-drift adjudication recorded two entries above, which named this as the next thing worth doing in `tools/`, and the standing verification doctrine in `CLAUDE.md`. No owner decision is required or implied. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — integration pass over the writer-primitive, platform-skip and light-guard increments: a fail-open hole in the new outcome, a trap in the new shared primitive, and the counts this log owes itself again

**Status:** LANDED. Fixes three defects that exist only in the combination, admits two repaired guards to the FAST tier, opens S-112 for a product defect one of them found, and corrects the figures the three entries above carry.

**Problem 1 — the new `PLATFORM-SKIP:` outcome was fail-OPEN, and the hole is one that text cannot close.** The runner scores a wrapper's outcome from its CAPTURED OUTPUT, and captured text cannot say who printed a line. So a wrapper that asserted nothing and merely ran a CHILD which printed the marker was scored a platform skip, with the suite exiting 0 — where before this outcome existed the same wrapper was a hard failure, because it printed no `PASS:`/`FAIL:` marker of its own. Demonstrated against the real runner with two synthetic wrappers, one relaying a child's marker and one echoing the text directly: `RUN 2 / PASS 0 / FAIL 0 / PLATFORM-SKIP 2`, suite exit 0. **Requiring the cause to name the wrapper's own stem does NOT fix it** — that was the first repair attempted here and it was measured not to work, because a relay names the stem just as easily; the probe scored both as skips again. What fixes it is requiring the wrapper's OWN SOURCE to emit the marker: the runner now refuses to score a wrapper whose file contains no such `echo`, which no relay can satisfy. Re-measured: the relay is a failure again, suite exit 1, while the two real emitters still skip correctly under a simulated Windows `uname` and `tools/test_gates_can_fail.sh` stays at 52 pass / 0 fail. The two halves compose — the runner refuses a wrapper that does not emit the marker, and that gate's D5 ratchet pins WHICH wrappers may contain the emission at all, today exactly two.

**Problem 2 — the new shared primitive carried a comment telling its next caller that an unsafe combination was safe.** `include/determ/util/restricted_write.hpp` gained a test-only `simulate_narrow_failure_errno`, documented as able to "turn a success into a REPORTED failure and never the reverse, so it cannot weaken a permission". It REPLACED the `::fchmod` call rather than overriding its result, and `O_CREAT|O_TRUNC` does not narrow a pre-existing file — the same fact the header states twelve lines earlier — so under the `Continue` policy, which is what two of the three call sites pass, a 0644 target stayed 0644 through the write. Not reachable today, because the only site that passes a non-zero errno also passes `Refuse`; but a shared primitive must not set that trap for the caller after next. The real `::fchmod` now always runs and only its result is overridden, and the comment says what the code does.

**Problem 3 — the integrator moved a figure and left its derived pair stale, one wave after recording that exact mistake.** `docs/SECURITY.md` carries the FAST membership in two places. Moving it 325 → 327 for the two wrappers admitted below, the second citation's derived clause was left reading "of which 325 ASSERT on Linux and macOS and 323 on Windows" — both superseded; it is 327 and 325. The entry of 2026-09-18 two above this one records the identical failure in the identical sentence, in my own words, with the identical root cause: no doc guard derives that pair, so it is green and wrong. Recording a lesson is not the same as having learned it.

**Two repaired guards admitted to FAST.** `tools/test_light_wait_surface.sh` and `tools/test_light_negative_footing.sh` are now in the `ONLY_PATTERN` alternation, taking the tier 325 → 327 (328 stems, one of which has no wrapper file, matching 327 of 571 on disk). They are pure source analysis, need no binary and run sub-second, and they had been RED for weeks OUTSIDE the tier — which is exactly why nothing noticed. Admitting them is the change that stops that specific rot recurring; leaving them repaired but unrun would have reproduced the condition that created them.

**S-112 opened** for the defect the wait-surface repair found: `determ-light outbox enqueue --wait N` is accepted and is silently a no-op. Verified independently of the track that reported it — `read_account_trustless` takes the wait as parameter 7 defaulted to 0 (`light/trustless_read.hpp`), `cmd_enqueue` calls it with four arguments (`light/outbox_cli.cpp:206`), the shared parser accepts `--wait` for every verb (`:66`), and `reconcile` does forward it (`:324`). Liveness, not soundness: a wait never changes WHICH value a verdict carries. The row records that the accept-and-ignore surface is **seven verbs wide** — one consumes the flag, one drops it where it matters, five accept it with no binding read — so that a reader who patches the one call site does not believe the surface is closed.

**Corrections this append-only log owes itself.** (a) The platform-skip entry above states the tier membership as **325**, the alternation as **326 stems**, the match as **325 of 571**, and that "both sentences in `docs/SECURITY.md` now say which is which"; integrated with the two admissions the figures are **327**, **328**, **327 of 571**, and the second sentence needed the correction described in Problem 3. (b) The light-guard entry above states that "neither wrapper is in FAST, so `ci_local` still does not run either of them"; both are, and it does. Neither entry was wrong when written.

**An environment incident on the build box, recorded because it could have silently corrupted a verdict and did not.** Building a fake-Windows sandbox, one track copied a shim over a symlink and replaced `/usr/bin/uname` for roughly 95 seconds (2026-09-18 08:26:00–08:27:36 UTC), during which every process on the box saw `uname -s` = `MINGW64_NT-10.0-22621`. Checked, twice and independently: exactly three files on the whole box were written inside that window, all three that track's own Windows-simulation transcripts, and all three were superseded by re-runs after the restore. No commit, no `ci_local` log and no mutant log falls inside it; all five `ci_local` logs record `platform Linux x86_64`. One artifact survives with a self-contradicting header (`BOUNDARY 1: Linux (MINGW64_NT-10.0-22621)`) recording the OPPOSITE of the claim it was made to support — the wrappers correctly took the Windows branch because `uname` was lying — and it is superseded, not evidence. **No conclusion in this tree rests on a measurement taken during that window.** The reason is timing, not design: nothing prevented it.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration. The only C++ touched is the comment and the ordering of two statements inside the test hook of `include/determ/util/restricted_write.hpp`; no call site, no diagnostic and no on-disk byte changes.

**Noticed and deliberately left alone, with the locus.** The restricted-write increment made TWO ungated Windows changes, not the one its entry names: the close error is now checked, and the file is now removed on failure, at all three sites. Both are fail-closed and right; only the first was named. `tools/test_light_negative_footing.sh`'s `MIN_CMDS = 11` is a cardinality floor rather than a membership ratchet, so cloning one command while deleting another keeps the count and hides the deletion — measured green with `cmd_verify_param_change` gone. `tools/test_light_wait_surface.sh`'s widened `WAIT_EXPR` can be satisfied by an unrelated `.wait` field on another object. `keyfile-recover --out` and `keyfile-decrypt --out` write unframed bytes and JSON respectively, so the "every container rejects a fragment" argument for removing a failed write's leftovers does not reach its hardest case — the conclusion survives and is stronger there, since nothing can reject an unframed fragment. `tools/test_light_wait_surface.sh` prints one declined leg as lowercase `skip:`, which the runner's skip column does not count.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **RUN 327 / PASS 327 / FAIL 0 / PLATFORM-SKIP 0**, 16 doc guards green.

**Authority:** the standing verification doctrine in `CLAUDE.md`, and the independent adversarial review of the integrated diff, which returned NOT SAFE AS WRITTEN on the three problems fixed here. No owner decision is required or implied. Recorded by Claude (Cowork session), 2026-09-18. This entry is append-only per convention and carries no tier marker in its body.

---

## 2026-09-18 — `determ-light outbox`: one parser served seven verbs and accepted every verb's flags for all of them, so the wait an operator asked for reached nothing

**Status:** LANDED. Closes S-112 (the instance) and S-113 (the class), resolves the `WH-6` scope question in `WaitHoldAndWaitSoundness.md`, turns a cardinality floor into a membership ratchet, and adds one FAST gate.

**Problem.** `light/outbox_cli.cpp` has ONE `parse_args` for all seven `outbox` verbs — `enqueue`, `submit`, `reconcile`, `status`, `replace`, `prune`, `recover`. That is reasonable: the flags spell the same things everywhere. What was not reasonable is that it ACCEPTED the union of every verb's flags FOR EVERY VERB. A flag a verb does not read changes no output, no exit code and no file, so nothing in the tree could observe it, and nothing did. Measured against the pre-fix binary: all 133 (verb, flag) pairs were accepted and only 43 are read, so **90 of the 133 were accept-and-ignore** — `outbox status --wait 30` exited 0 in 12 ms having waited for nothing, `outbox recover --timeout-ms 1` and `outbox replace --json` likewise. The instance that was already on the ledger, S-112, is the one that costs something: `cmd_enqueue` reserves its nonce against a committee-verified `next_nonce` read through `read_account_trustless`, that read anchors at the chain HEAD, and it was called with FOUR arguments where the wait is parameter seven and defaults to 0. So `outbox enqueue --wait 30` was accepted, the head-anchored successor binding could not complete, and the operator got the conservative local fallback while believing the reader had been told to block.

**Reproduced by EXECUTION before the fix, twice, both transcripts in the audit dir.** (1) Against the pre-fix binary, all seven verbs accepted `--wait 30` and returned in 12-19 ms of wall clock; eleven further wrong-verb flags were accepted the same way. (2) Against the pre-fix product source with a temporary harness in the test file only — no product file touched — `cmd_enqueue`'s four-argument call was issued next to the same read with the wait forwarded, over an in-process committee-signed fixture chain whose next block lands on the third successor poll: the shipped call polled the successor **once** and failed closed with a message that never mentions a wait; the forwarded call polled **three** times and produced the hint. That is the defect, executed, not argued.

**The change, in three parts.** (a) The hint read is now `light/outbox.cpp::nonce_hint_trustless`, one named route whose `wait_seconds` parameter has **no default** — the omitted-defaulted-argument shape that caused S-112 no longer compiles — and `cmd_enqueue` passes `a.wait` into it. (b) `light/outbox_cli.cpp` gains `verb_flags()`, the per-verb set of flags each verb ACTS ON; anything outside a verb's set is refused before the branch that would parse it, with a diagnostic naming the flag and the verb and a non-zero exit, exactly as an unknown argument already was. The union used to tell "you used a real flag on the wrong verb" from "you mistyped" is DERIVED from that same table, so the two cannot disagree. Refusal, not a warning, is the choice: this repo fails closed, the parser already threw on an unknown arg, and every flag removed from a verb's surface carries an operator expectation that was not being met — `--wait` above all. (c) The usage block and `docs/CLI-REFERENCE.md` now state the enforced sets. Two of them were wrong in the other direction and are corrected: `--timeout-ms` is READ by `enqueue` and `reconcile` and was advertised on neither.

**This is a behaviour change on a shipped command, deliberately.** `outbox status --wait 30`, `outbox replace --json`, `outbox prune --rpc-port N` and 87 other pairs now exit 1 instead of 0. Nothing in the tree used one: `tools/`, `docs/`, `README.md`, `sdk/`, `dapps/`, `.github/` and `sim/` were grepped and every `outbox` invocation in them stays inside its verb's set. An operator script that used one was not having its request honoured either way; it now finds out. The per-INVOCATION refusal added after the review (below) is six more configurations that now exit 1: `enqueue --rpc-port/--timeout-ms/--wait` beside an explicit `--nonce`, `enqueue --timeout-ms/--wait` without `--rpc-port`, and `reconcile --state` without `--resume`. ONE in-tree invocation was inside that set and is corrected rather than grandfathered — `tools/test_light_outbox.sh` passed `--rpc-port 1` next to an explicit `--nonce 3`, which is precisely the defect in miniature: a human wrote a flag that did nothing and nothing told them.

**An independent adversarial review then found the headline false INSIDE one verb, and the gate unable to see a flag added to the parser. Both are fixed here, not documented around.** (1) `cmd_enqueue`'s nonce-reservation block is the only reader of `--rpc-port`, `--timeout-ms` and `--wait`; it is skipped whole under an explicit `--nonce` and opens no socket without `--rpc-port`, so all three were still accepted-and-ignored there — the verbatim S-112 symptom with one fewer flag, inside the verb the increment is named after. `anchored_head` loads the anchor `--state` names only on the `--resume` path, so `reconcile --state` without `--resume` was the same. Rather than qualify the claim, the claim is made true: `refuse_inert_flags()` refuses a flag the verb reads but that nothing reads AS INVOKED, naming the flag AND the flag whose presence or absence made it inert, before anything is locked or written. Which pairs those are was MEASURED, not inferred — every accepted (verb, flag) pair was driven in every configuration its verb admits and two runs differing only in that flag were compared on exit code, output, the bytes of the outbox afterwards and wall clock; exactly six triples came back inert and exactly those six are refused. `submit --now` also reads inert against a slot that has never failed, and is deliberately NOT refused: that is state-dependent, not configuration-dependent, and refusing it would refuse a correct command. (2) The gate hard-coded a 19-element flag list, so a 20th flag with a parser branch and a `verb_flags()` entry but no usage-text line escaped all four legs. Confirmed by running exactly that mutant against the pre-fix tree: GREEN, 57 pass / 0 fail, while `outbox status --dry-run` exited 0 having ignored the flag. The universe is now DERIVED from the binary (each verb is provoked into its own wrong-verb refusal, which prints its whole accepted set; the union of the seven is the universe), the hand-pinned contract stays as the fixed point it is measured against, and leg B now closes over leg A: every pair the matrix measured as accepted must be probed in B or declined in B by name. The same mutant is now RED on three assertions.

**The gate — `tools/test_light_outbox_flag_surface.sh`, FAST, 65 assertions, and not one of them greps a source file.** The property it asserts is the class, not the instance, in two halves: FOR EVERY (VERB, FLAG) PAIR THE PARSER RECOGNIZES, THE VERB EITHER OBSERVABLY ACTS ON THE FLAG OR REFUSES IT; and a flag the verb does read but that nothing reads in the configuration given is refused there too. **A** derives the flag universe from the binary's own refusal diagnostic, drives all 7 x |universe| pairs through the binary and classifies each from what the process did — accepted, refused by this verb, refused as INERT in this invocation, or no longer recognized by the parser at all, four outcomes because conflating any two would let a mutant read as something it is not — then requires THREE sets to agree per verb: what the binary accepts, what the binary says it accepts, and the hand-pinned contract. Accepting one more flag is RED, refusing one it reads is RED, and every verb must refuse at least one flag, a non-vacuity floor without which the "every refusal is fail-closed and by name" assertion passes on a verb that refused nothing. **B** requires every accepted pair to change something observable between two runs that differ only in that flag: exit 3 on an unreadable-meta directory for `--outbox` on all seven verbs, a JSON object for `--json`, exit 6 for a foreign `--genesis` or `--keyfile`, a moved acknowledged tx hash for `--to`/`--amount`/`--fee`/`--payload-hex`, fee-bump versus re-issue on `replace`, exit 8 for a reused `--idempotency-key`, exit 4 for `--max-messages`, the record's own failure counter for `--now`, the port named in the binary's own transport diagnostic for `--rpc-port`, and a **~1 s wall-clock window** for `--timeout-ms` on all three network verbs against a local peer that accepts a connection and never answers. **B5** requires each of the six measured inert triples to be refused fail-closed in its inert configuration, to name its cause, to leave no directory/lock/record behind, and NOT to be refused in the configuration that reads it — a refusal asserted only in one direction would pass on a binary that refused the flag everywhere. **B then closes over A**: every pair A measured as ACCEPTED must be probed in B or declined in B by name, and nothing may be registered that the binary does not accept. **C** requires `determ-light help` to advertise exactly the set leg A MEASURED the binary accepting — both sides are the running binary, so no comment and no list in the script can satisfy it. **D** runs the new in-process `selftest-outbox-hint-wait`, which drives the real nonce-hint read over the fixture chain and asserts one extra successor poll per wait-second, the fail-closed diagnostic naming the wait, and the verified hint existing only when the wait carried through.

**What the gate declines, by name, banking no pass.** Seven (verb, flag) pairs' effects cannot be reached without a daemon: `--older-than` and `--include-unlocated` select among PROVEN-consumed slots, which take a daemon-verified inclusion to produce; `--resume`, `--state`, `--wait` and `--json` on `reconcile` are all read past `connect_and_pin`; and `--wait` on `enqueue` bounds a successor poll that a peer which only accepts never reaches. Each prints a `SKIP:` naming the cause and the live gate that covers it, and each is registered as a (verb, flag) pair so the coverage assertion accounts for it rather than leaving a hole shaped like a decline. And the gate does NOT cover `cmd_enqueue`'s own forwarding INTO the route — leg D drives the route, because reaching the verb's read needs a committee-signed chain behind a socket. That call site is covered by `tools/test_light_wait_surface.sh`'s derived-helper check and by the route's parameter having no default; mutant M5 below is exactly that drop and the split is measured, not assumed.

**Mutants, each against a REBUILT binary, and the harness refuses a verdict unless a marker it injected is found in the edited source AND in the relinked binary.** M1 `nonce_hint_trustless` reads with a literal 0 (S-112 re-opened inside the route) -> the new gate RED on leg D and `test_light_wait_surface` RED. M2 `outbox status` accepts `--wait` again -> new gate RED on leg A. M3 the usage block advertises `--now` on `outbox status` -> new gate RED on leg C. M4 `outbox submit` refuses `--now`, which it acts on -> new gate RED in both directions. M5 `cmd_enqueue` passes a literal 0 into the route -> `test_light_wait_surface` RED, new gate GREEN, which is the residual named above. M6 the refusal is downgraded to a stderr warning and the flag accepted anyway -> new gate RED on five verbs. N1 `refuse_inert_flags` accepts every inert flag again (the review's B1 re-opened) -> RED. N2 the same refusal made BLANKET, refusing the three flags even where they ARE read -> RED on the enabling direction. N3 the inert refusal keeps naming the flag but stops naming the flag that made it inert -> RED. N4 the binary stops printing the accepted set the universe is derived from -> RED, fail-closed, rather than a silently empty universe. And the review's own mutant: `--dry-run` added to `status`'s set with a parser branch that reads nothing and no usage-text line -> GREEN 57/0 before this change, RED on three assertions after it. Three more live inside the guards' own SELFTEST modes: RH (the route re-opened) and RJ (the call site) in the wait-surface guard, and N13 below.

**Two guards this track owns, repaired.** `tools/test_light_wait_surface.sh`: its KNOWN GAP list named S-112 and is PINNED in both directions, so fixing the product made the guard RED until the entry was deleted — by design, and the entry is now gone; the mechanism stays, the header records what closed the gap, and the two new selftest cases RH/RJ replace the old "gap closed but entry left standing" case with the two ways the gap can come back. The guard now derives `nonce_hint_trustless` as a binding route on its own. One consequence is recorded rather than hidden: `light/outbox_selftest.cpp` used to be excused by the derived carve-out because it named no wait at all, and it now exercises the wait deliberately, so it is no longer excused and every binding call in it passes a named wait — including the zero the control read uses, because that fixture's successor is always one poll away. `tools/test_light_negative_footing.sh`: its `MIN_CMDS = 11` was a CARDINALITY FLOOR, and clone-one-delete-one walks straight past a floor — delete one pinned command's negative-verdict surface, clone another, and the count is still 11. It is now a MEMBERSHIP ratchet over the eleven measured command names: additions stay free (a new negative-verdict command joins the derived set with no edit), a removal or a substitution is RED, and a deliberate rename is RED and must be re-pinned, which is the point of a ratchet. Selftest case N13 injects exactly the clone-one-delete-one shape and requires RED; it fires on the membership check alone, with the count unchanged at 11.

**WH-6 resolved: the STATEMENT is authoritative and the ENUMERATION was incomplete.** `docs/proofs/WaitHoldAndWaitSoundness.md` §4.6 states the rule for "every trust-minimized reader" and then enumerated `light/main.cpp` commands only. `WH-SN` (§5) quantifies over "any binding consumer of §4.6", so a reading confined to one file would leave the theorem silent about readers that exist — and that silence is how S-112 went unnamed, since nothing said the `outbox` verbs owed a forward. The enumeration now carries `outbox enqueue` and `outbox reconcile`, §8 counts nineteen binding consumers instead of seventeen, and the "non-state commands correctly omit `--wait`" paragraph records that the five `outbox` verbs with no binding read were the one place where that sentence was false and are now enforced. No lemma is re-derived; `WH-1`-`WH-5` already applied to those readers, which reach the identical helper.

**Riders: none.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, no new dependency. The signed bytes, the record format, the durability contract, the exit codes and every existing message are untouched.

**What does change beyond the above, stated because it is a change.** `cmd_enqueue`'s F-4 retry now wraps the genesis pin as well as the read, because the pin is part of the route. On the rare TAMPERED re-read (at most two retries) the daemon's block 0 is fetched again. That is one extra RPC on a retry and it re-verifies that the daemon is still on our chain before the reservation is steered — strictly more conservative, and named here rather than left to be discovered.

**Noticed and deliberately left alone, with the locus.** (a) The S-113 CLASS is closed for `outbox` only. A measured sweep found `light/outbox_cli.cpp:108` is the tree's only parser shared across verbs, and the per-command parsers in `light/main.cpp` and `wallet/main.cpp` do refuse unknown arguments (probed live). But a per-command parser can still bind a flag it never reads, and nothing gates that. One such site is confirmed BY EXECUTION: `determ genesis-tool build <cfg> --wait 5 --older-than 9` exits 0 having ignored both, because the parser at `src/main.cpp:5475` scans its argv only for `--json`. `src/main.cpp` belongs to another track this wave; the site is named, not touched. (b) The same method — drive the binary, require every accepted flag to change something observable — would cover `determ-light`'s 76 command handlers, `determ-wallet` and `determ`; that is its own increment and is not claimed here. (c) `tools/test_light_outbox_flag_surface.sh` runs in ~13.7 s and `tools/test_light_outbox.sh` next to it in ~9.4 s (2026-09-18, 2 cores under two concurrent builds; ~9.2 s and ~7 s unloaded). Most of it is the deliberate real sleeps of the timing windows and ~140 process launches each; neither is reducible without giving up the property. The "<5s each" sentence in `docs/SECURITY.md` and `tools/run_all.sh` was already false before this increment and is corrected rather than left standing, in both places. (d) The wrong-verb and unknown-arg diagnostics print their prefix twice (`outbox status: outbox status: ...`): `parse_args` builds the prefix into the message and `fail()` prepends it again. It is cosmetic, it is this increment's own message, and it is named here rather than fixed, so the diff stays the review's two blockers and the four findings it named. The new inert diagnostic does not repeat it. (e) `submit --now` is a no-op against a slot that has never failed — state-dependent, not configuration-dependent, so it is measured and left accepted; the gate arms a backoff with a first failed submit before measuring it. (f) The review's finding 1 stands: the `--now` probe's backoff window (`BACKOFF_BASE_S = 2`, second granularity) is the tightest timing dependence in the gate. It is fail-closed (a miss is RED) and it was not touched here.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **RUN 328 / PASS 328 / FAIL 0 / PLATFORM-SKIP 0**, 16 doc guards green. The FAST tier moves 327 -> 328 and the three derived figures in `docs/SECURITY.md` move with it: 328 -> 329 stems, 571 -> 572 wrappers on disk, and "327 assert on Linux and macOS and 325 on Windows" -> 328 and 326.

**Authority:** the standing verification doctrine in `CLAUDE.md`, the S-112 row's own statement that the fix needs "a decision about the five silent acceptors and a decision about whether WH-6's enumeration or its statement is authoritative", and the fail-closed disposition this repo applies to every other unparseable input. No owner decision is required: the accepted surface shrinks to what was already documented, and nothing in the tree used the removed pairs.

---

## 2026-09-18 — the secret-source convention leaves light/: eighteen determ-wallet and determ parse sites stop needing the private key on argv, and the primitive that was written twice becomes one header

**Status:** LANDED. Opens and partially mitigates S-114 (`determ-wallet`) and S-115 (the `determ` daemon), promotes `light/seed_source.{hpp,cpp}` to `include/determ/util/secret_source.hpp` behaviour-preservingly, and adds one FAST gate. The raw flags are NOT removed; the class is NOT closed and this entry names every site left.

**The problem, reproduced by EXECUTION before anything was written** (`/root/audit/z2/REPRO-2026-09-18.txt`). A running process's argument vector is world-readable at `/proc/<pid>/cmdline` — measured `-r--r--r-- root:root`, and read there by a process in a different session with no relationship to the child — `ps -o args=` prints the same line to every user on the host, and the operator's interactive shell writes it verbatim into `$HISTFILE` (all three legs executed, not argued from source). A real Ed25519 private key minted by `determ-wallet account-create-batch` was recovered out of `/proc/<pid>/cmdline` of a `determ-wallet account-import --priv <key>` child, and a real keyfile passphrase out of a `determ account create --passphrase <pw>` child; both children were pinned in a deterministic window by making their `--out` a FIFO, so there is no race and no sampling luck. This is S-110 — reproduced and closed for `determ-light` on 2026-09-17 — in the other two binaries, where it had been left unfixed BY NAME: the S-110 entry's "not in scope" paragraph names `determ-wallet keyfile-create --priv` and `determ-wallet account-import --priv` as deliberately deferred. Twelve `determ-wallet` parse sites and six `determ` parse sites were affected. For a private seed, a master seed, a Shamir secret, an RPC HMAC secret or a keyfile passphrase, that argv string IS the secret.

**The change, and it is ADDITIVE.** Each of the eighteen gained a `--<name>-from <file:path|env:NAME|prompt>` twin: `shamir-split --secret-from`, `envelope encrypt/decrypt --password-from`, `account-derive-batch --seed-from`, `account-import --priv-from`, `backup-create --secret-from`, `keyfile-create --priv-from`, `create-recovery --seed-from` and `--password-from`, `recover --password-from`, `message-sign --priv-from`, `rpc-auth --secret-from`; and in the daemon `account create/decrypt --passphrase-from` and the `--priv-from` of `submit-param-change`, `submit-merge-event`, `submit-dapp-register`, `submit-dapp-call`. The raw flags keep working with the same accepted inputs, the same stdout bytes and the same exit codes; they additionally print the named `WARNING[seed-on-command-line]` line to stderr naming the `-from` alternative. Both forms of one secret at once are refused. A missing or empty file, an unset or empty variable, an empty `file:` path or `env:` name, and an unknown scheme are each refused with a named diagnostic and exit 1. The `DETERM_PASSPHRASE` fallback of the daemon's two account subcommands is unchanged and is still consulted last.

**The primitive that had been written twice is now written once.** `include/determ/util/secret_source.hpp` is header-only for the reason `include/determ/util/restricted_write.hpp` records and which was re-checked against `CMakeLists.txt` rather than assumed: `determ`, `determ-wallet` and `determ-light` are three separate `add_executable` targets and the only library between them, `determ-crypto-c99`, takes only `.c` sources. `light/seed_source.cpp` and `wallet/main.cpp::passphrase_from_source` are now forwarders to it. The two pre-existing copies agreed on every diagnostic string, the three source forms and the exit code, and differed on exactly three things — the noun in the diagnostics, the prompt label, and whether the value is whitespace-trimmed. Those three are the `SecretPolicy` fields; everything else is the mechanism. The trim difference is not cosmetic and is now pinned by a mutant: a passphrase may legitimately begin or end with a space, so a `file:` line loses only its trailing CR/LF and `env:`/`prompt` values are taken verbatim, while a hex seed is trimmed at both ends.

**The move is behaviour-preserving, and the proof is that the gate written for the old code is green UNCHANGED.** `tools/test_light_seed_source.sh` was not edited — not one character — and reports the same 28 assertions passed, including its own in-gate reproduction of S-110 and its `/proc/<pid>/cmdline` reads. The mutant that proves the unchanged gate can still fail is **M4b**, which flips `light/seed_source.hpp`'s `SEED_POLICY` binding to PASSPHRASE — the single point both live light entry points share — and turns that gate RED at five assertions. It is M4b and not M4 because **M4 came back GREEN**, and that is recorded below rather than renumbered away.

**The three sources are RANKED, in every surface an operator reads.** The first cut listed `file:<path> | env:<NAME> | prompt` flat in all five new surfaces — the S-114 and S-115 ledger rows, the two new `docs/CLI-REFERENCE.md` convention rows and the two binaries' top-level usage blocks — while `docs/SECURITY.md`'s own S-110 row, unchanged since 2026-09-17, already said `file:` with restrictive permissions is the strongest of the three because `env:` stays visible in `/proc/<pid>/environ` to the same UID. The ranking existed only in the header's source comment, which an operator never reads. All five surfaces now carry it, plus the thing that was nowhere said: **`SECRET=... determ-wallet account-import --priv-from env:SECRET` puts the secret straight back into the shell history file**, which is one of the three leaks this entry exists to close. `env:` is worth having when the variable comes from a parent process, a CI secret store or a systemd credential — not when it is typed on the command line.

**The unknown-scheme refusal no longer echoes a long specifier.** It printed `unknown private key source '<spec>'` verbatim. The likeliest way to reach it is the typo `--priv-from <the key itself>` — one character from `--priv <key>`, and the two flags now sit next to each other in every usage string. `/proc/<pid>/cmdline` dies with the process; stderr goes into CI logs, ticket attachments, terminal scrollback and `script` transcripts, which do not, so echoing it converts a transient exposure into a stored one. Specifiers of 16 characters or fewer are still echoed verbatim — every refusal any gate greps for is inside that bound, so no shipped diagnostic changed — a longer one with a colon in its first 16 characters keeps the scheme-looking prefix (`fille:<15 chars elided>`, which is what makes a mistyped scheme diagnosable), and a longer one with no colon is not echoed at all (`<64 chars elided>`). B21b and B21c assert it; M10 is RED.

**One stable marker, deliberately.** Every warning in all three binaries prints `WARNING[seed-on-command-line]`, keeping the historical noun even where the material is a passphrase or an HMAC secret. It is the spelling S-110 shipped and `docs/SECURITY.md` documents; a second spelling would mean an operator grepping the documented marker silently misses two binaries. The sentence under the marker names the actual material and the actual flags.

**The gate.** `tools/test_secret_on_argv.sh`, FAST, 102 assertions, four sections. A EQUIVALENCE (14): byte-identical stdout between the two forms on the deterministic commands, and for the randomized ones — which mint a fresh salt, nonce or polynomial, so byte-equality is not available — the artifact the `-from` form produces reconstructs to the SAME secret. The password fixture carries leading, trailing and internal spaces, so a source that trimmed whitespace would produce a container nobody can open. A5/A6 catch that through the `file:` branch; **A5b/A6b catch it through `env:` and `prompt`, which are a DIFFERENT function** — `detail::normalize_value`, one branch over from `normalize_line` — and were unasserted in the first cut, so a mutant trimming in that one function left all 98 assertions green. Review found it on 2026-09-18; M7b is the mutant that now kills it. B SOURCES (45): for EVERY one of the eighteen pairs, a missing `file:` source is refused with a named diagnostic that names that flag and exits 1 — the assertion that proves the flag is parsed and routed at all, because an unwired flag is ignored and the command prints its required-argument error instead — and both forms at once are refused; plus the other source shapes on representatives, and that an unrecognised specifier longer than 16 characters is ELIDED rather than echoed back. C WARNING (38): for every one of the eighteen, the raw flag warns by name and names its twin, the `-from` form does not, and the warning is a WARNING — the raw form still emits its output and exits 0. **D PROCESS TABLE (5), the property itself, read from the kernel and not from a grep of the source**: with `-from` the secret is ABSENT from the running child's `/proc/<pid>/cmdline` and with the raw flag it is PRESENT there, both children pinned by a FIFO.

**The window is SYNCHRONISED on the kernel, and the first cut of this increment got it wrong.** It polled `/proc/<pid>/comm` until the child had exec'd and then slept 100 ms, and the four documents that described it — the wrapper's header, its own PASS text, the S-115 ledger row and an earlier draft of this entry — all said the sample was taken "strictly AFTER the envelope was sealed". Independent review measured it, 5 runs out of 5: at the sampling instant the daemon child was `state=R`, `wchan=0`, with the `--out` FIFO NOT yet open — still inside Argon2id, which costs 151-175 ms while the sample landed at ~110 ms. The window that was actually measured was "100 ms after exec". The wallet leg happened to be correct (5 ms of work, so it was genuinely blocked), which is exactly how a timing coincidence passes for a property. **The gate now waits until `/proc/<pid>/wchan` names the FIFO rendezvous (`wait_for_partner`) before sampling** — the same thing the wallet leg was accidentally achieving — with the existing 25 s watchdog as the bound; if the child never blocks, the leg FAILS rather than banking a pass, and on a kernel that does not name the wait it prints `SKIP:` and banks nothing. Measured after the fix: the daemon leg samples at t+169..192 ms, `state=S wchan=wait_for_partner`, 5 runs of 5. **The proof that it was worth doing is mutant M9**, which writes the resolved passphrase into the argv block AFTER `envelope::encrypt` and `serialize` have run: RED against the synchronised gate, **GREEN against a byte-for-byte replica of the old timed one**. This is wave-doctrine lesson 16 — "for a 'the secret is absent from X' property, sample at or after the last moment the code could have put it there" — recorded one day earlier and repeated here anyway; recording a lesson is not the same as having learned it.

**What the D window still does not cover, said here rather than discovered later:** an argv write AFTER that `open(2)` returns, i.e. during the `--out` write or between it and exit. A FIFO can pin a process at an open and nowhere else. That residue is a few hundred microseconds of a code path that holds no secret, and it is not zero; the gate's header says so too.

Where `/proc/<pid>/cmdline` cannot be read the section prints `SKIP:` and banks nothing, and A, B and C still assert there; this is not a wrapper-level platform skip, because most of what it gates exists on every platform.

**The mutants — twelve, INCLUDING the two that came back GREEN, because a record that reports a cleaner result than the harness measured is worse than the defect it hides.** Each was verified to have reached BOTH the source and the RELINKED binary by a marker grep before any verdict was read; the harness refuses a verdict otherwise, and also refuses one if the build failed or the binary's mtime did not move. The transcripts are `/root/audit/z2/mutants/run.log`, `/root/audit/z2/fix/mutants-M7b-M10.log` and `/root/audit/z2/fix/B2-M9-two-windows.txt`.

RED: **M1** wallet argv-writeback after resolving `--priv-from` → D1. **M2** the named warning dropped → all eighteen C "raw warns" assertions, in both binaries, plus C37. **M3** an unopenable `file:` source accepted instead of refused → B. **M5** the both-at-once refusal dropped → B. **M6** a `file:` source resolving to the wrong value while every refusal still fires → A, and only A. **M7** the passphrase policy trimming whitespace like the hex policies → A5/A6/A10. **M8** daemon argv-writeback immediately after resolving `--passphrase-from` → D3. **M4b** `light/seed_source.hpp` binding `SEED_POLICY` to PASSPHRASE → five assertions of the UNCHANGED light gate. **M7b** `detail::normalize_value` trimming for every policy — the `env:`/`prompt` half of the no-trim rule, with the `file:` half left correct → A5b/A6b. **M9** the daemon writing the resolved passphrase onto argv AFTER the envelope is sealed and serialized → D3. **M10** the unknown-source refusal echoing the whole specifier again → B21b/B21c.

**GREEN, and each one is a finding this increment would otherwise not have made:**

* **M4** — `determ::light::seed_hex_from_source` made to forward on the PASSPHRASE policy instead of SEED. `tools/test_light_seed_source.sh` stayed green, exit 0, zero FAIL lines. The harness's own words are "THE GATE CANNOT SEE THIS MUTANT. That is a hole, not a pass." The cause is the finding: **after the promotion, three of the five `determ::light::` forwarders have NO CALLER inside `determ-light`** — `seed_hex_from_source`, `warn_seed_on_command_line` and `zero_secret_string` (verified by grep over `light/`: callers exist only for `resolve_seed_hex`, `zero_secret_bytes`, `SeedScrub` and `scrub_on_scope_exit`). An editor who opens `light/seed_source.cpp` to change one of those three bodies changes nothing, runs the gate, sees 28 green and believes light's behaviour moved. It did not. They are kept (they are the documented S-110 names and deleting declarations is not part of a behaviour-preserving move) and the fact is written into `light/seed_source.hpp` at the locus. M4 was then re-aimed as M4b at the one point that CAN still drift. **A mutant coming back GREEN is what found this; reading the code had not.**
* **M7** — GREEN on its FIRST run. The gate's password fixture was `'p a s s  w/ spaces'`, internal spaces only, so trimming was a no-op on it and the rule the mutant erased was invisible. The fixture now carries LEADING and TRAILING spaces, which is the case the passphrase policy exists for, and M7 is RED. The gate would otherwise have claimed to pin a rule it could not see.

Neither of those two facts survived into the first draft of this entry or the first draft of the S-114/S-115 ledger rows: both said "mutants M1-M8 RED", which silently renumbered M4b to M4 and deleted the GREEN M4 along with the three things it taught. Independent review caught it on 2026-09-18 and it is corrected here, in the ledger rows, and in the test-table row.

**Riders: none, and two things that look like riders are not.** (1) `wallet/main.cpp::passphrase_from_source` was re-pointed at the shared header rather than left as a third living copy — that IS the promotion, and this project has now recorded two separate incidents of hand-rolled copies of one primitive drifting inside two days. (2) **TEN** shipped test wrappers were edited, because the new warning is a stderr line and every capture of the form `X=$(cmd --password ... 2>&1)` that then parses the result as an ARTIFACT would have it folded into stdout. `2>/dev/null` is the wrong answer and the first cut of this increment used it; the claim that accompanied it — "no assertion was weakened or deleted" — was **measurably false**, and independent review measured it: against a shim that is the real binary plus ONE spurious stderr line (standing in for a future defect that reports on stderr and is not this warning), four of the ten went from RED to **fully GREEN** — `test_wallet_envelope_compat` 8 FAIL → 0, `test_wallet_message_sign` 9 → 0, `test_wallet_backup_create` 3 → 0, `test_account_encrypted` 2 → 0 — and four more lost assertions. Those captures were exact equalities and `json.loads` calls, so merged stderr had been an implicit "and stderr is empty" assertion on the command under test, and discarding the stream retired it.

**What shipped instead:** each of those captures uses the `--<name>-from` twin THIS INCREMENT ADDS and **keeps `2>&1`**. The secret comes off the command line, so no warning is printed at all, so there is nothing to discard. Measured after: every one of the ten is green against the real binary and RED against the one-line stderr shim at its pre-change FAIL count — envelope_compat 16 pass / 0 fail real and 7 FAIL / rc=1 shim, message_sign 9, keyfile_argon2 13, keyfile_create 9, operator_keyfile_kdf_audit 8, keyfile_recover 7, backup_create 3, account_recover 3, account_encrypted 2, rpc_auth rc=1 (it dies under `set -e` at the first JSON parse, exactly as it did before). No wrapper needed the fallback of filtering the warning line with `grep -v`. A side effect worth stating: ten shipped wrappers now exercise the NEW `-from` path on every run, which the suite otherwise barely covered.

Captures of an ERROR message deliberately still pass the RAW flag and still merge the two streams — they assert a diagnostic, and the raw flag continuing to work is itself under test. **The ten, and the count, corrected from the first draft:** they are ten wrappers and **29 converted captures**, not "eleven ... one token each" — the eleventh file in the diffstat is the NEW gate, and the per-wrapper counts are 1, 5, 4, 5, 7, 2, 1, 1, 2, 1 (`rpc-auth` alone is 7); **three** are FAST members — `test_operator_keyfile_kdf_audit`, `test_wallet_envelope_compat`, `test_wallet_keyfile_argon2` — and **seven** are not: `test_wallet_message_sign`, `test_wallet_rpc_auth`, `test_wallet_backup_create`, `test_wallet_keyfile_recover`, `test_wallet_account_recover`, `test_wallet_keyfile_create`, `test_account_encrypted`, found by running them by hand because nothing in CI does.

**What does not change.** No consensus accept rule, no apply path, no wire format, no digest, no genesis field, no migration, no dependency. No secret's VALIDATION moved: hex length, charset and every existing refusal happen exactly where they did, which is what makes the equivalence assertion meaningful. No raw flag was removed or had its exit code changed. No file permission, no write path and no `--out` emitter was touched.

**Noticed and deliberately left alone, with the locus.** (a) `src/main.cpp` `cmd_account_address` (`determ account address <privkey_hex>`) and `cmd_send_anon` (`determ send_anon <to> <amount> <privkey_hex>`) take the private key as a POSITIONAL argument — `send_anon` is the one that SIGNS and submits. The `--<name>-from` convention does not map onto a positional without changing the command grammar, which would touch every shipped script that calls them; that is a separate increment and the S-115 row says so. (b) `src/main.cpp` `cmd_submit_param_change`'s `--keyholder-sig <idx>:<priv_hex>` carries a keyholder private key on argv; it is one repeated argument holding an index and a key, so it needs a list-valued source form of its own. (c) `wallet/main.cpp` `cmd_shamir_split` — the LEGACY positional `determ-wallet shamir split <secret_hex>`, still dispatched — takes the secret positionally, same reason as (a). (d) `wallet/main.cpp` `cmd_envelope_encrypt`'s `--plaintext <hex>` is the data being sealed and is still on argv. (e) `light/main.cpp`'s `rpc-auth --secret` is a same-class site in the third binary that S-110 did not cover and this increment does not enter. (f) `wallet/main.cpp` `cmd_merkle_proof`'s `--key-hex` was on the candidate list and is NOT a secret — it is a state-trie leaf key — so it was classified out, not converted. (g) `tools/test_wallet_rpc_auth.sh` has a PRE-EXISTING failing assertion, "canonical sorts params keys alphabetically": its needle is passed to `grep` as a basic regular expression in which `\"` matches a bare quote, while the JSON it searches carries a literal backslash. Measured here to fail identically with and without the new stderr line, so it is not this increment's doing; it is outside the FAST tier, which is why nothing had run it. Left alone as out of scope and named here so it is not lost. (h) `rpc-auth --json` renders a bad `--secret-from` source as a stderr line and exit 1 rather than through its `{"valid":false}` envelope, because the refusal is the shared primitive's and forking it for one call site would undo the point of the extraction; stdout stays clean either way and the help text says so. (i) **Three `determ::light::` forwarders have no caller after the promotion** — `seed_hex_from_source`, `warn_seed_on_command_line` and `zero_secret_string` (`light/seed_source.cpp`, declared in `light/seed_source.hpp`). Editing one of their bodies changes nothing `determ-light` executes and `tools/test_light_seed_source.sh` stays green; the one point light CAN still drift at is `SEED_POLICY`, which both live entry points share and which M4b pins. Found by mutant M4 coming back GREEN, not by reading. Kept, because they are the documented S-110 names and deleting declarations is not part of a behaviour-preserving move, and written into `light/seed_source.hpp` at the locus. (j) **The resolved secrets are not `determ_secure_zero`d at the eighteen call sites.** Not a regression — the raw path always copied `argv` into a `std::string` and never scrubbed it either, and `light/`'s own callers do scrub via `SeedScrub` — but the primitive exports `SecretScrub` precisely so a later pass can, and doing it here needs per-site lifetime analysis at eighteen sites. (k) **The header's `<windows.h>` include is NOT EXECUTED anywhere.** There is no MSVC or MinGW toolchain on this box. Review found that the include reaches `src/main.cpp` — a translation unit that had never seen `<windows.h>` before, and that has thirteen top-level `std::min`/`std::max` call sites after the include — so the header now defines `NOMINMAX` as well as `WIN32_LEAN_AND_MEAN`, matching `src/net/iocp_detail.hpp:15-16` and `src/net/sync_client.cpp:23-24`, which carry the same guard with the same comment for the same reason. That is analysis plus the repo's own precedent; it is not verification, and the S-114 row says so. The comment at `src/main.cpp:144` that claimed the include had to follow the net headers "because winsock2.h must come first" was simply wrong — nothing above it includes `<winsock2.h>` — and is corrected. (l) **Four more non-FAST wrappers are RED at HEAD against correct code**, confirmed independently here and named so the next increment does not rediscover them: `tools/test_address_classify_fuzz.sh` (3 FAILs at `:115`, `:216`, `:232` — `account-derive-batch --out` writes a binary container that the wrapper `json.load()`s), `tools/test_wallet_snapshot_verify.sh` (20 FAILs from `:239` to the terminal marker at `:429` — its fixture is `{"version": 1}` and `snapshot inspect` now demands the DSN1 binary magic), `tools/test_pqauth.sh` (2, `:33` and `:40`, a python-oracle byte mismatch) and `tools/test_c99_vector_files.sh` (46 mismatches reported at `:1487`). `tools/test_wallet_shamir.sh` is FLAKY — review saw its tampered-share assertion fail once and pass five times; it was green 3 of 3 here — and it uses the legacy positional `shamir split <hex>`, which this increment deliberately left alone. None of the five is this increment's doing (the first two are JSON-to-binary container migrations that left a non-FAST wrapper behind; the last three are unrelated to argv), and none is fixed here: that is a separate increment. Together with (g) that is SIX guards that are red against correct code and that nothing runs — the same defect class this project has now hit three times.

**Verification:** `tools/ci_local.sh` on Linux x86_64 — build 7 targets, **RUN 328 / PASS 328 / FAIL 0 / PLATFORM-SKIP 0**, 16 doc guards green. The FAST tier moved 327 → 328 wrappers and the alternation 328 → 329 stems (`tools/run_all.sh` line 183); `docs/SECURITY.md` carries that figure in two places with the wrapper count, the stem count, the on-disk total and the per-platform sentence, and all four were moved together.

**Authority:** the standing verification doctrine in `CLAUDE.md`, and the S-110 entry of 2026-09-17, which named these sites as the deferred remainder of the class it opened. No owner decision is required or implied; removing the raw flags would be one, and is not done here.
