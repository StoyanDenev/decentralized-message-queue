...
## 11. Amendment 2026-07-20 — DSSO assertion re-engineering withdrawn; design realigned to the paper

**Authority: Stoyan Denev, 2026-07-20 (`DECISION-LOG.md` 2026-07-20; `v2.25-DSSO-DAPP-SPEC.md`).** §10 (2026-07-15) described DSSO as "DLT-B" with a block-anchored DAPP_CALL assertion whose truth reduced to a user credential signature. That assertion mechanism was AI drift — a Determ-specific re-engineering, not the design of *Identity provider in an environment of mutual distrust* (academia.edu/80188125). Owner decision: keep the paper's design, improved only by OPAQUE (for SRP) and a t-of-n, unordered threshold OPRF (for the sequential all-node chain); the RP token is the paper's hash challenge-response, not a signature. Consequence for this NOTICE: FROST is not merely removed but **structurally unnecessary** — the paper co-signs nothing — so nothing in DSSO composes with FROST, and the `v2.26-ROTATION-SPEC.md` KR-5 "cross-epoch DKG guard" is marked N/A (no DKG ceremony exists). The §3 re-introduction bars stay fully in force. The "DLT-A/-B" labels are retired.
