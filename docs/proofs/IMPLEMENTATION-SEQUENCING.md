...
**Bundle 1 v2.10 FROST REMOVED 2026-06-07.** Per `FROST_DEVIATION_NOTICE.md` and `DECISION-LOG.md 2026-06-07`, FROST was identified as a Claude-introduced design deviation, not part of Stoyan Denev's original Determ design. Re-examination found FROST not load-bearing for any v1.1 property (commit-reveal already provides unbiasable block randomness; K individual Ed25519 sigs already provide block authentication; DSSO works via the threshold-OPAQUE composition — `v2.25-DSSO-DAPP-SPEC.md`, DECISION-LOG 2026-07-15 — using primitives already shipped). **Bundle 1 v2.10 FROST chain-wiring is removed from substrate scope.** The FROST C99 implementation under `src/crypto/frost/` was retained as a library and then DELETED from the tree 2026-07-09 (pre-launch register B2; `FROST_DEVIATION_NOTICE.md` §8 — git history + the audit docs are the design record); it was NOT part of the v1.1 consensus path, NOT part of the v1.1-locked formal-verification surface, and any re-introduction requires Stoyan's explicit sign-off per `FROST_DEVIATION_NOTICE.md §3`.
