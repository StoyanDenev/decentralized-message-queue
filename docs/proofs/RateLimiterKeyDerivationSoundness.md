...
> **Environment note (doc-consolidation inc.4 drift-repair) — derivation substrate SUPERSEDED; IPv4 preserved, IPv6 re-verification pending.** `asio` is deleted from the tree (native `net::` IOCP/epoll backends behind `net::Transport` — see `MinixTacticalProfile.md`), so `addr_str` no longer comes from `asio::ip::address::to_string()`. The endpoint string is now produced by `net::Transport::remote_endpoint()` (`include/determ/net/transport.hpp:54`; `Peer::address()` reads it at `src/net/peer.cpp:10`), whose backends format via `inet_ntop` (`src/net/reactor_transport.cpp:30-34`, `src/net/iocp_transport.cpp:61-65`). **IPv4:** `inet_ntop(AF_INET, …)` yields the same canonical dotted-quad, so T-1's IPv4 per-host soundness carries over unchanged. **IPv6:** the §1.3 RFC 5952 form and the zone-id (`%eth0`) handling that T-4 (K3 hextet manipulation) and the §3.3 strip analysis reason about are `asio::to_string()`-specific; the native `format_endpoint` shown is `inet_ntop`-based and, *as written, has only a `sockaddr_in`/`AF_INET` path* (no IPv6 branch). **The IPv6 results (T-4 and the §3.3 canonicalization/strip claims) are therefore re-verification pending** — they must be re-checked against the native `remote_endpoint()` string form (its RFC 5952 conformance, and whether an IPv6 endpoint is formatted at all) before they can be relied on. The analysis below is retained as the finding's original context under the asio environment.
