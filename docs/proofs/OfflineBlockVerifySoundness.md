...
- `cmd_committee_signature_verify` (`wallet/main.cpp:8623`) — the SIGS gate: per-signer `crypto_sign_verify_detached` over `digest_bytes`, sentinel-zero abstention handling, and the committee-anchored `required = (2*|pubkey_of|+2)/3` quorum (D2-hardened 2026-07-27; see §0).
...
3. **SIGS** — attempted **only if BOTH `--committee` AND `--block-digest` are supplied, else SKIP**: BFT-style `⌈2K/3⌉`-quorum Ed25519 committee-signature verification over the **OPERATOR-SUPPLIED** `block_digest` via libsodium, where `K` is the operator-supplied committee size. *(Reuses `cmd_committee_signature_verify` verbatim — there is no `--bft` switch; the quorum is the component's committee-anchored `⌈2K/3⌉`, D2-hardened from the old attacker-loosenable `⌈2·present/3⌉`.)*
...
- **(D2) The quorum — RESOLVED, then HARDENED (WCSV-QUORUM, 2026-07-27).** The shipped `cmd_block_verify` reuses `cmd_committee_signature_verify` verbatim, so its quorum is the component's. That component computed `required = ⌈2·present/3⌉` over `present_count` = the number of NON-sentinel signatures the **untrusted block carries** — and *that denominator is attacker-controllable*: padding K−1 creator slots with the all-zero sentinel (counted as abstention, not failure) collapses `present_count` to 1, so `required = ⌈2/3⌉ = 1` and a **single** valid committee signature "meets quorum" — a 1-of-K downgrade the daemon (`required_block_sigs` over the committee size) always rejects. This was a real **false-PASS** gap: the §3.3 fail-closed claim below silently assumed the threshold could only be *stricter* than needed, but a present-derived denominator makes it *looser*. **Fix:** the denominator is now the OPERATOR-SUPPLIED committee size `K = |pubkey_of|`, so `required = ⌈2K/3⌉` over the committee — the attacker can neither pad sentinels nor shrink `creators[]` to lower it. (Still the lean BFT-style `⌈2/3⌉` check; supplying the full K for a BFT round is fail-SAFE — it can only demand MORE sigs.) Gate: `tools/test_wallet_committee_quorum.sh` → `selftest-committee-quorum` (falsify-on-mutant: reverting to `present_count` re-passes the 1-of-3 sentinel-padded block). BV-2 / BV-3 below are stated over the corrected `⌈2K/3⌉` quorum.

Neither D1 nor D2 affects the *cryptographic* soundness of the underlying checks (BV-1, BV-2); they fixed which field STRUCTURE reads (D1) and which committee-anchored quorum SIGS enforces (D2). With the D2 hardening, §3.3's fail-closed argument now holds as stated: the threshold `⌈2K/3⌉` is over the operator's committee size (attacker-immutable), so it can only be *stricter* than the round's need — a false FAIL, never a false PASS. (Before the fix this claim did NOT hold, since `present_count` let the adversary drive the threshold down to 1.)
...
sig_valid = (rc == 0);
...
size_t committee_k = pubkey_of.size();                 // OPERATOR-supplied committee size
size_t required = (2 * committee_k + 2) / 3;           // ceil(2K/3) over the committee — NOT
bool pass = (committee_k > 0) && (valid_count >= required);  // over the block-carried present_count
```
...
**Statement.** Under A1, a SIGS PASS implies there exist at least `required` valid Ed25519 signatures, by distinct committee members whose public keys are in the supplied `--committee`, **over the operator-supplied digest `d`** — where `required = ⌈2K/3⌉` over the OPERATOR-SUPPLIED committee size `K = |pubkey_of|` (the D2-hardened, committee-anchored quorum; NOT the old attacker-loosenable `⌈2·present/3⌉`; see §0 D2). **This soundness is conditional on `d` being the block's true digest.** The wallet cannot recompute the true digest (no chain-library link ⇒ no `compute_block_digest`), so if the operator supplies a wrong `d`, SIGS verifies signatures over the wrong message and the verdict is **vacuous** — it neither implies nor denies that the block carries valid signatures over its *real* digest.
...
The reused `cmd_committee_signature_verify` computes `required = ⌈2K/3⌉` over the operator-supplied committee size `K` (D2-hardened 2026-07-27) — it has no full-K-of-K MUTUAL_DISTRUST branch and no `--bft` switch. The §0 SPEC's original two-mode ask (full K-of-K *without* `--bft`, `⌈2K/3⌉` *with*) was RESOLVED to option (b): the composite inherits the component's committee-anchored `⌈2K/3⌉`. **RESOLVED — plus a correction:** the pre-hardening `⌈2·present_count/3⌉` denominator was NOT merely "stricter than intended" — it was attacker-*loosenable* (sentinel-padding drove it to 1), a genuine false-PASS that the fail-closed argument had missed; anchoring to `K` (attacker-immutable) is what makes BV-3's "stricter can only false-FAIL" hold. An operator supplying the full K for a BFT round gets a fail-SAFE stricter threshold.
