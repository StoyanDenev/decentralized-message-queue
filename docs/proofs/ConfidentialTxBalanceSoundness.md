...
- **Timing side channels** — CT-hardening of the provers is now SHIPPED (2026-07-06): the modexp/MSM no longer branch on secret scalars, so both provers are constant-time for honest inputs (NC-3, L-4). This document's claims remain functional (not a timing proof).
...
- **NC-3 — Constant-time hardening SHIPPED (2026-07-06); this document still asserts only functional soundness.** The group exponentiation (`modexp_c`, now a branchless fixed-window square-and-multiply) and the multi-exponentiation (the zero-scalar skips removed — P-256 via the pt-domain `determ_p256_msm_ct`, `Z_p*` by always exponentiating) no longer branch on secret scalars; combined with the already-CT P-256 scalar-mult ladder and `scalar_mul_mod_n` / the byte-loop mod-`n`/`q` scalar ops, **both provers are now constant-time for their own honest inputs** (the only residual branches are `scalar >= order` / point-decode validity rejects, which never fire for a prover's own honest scalars). Byte-output-invariant + independently audited; see `ConstantTimeInventory.md` §2.10 and `CRYPTO-C99-SPEC.md` §3.20. This soundness document's claims remain **functional** (completeness/soundness/hiding); the timing posture is now hardened rather than merely deferred (L-4).
...
- **L-4 — Timing not formally proven here (but hardened).** The provers' modexp/MSM were CT-hardened 2026-07-06 (NC-3) so they no longer branch on secret scalars; this document nonetheless asserts only functional soundness, not a machine-checked timing proof (the empirical `ct-timing-probe` measurement is the separate operator step — CRYPTO-C99-SPEC §3.12 / [`ConstantTimeInventory.md`](ConstantTimeInventory.md)).
...
- **Non-claims (NC-1..NC-4).** Amount privacy only (not identity/graph/fee/existence); not a consensus/wallet primitive (owner-gated integration); CT hardening of the provers is SHIPPED (2026-07-06) but this doc still proves only functional soundness, not timing; not post-quantum.
