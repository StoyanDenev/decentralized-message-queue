...
| 2 | Deterministic Simulation Framework (DSF) | 🔥 spec resolved (`docs/proofs/DSF-SPEC.md`) + **increments 1-14 shipped** (self-contained `sim/` core + `determ-dsf` + 83 baked scenarios incl. the §Q5 generator's NINE templates (broadcast + agreement + ratchet + quorum + conservation + reconciliation + crash/recover + partition/heal + rotation-fairness — every simulator fault dimension AND every recorded §Q7 candidate family covered) + the inc-5 §Q5/§Q6 `--generate N --seed S [--gen-seed G] [--template broadcast\|agree\|ratchet\|quorum\|conserve\|recon\|crashrec\|partition\|rotation]` CLI for reproducible `gen_run_NN` variants (inc-13's `--gen-seed` pins profiles while `--seed` sweeps realizations) + the `operator_dsf_sweep.sh` overnight sweep runner, `tools/test_dsf_{core,inc2,inc3,inc4,inc5,inc6,inc7,inc8,inc9,inc10,inc11,inc12,inc13,inc14}.sh`); §Q1/§Q2 consensus-injection + FA4 wiring pending |
...
`docs/proofs/DSF-SPEC.md` **and its framework has shipped (increments 1-14:
the self-contained `sim/` core + 83 baked scenarios incl. the §Q5 generator's
nine templates (broadcast + agreement + ratchet + quorum + conservation + reconciliation + crash/recover + partition/heal + rotation-fairness) + the inc-5 §Q5/§Q6 `--generate N --seed S
[--gen-seed G] [--template]` reproducible-variant CLI).** It is the canonical
extension for trace-level properties. NOTE: the increment-1-14 DSF scenarios run
