...
**Gate (executed).** This validator re-derivation is now enforced by `determ test-abort-cert-validation` (`tools/test_abort_cert_validation.sh`, FAST both platforms — register claim **T-3**, `ProofClaimGateTraceability.md` §3m). Before it, no test drove `check_delay`: mutating either compare (`validator.cpp:446` `delay_output` or `:433` `delay_seed`) to `if (false)` passed the whole suite. The gate adds a public const seam `BlockValidator::check_delay_for_test(const Block&)` (same forwarder pattern as `check_transactions_for_test`) and drives it on the subcommand's canonical commit-reveal block: a positive control (the honest `delay_seed`/`delay_output` pair clears `check_delay` — observable *only* via the seam, since the hand-built block would not survive the later block-sig/digest gates of full `validate()`), plus two negative legs (a one-byte-tampered `delay_output` → `"delay_output mismatch"`, a tampered `delay_seed` → `"delay_seed mismatch"`). *Falsify-on-mutant (both compares, each independently):* `:446 → if(false)` flips only the delay_output leg RED; `:433 → if(false)` flips only the delay_seed leg RED (the tampered seed falls through to `:446`, which emits `"delay_output mismatch"` — not the seed-specific string — so the substring-keyed leg goes red). The control and the other leg stay green in each case.

**Gate (executed) — the third derived value, `cumulative_rand`.** The claim above covers *all three* derived values, but `check_delay` re-derives only `delay_seed`/`delay_output`; `cumulative_rand` is re-derived and rejected by a *separate* accept-rule, `BlockValidator::check_cumulative_rand` (`validator.cpp:528-536`: `expected = SHA256(prev_cumulative_rand ‖ delay_output); if (expected != b.cumulative_rand) return {false, "cumulative_rand incorrect"}`), wired unconditionally into `validate()` as gate #10 — *after* `check_block_sigs` (#9). Until this leg it had **no** falsify-on-mutant coverage: because gate #10 sits past the block-sig gate, the hand-built commit-reveal block dies before reaching it, so mutating the compare (`:535 → if(false)`, i.e. *accept any beacon value*) passed the whole suite green — a latent accept-widening on the value that feeds committee selection (`resolve_epoch_rand → epoch_committee_seed`), the block lottery (`first8(cumulative_rand) % M`), and `derive_delay` activation. The gate now adds a second const seam `check_cumulative_rand_for_test(const Block&, const Chain&)` (same forwarder pattern) and drives it in `test-abort-cert-validation`: a positive control (a block whose `cumulative_rand` is canonicalized against the genesis-head `prev_rand` clears the gate — observable only via the seam) plus one negative leg (a one-byte-tampered `cumulative_rand` → `"cumulative_rand incorrect"`). *Falsify-on-mutant:* `:535 → if(false)` returns `{true,""}` for the tampered value, dropping the substring and flipping the negative leg RED while the positive control stays green. This closes the T-3 coverage gap for the third derived value; it changes no consensus behavior (the check already shipped and is correct).
...
*Signing happens strictly inside the gated region.* `start_block_sig_phase` (`node.cpp:992`) is the sole producer of this node's `BlockSigMsg`: it builds the tentative block from the `σ`-ordered contribs (re-looking-up each `pending_contribs_[d]` and `return`ing if absent, `node.cpp:1000-1005`), computes the digest, calls `make_block_sig`, stores it, and broadcasts (`node.cpp:1045-1053`). It is reached only via `enter_block_sig_phase` (`node.cpp:935-938`, inside a `net::EventLoop::post` — `asio::post` in earlier revisions; `asio` is deleted from the tree). There is no other call path that emits a `BlockSigMsg` for a fresh round. Hence the Phase-2 signature is emitted strictly after the K-complete gate.

*The digest binds the sealed set.* The digest signed in `start_block_sig_phase` is `compute_block_digest(tentative)` over the K `σ`-ordered contribs; `compute_block_digest` appends every `creator_dh_inputs[i]` (`producer.cpp:620`). So the signature attests over the *complete* ordered commitment set — none missing, order fixed by `σ`.
...
| `cumulative_rand` chaining (T-3) | `producer.cpp:819-822` | `check_cumulative_rand` (`validator.cpp:528-536`), gated via `check_cumulative_rand_for_test` |
| Phase-1 unanimity at finalize | `node.cpp:1066-1073` (`try_finalize_round`) | — |
| Digest excludes secrets (T-1.1) | `producer.cpp:555-564`, `:608-693` | `validator.cpp:443` |
