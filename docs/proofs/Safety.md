...
`compute_block_digest(B)` (Preliminaries §1.3; `src/node/producer.cpp:757–899`) is `SHA256` over the ten v1-base fields `{index, prev_hash, tx_root, delay_seed, consensus_mode, bft_proposer, creators[], creator_tx_lists[][], creator_ed_sigs[], creator_dh_inputs[]}` (appended unconditionally, `producer.cpp:759–769`) **plus nine conditional dimensions**, each appended behind a presence gate so a block that does not carry the field keeps a byte-identical v1 digest: the reconciled `inbound_receipts` root (`:778`), the reconciled `equivocation_events` root (`:799`), the reconciled `abort_events` root (`:805`), `partner_subset_hash` (`:823`), the canonical `timestamp` (`:838`), `signature_form` (`:850`), `eligible_count` + `source_shard_id` (`:879–881`), and the `shard_tip_records` root (`:892`). Every one of these is likewise a field of `signing_bytes` (which covers all `Block` fields except `creator_block_sigs`), so `signing_bytes(B) = signing_bytes(B')` forces equality of the full digest-bound set; the digest then matches up to a SHA-256 collision (probability ≤ `2⁻¹²⁸` under A2). The rationale for each conditional dimension — why binding it is safe (deterministic or reconciled, never a raw gossip-async view) — is enumerated in §5.3's "Full closure" note.   ∎
...
- **"≤ 1 block instance per digest" (S-030 D2).** T-1 says "at most one *digest* finalizes per height" — a single Hash value. It does NOT directly say "at most one *block instance* per height." The K-of-K committee signs `compute_block_digest()`, which is narrower than `Block::signing_bytes()` (it excludes Phase-2-reveal-time fields and several evidence/receipt list fields). Two block instances differing only in those excluded fields share the same digest; both pass K-of-K signature verification.
  ...
  **Validator re-derivation witness (timestamp median arm, falsify-on-mutant).** The consensus-layer timestamp binding above is load-bearing only if the *validator* actually re-derives and enforces the lower-median. `BlockValidator::check_timestamp` (`validator.cpp:1761-1768`) does: on a reconciled block (non-empty `creator_proposer_times`) it rejects a size mismatch (`:1762`), a zero committed proposer time (`:1764`), and `b.timestamp != reconcile_median_time(creator_proposer_times)` (`:1766`); legacy blocks (empty vector) fall through to the ±30s wall-clock bound only. Before the round-3 proof-claim audit (`wf_97a30e14`, the same class as the `check_cumulative_rand` gap closed in `1b53344`) this median arm was driven by **no** test — the sole gate hitting the `check_timestamp_for_test` seam used an *empty* `creator_proposer_times`, exercising only the ±30s bound, so the mutant `validator.cpp:1766 → if(false)` (accept any timestamp on a reconciled block, letting a producer grind the digest-bound timestamp) passed the whole suite green. `determ test-block-timestamp` (`tools/test_block_timestamp.sh`) now drives the seam with a non-empty `creator_proposer_times`: a positive control (`timestamp == lower-median`, with a `VirtualClock` pinned to the median so it also clears the ±30s bound) plus three negative legs — one per reject arm — each asserting its substring. The clock is pinned to the median so the median arm, not the ±30s bound, is the sole rejecter of the `timestamp != median` leg (which flips RED under the mutant).
...
- **T-L4 (balance/nonce trust via state-proof composition).** Composes with **L-1.3 (state-root inclusion semantics — apply-time gate)** and the FA-Apply suite (chain apply-determinism). T-L4 chains T-L1 + T-L2 + T-L3 with the S-042 committee-binding (`committee_bound_state_root`, `light/trustless_read.cpp:335-437`) covering the race window — the gap between head-anchor time `vc.height` and state-proof time `proof_height`. The mitigation's load-bearing step is a stale-height gate (`light/trustless_read.cpp:528-533`) followed by binding `anchor_index = proof_height − 1` (`:545`): the helper recomputes the full anchor block's `block_hash` and requires the committee-signed **successor**'s `prev_hash` to equal it (`:424-432`), transitively committing the anchor's `state_root` forward. The anchor header's own digest excludes `state_root`, so signing it does not bind it — only the successor's signature does (the head fails closed with no signed successor unless `--wait` polls). The S-042 binding is the light-client-side analog of L-1.3 — both bind the state-root commitment to the canonical chain at the apply boundary. (Standalone temporal proof: `StateProofRaceWindowSoundness.md` PRW-1..PRW-5.)
...
- A stale or forked state-proof (different state_root than what the committee signed at the head) is caught by T-L4 (the S-042 stale-height gate + `committee_bound_state_root` successor `prev_hash` binding).
...
- `light/trustless_read.cpp::read_account_trustless` (lines 439-599) — the composite trustless-read flow; the S-042 committee-binding cited by T-L4 lives in `committee_bound_state_root` (lines 335-437), called at lines 546-548 with the stale-height gate at lines 528-533 and the attested-vs-proof_root check at lines 549-555; the cleartext-vs-value_hash cross-check binding the daemon's `account` reply lives at lines 573-593.
