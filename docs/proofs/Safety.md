...
- **"≤ 1 block instance per digest" (S-030 D2).** T-1 says "at most one *digest* finalizes per height" — a single Hash value. It does NOT directly say "at most one *block instance* per height." The K-of-K committee signs `compute_block_digest()`, which is narrower than `Block::signing_bytes()` (it excludes Phase-2-reveal-time fields and several evidence/receipt list fields). Two block instances differing only in those excluded fields share the same digest; both pass K-of-K signature verification.
  ...
  **Full closure (v2.7 F2 view reconciliation — pool-fed dimensions + `partner_subset_hash` SHIPPED; only `timestamp` pending).** Extends `compute_block_digest()` to cover the ✗-row fields directly, via Phase-1 view reconciliation with **per-field heterogeneous rules**: union (with per-event V11 verifiability) for `equivocation_events`, union (with per-event V10 verifiability) for `abort_events`, intersection for `inbound_receipts` — all three SHIPPED (commits `a727cb2` / `48c4b45`); deterministic derivation for `cross_shard_receipts` (pinned via `tx_root`) and `partner_subset_hash` (bound directly when non-zero, commit `8585a50` — deterministic from merge state, no reconciliation needed); and the assembler-proposes-members-bound-check for `timestamp` (the one piece still pending, §5 / F2-SPEC §6.3). For the shipped dimensions, K-of-K signatures gather only over the reconciled canonical view → divergent instances cannot both be K-of-K-signed → literal "≤ 1 instance per height" enforced at the consensus layer. Full analysis (including comparison of the two closure paths): `S030-D2-Analysis.md` §3.5. Implementation specification (all 9 historical open design questions formally resolved; pending pre-implementation review of the 5 decisions in §6): `F2-SPEC.md`. Estimated cost from spec-review acceptance: ~3-4 days.
