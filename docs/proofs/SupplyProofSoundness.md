...
> **Implementation status (R41): spec-level — command deferred.** The `determ-light supply-trustless` command was implemented and then **reverted** this round: the five A1 counters increment **every block** (subsidy), and the daemon's `state_proof` RPC returns only the leaf `value_hash` (never the raw value — `src/node/node.cpp::rpc_state_proof`), so the client must cross-check against a separately-fetched `chain_summary` cleartext. Because the light client performs many sequential RPCs (genesis anchor → chain walk → per-counter proofs) before reading the cleartext, the head advances and the per-block counters systematically mismatch — the command reports `TAMPERED` against an *honest* daemon. The stake (`s:`) and balance (`a:`) reads do not hit this because those values are stable across blocks. A correct implementation needs daemon-side support — either a height-pinned counter read (`chain_summary --at-height H`) or the `state_proof` RPC returning the raw value bytes alongside `value_hash` so cleartext and proof are atomic at one height. That is a real v2.x design item. **The cryptographic soundness below (SU-1..SU-4) is unaffected and stands as the spec for that future implementation** — only the cleartext-sourcing race, an availability/consistency concern orthogonal to the soundness argument, is deferred. (Precedent: `StateRootAnchorSoundness.md` shipped spec-level before its `verify-state-root` command.)
...
The `determ-light supply-trustless` composite command, which (mirroring `read_account_trustless` at `light/trustless_read.cpp:188-350` and the `s:` sibling `read_stake_trustless`) executes, for each of the five supply counters:
...
4. **Merkle inclusion verify** — `verify_state_proof(proof, root)` (`light/verify.cpp:285-349`), delegating to `crypto::merkle_verify` (`src/crypto/merkle.cpp:113-141`). (T-L3 / SU-2.)
5. **Race-window anchor of the proof root to a committee-signed header** — the three-branch `proof_height < / == / > vc.height` dispatch (`light/trustless_read.cpp:226-307`). (T-L4 §4.4.1 / SU-1.)
...
This is exactly the chain of bindings `StateRootAnchorSoundness.md` SR-1 establishes, applied here to the height the `c:` proofs are anchored at. Suppose the invocation anchors `R_A ≠ state_root_T(h)`. Two exhaustive cases (the SR-1 case split, reproduced for the `c:`-proof anchoring at `trustless_read.cpp:269-285`):
...
**Proof.** This is the direct application of `MerkleTreeSoundness.md` **MT-4** (inclusion-proof soundness) at the `k:c:` leaf, equivalently `LightClientThreatModel.md` T-L3 specialized to the supply-counter namespace and structurally identical to `StakeProofSoundness.md` SP-2. `verify_state_proof` delegates to `crypto::merkle_verify(R, key_bytes, value_hash_A, target_index, n, sibs)` (`light/verify.cpp:333-335`), which recomputes the leaf hash `c_0 = \text{merkle\_leaf\_hash}(\text{key}_c(\text{NAME}), \text{value\_hash}_A) = H(\texttt{0x00} \| u32\_be(|\text{key}_c(\text{NAME})|) \| \text{key}_c(\text{NAME}) \| \text{value\_hash}_A)` and walks up the sibling chain (`merkle.cpp:129-138`), accepting iff the exact-consume gate (`proof_idx == proof.size()`) holds **and** the recomputed root equals `R`.
...
3. *Therefore the light client's recompute equals the chain's.* Composing (1) + (2): the five values the light client reads are the genuine post-apply counters, and plugging them into the same closed form yields the same `expected_total` the daemon's honest chain holds at `h`. A daemon that wishes to mislead the operator about supply must therefore either (i) tamper an individual counter leaf — caught by SU-2 (the tampered value-hash fails root-equality), or (ii) anchor different counters to different roots — caught by the single-root precondition (§4.4: at most one root matches the committee-verified head, so the other proofs throw at `trustless_read.cpp:269-275`), or (iii) present a wholly different but still committee-signed state at a different height — caught by SU-1's SR-2/SR-3 genesis/height binding. In every case the attack reduces to assumption A1 or A2.

**What SU-3 does *not* give (the leaf_count caller-trust boundary).** SU-3's recompute checks the *identity among the five committed counters*. It does **not**, by itself, confirm that the committed counters match the *sum over the account/stake leaves* (`live_total_supply()`), because the light client does not read every `a:`/`s:` leaf — and even if it wished to, it could not enumerate them without trusting the daemon's `leaf_count` (the **S-040** caller-trust limitation; `MerkleTreeSoundness.md` §6.2, SU-4 below). The full apply-layer guarantee `live_total_supply() == expected_total()` is enforced *on every full node* at apply tail (`chain.cpp:1397-1419`) and is the property the *chain* maintains; SU-3 lets the light client confirm the *counter half* of that identity is internally consistent and committee-committed, which is the trust-minimized read's deliverable. The **S-040 leaf_count caller-trust boundary** is therefore where SU-3's guarantee stops: the light client trusts that the daemon's `leaf_count` (used to verify each proof's tree shape) is genuine, sourced from the same committee-anchored envelope (SU-4); it does not independently enumerate the account set to re-derive `live_total_supply()`.
...
### 4.4 Theorem SU-4 (`leaf_count` caller-trust boundary; single-envelope + single-root sourcing)

**Statement.** SU-2's soundness is stated with `n` = the *genuine* leaf count of the tree `R` commits to, and SU-3's cross-counter check requires all five proofs to anchor to a *single* `R`. Because `merkle_verify` does **not** bind `leaf_count` into any leaf or inner hash (the **S-040** caller-trust invariant; `MerkleTreeSoundness.md` §6.2), the soundness of the `c:` read additionally requires that the light client source `leaf_count` from the *same committee-anchored origin* as `state_root`, and anchor every counter proof to the one committee-verified head. The shipped `supply-trustless` flow discharges both obligations: per counter, `state_root` and `leaf_count` arrive in one `rpc_state_proof` reply, that reply's `state_root` is committee-anchored by SU-1 in the same invocation, and all five proofs are checked against the same `vc.head_state_root` (or the same `proof_height`-anchored header). Hence SU-4 holds operationally and both the split-source and split-root attacks are unreachable.

**Why the obligation exists.** `merkle_verify` consumes `leaf_count` only to drive the number of levels and the per-level duplication parity (`merkle.cpp:130-138`); it is not an input to `merkle_leaf_hash` or `merkle_inner_hash`. Two distinct `(target_index, leaf_count)` pairs that yield the *same walk shape* verify identically — the concrete failure pinned by `determ test-merkle-proof-tampering` scenario #12 (claiming `leaf_count = 8` for a genuinely-5-leaf tree at index 2 verifies as `true`). An attacker who could feed the verifier a *wrong* `leaf_count` decoupled from the trusted root could, in principle, make a proof for one tree shape validate against a root of a different shape.

**Why the shipped flow is safe.** The danger is exclusively the *split-source* case (proof + `leaf_count` from one untrusted channel, `state_root` from a different trusted channel) and, for the five-counter read specifically, the *split-root* case (five proofs anchored to five different roots). In `supply-trustless`:
...
2. The light client anchors *that reply's* `state_root` to a committee-signed header (SU-1) in the same invocation, then verifies the `c:` proof against the anchored root using *that reply's* `leaf_count` (`verify.cpp:308-335`). There is no second channel; the `(state_root, leaf_count)` pair is consistent-by-construction because the daemon computed both from one locked state snapshot, and the anchoring step rejects any `state_root` the committee did not sign.
3. *Single-root precondition (supply-specific).* For SU-3's cross-counter identity to be meaningful, all five counter proofs must commit to the *same* state. The flow enforces this because each proof's `state_root` is matched against the *one* committee-verified head (`vc.head_state_root`, or the header at `proof_height − 1`): a daemon serving counter A against root `R₁` and counter B against root `R₂ ≠ R₁` can have at most one of `R₁, R₂` equal the committee-anchored head, so the other proof throws at the race-window dispatch (`trustless_read.cpp:269-275`). The light client therefore reads five counters that are all committed under the same `R`.

If a malicious daemon serves an inconsistent `(state_root, leaf_count)` — e.g., the genuine `leaf_count` but a tampered `state_root` — the tampered root fails SU-1's committee anchoring. If it serves a tampered `leaf_count` with the genuine `state_root`, then by SU-2's exact-consume + range gates the wrong walk shape generically fails root-equality against the genuine `R` for the genuine `c:` leaf; and even in the degenerate same-walk-shape coincidence of S-040, the *value* the verifier accepts is still the genuine `c:` leaf at that position under the genuine root — the daemon gains no ability to substitute a *wrong* counter value, only (at most) to mislabel which sorted slot it occupies, which the `key_c(NAME)`-binding of SU-2 still ties to `NAME`. The operational obligation is therefore met: **source `leaf_count` from the same committee-anchored envelope as `state_root`, and anchor every counter proof to the one committee-verified head** (`MerkleTreeSoundness.md` §6.2; `docs/SECURITY.md` §S-040), which the single-reply shape plus the shared-head anchoring enforce structurally.

**Status.** S-040 is a registered **Low/Op** caller-trust item, orthogonal to SU-2's soundness (which holds with the genuine `leaf_count`). The structural fix — domain-separating `leaf_count` into the leaf hash — breaks v1 wire compatibility and is tracked for a future v2.x flag-day (`MerkleTreeSoundness.md` §6.2). SU-4 records that the shipped `supply-trustless` flow does not split the source or the root and is therefore unaffected.

**Concrete-security bound.** SU-4 adds no independent term beyond SU-1 + SU-2 + SU-3 under single-envelope + single-root sourcing; a split-source or split-root integrator would expose the S-040 surface, which SU-4 directs them to avoid.   ∎
...
SU-3's recompute confirms the five committed counters satisfy `expected_total = genesis_total + Σsubsidy + Σinbound − Σslashed − Σoutbound` — the *right-hand side* of the A1 invariant. It does **not** confirm that this equals `live_total_supply()` (the sum over every `a:` balance + `s:` locked stake leaf), because the light client does not read every account/stake leaf (doing so would require enumerating the tree, which trusts the daemon's `leaf_count` — S-040, §6.3). The full equation `live_total_supply() == expected_total()` is enforced on every full node at apply tail (`chain.cpp:1397-1419`) and is the *chain*'s guarantee; the light client's trust-minimized deliverable is the committee-committed, internally-consistent counter set. An operator wanting the full live-supply cross-check runs a full node (which enforces it) or accepts the chain-level guarantee that the apply gate has never thrown on any accepted block.
...
### 6.3 S-040 leaf_count caller-trust (SU-4)

Recorded in §4.4: sound under the shipped single-envelope + single-root sourcing; a future split-source integrator must source `leaf_count` from the committee-anchored envelope and anchor every counter proof to the one committee-verified head. Low/Op; structural fix tracked for a v2.x wire-breaking flag-day.
...
|---|---|---|---|
| SU-1 | committee-anchor (race-window) in the read flow | `light/trustless_read.cpp:226-307` | Anchor each `c:` proof's `state_root` to a committee-signed header; the three-branch `proof_height < / == / >` dispatch. |
| SU-1 | `verify_block_sigs` | `light/verify.cpp:190-283` | Per-block Ed25519 K-of-K committee-sig verify over `light_compute_block_digest`. |
| SU-1 | `verify_headers` | `light/verify.cpp:104-188` | prev_hash continuity walk from genesis (SR-2 genesis-binding). |
| SU-1 | `anchor_genesis` | `light/trustless_read.cpp:52-79` | T-L1 genesis pin (`compute_genesis_hash` byte-compare). |
| SU-2 | `verify_state_proof` | `light/verify.cpp:285-349` | Parse `c:` proof JSON, delegate to `merkle_verify`; optional `--state-root` override. |
...
| SU-E | `read_account_trustless` (template for `read_supply_trustless`) | `light/trustless_read.cpp:188-350` | The composite read skeleton; `supply-trustless` differs by `namespace="c"`, repeated per counter against the same head. |
...
| `tools/test_merkle_proof_tampering.sh` + `determ test-merkle-proof-tampering` (15 scenarios) | SU-2 tamper rejection; SU-4 / S-040 `leaf_count` limitation (#12). |
...
- **Theorems.** SU-1 (committee-signed `state_root` binds the `c:` counter leaf — reduces to an assumption-A1 forgery on `h+1` or an A2 collision at `h`, via SR-1); SU-2 (`k:c:` Merkle inclusion soundness — MT-1/MT-3/MT-4 at the `k:c:` leaf, MT-2 domain separation for the composite-key binding); SU-3 (the verified counters let the light client recompute the A1 supply identity itself — consuming I-6 / T-12, noting the S-040 `leaf_count` caller-trust boundary; no `a:`/`s:` analog); SU-4 (`leaf_count` caller-trust boundary — S-040, discharged by single-envelope + single-root sourcing). Corollary SU-E composes them with T-L1 + T-L2 to the same `≤ 2⁻⁹²` end-to-end bound as the balance/stake reads (up to a `5×` Merkle-term factor for the five-leaf read).
...
- **Known limitations.** SU-3 checks the counter-identity, not the full `live_total_supply()` equation (§6.1); single-shard scope, no cross-shard aggregate (§6.2); S-040 `leaf_count` caller-trust, mitigated by single-envelope + single-root sourcing (§6.3). None undermine the per-invocation soundness claim.
...
- `light/trustless_read.cpp:188-350` — `read_account_trustless` (template for `read_supply_trustless`); race-window dispatch l.226-307.
...
- `docs/SECURITY.md §S-040` — `leaf_count` caller-trust invariant (SU-4).
