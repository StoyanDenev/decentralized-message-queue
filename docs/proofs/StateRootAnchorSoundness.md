...
| `verify_headers` | `light/verify.cpp:104-188` | prev_hash walk genesis → H (§1 step 3, `SR-2`). |
| `committee_bound_state_root` | `light/trustless_read.cpp:333-411` (decl `trustless_read.hpp:174-205`) | **The implemented §3.3/§6.4 binding:** full-block recompute via the `"block"` RPC + committee-signed successor `prev_hash` match; head fails closed (`:366-375`). All trust-minimized readers route their reported `state_root` through it. |
