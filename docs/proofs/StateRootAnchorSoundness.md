...
| `committee_bound_state_root` | `light/trustless_read.cpp:335-437` (decl `trustless_read.hpp:221-224`) | **The implemented §3.3/§6.4 binding:** full-block recompute via the `"block"` RPC + committee-signed successor `prev_hash` match; head fails closed (`:388-401`). All trust-minimized readers route their reported `state_root` through it. |
...
| `tools/test_light_state_root_binding_guard.sh` | (offline; S-042 falsify-on-mutant) | the static source-guard + `SELFTEST` regressions (R1-R6) that mechanize `committee_bound_state_root`'s binding arms — `full-block recompute` (I3e), `successor prev_hash` binding (I3b/I3d), block-fetch (I3a), the `SECURITY` throw (I3c) — each injected-and-flagged. This is the actual falsify-on-mutant witness for the binding claim. |
| `tools/test_light_state_root_binding.sh` | (offline; fail-closed witness) | asserts the reader fails closed on a dead RPC port and on a missing genesis (structural fail-closed only). ⚠ It does NOT perform the swapped-`state_root` REJECT or head-no-successor assertions previously credited here — its own header defers the `state_root`-swap forgery-catch to a **CI / WSL2 cluster leg** (skipped gracefully offline). The swap-reject coverage lives in the guard test above. |
