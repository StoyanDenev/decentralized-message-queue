...
**K-of-K assumption (locked at v1.1 under no-migrations).** The 1/K split assumes K-of-K signing — every registered committee member signs every block (the v1.1 default consensus mode per §6.2). Under v1.1, blocks are authenticated by K individual Ed25519 signatures (no aggregation; no threshold signing). The 1/K distribution rule is therefore unambiguous: each of the K signers receives exactly 1/K of the priority tip. **FROST was removed from v1.1 chain consensus path 2026-06-07** per `FROST_DEVIATION_NOTICE.md` and `DECISION-LOG.md 2026-06-07`; the prior forward-compat note about t-of-n FROST is therefore moot under v1.1 + no-migrations.
