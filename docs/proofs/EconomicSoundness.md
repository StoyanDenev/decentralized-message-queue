...
- **NEF formula choice.** E1's canonical design is now lottery distribution with optional per-block win cap (`plan.md` §E1): each first-time REGISTER draws against `cumulative_rand` for a fixed `nef_grant` with probability `1/nef_probability_denom`, capped at `nef_max_wins_per_block` wins per block. The proof of A1's unitary-supply invariant holds for any sum-preserving distribution rule. All three branches of the lottery + cap apply path are sum-preserving: (a) win-with-transfer moves `nef_grant` from pool to registrant; (b) lottery-loss is a no-op; (c) cap-hit is a no-op. None mints or burns. Earlier geometric (`pool/2`) and harmonic (`pool · 2 / (n · (n+1))`) variants are retired from the design; the shipped geometric implementation in `chain.cpp` is flagged for lottery + cap rewrite under the E-track reconciliation item.
