...
- **Slash effects on supply — now identically zero.** T-S7's A1 invariance references `accumulated_slashed`, and as of 2026-09-16 **no apply path credits it**: the equivocation forfeiture was removed by D4 (O-1 step 3a) and the Phase-1 abort deduction by D13. `block_slashed` is frozen at 0 and `accumulated_slashed_` never grows; the counter and its `c:accumulated_slashed` state-root leaf are retained for A1's ledger shape and for chains that carry a non-zero value in a restored snapshot. T-S7 is unaffected either way — it treats `accumulated_slashed` as a given counter that other apply paths populate; its preservation under T-S7 follows from FA11 T-12's induction step.
...
| `EquivocationSlashing.md` (FA6) | The equivocation evidence channel. It no longer advances `accumulated_slashed` (D4); the composition with subsidy via FA11 T-12 is the trivial one, `Δ = 0`. |
