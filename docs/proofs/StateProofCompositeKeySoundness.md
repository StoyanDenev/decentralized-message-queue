...
- **The `leaf_count` caller-trust gap.** `leaf_count` is returned but not bound into the leaf hash (S-040; `MerkleTreeSoundness.md` §6.2); mitigation is to source it from the same anchor as `state_root`. Orthogonal to reconstruction; `ReceiptInclusionProofSoundness.md` RI-4 owns it.
