...
The `i|m|p` branch of `Node::rpc_state_proof` (`src/node/node.cpp:3406–3432`) and its three obligations:
...
The daemon-side reconstruction (`node.cpp:3406–3429`):
...
3. **`crypto::merkle_verify(root, key_bytes, value_hash, target_index, leaf_count, proof)`** (`src/crypto/merkle.cpp`) — the verifier re-derives the leaf hash `merkle_leaf_hash(key_bytes, value_hash) = H(0x00 ‖ u32_be(|key|) ‖ key ‖ value_hash)` (`merkle.cpp:42–51`; recomputed inside `merkle_verify` at `merkle.cpp:140`) from the **returned `key_bytes`** and walks the sibling chain to the root. The light client calls it at `light/verify.cpp:378–380` over `key_bytes = from_hex(proof_json["key_bytes"])` (verify.cpp:349–350). This is the load-bearing fact for SP-CK-3: verification consumes `key_bytes`, not `(namespace, key)`, so soundness is decided by whether `key_bytes` rolls up to the committee-signed root, independent of who produced it.

---

## 4. Security theorems

Throughout, fix a composite namespace `ns ∈ {i,m,p}`, write `want(ns) ∈ {40, 4, 12}` for the canonical body width (`node.cpp:3417–3419`), and let `body` be the caller-supplied hex-decoded suffix. Let `recon(ns, body) := ns[0] ‖ ':' ‖ body` be the daemon's reconstructed key (`node.cpp:3426–3429`) and `bsl(·)` the corresponding `build_state_leaves` key (`chain.cpp:331–378`).
...
**Proof.** The `i|m|p` branch executes in strict order (`node.cpp:3406–3432`):
...
Then under a fully-Byzantine `A_daemon`, an honest verifier that (i) re-derives `merkle_leaf_hash(key_bytes, value_hash)` from the returned `key_bytes` (parsed at `light/verify.cpp:349–350`; the leaf hash is recomputed inside the `determ::crypto::merkle_verify` call at `light/verify.cpp:378–380`, `src/crypto/merkle.cpp:140`), (ii) checks the path against a **committee-signed** `state_root` (SR-1), and (iii) checks `key_bytes` equals the canonical key it can recompute from its own logical inputs, never accepts a proof for a key other than the one it named — **regardless of which route produced `key_bytes`**.

**Proof.** Verification (the `merkle_verify` call at `light/verify.cpp:378–380`) consumes `key_bytes` and `value_hash`, not `(namespace, key)`. So the only daemon-controlled inputs that can influence the verdict are `(key_bytes, value_hash, target_index, leaf_count, proof, state_root)`.
...
- **F-4 (head-only `state_root`).** `rpc_state_proof` returns `compute_state_root()` + `height()` at the *current* head (`node.cpp:3451–3452`); per-height historical proofs are the head-only-RPC constraint `AccountHistorySoundness.md` §6.4 documents. Orthogonal to reconstruction.
