...
### F-5 (Misbehaviour during merge: refugee evidence is recorded on the home chain)

Refugees misbehaving on T's merged block have the evidence relayed to S (their home chain) via the FA6 `EquivocationEvent` mechanism. Since D4 (2026-09-16) that evidence carries NO L1 consequence on either chain — no stake moves, no registrant is deactivated — so the *eligibility* half of the old statement is gone: a refugee equivocator stays eligible on S and the merge path removes nobody. What survives is relay + record; the economic response belongs to the L2 bond policy (D22). T-4's safety preservation never consumed the consequence (it is a signature-counting argument over the merged committee), so it composes with FA6 unchanged.
...
- F-5: Refugees misbehaving on T's merged block have their evidence relayed to S (home chain) via FA6; already shipped, and since D4 it is a record with no L1 consequence — no eligibility change on either chain.
