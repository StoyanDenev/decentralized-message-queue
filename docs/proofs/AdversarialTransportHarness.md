...
Fault model + `test-fa-partition-virtual` SHIPPED; hardened 2026-07-15 (§3.4) after the first CI run — mesh barrier, quiescence barrier, head−1 prefix witness, `maj_live` gating, 1 s phase timers, failure diagnostics. Consensus: **S-050 stall valve SHIPPED** (the absorbing round-state-fork livelock class is now non-absorbing); **S-051 pool-exhaustion halt OPEN** (owner-gated protocol change). Post-fix stability: wedge-class failures 5/30 → 0/30 across pinned-core and starved loops, both platforms. The reliable loss-liveness gate and deterministic fork reproduction remain the **virtual-TIME** follow-on. (S-048 itself is CLOSED since A4 — `Node::maybe_reorg_to_locked`; §3.1's account of the loss→fork mechanism predates that fix and stands as history.)
