...
- **Known limitations.** One-sided / no provable non-application (§6.1, RI-3); genesis-only committee map (§6.2); S-040 `leaf_count` caller-trust, mitigated by single-envelope sourcing (§6.3); `i:` served by the generic `state_proof` RPC via R-ext (§6.4 — formerly a sourcing obligation, now shipped: hex-encoded composite key body); presence-only semantics, no amount/recipient/dst-shard in the leaf (§6.5); per-chain, not cross-shard-global (§6.6). None undermine the per-invocation `APPLIED` soundness claim.
