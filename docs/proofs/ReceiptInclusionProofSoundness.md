...
2. **The `i:` namespace is NOT served by the generic `state_proof` RPC.** `Node::rpc_state_proof` (`src/node/node.cpp:3287-3336`) explicitly handles only the simple-key namespaces `a|s|r|d|b|k|c` and rejects `ns == "i"` (and `m`/`p`) with *"unsupported namespace; use a|s|r|d|b|k|c"* (`node.cpp:3294`, `:3312-3313`): the comment reads *"composite-key namespaces (i/m/p) are out of scope for this RPC."* Therefore `verify-receipt-inclusion` cannot reuse the `{{"namespace","i"},{"key",domain}}` call shape the way `stake-trustless` reuses `{{"namespace","s"},...}`. It must either (a) extend the daemon RPC to build the composite `i:`-key, or (b) construct the `i:` key bytes **client-side** and feed them to the verifier via the `key_bytes` field that `verify_state_proof` already consumes (`light/verify.cpp:304-305`). §1.4 and §6.4 make this sourcing boundary explicit; the cryptographic soundness (RI-1..RI-3) holds for whichever sourcing I3 ships, because the verifier re-derives the leaf hash from `key_bytes` and gates on root-equality regardless of how `key_bytes` arrived.
...
Its logical pipeline mirrors `read_account_trustless` (`light/trustless_read.cpp:188-350`) with the `i:` composite key:
...
2. **Header-chain walk + per-block committee-sig verify** — `verify_chain_to_head` (`light/trustless_read.cpp:81-186`), invoking `verify_headers` (`light/verify.cpp:104-188`) per page and `verify_block_sigs` (`light/verify.cpp:190-283`) per block, end-to-end from block 0. (T-L2.) This yields a committee-anchored `state_root` (SR-1).
3. **State-proof fetch for the `i:` leaf** — the composite key `"i:" + src_be8(S) + H` must be presented to the daemon's proof builder. Because `Node::rpc_state_proof` does **not** serve the `i:` namespace (`node.cpp:3294`, the i/m/p out-of-scope comment), I3 sources the proof by one of the two routes in §1.4. The reply carries `(key_bytes, value_hash, target_index, leaf_count, proof, state_root, height)`.
4. **Merkle inclusion verify** — `verify_state_proof(proof, root)` (`light/verify.cpp:285-349`), delegating to `crypto::merkle_verify` (`src/crypto/merkle.cpp:113-141`), with the verifier re-deriving the leaf hash from the supplied `key_bytes`. (T-L3 / RI-2.)
5. **Race-window anchor of the proof root to a committee-signed header** — the three-branch `proof_height < / == / > vc.height` dispatch (`light/trustless_read.cpp:226-307`). (T-L4 §4.4.1 / RI-1 / `LightClientThreatModel.md` L-5.)
...
1. **Composite-key construction + RPC-coverage gap.** The `i:` key is not an ASCII `domain` string but the 42-byte composite `"i:" + u64_be(S) + H`. Because `Node::rpc_state_proof` declines the `i:` namespace (`node.cpp:3294`, `:3312-3313`), the proof for an `i:` leaf is **not** obtainable via the existing `{{"namespace","i"},{"key",…}}` call. §1.4 / §6.4 fix the two admissible sourcings (extend the RPC, or build `key_bytes` client-side and let the verifier consume them).
...
Suppose the invocation anchors `R_A ≠ state_root_T(h)`. The SR-1 case split (`StateRootAnchorSoundness.md` §4.1, reproduced for the `i:`-proof anchoring at `trustless_read.cpp:269-285`):
...
### 4.4 Theorem RI-4 (`leaf_count` caller-trust boundary; single-envelope sourcing)

**Statement.** RI-2's soundness is stated with `n` = the *genuine* leaf count of the tree `R` commits to. Because `merkle_verify` does **not** bind `leaf_count` into any leaf or inner hash (the **S-040** caller-trust invariant; `MerkleTreeSoundness.md` §6.2), the soundness of the `i:` read additionally requires that the light client source `leaf_count` from the *same committee-anchored origin* as `state_root`. The shipped flow discharges this: under sourcing (R-ext), `state_root` and `leaf_count` arrive in one `rpc_state_proof`-shaped reply, computed atomically under the daemon's `state_mutex_` read lock (`node.cpp:3289`) over a single consistent state, and that reply's `state_root` is committee-anchored by RI-1 in the same invocation; under sourcing (R-cli), the proof envelope (sans the client-built `key_bytes`) is still one reply with a single `(state_root, leaf_count)` pair. Hence RI-4 holds operationally and the split-source attack is unreachable.
...
**Why the shipped flow is safe.** The danger is exclusively the *split-source* case: proof + `leaf_count` from one untrusted channel, `state_root` from a different (trusted) channel, with the two inconsistent. In `verify-receipt-inclusion`:

1. The proof envelope returns `state_root`, `leaf_count`, `target_index`, `value_hash`, and `proof` as fields of **one** JSON object, with `leaf_count = build_state_leaves().size()` (`chain.cpp:456`) — the genuine count for the very tree whose root is `compute_state_root()` in the same reply (`node.cpp:3333` under R-ext; the same `Chain::state_proof` call under R-cli).
2. The light client anchors *that reply's* `state_root` to a committee-signed header (RI-1) in the same invocation, then verifies the `i:` proof against the anchored root using *that reply's* `leaf_count` (`verify.cpp:308-335`). There is no second channel; the `(state_root, leaf_count)` pair is consistent-by-construction because the daemon computed both from one locked state snapshot, and the anchoring step rejects any `state_root` the committee did not sign.

If a malicious daemon serves an inconsistent `(state_root, leaf_count)` — the genuine `leaf_count` but a tampered `state_root` — the tampered root fails RI-1's committee anchoring. If it serves a tampered `leaf_count` with the genuine `state_root`, then by RI-2's exact-consume + range gates the wrong walk shape generically fails root-equality against the genuine `R` for the genuine `i:` leaf; and even in the degenerate same-walk-shape coincidence of S-040, the value the verifier accepts is still the genuine `i:` leaf at that position under the genuine root — the daemon gains no ability to substitute a *wrong* `(S, H)` receipt, only (at most) to mislabel which sorted slot it occupies, which the `key_i(S, H)`-binding of RI-2 still ties to `(S, H)`. The operational obligation is met: **source `leaf_count` from the same committee-anchored envelope as `state_root`** (`MerkleTreeSoundness.md` §6.2; `docs/SECURITY.md` §S-040), which the single-reply shape enforces structurally.

**Status.** S-040 is a registered **Low/Op** caller-trust item, orthogonal to RI-2's soundness (which holds with the genuine `leaf_count`). The structural fix — domain-separating `leaf_count` into the leaf hash — breaks v1 wire compatibility and is tracked for a future v2.x flag-day (`MerkleTreeSoundness.md` §6.2). RI-4 records that the shipped `verify-receipt-inclusion` flow does not split the source and is therefore unaffected.

**Concrete-security bound.** RI-4 adds no independent term beyond RI-1 + RI-2 under single-envelope sourcing.   ∎
...
Like every other light-client verifier, `verify-receipt-inclusion` seeds its committee map from genesis `initial_creators` (`build_genesis_committee`, `trustless_read.cpp:43-50`) and requires every creator in the `0..h` walk to be in that map (`verify_block_sigs`, verify.cpp:223-229). On a chain with mid-chain REGISTER/DEREGISTER that rotated the committee, the walk **fails closed** at the first non-`K_0` signer → `UNVERIFIABLE` — a positive safety property, never a wrong verdict. This is the shared `K_0` caveat (`LightClientThreatModel.md` §6.5 + F-1, `StakeProofSoundness.md` §6.3, `StateRootAnchorSoundness.md` §6.2), inherited unchanged.

### 6.3 S-040 leaf_count caller-trust (RI-4)

Sound under the shipped single-envelope sourcing; a future split-source integrator must source `leaf_count` from the committee-anchored envelope. Low/Op; structural fix tracked for a v2.x wire-breaking flag-day (`MerkleTreeSoundness.md` §6.2).

### 6.4 `i:` namespace not served by the generic `state_proof` RPC (sourcing obligation)

`Node::rpc_state_proof` declines the `i:` namespace (`node.cpp:3294`, `:3312-3313`: *"composite-key namespaces (i/m/p) are out of scope for this RPC"*). I3 must therefore use sourcing (R-ext) — extend the daemon RPC (or add a `receipt_proof` RPC) to build the composite key — or (R-cli) — construct `key_i(S, H)` client-side and feed it as `key_bytes` (§1.4). Both are sound (RI-2 re-derives the leaf hash from `key_bytes` and gates on root-equality regardless of sourcing); (R-cli) is strictly stronger on key-integrity (the key never originates from the daemon) and is the recommended default. This is a *plumbing* obligation on I3, not a soundness gap: whichever route ships, the §4.2.1 key + marker cross-check pins the proven leaf to the operator's `(S, H)`. (Confirmed against the present tree: `node.cpp:3296` lists `a|s|r|d|b|k`, `:3305` adds `c`, and the `else` at `:3312` rejects `i`; the verifier `verify_state_proof` consumes `key_bytes` directly at `verify.cpp:304-305`, so R-cli requires no verifier change.)
...
| RI-1 (T-L1) | `anchor_genesis` | `light/trustless_read.cpp:52-79` | Genesis-hash anchor (reused; `LightClientThreatModel.md` T-L1). |
...
| RI-4 | proof envelope (`state_root` + `leaf_count`) | `src/node/node.cpp:3325-3335`; `src/chain/chain.cpp:456` | Single-reply `(state_root, leaf_count)` from one locked state (S-040 single-envelope). |
| §6.4 (RPC gap) | `Node::rpc_state_proof` namespace gate | `src/node/node.cpp:3287-3336` (`i:` declined at `:3294`, `:3312-3313`) | `i:` out of scope → R-ext or R-cli sourcing required. |
...
| `tools/test_merkle_proof_tampering.sh` + `determ test-merkle-proof-tampering` (15 scenarios) | RI-2 tamper rejection; RI-4 / S-040 `leaf_count` limitation (#12); key-binding (#14-#15). |
...
- **Known limitations.** One-sided / no provable non-application (§6.1, RI-3); genesis-only committee map (§6.2); S-040 `leaf_count` caller-trust, mitigated by single-envelope sourcing (§6.3); `i:` not served by the generic `state_proof` RPC → R-ext/R-cli sourcing obligation on I3 (§6.4); presence-only semantics, no amount/recipient/dst-shard in the leaf (§6.5); per-chain, not cross-shard-global (§6.6). None undermine the per-invocation `APPLIED` soundness claim.
...
- `light/verify.cpp:285-349` — `verify_state_proof` (consumes `key_bytes` at `:304-305`).
- `light/trustless_read.cpp:188-350` — `read_account_trustless` (template for the `i:` read); race-window dispatch `:226-307`.
...
- `docs/SECURITY.md §S-040` — `leaf_count` caller-trust invariant (RI-4).
