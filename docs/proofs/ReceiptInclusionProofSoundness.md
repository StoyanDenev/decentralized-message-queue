...
2. The light client anchors *that reply's* `state_root` to a committee-signed header (RI-1) in the same invocation, then verifies the `i:` proof against the anchored root, the wrapper re-binding the reply's `leaf_count` to `R` (`verify.cpp:308-335`). A tampered `state_root` fails RI-1's committee anchoring; a tampered `leaf_count` fails the wrapper re-derivation against the genuine `R`. Either deception yields `UNVERIFIABLE`, never a false `APPLIED`.
