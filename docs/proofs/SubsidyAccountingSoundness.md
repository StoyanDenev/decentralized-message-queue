...
`expected_total()` (`include/determ/chain/chain.hpp:590–597`) is the closed form:
...
The sixth term `− accumulated_shielded_` (§3.22, `chain.hpp:898`) subtracts value moved out of the transparent live sum into the confidential pool by SHIELD / UNSHIELD / CONFIDENTIAL_TRANSFER; it is **conditional/zero on shield-free chains** (its state-root leaf `c:accumulated_shielded` is emitted only when non-zero, `chain.cpp:502–503`), so the transparent identity degenerates to the classic five-term form on any chain that never shields. The reward pipeline this proof covers never touches `accumulated_shielded_`.
...
**Gate (executed).** SB-3 corollary 3 (*no silent wrap on the reward path*) is now enforced by `determ test-overflow-paths` (`tools/test_overflow_paths.sh`, FAST on both platforms — register claim **SB-3**, `ProofClaimGateTraceability.md` §3j). No prior test drove a *committee creator* near the u64 ceiling (all subsidy/fee tests use small balances), so the per-creator `checked_add_u64` at `chain.cpp:1761` had no negative test. The gate stages a sole creator at `UINT64_MAX − 5` and a FLAT `block_subsidy = 100`: the per-creator credit must throw `S-007 "per-creator"` and the A9 rollback must leave the balance and `state_root` byte-identical (no partial mint), while a boundary control (credit to *exactly* `UINT64_MAX`) is accepted — pinning the strict-greater check from both directions. *Falsify-on-mutant:* replacing the guard at `:1761` with a raw `bal += per_creator` makes the creator's balance wrap silently with no throw and no rollback; the overflow + both rollback assertions flip RED while the boundary control stays green. The `:1749` (join) and `:1769` (dust) guards are the identical idiom; the per-creator site is the falsifiable representative.
...
live_total_supply(state_{n+1}) − live_total_supply(state_n)
    = subsidy_this_block + accumulated_inbound delta
      ...
      − accumulated_shielded delta
```

with **fees contributing zero net** (the per-tx debit `−fee` cancels exactly against the per-creator distribution credit `+per_creator + dust`, which sum to `+total_fees`), **subsidy contributing exactly `+subsidy_this_block`** matched 1:1 by the `accumulated_subsidy_ += subsidy_this_block` advance at `chain.cpp:1391`, and the **shielded channel contributing `−accumulated_shielded delta`** (§3.22: SHIELD moves `+A` into the confidential pool, dropping the transparent sum by `A` while `accumulated_shielded_` rises by `A`; UNSHIELD / CONFIDENTIAL_TRANSFER reverse it — zero on every block this reward proof concerns and on any shield-free chain). Three corollaries:
...
The remaining `expected_total` terms — `accumulated_inbound_`, `accumulated_outbound_`, `accumulated_slashed_`, and `accumulated_shielded_` — are populated by the cross-shard and §3.22 confidential-pool channels (`CrossShardReceipts.md` FA7 and the SHIELD / UNSHIELD / CONFIDENTIAL_TRANSFER apply branches at `chain.cpp:1037/1081/1172`) — and, as of 2026-09-16, `accumulated_slashed_` is populated by NOTHING: the equivocation forfeiture (D4) and the Phase-1 abort deduction (D13) were both retired, so that term is frozen at whatever a restored snapshot carries and is 0 on any chain built from genesis. All of this is independent of the reward pipeline; the NEF channel (`chain.cpp:823–833`) is an intra-supply transfer touching no counter (`EconomicSoundness.md` T-13, `NefPoolDrain.md` T-N5). Summing all channels yields exactly the decomposition in the statement, and the apply-tail assertion confirms it block-by-block. ∎

**Code witness.** `src/chain/chain.cpp:1286,1390` (the matched distribution + mint gates); `src/chain/chain.cpp:1391` (the gated mint); `src/chain/chain.cpp:1393–1395` (inbound/outbound/slashed counter advances); `src/chain/chain.cpp:1397–1419` (A1 closure assertion + rollback throw); `src/chain/chain.cpp:1280,1292,1300` (the three S-007 credit guards on the reward path); `src/chain/chain.cpp:548–553` (`live_total_supply`); `include/determ/chain/chain.hpp:590–597` (`expected_total`, six-term incl. `−accumulated_shielded_`).
...
**Test witness.** `tools/test_supply_invariant.sh` (direct A1 assertion on synthetic per-counter deltas); `tools/test_supply_lifecycle.sh` (A1 closure after every block across TRANSFER / STAKE / UNSTAKE / REGISTER+NEF / DEREGISTER / an equivocation event + a Phase-1 abort, both of which now move no stake / FLAT + LOTTERY subsidy / E4 exhaustion / cross-shard inbound+outbound); `tools/test_fee_distribution_edge.sh` ("A1 invariant across all distribution scenarios"); `tools/test_cross_shard_supply_invariant.sh` (the FA-Apply-17 aggregate); `tools/operator_supply_check.sh` (operator-facing offline A1 audit from snapshot data).
...
- **Slash and cross-shard supply contributions.** SB-3's decomposition references `accumulated_slashed_`, `accumulated_inbound_`, `accumulated_outbound_`, and `accumulated_shielded_` as given counters. `accumulated_slashed_` has had **no producer since 2026-09-16** (D4 removed the equivocation forfeiture, D13 the abort deduction), so its delta is identically 0 and the decomposition is sound a fortiori; the remaining derivations are `CrossShardReceipts.md` (FA7) / `CrossShardSupplyConservation.md` (FA-Apply-17), and the §3.22 SHIELD / UNSHIELD / CONFIDENTIAL_TRANSFER apply branches (`chain.cpp:1037/1081/1172`).
...
| `include/determ/chain/chain.hpp:590–597` | `expected_total()` closed form (six-term, incl. `−accumulated_shielded_`). |
