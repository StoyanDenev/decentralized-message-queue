...
- **RP-3** (value-hash cleartext cross-check) closed — the light-client recomputation (`light/main.cpp:4614-4621`) is byte-identical to the committed-leaf computation (`chain.cpp:300-307`); a daemon lie about `ed_pub`/lifecycle/`region` needs an A2 collision. **Executed against `A_daemon`** by `tools/test_light_registrant_tamper.sh`: a `tools/rpc_tamper_proxy.py` MITM rewrites one `account`-reply `registry` field (`registered_at`, then `region`) while forwarding the honest proof, and the client returns `UNVERIFIABLE`/exit 3 with the account-registry mismatch detail (a pass-through control reaches `INCLUDED`/exit 0, so the gate is non-vacuous; the falsify at `light/main.cpp:6112` flips the tamper legs to a false `INCLUDED`). See `ProofClaimGateTraceability.md` §3e.
