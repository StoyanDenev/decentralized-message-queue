...
The read's logical pipeline mirrors `read_account_trustless` (`light/trustless_read.cpp:188-350`) with the `a:`-namespace argument replaced by `"r"` and the value-hash decode replaced by the `RegistryEntry` encoding; the shipped implementation is `cmd_verify_registrant` (`light/main.cpp:4456-4773`):

1. **Genesis anchor** — `anchor_genesis(rpc, genesis_O)` (`light/trustless_read.cpp:52-79`; invoked at `light/main.cpp:4499`): compute `compute_genesis_hash(genesis_O)` locally, fetch block 0, byte-compare. (T-L1.)
2. **Header-chain walk + per-block committee-sig verify** — `verify_chain_to_head` (`light/trustless_read.cpp:81-186`; invoked at `light/main.cpp:4511`), invoking `verify_headers` + `verify_block_sigs` end-to-end from block 0, capturing `vc.head_state_root` + `vc.height`. (T-L2.) This yields a committee-anchored `state_root` (SR-1). If `vc.head_state_root` is empty the read throws (S-033 not active — `light/main.cpp:4512-4517`).
...
6. **Race-window anchor of the proof root to a committee-signed header** — the three-branch `proof_height < / == / > vc.height` dispatch (`light/main.cpp:4637-4710`, structurally identical to `verify-param-change` / `trustless_read.cpp:226-307`). (T-L4 §4.4.1 / RP-1; the standalone form is `StateProofRaceWindowSoundness.md` PRW-1..PRW-5.)
...
**Head-block boundary.** If the operator queries a registrant at the chain *head*, that block's `state_root` has no signed successor yet; the shipped flow handles this via the race-window dispatch matching a head-height proof (`proof_height == vc.height`) byte-for-byte against `vc.head_state_root` (`light/main.cpp:4680-4684`), itself extracted from the committee-signed header walk; a future-height proof (`proof_height > vc.height`) is anchored by fetching and committee-verifying the header at `proof_height − 1` and walking the prev_hash link forward (`light/main.cpp:4657-4684`); a past-height proof (`proof_height < vc.height`) is rejected as stale (`light/main.cpp:4644-4650`). This is `LightClientThreatModel.md` L-5 / `StateProofRaceWindowSoundness.md` PRW-1..PRW-3. RP-2 below proves Merkle-path soundness *given* such an `R`; RP-1 is the committee-binding of `R`.
...
|---|---|---|---|
| RP-1 | Committee-signed `state_root` binds the `r:` leaf | `light/main.cpp:4637-4710` (race-window anchor); `src/node/producer.cpp::compute_block_digest`; `src/chain/block.cpp::signing_bytes` | `StateRootAnchorSoundness.md` SR-1/SR-2/SR-3; `LightClientThreatModel.md` T-L2; `StateProofRaceWindowSoundness.md` PRW-1..PRW-3 |
...
- **RP-1** (committee-anchored root) closed by inheritance from `StateRootAnchorSoundness.md` SR-1 + the shared race-window anchor (`light/main.cpp:4637-4710`); reduces to A1 + A2, bound `≤ K·2⁻¹²⁸ + 2⁻¹²⁸`.
