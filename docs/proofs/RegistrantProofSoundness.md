...
The read's logical pipeline mirrors `read_account_trustless` (`light/trustless_read.cpp:188-350`) with the `a:`-namespace argument replaced by `"r"` and the value-hash decode replaced by the `RegistryEntry` encoding; the shipped implementation is `cmd_verify_registrant` (`light/main.cpp:4164-4492`):

1. **Genesis anchor** — `anchor_genesis(rpc, genesis_O)` (`light/trustless_read.cpp:52-79`; invoked at `light/main.cpp:4204`): compute `compute_genesis_hash(genesis_O)` locally, fetch block 0, byte-compare. (T-L1.)
2. **Header-chain walk + per-block committee-sig verify** — `verify_chain_to_head` (`light/trustless_read.cpp:81-186`; invoked at `light/main.cpp:4216`), invoking `verify_headers` + `verify_block_sigs` end-to-end from block 0, capturing `vc.head_state_root` + `vc.height`. (T-L2.) This yields a committee-anchored `state_root` (SR-1). If `vc.head_state_root` is empty the read throws (S-033 not active — `light/main.cpp:4217-4222`).
3. **State-proof fetch for the `r:` namespace** — `rpc.call("state_proof", {{"namespace","r"},{"key",domain}})` (`light/main.cpp:4225-4226`). The daemon's `Node::rpc_state_proof` (`src/node/node.cpp:3355-3429`) handles `"r"` in the simple-key branch (`node.cpp:3382`: `if (ns == "a" || ns == "s" || ns == "r" || ns == "d" || ns == "b" || ns == "k")`), building the prefixed key `"r:" + domain` byte-for-byte as `build_state_leaves` does, and returning `(key_bytes, value_hash, target_index, leaf_count, proof, state_root, height)`.
4. **Key binding** — bind `proof.key_bytes == "r:" + domain`, recomputed locally (`light/main.cpp:4209-4212` builds `local_key`; the byte-compare is at `light/main.cpp:4261-4270`). A mismatch ⇒ `UNVERIFIABLE`.
5. **Value-hash cleartext cross-check** — fetch the cleartext registrant via the `account` RPC (`light/main.cpp:4277`), recompute the canonical `RegistryEntry` value-hash from the daemon-asserted cleartext fields (`light/main.cpp:4314-4321`), and require equality against the proof's verified `value_hash` (`light/main.cpp:4323-4332`). (RP-3 / §4.3.)
6. **Race-window anchor of the proof root to a committee-signed header** — the three-branch `proof_height < / == / > vc.height` dispatch (`light/main.cpp:4344-4411`, structurally identical to `verify-param-change` / `trustless_read.cpp:226-307`). (T-L4 §4.4.1 / RP-1; the standalone form is `StateProofRaceWindowSoundness.md` PRW-1..PRW-5.)
7. **Merkle inclusion verify** — `verify_state_proof(proof, anchor_root)` (`light/verify.cpp:285-349`; invoked at `light/main.cpp:4417`), delegating to `crypto::merkle_verify` (`src/crypto/merkle.cpp:113-141`). (T-L3 / RP-2.)
8. **Lifecycle derivation** — on `INCLUDED`, derive `ACTIVE`/`INACTIVE` from the committee-certified `(active_from, inactive_from)` against the committee-verified anchored head height (`light/main.cpp:4432-4435`). (RP-5 / §4.5.)

The verdict tri-state (mirroring `verify-dapp-registration` / `verify-param-change`):

| Verdict | Exit | Condition |
|---|---|---|
| `INCLUDED` | 0 | steps 1–8 pass: the `r:` leaf is committed under the committee-signed `state_root`, `key_bytes` matches the locally-recomputed `"r:" + D`, and `value_hash` matches the recomputed `RegistryEntry` encoding. The `RegistryEntry` is committee-anchored truth, plus a verified `ACTIVE`/`INACTIVE` verdict. (`light/main.cpp:4422-4436, 4487`.) |
| `NOT_INCLUDED` | 0 | the daemon returns the literal `not_found` for the *exact* `"r:" + D` key, **and** the `account` RPC's `registry` is also null/absent (cross-check). A sound verified negative: no such validator is registered at the verified head. Exit 0 (a verified answer) matches the whole InclusionVerdict reader family — `verify-dapp-registration` / `-receipt-inclusion` / `-merge-state` / `-param-change` all return 0 for INCLUDED *and* NOT-INCLUDED; only UNVERIFIABLE is non-zero. (`light/main.cpp:4235-4239, 4485-4486`.) |
| `UNVERIFIABLE` | 3 | any of: a non-`not_found` daemon refusal; `key_bytes ≠ "r:"+D`; value-hash/cleartext mismatch; `merkle_verify` rejects; a `not_found` contradicted by a non-null cleartext registry; a present `r:` proof contradicted by a null cleartext registry. Fail-closed — refuses to assert either way. (`light/main.cpp:4246-4256, 4266-4270, 4326-4332, 4419-4420, 4485`.) |
| (transport/args) | 1 | usage error, RPC open failure, malformed hex, or any thrown exception. (`light/main.cpp:4176-4184, 4488-4491`.) |
...
- never returns a false `NOT_INCLUDED` (a withheld/inconsistent leaf yields `UNVERIFIABLE`, never an authoritative negative);
...
The read reuses the four light-client primitives unchanged — genesis anchor (`LightClientThreatModel.md` §3.1), committee-sig verify (§3.2), header continuity (§3.3), and Merkle state-proof verify (§3.4) — composed by the same `read_*_trustless` skeleton, and the same three-branch race-window anchor `verify-param-change` / `verify-dapp-registration` use. The pipeline diagram of `LightClientThreatModel.md` §3.5 applies with the state-proof stage targeting the simple ASCII key `key_r(D) = "r:" + D`. Two specializations distinguish the `r:` read from the balance read:
...
**Head-block boundary.** If the operator queries a registrant at the chain *head*, that block's `state_root` has no signed successor yet; the shipped flow handles this via the race-window dispatch matching a head-height proof (`proof_height == vc.height`) byte-for-byte against `vc.head_state_root` (`light/main.cpp:4406-4411`), itself extracted from the committee-signed header walk; a future-height proof (`proof_height > vc.height`) is anchored by fetching and committee-verifying the header at `proof_height − 1` and walking the prev_hash link forward (`light/main.cpp:4351-4405`); a past-height proof (`proof_height < vc.height`) is rejected as stale (`light/main.cpp:4344-4350`). This is `LightClientThreatModel.md` L-5 / `StateProofRaceWindowSoundness.md` PRW-1..PRW-3. RP-2 below proves Merkle-path soundness *given* such an `R`; RP-1 is the committee-binding of `R`.
...
1. **Key equality.** `key_bytes == key_r(D)`, where `key_r(D) = "r:" + D` is built locally from the operator's `--domain` argument (`light/main.cpp:4209-4212`). The verifier re-derives the canonical key and byte-compares against the daemon-returned `key_bytes` (`light/main.cpp:4261-4270`), rejecting any mismatch — pinning the proven leaf to *this* domain `D`. Because the same `D` also keys the `a:`/`s:`/`d:` leaves, the namespace byte in `key_r(D)` is load-bearing (RP-2 (ii)).
2. **Value-hash equality.** `value_hash == H(\text{preimage}(r_A))`, where the verifier recomputes the canonical §1.2 preimage (`ed_pub ‖ registered_at ‖ active_from ‖ inactive_from ‖ |region| ‖ region`) from the daemon-asserted cleartext registry fields (`light/main.cpp:4314-4321`). This confirms the leaf commits the asserted entry and not some other content. The **field order, the `u64_be` width of each scalar and of the `region` length prefix, and the 32-byte raw `ed_pub`** are all part of the canonical preimage; the verifier decodes the 64-hex `ed_pub` back to its 32 raw bytes and rejects if it is not 32 bytes (`light/main.cpp:4302-4307`).
...
computed by the light client at `light/main.cpp:4314-4321` is **byte-identical** to the committed-leaf computation at `src/chain/chain.cpp:300-307`, so equality against the proof's verified `value_hash` (`light/main.cpp:4325`) is the binding from cleartext to commitment.

**Proof.** The `account` RPC (`node.cpp:2722-2748`) returns, for a registered domain, a `registry` object with `ed_pub` (hex), `registered_at`, `active_from`, `inactive_from`, and `region` (`node.cpp:2727-2731`) — sourced from the same `registrants_` map (`node.cpp:2716-2719`) that `build_state_leaves` hashes. The light client decodes `ed_pub` from 64-hex to 32 raw bytes (`light/main.cpp:4302`), reads the three `u64` height fields and the `region` string (`light/main.cpp:4294-4298`), and feeds them to a `SHA256Builder` in the exact field order and width of the `chain.cpp:300-307` `r:` branch. Because `SHA256Builder::append(uint64_t)` is big-endian on both sides and the `region` length prefix is a `u64` on both sides, the two preimages are byte-equal iff the cleartext fields equal the committed fields.

Now suppose the daemon serves a cleartext `r_A` differing from the genuine committed `r_T` in any field, alongside the genuine committed `value\_hash_r^T`. The cross-check recomputes `H(\text{preimage}(r_A))` and compares to `value\_hash_r^T`. For the comparison to pass, `H(\text{preimage}(r_A)) = H(\text{preimage}(r_T))` with `\text{preimage}(r_A) ≠ \text{preimage}(r_T)` — a SHA-256 collision, `≤ 2⁻¹²⁸` (A2). The **injectivity** of the encoding makes `\text{preimage}(r_A) ≠ \text{preimage}(r_T)` follow from `r_A ≠ r_T`: the lone variable-length field (`region`) is length-prefixed, so a difference in any field — including a `region` of a different length, or one whose bytes shift across the boundary — produces a distinct preimage with no parsing ambiguity, keeping the cross-check unambiguous. Conversely, if the daemon keeps the cleartext honest but serves a tampered `value\_hash_A ≠ value\_hash_r^T`, the cross-check fails directly (`H(\text{preimage}(r_T)) ≠ value\_hash_A`) → `UNVERIFIABLE` (`light/main.cpp:4326-4332`); and if it tampers *both* consistently, RP-2's root-equality catches it (a wrong committed value-hash fails the Merkle walk against `R`). The cross-check thus binds the displayed `ed_pub` / lifecycle / `region` to committee-anchored truth, identically to the balance read's L-4 and the `d:` read's DR-2(iii).

**Concrete-security bound.** `Pr[A_daemon defeats RP-3] ≤ 2⁻¹²⁸` (A2). Folds into RP-2; adds no independent end-to-end term.   ∎

### 4.4 Theorem RP-4 (one-sided verifier; sound `NOT_INCLUDED` only via the daemon's literal `not_found`)

**Statement.** The `r:` read is a **one-sided** verifier in the cryptographic sense (no Merkle non-membership proof exists), but it emits a **sound `NOT_INCLUDED`** verdict — a *verified daemon-honesty negative* — conditioned on the conjunction:

1. the daemon returns the literal `not_found` error for the **exact** `"r:" + D` key (`light/main.cpp:4231-4239`), **and**
2. the `account` RPC's `registry` for `D` is also null/absent (`light/main.cpp:4242-4244`).

If (1) holds but (2) fails — the daemon says `not_found` for the `r:` proof yet returns a *non-null* registry over `account` — the verifier refuses `NOT_INCLUDED` and emits `UNVERIFIABLE` (inconsistent daemon, `light/main.cpp:4245-4251`). Symmetrically, if a *present* `r:` proof is served but the cleartext registry is **null**, the verifier throws (`light/main.cpp:4284-4291`) — a null registry with a present `r:` proof is an inconsistent daemon, fail closed. No false `NOT-REGISTERED` is ever emitted, because the verdict is keyed on the daemon's own literal absence claim cross-checked against its own cleartext, and any contradiction collapses to `UNVERIFIABLE`.

**Analysis.** The sorted-leaves balanced binary Merkle primitive supports **positive membership only** (`MerkleTreeSoundness.md` **MT-5**): there is no native non-membership (absence) proof. `Chain::state_proof` returns `std::nullopt` for an absent key (`chain.cpp:449`), surfaced by the daemon as `{"error":"not_found"}` (`node.cpp:3428`). An honest light client receiving this can only conclude *"this daemon did not give me a proof,"* which an all-Byzantine peer set could produce by withholding. The read therefore does **not** treat `not_found` as a cryptographic absence proof; instead it treats `NOT_INCLUDED` as a sound *verified negative about this daemon's consistent claim of absence*, gated on the two-sided agreement of (1)+(2). The caller contract of `NegativeVerdictSoundness.md` **NV-6** applies: a `r:` `NOT_INCLUDED`/`not_found` is "no membership proof obtained" for value-affecting decisions against a withholding adversary — the operator falls back to a multi-peer cross-check (out of scope) where authoritative absence matters.

**Why the cross-check matters.** A daemon attempting attack §2(b) (suppress a genuine registration) cannot land the verifier in a *false* `NOT_INCLUDED`: if it serves `not_found` for the `r:` proof while the genuine registrant is still reflected in its own `account` cleartext, condition (2) fails and the verdict is `UNVERIFIABLE`, operator-visible. To force a false `NOT_INCLUDED` the daemon would have to *also* lie in the cleartext (return a null registry) — at which point both surfaces consistently deny the registration, and the verdict is the honest "this daemon claims `D` is not registered." The verifier never *asserts* absence as committee-anchored truth; it asserts only that the daemon is internally consistent in denying it. The soundness goal (§2) — never a false `INCLUDED`, never a false `NOT_INCLUDED`, fail closed on inconsistency — is fully met.   ∎
...
**Statement.** The read's exit codes are sound: `INCLUDED` → 0 and `NOT_INCLUDED` → 0 are emitted **only** on a verified answer (the full RP-2 conjunction, resp. the RP-4 two-sided `not_found` agreement); `UNVERIFIABLE` → 3 on any detected inconsistency; args/transport errors → 1. **There is no code path from a detected inconsistency to a false `INCLUDED`.** (Per `light/main.cpp:4485-4487`: `UNVERIFIABLE` returns 3, else returns 0 — so `INCLUDED` AND `NOT_INCLUDED` are the exit-0 "sound verified answer" tier and `UNVERIFIABLE`/error are the non-zero fail-closed tier, matching the whole InclusionVerdict reader family.)

**Proof.** By inheritance from `LightClientThreatModel.md` Lemma **L-6** for the reused T-L1/T-L2/T-L3 surfaces (`verify_headers`, `verify_block_sigs`, `verify_state_proof` each set `r.ok=false` + `r.detail` on every error branch and return to a caller that throws or sets `UNVERIFIABLE`; `anchor_genesis` throws on genesis mismatch — `light/main.cpp:4488-4491` catches every `std::exception` to exit 1), plus the §4.2.1 structural property that `INCLUDED` is reached *only* on the conjunction (RP-1 anchored ∧ RP-2 root-equality ∧ key-equality ∧ value-hash-equality, set at `light/main.cpp:4422`). Every failure branch sets `UNVERIFIABLE` or throws: a non-`not_found` daemon refusal (`light/main.cpp:4252-4256`), `key_bytes` mismatch (`light/main.cpp:4266-4270`), an account-RPC refusal or null-registry on a present proof (`light/main.cpp:4278-4291`, throws), value-hash mismatch (`light/main.cpp:4326-4332`), a stale or non-matching proof root (`light/main.cpp:4344-4350, 4375-4381, 4406-4411`, throws), and `merkle_verify` rejection (`light/main.cpp:4419-4420`). The `not_found`-but-non-null-registry contradiction sets `UNVERIFIABLE` (`light/main.cpp:4245-4251`). There is no path from any of these to `INCLUDED`. This is the per-command instance of L-6; the throw-discipline is structural.   □

### 4.7 End-to-end composition

**Corollary RP-E (trust-minimized registrant read).** Under A1 + A2, `verify-registrant --domain D` yields an `INCLUDED` verdict bound, end-to-end, to a committee-signed `state_root` anchored to the operator's pinned genesis (with a verified `ACTIVE`/`INACTIVE` classification), a sound `NOT_INCLUDED`, or a fail-closed `UNVERIFIABLE`. Composing RP-1 (committee-anchored root) + RP-2 (`r:` Merkle inclusion + key/value-hash binding, including the inherited S-040 `leaf_count` root-binding) + RP-3 (value-hash cleartext cross-check) + RP-5 (sound lifecycle derivation) with the genesis-anchor + header-walk steps (T-L1 + T-L2), and taking the union bound over the pipeline:
...
|---|---|---|---|
| RP-1 | Committee-signed `state_root` binds the `r:` leaf | `light/main.cpp:4344-4411` (race-window anchor); `src/node/producer.cpp::compute_block_digest`; `src/chain/block.cpp::signing_bytes` | `StateRootAnchorSoundness.md` SR-1/SR-2/SR-3; `LightClientThreatModel.md` T-L2; `StateProofRaceWindowSoundness.md` PRW-1..PRW-3 |
...
| RP-3 | Value-hash cleartext cross-check binding | recompute `light/main.cpp:4314-4321` ≡ committed `src/chain/chain.cpp:300-307`; compare `light/main.cpp:4325`; cleartext source `src/node/node.cpp:2725-2735` | `LightClientThreatModel.md` L-4; `StakeProofSoundness.md` SP-2; `DAppRegistryReadSoundness.md` DR-2(iii) |
| RP-4 | One-sided verifier; sound `NOT_INCLUDED` via `not_found` + cleartext corroboration | `light/main.cpp:4231-4251` (`not_found` + null-registry cross-check); `light/main.cpp:4284-4291` (present-proof + null-registry → throw); `src/chain/chain.cpp:449` (`nullopt`); `src/node/node.cpp:3428` (`not_found`) | `MerkleTreeSoundness.md` MT-5; `NegativeVerdictSoundness.md` NV-6 |
| RP-5 | ACTIVE/INACTIVE is a sound derived predicate | `light/main.cpp:4432-4435`; struct sentinel `include/determ/chain/chain.hpp:38` | `DAppRegistryReadSoundness.md` DR-6 |
| RP-6 | Fail-closed exit-code discipline | `light/main.cpp:4483-4491` (exit codes + catch-all); error branches throughout `4231-4420` | `LightClientThreatModel.md` L-6; `DAppRegistryReadSoundness.md` DR-5 |
| RP-E | End-to-end composition `≤ 2⁻⁹²` | `light/main.cpp:4164-4492` (`cmd_verify_registrant`) | T-L1 + T-L2 + RP-1 + RP-2 + RP-3 + RP-5 |
...
- **RP-1** (committee-anchored root) closed by inheritance from `StateRootAnchorSoundness.md` SR-1 + the shared race-window anchor (`light/main.cpp:4344-4411`); reduces to A1 + A2, bound `≤ K·2⁻¹²⁸ + 2⁻¹²⁸`.
...
- **RP-3** (value-hash cleartext cross-check) closed — the light-client recomputation (`light/main.cpp:4314-4321`) is byte-identical to the committed-leaf computation (`chain.cpp:300-307`); a daemon lie about `ed_pub`/lifecycle/`region` needs an A2 collision.
- **RP-4** (one-sided verifier) closed honestly — the sorted-leaves primitive (MT-5) admits no non-membership proof, so `NOT_INCLUDED` is a daemon-honesty negative gated on `not_found` + cleartext corroboration; a withheld/inconsistent leaf collapses to `UNVERIFIABLE`, never a false negative; a null registry with a present proof fails closed.
