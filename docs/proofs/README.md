...
| FA-Apply-15 | [MultiEventComposition.md](MultiEventComposition.md) | Multi-event block composition contract (T-M1..T-M7): an apply-layer block containing TRANSFER + STAKE + DEREGISTER + UNSTAKE + REGISTER + DAPP_REGISTER + DAPP_CALL + PARAM_CHANGE + MERGE_EVENT + AbortEvent + EquivocationEvent + CrossShardReceipt — together in a single block — composes safely (T-M1: per-event independence — each event's apply mutation is isolated from sibling-event mutations so per-account / per-stake / per-registry effects do not interfere; T-M2: A1 unitary-supply invariance — the per-block sum of (balance + stake_locked + accumulated_slashed + nef_pool + subsidy_minted) deltas zeroes to the protocol's expected delta regardless of event mix; T-M3: deterministic event-order resolution — events apply in the canonical order defined at `chain.cpp` apply branches (TRANSFER first, MERGE_EVENT last; intra-tx order via vector index) so replays produce byte-identical state; T-M4: nonce-monotonicity across multi-tx multi-account composition — each account's nonce advances exactly once per tx the account signs, regardless of sibling-tx interference; T-M5: dedup-set composability — multiple CrossShardReceipt entries in the same block each independently mark distinct triples in the `applied_inbound_receipts` set without colliding with sibling-block receipts; T-M6: skip-vs-charge composition — an event that silently skips (insufficient-balance STAKE, pre-unlock UNSTAKE, non-owner DAPP_DEREGISTER, etc.) does not destabilize the apply path of sibling events; T-M7: failure isolation — a malformed event in position K does not corrupt the apply mutations of events 0..K-1 nor poison the apply path for events K+1..N-1). Cross-references `tools/test_block_event_composition` + `tools/test_multi_tx_block` + `tools/test_chain_apply_block` regressions; companion analytic to FA-Apply-3 (nonce monotonicity), FA-Apply-6 (fee composition), FA-Apply-7 (subsidy composition), FA-Apply-9 (cross-shard receipt dedup), FA-Apply-10 (equivocation slashing apply), and FA-Apply-11 (abort apply) — FA-Apply-15 closes the conditional that each per-event invariant composes safely under the worst-case fully-mixed event-set within a single block, which none of the per-event FA-Apply-N analytics alone covers. | ✓ |
...
| FB20 | [tla/MultiEventComposition.tla](tla/MultiEventComposition.tla) — multi-event block composition state machine (companion to FA-Apply-15; 6 invariants TypeOK / PerEventIndependence / A1ConservationAcrossComposition / DeterministicOrderResolution / NonceMonotonicAcrossEvents / DedupSetComposable + 2 temporal props EventualMultiEventApply / SkipDoesNotPoisonSiblings) | ✓ spec, model-check pending TLC install |
