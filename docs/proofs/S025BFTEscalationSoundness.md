...
**Status note (S-045, 2026).** The **shipped genesis-default `bft_escalation_threshold` is now `1`** (was 5), lowered by the S-045 fix (`docs/SECURITY.md` §S-045; derivation `AbortCascadeLiveness.md` T-4 / §4.3). This document's `A_premature` gate-2 soundness is UNAFFECTED — the validator still recomputes `total_aborts` from the block's own `b.abort_events` (each requiring the F-a `max(2, K−1)` claim quorum, so a single Byzantine node cannot manufacture aborts), so a lower threshold cannot force escalation without genuine abort certificates. What θ=1 changes is *reachability*, not soundness: at `M=K` the per-stuck-height abort-event ceiling is `K−1`, so any default `> K−1` froze the escalation counter below the threshold forever (the S-045 wedge); θ=1 is reached by the first abort event, so the counter can never freeze below it, and gate 3 (`avail < k_target`) still prevents escalation from firing while an MD margin exists. The `θ = 5` figures below (§Gate-2, the worked example, and the §"tuning" table) therefore describe the **pre-S-045 regime** and are retained as the operational-patience analysis; the shipped default trades that patience for guaranteed reachability, which is correct because at zero MD margin (`M=K`) there is no MD round to be patient *for*.
...
- **Gate 2 (`total_aborts >= bft_escalation_threshold`):** observed *sustained* abort signal. `current_aborts_` is the in-memory list of `AbortEvent` records the node has gathered for the *current* height attempt; aborts accumulate as the producer fails to gather K ContribMsgs and K BlockSigMsgs from a stable committee. The genesis default `bft_escalation_threshold` is **1** post-S-045 (`src/chain/genesis.cpp`; also `src/node/node.cpp` config loader) — escalation is reachable on the first abort event (see the status note above; the pre-S-045 default was 5). Operators can still tune it higher (longer MD patience, at the cost of the S-045 reachability caveat at `M=K`) via `GenesisConfig::bft_escalation_threshold`.
...
**Worked example (pre- vs post-S-045).** At the pre-S-045 `threshold = 5`, three transient aborts (a producer pause, a peer disconnect, a brief network split) did not escalate; by the fifth sustained abort the chain had spent ~5 × `tx_commit_ms + block_sig_ms + abort_claim_ms` in unsuccessful K-of-K attempts — enough to confirm K-of-K was not progressing, so escalation fired. That "patience" reasoning is sound **only when there is an MD margin to be patient for** (`M > K`): there, gate 3 (`avail < k_target`) keeps the round in MD mode across transient aborts regardless of the threshold, so a low threshold does not cause premature escalation. But at **`M = K`** (zero MD margin) the very first abort drops `avail` below `k_target`, and the per-stuck-height abort-event ceiling is `K−1` — so a threshold `> K−1` (e.g. 5) is *never reached*, freezing the counter and wedging the height permanently (S-045). The **shipped default `threshold = 1`** resolves this: it is reached by the first abort event, so escalation is reachable whenever it is *viable* (`avail ∈ [k_bft, K)`) and gate 3 still gates it — it does **not** convert MD safety into BFT safety while an MD round is still formable, because in that regime gate 3 has not fired. (A large `threshold = 50` would still stall for many rounds before recovery, and is unreachable at any `M=K` pool.)
...
The **shipped default `bft_escalation_threshold = 1`** (`src/chain/genesis.cpp`; post-S-045, was 5) makes escalation reachable on the first abort event — the correct default because at `M=K` (the `cluster`/`tactical` postures) no higher threshold is reachable at all (S-045). Operators with an MD margin (`M > K`) who want more MD patience across transient aborts may tune it **up** to `≤ K−1` per stuck-height reachability, or higher on a pool provisioned `N ≥ θ + k_bft`; gate 3 anyway keeps margin-having pools in MD mode regardless of the threshold. Profile-specific illustrations (the "worst-case stall" columns describe the *pre-fix* θ=5 patience regime, retained as tuning guidance):

| Profile | Block cadence | Shipped / recommended threshold | Stall time at that threshold |
|---|---|---|---|
| `tactical` (M=K=3) | 20 ms blocks | 1 (default; ≤ K−1 required at M=K) | ~1 × 0.05 s = ~0.05 s |
| `single_test` | 50 ms blocks | 1 (default) | ~1 × 0.125 s |
| `regional` (M=5/K=4) | 300 ms blocks | 1 default; up to ≤ 3 with MD margin | ~θ × 0.75 s |
| `global` (M=7/K=5) | 600 ms blocks | 1 default; up to ≤ 4 with MD margin | ~θ × 1.5 s |
| Operator-tuned safety-priority (`M > K`) | varies | up to K−1 (or grow pool ≥ θ+k_bft) | accepts longer MD stall to delay escalation under intermittent issues |
...
Three-node mesh with K=3, `bft_enabled = true`, `bft_escalation_threshold = 1` (now the shipped genesis default post-S-045; the test pins it explicitly). The script:
