...
where (from `chain.cpp:699-704` and `chain.hpp:590-596`)
...
This list is **complete** for the shipped `expected_total()` (`chain.hpp:590-596`, six signed counters). Fields 1–7 match `CrossShardSupplyConservation.md` §2.2 (seven fields, no separate `outbound_receipts_` member; the source-side in-flight signal is the monotone counter `accumulated_outbound_`); field 8, `accumulated_shielded_`, is the §3.22 confidential-pool counter added after that proof (SHIELD/UNSHIELD/CONFIDENTIAL_TRANSFER never cross a shard boundary — the UNSHIELD apply rejects a cross-shard `tx.to` at `chain.cpp:1059` — so it does **not** participate in the K-shard in-flight machinery of SI-3; see §5 lim. 7). `live_total_supply()` (`chain.cpp:699-704`) sums *only* fields 1+2 — there is no pseudo-account pool or separate fee escrow on the conservation path: fees flow through `accounts_` via the creator distribution at `chain.cpp:1755-1774` (SI-1, fee case), and shielded value is held opaquely in the confidential pool, accounted by field 8 (`chain.hpp:585-589`). `expected_total()` (`chain.hpp:590-596`) is the signed sum of fields 3–8. The post-apply assertion `actual == expected` (`chain.cpp:1866-1888`) ties (1+2) to (3+4+6−5−7−8) on every block.
...
Following `CrossShardSupplyConservation.md` §2.3-§2.4, a unit of value is **in flight** once a source shard `S` has incremented `accumulated_outbound_` (via `block_outbound += tx.amount` at `chain.cpp:1005`, folded at `1863`) but before the destination shard `D` has incremented `accumulated_inbound_` (via `block_inbound += r.amount` at `chain.cpp:1846`, folded at `1862`). The K-shard aggregate that conserves is FA-Apply-17's **Form B** (`main.cpp` `aggregate_conserved()` lambda):
...
**Legacy back-solve preserves the identity by construction.** If `genesis_total` is absent from `S` (legacy snapshot), `restore_from_snapshot` back-solves it (`chain.cpp:2628-2637`):
...
This is the rearrangement of `A1` (`genesis_total = live − (subsidy + inbound) + (slashed + outbound)`), so the restored chain satisfies `live == expected_total` *by construction*, and the back-solved `genesis_total_` equals the original (because the original also satisfied `A1`). **The back-solve omits `accumulated_shielded_`, and this is sound:** it runs *only* when `genesis_total` is absent — the legacy (pre-A1) snapshot path — and every such snapshot predates §3.22, so `accumulated_shielded_` is `0` there and the six-term identity collapses to the five-term rearrangement above. Any post-§3.22 snapshot always carries `genesis_total` (unconditionally emitted at `chain.cpp:2212`) and therefore takes the explicit-value branch (`chain.cpp:2629`), never the back-solve. Either way all six counters + both sums are reproduced and `A1(C₁)` holds. Moreover S-033 binds the round trip cryptographically: the snapshot tail head's `state_root` covers the `c:` namespace (the five always-on counters at `chain.cpp:494-498` plus the conditional `c:accumulated_shielded` at `502-503`) plus `a:`/`s:`, so a tampered counter or balance fails the restore-side G2 recompute (`chain.cpp:2657`-region) — that is the Byzantine-bytes defense of §5, reducing to A2, distinct from the apply-correctness statement SI-2 makes for honest snapshots.
...
7. **§3.22 confidential pool is single-shard; SI-3's Form-B aggregate is stated without it.** The `accumulated_shielded_` term (field 8) is fully covered by the single-shard SI-1 (apply) and SI-2 (snapshot) surfaces — every SHIELD/UNSHIELD/CONFIDENTIAL_TRANSFER move is a *within-shard* reclassification between the transparent live sum and the opaque pool, and the UNSHIELD apply rejects a cross-shard `tx.to` (`chain.cpp:1053-1059`), so no shielded value ever crosses a shard boundary. Consequently the K-shard **Form-B** aggregate recapped in SI-3 (`Σ_s [ live + outbound + slashed − inbound − subsidy ]`), inherited verbatim from `CrossShardSupplyConservation.md` (FA-Apply-17), does **not** carry a `+ accumulated_shielded_` term: on a shard that has run shield ops it evaluates to `genesis_total(C_s) − accumulated_shielded(C_s)` rather than `genesis_total(C_s)`, because shielded value has left the transparent `live` sum but is not booked into any cross-shard accumulator. The real per-shard supply is `live + accumulated_shielded_` (`chain.hpp:585-589`), and the per-shard `A1` that SI-1/SI-2 establish is the six-term identity in §1.1. Reconciling FA-Apply-17's Form-B with the confidential pool (adding the `+ accumulated_shielded_` term to Form B) is tracked as drift in `CrossShardSupplyConservation.md`, out of this proof's scope; SI-3 as stated is exact for shield-free shards and for the cross-shard flow of transparent value on any shard.
...
| `expected_total()` | `include/determ/chain/chain.hpp:590-596` | the `genesis + subsidy + inbound − slashed − outbound − shielded` formula, the A1 RHS |
