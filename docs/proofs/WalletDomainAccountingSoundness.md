...
- **Cross-shard completeness.** For a cross-shard `TRANSFER`, the credit to `to` is delivered as a *receipt on the destination shard* (`b.inbound_receipts` handling in `apply_transactions`), outside the walk over `D`'s home-shard blocks. The single-shard tally sees the outbound debit but not the inbound credit if `D` is the receiver on another shard. (WA-5 (3).)
...
- *DAPP_CALL, `from == D` (same-shard).* The chain does `sender.balance -= (amount + fee)`, `accounts_[to].balance += amount` — "same semantics as TRANSFER's same-shard leg." The command records `dapp_spend += amount` + `fees_paid += fee`. Exact.
- *DAPP_CALL, `to == D`.* The chain credits `accounts_[to].balance += amount` (`D` is the DApp). The command records `credits_received += amount`. Exact.
...
**Statement.** Under the **assume-applied** premise — every included transaction is counted as if it applied as signed — `tx_flow_net` (§3.2) equals the exact signed sum of `D`'s transaction-field flows over `T_D`. The premise is violated in exactly two cases, both of which the tally over-counts and which are therefore absorbed, by construction, into `non_tx_delta`:
...
Because the reconciliation identity (WA-3) defines `non_tx_delta := b_D^auth − tx_flow_net`, and `b_D^auth` reflects the *true* applied state (no phantom debit for V1, no net movement for V2), any over-count in `tx_flow_net` appears as an equal, opposite term in `non_tx_delta`. Thus V1/V2 are **not** silent errors — they are exactly the discrepancies the surfaced `non_tx_delta` is designed to expose. The command does not attempt to *detect* V1/V2 from tx history (V2's success-vs-failure depends on `unlock_height` and the `locked` amount at apply time, which are not determinable from the tx alone, per the R57 contract); it reports `tx_flow_net` honestly as the assume-applied sum and lets `non_tx_delta` carry the residual. ∎
