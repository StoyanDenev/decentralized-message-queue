...
3. **Tallies the eight accounting quantities** of §3.2 (`credits_received`, `debits_sent`, `dapp_spend`, `staked`, `unstaked`, `fees_paid`, `blocks_produced`, and the derived `tx_flow_net`).
...
- **Cross-shard completeness.** For a cross-shard `TRANSFER`/`DAPP_CALL`, the credit to `to` is delivered as a *receipt on the destination shard* (`b.inbound_receipts` handling in `apply_transactions`), outside the walk over `D`'s home-shard blocks. Because the tally holds no shard geometry, it does more than *omit* the inbound credit: on the sending shard it **tallies a same-block `credits_received += amount` for the cross-shard `to`** (assume-applied V4, WA-2) — an over-count absorbed into that recipient's `non_tx_delta`. On a single-shard chain (`shard_count ≤ 1`, the default) this cannot occur. (WA-5 (3), WA-2 V4.)
...
| DAPP_CALL | `from == D` (same-shard) | `sender.balance -= (amount + fee)`; `accounts_[to].balance += amount` (the DApp is credited, "same semantics as TRANSFER's same-shard leg"). | `dapp_spend += amount`; `fees_paid += fee` |
| DAPP_CALL | `to == D` | `accounts_[to].balance += amount` (`D` is the DApp being credited). | `credits_received += amount` |
...
```
  credits_received = Σ amount over {TRANSFER, DAPP_CALL} with to == D (same-shard)
  debits_sent      = Σ amount over TRANSFER            with from == D
  dapp_spend       = Σ amount over DAPP_CALL           with from == D
  ...
  tx_flow_net = credits_received + unstaked
              − debits_sent − dapp_spend − staked − fees_paid
```
...
**Statement.** For every transaction `tx ∈ T_D`, the accounting category the command assigns `tx` corresponds exactly to the balance effect `apply_transactions` (`src/chain/chain.cpp`) produces for `tx`'s `TxType` and `D`'s role. Formally: the six flow categories (`credits_received`, `debits_sent`, `dapp_spend`, `staked`, `unstaked`, `fees_paid`) partition each transaction's effect on `D`'s balance and locked-stake into the *same* signed contributions the chain applies, per the §3.1 table.
...
- *DAPP_CALL, `from == D` (same-shard, active DApp).* The chain does `sender.balance -= (amount + fee)`, `accounts_[to].balance += amount` — "same semantics as TRANSFER's same-shard leg." The command records `dapp_spend += amount` + `fees_paid += fee`. Exact **on this happy path**; a DAPP_CALL to an unregistered/inactive DApp or with invalid topic/framing is fee-only on-chain and is the assume-applied V3 case (WA-2), absorbed into `non_tx_delta`.
...
**Statement.** Under the **assume-applied** premise — every included transaction is counted as if it applied as signed — `tx_flow_net` (§3.2) equals the exact signed sum of `D`'s transaction-field flows over `T_D`. The premise is violated in exactly four cases (V1–V4 below), each of which the tally over-counts on the `amount` axis and which are therefore absorbed, by construction, into `non_tx_delta`. In every case the **fee** side is exact (the chain does charge the fee on all four paths), so the discrepancy is confined to the moved principal.

- **(V1) Insufficient-balance apply-skip.** A TRANSFER / STAKE / DAPP_CALL whose `sender.balance < (amount + fee)` at apply time hits `continue` in `apply_transactions` and mutates nothing (balance and nonce unchanged), yet the tally counts its `amount`/`fee`. (The validator normally rejects these before inclusion; the `continue` is the apply-time safety net.)
...
- **(V3) DAPP_CALL fee-only no-op** *(R60 audit finding)*. A `DAPP_CALL` whose target DApp is unregistered/inactive, or whose topic/payload framing is invalid, or which is cross-shard, takes one of the early `break` paths in `apply_transactions` (`src/chain/chain.cpp` DAPP_CALL case: missing/inactive DApp, empty payload, topic-len overrun, unknown topic, malformed ct framing, cross-shard) — each of which does `charge_fee(sender, tx.fee)` + `sender.next_nonce++` then **breaks**, never reaching the `amount` debit/credit. The tally, keying only on `type == 10`, still adds `dapp_spend += amount` on the sender and `credits_received += amount` on the receiver (if `to == D`). The over-count is `+amount` on `dapp_spend` and `+amount` on the receiver's `credits_received`, both absorbed into the respective `non_tx_delta`. The command cannot detect these paths from the block walk (they depend on `dapp_registry_` / topic state the wallet does not hold), so it reports the assume-applied sum honestly.
- **(V4) Cross-shard TRANSFER/DAPP_CALL receiver credit** *(R60 audit finding)*. On a sharded chain (`shard_count > 1`), a cross-shard `TRANSFER`/`DAPP_CALL` debits the sender on its home shard but routes the recipient credit **off-shard** (`block_outbound += amount`; the credit lands via `inbound_receipts` on `to`'s home shard, `chain.cpp` `is_cross_shard`). The tally, which fetches no shard geometry, credits `to` on the *sending* shard for any `type ∈ {0,10}` — so if a cross-shard recipient `D` is audited against the sending shard's daemon, `credits_received` is over-counted by `amount` (a same-block credit the recipient never received on this shard), absorbed into `D`'s `non_tx_delta`. On a single-shard chain (`shard_count ≤ 1`, the default) `is_cross_shard` is always false, so V4 cannot occur and every credit is exact. This refines §3.1/WA-1: the `credits_received += amount` classification is exact for a **same-shard** receiver; a cross-shard receiver credit is an assume-applied artifact, not the mere omission the earlier draft implied.
...
- (V1): the `sender.balance < cost` guard's `continue` (present in the TRANSFER, STAKE, and DAPP_CALL cases of `apply_transactions`) skips the whole tx — no debit, no nonce bump. The tally, seeing the tx included, still adds its fields. The over-count is `+(amount + fee)` for a skipped debit-type tx.
...
- (V3): the DAPP_CALL early-`break` paths charge the fee and advance the nonce but never apply the `amount` debit/credit. The tally's `dapp_spend += amount` (sender) and `credits_received += amount` (receiver `to == D`) over-state the true `0` on the amount axis; the fee is exact.
- (V4): a cross-shard TRANSFER/DAPP_CALL applies no local credit to `to` (the credit is an off-shard receipt). The tally's `credits_received += amount` for a cross-shard recipient over-states the true `0` on this shard.
