...
>
> **UPDATE (commit 850d2c3): the inbound-view commit-binding is now LIVE** — the
> producer binds `view_inbound_root` into the Phase-1 commit, the block carries
> per-creator view roots, and the validator recomputes the creator commit with
> them (verified: full cross-shard suite + FAST 147/147). **Site 3 is now also
...
> specification but live behavior. **The equivocation/abort dimensions are now also
> shipped (commit `48c4b45`)** via the same carry→reconcile→digest pattern with the
> UNION rule (subset membership + zero-root v1-sentinel handling — see
> `EqAbortViewDigestExtension.md`), so the digest-coverage claim is live behavior
> for all three pool-fed input fields. The apply-side determinism the theorem
> consumes (S-033 state_root over the reconciled inputs) holds for all three.
...
- **D2 (gossip-async-vs-canonical-view divergence).** The K-of-K Phase-2 signature surface (`compute_block_digest`) covered a subset of the fields the apply path consumed. Pool-fed fields — `equivocation_events`, `abort_events`, `inbound_receipts` — were included in `signing_bytes` but not in `compute_block_digest`, so two distinct block instances could share the same digest (and therefore the same K signatures) but differ in these fields. Honest nodes applying the two instances would diverge in state for one block. Closed by F2 (consensus-layer): each committee member commits to their view of the three pool-fed fields at Phase-1 commit time via `make_contrib_commitment`'s view-root parameters; the validator enforces that the block body's `equivocation_events` / `abort_events` are a **subset** of the committee-wide `reconcile_union`, and `inbound_receipts` a subset of `reconcile_intersection`, of the K committed views (`check_eqabort_reconciliation` + `check_inbound_receipts`); and `compute_block_digest` now binds the admitted sets, so the K signatures attest to exactly them (closing the same-digest-different-fields gap). Omission of a witnessed entry stays possible — an accepted liveness relaxation (§6 F-2) — but cannot cause a *silent* cross-node divergence, because the admitted set is digest-bound. Plus apply-layer: the same state_root + producer wiring that closes D1.
...
3. The canonical event lists `(B.equivocation_events, B.abort_events, B.inbound_receipts)` (each list verified at validation-time to be a **subset** of `reconcile_union` / `reconcile_intersection` over the K committed contrib views — `check_eqabort_reconciliation` / `check_inbound_receipts` — with the admitted sets bound into the digest).
...
The F2-committed view roots enter the state through the canonical event lists (the shipped subset checks require `B.equivocation_events ⊆ reconcile_union(K contribs' view_eq_lists)` and `B.inbound_receipts ⊆ reconcile_intersection(...)`, and `compute_block_digest` binds the admitted sets into the K-of-K signature — §2 T-2/T-3/T-4, §6 F-2); the canonical event lists enter the state through Phases 3, 4, 5 of `apply_transactions`. Therefore the view roots fully determine the contribution of the three pool-fed channels to the post-apply state (modulo A2 on the Merkle root binding from member view-list to view-root).

Composing: for a fixed block `B`, its admitted event lists are pinned by the K-of-K signature over `compute_block_digest` — which on an F2-active block binds the admitted eq/abort/inbound sets (§6 F-2) — so every honest receiver applies the identical lists ⇒ identical Phase 3/4/5 mutations ⇒ identical post-state (via the per-Phase determinism arguments of FA-Apply-9 / FA-Apply-10 / FA-Apply-11). ∎

**Implementation citations.** `src/node/validator.cpp:55–57` (validate routing) + `:1461–1497` + `:1554–1629` (`check_inbound_receipts` + `check_eqabort_reconciliation` — subset admission at validate-time); `src/node/producer.cpp:778–810` (admitted-set digest binding); `src/chain/chain.cpp:828` (`apply_transactions`); `src/chain/chain.cpp:1949–1965` (S-033 gate); `MultiEventComposition.md` T-M1 (Phase ordering); `F2ViewReconciliationAnalysis.md` T-3 (order-independence) + T-5 (union censorship-resistance) + T-6 (intersection conservative).

### T-2 — F2 View Root → Inbound Receipt Subset Admission

**Statement.** For every honestly produced block `B` and every honest receiver, the `view_inbound_root` field in `B.contrib_commitments[]` cryptographically commits each contributor to a specific finite set of `CrossShardReceipt` hashes; the shipped `check_inbound_receipts` check at the receiver (`src/node/validator.cpp:1461–1497`) enforces that every receipt in `B.inbound_receipts` lies in the intersection (per F2-SPEC §Q1) of the K contributors' committed view-lists — a **subset** rule, not exact-cardinality; and the apply-layer Phase 5 loop at `src/chain/chain.cpp:1832–1850` consumes exactly the admitted set. No **extra** receipt survives (subset-checked), and no admitted receipt can be silently **stripped** after signing (digest-bound, below); but a producer *may* omit a witnessed receipt — that is an accepted liveness relaxation, not a rejected fault (see Operational consequence). Formally:

```
For all r ∈ CrossShardReceipt(B):
    r ∈ B.inbound_receipts
        ⟹ ∀ k ∈ {1, …, K} : hash_cross_shard_receipt(r) ∈ contribs[k].view_inbound_list   (subset rule — check_inbound_receipts)
        ⟹ Phase 5 of apply(S, B) applies exactly r once
            (modulo dedup against applied_inbound_receipts_ via FA-Apply-9 T-R1 / T-R2)

    The converse does NOT hold: a receipt witnessed by all K MAY be omitted from B.inbound_receipts
    (an accepted union-persistence liveness relaxation — re-proposed in a later round). The digest
    binding below fixes the ADMITTED set at signature time; it does not force maximality.
```

**Proof sketch.** The subset direction rests on `F2ViewReconciliationAnalysis.md` T-6 (IntersectionConservative): a hash appears in `reconcile_intersection` iff it appears in every member's list. The shipped `check_inbound_receipts` (`validator.cpp:1461–1497`) authenticates each per-creator carried view-list against its committed root and rejects any admitted receipt whose key is not in `reconcile_intersection` of those views (`validator.cpp:1490–1496`). Crucially it does **not** require the block to carry every intersection member — so the check bounds the admitted set from above, not below. The removal gap this leaves (a relayer stripping an admitted receipt after the K-of-K signature) is closed separately by the digest: `compute_block_digest` appends a root over the sorted admitted receipt keys (`src/node/producer.cpp:778–784`), so the Phase-2 signatures attest to the exact admitted set and a strip changes the digest. The implication from membership to Phase 5 application is the apply-path loop body at `chain.cpp:1832–1850`:
...
for (auto& r : b.inbound_receipts) {
    ...
    // ... overflow-checked credit + dedup-insert ...
    applied_inbound_receipts_.insert(key);
    block_inbound += r.amount;
}
```

Every receipt in `B.inbound_receipts` is iterated; fresh receipts are applied; duplicate receipts are silently skipped per FA-Apply-9 T-R2. The Phase 5 loop produces no extra applications (it only iterates the wire-pinned vector). "No missing applications" holds relative to the **admitted** set (every admitted receipt is processed) — it does not assert that the admitted set is the maximal intersection, since omission is permitted. The dedup-set's restore equivalence (FA-Apply-12 T-AR3) preserves the predicate across snapshot ↔ apply boundaries.
...
**Operational consequence.** A Byzantine producer who attempts to insert an "extra" inbound receipt into `B.inbound_receipts` (not in the committee-wide intersection) is rejected by `check_inbound_receipts` at every honest receiver (`validator.cpp:1490–1496`: the admitted key is not in the intersection set), blocking the block. **Omission is different.** The check is subset-only — every admitted receipt must be in the intersection, but the block need not carry every intersection member — so a producer that omits a legitimately-witnessed receipt produces a *valid* block. This is an accepted **union-persistence liveness relaxation**, not a rejected fault: the omitted receipt's `(src_shard, tx_hash)` stays unapplied in `applied_inbound_receipts_`, so it is re-proposed in a later round rather than lost. What omission cannot do is diverge *silently* across receivers — `compute_block_digest` binds a root over the admitted receipt keys (`producer.cpp:778–784`), so the K-of-K signature attests to the exact admitted set and a post-signature strip changes the digest. The F2 intersection rule still gives the **conservative-credit** posture per F2-SPEC §Q1: a single corrupt cross-shard relayer cannot unilaterally cause a credit, because inclusion requires unanimous witness.

**Implementation citations.** `F2ViewReconciliationAnalysis.md` T-6; `MakeContribCommitmentBackwardCompat.md` T-2 + T-2.1; `src/node/validator.cpp:1461–1497` (`check_inbound_receipts` — subset admission); `src/node/producer.cpp:778–784` (inbound-set digest binding); `src/chain/chain.cpp:1832–1850` (Phase 5 loop); `CrossShardReceiptDedup.md` (FA-Apply-9) T-R1 + T-R2; `AppliedReceiptRestore.md` (FA-Apply-12) T-AR1..T-AR7.
...
### T-4 — F2 Abort Root → AbortEvent Subset Admission

**Statement.** For every honestly produced block `B`, the `view_abort_root` field in `B.contrib_commitments[]` commits each contributor to a specific finite set of `AbortEvent` hashes; the shipped `check_eqabort_reconciliation` check (`src/node/validator.cpp:1554–1629`, abort dimension) enforces that every event key in `B.abort_events` lies in the union of the K contributors' committed view-lists — a **subset** rule, not exact-cardinality; and the apply-layer Phase 3 loop at `src/chain/chain.cpp:1782–1797` consumes exactly the admitted set:

```
For all ae ∈ AbortEvent(B):
    ae ∈ B.abort_events
        ⟹ ∃ k ∈ {1, …, K} : hash_abort_event(ae) ∈ contribs[k].view_abort_list   (subset rule — check_eqabort_reconciliation)
    ...
    The converse does NOT hold: an abort event witnessed by some honest member MAY be omitted
    (an accepted union-persistence liveness relaxation — re-proposed later). The digest abort-root
    binding fixes the ADMITTED set at signature time; it does not force maximality.
```
...
**Operational consequence.** Symmetric to T-3. A Byzantine producer cannot insert a spurious abort event (the subset check rejects a non-union entry; even a union-member entry is gated by the per-event V10 quorum check before apply). It *can* omit an abort event observed by an honest contributor — the subset check permits omission — but the omitted event persists in honest pools and lands in a later block. The union rule is thus an **eventual**-inclusion censorship-resistance posture for Phase-1 round-disruption evidence, with the digest abort-root (`producer.cpp:805–810`) fixing the admitted set at signature time.

**Implementation citations.** `F2ViewReconciliationAnalysis.md` T-5; `src/node/validator.cpp:1554–1629` (`check_eqabort_reconciliation` — subset admission, abort dimension); `src/node/producer.cpp:805–810` (abort-set digest binding); `src/chain/chain.cpp:1782–1797` (Phase 3 loop); `AbortEventApply.md` (FA-Apply-11) T-A1, T-A2, T-A3, T-A7, T-A8; `Censorship.md` V10 (Phase-1 abort cryptographic well-formedness, prerequisite to F2 reconciliation); `MultiEventComposition.md` T-M6 Case 1 (Phase 3 + Phase 4 cascade interaction).
...
**Operational consequence.** Any state divergence — whether caused by a node-local bug, a corrupted at-rest chain.json (S-021 detects), a Byzantine producer who lies about `state_root` (S-033 gate detects), a Byzantine producer who inserts a non-committee event/receipt (the subset checks `check_eqabort_reconciliation` / `check_inbound_receipts` detect), or a relayer who strips an admitted entry after signing (the `compute_block_digest` set-binding detects) — surfaces at the S-033 gate or at an earlier accept-path check. (Omission of a witnessed entry is *not* a divergence channel: it is an accepted liveness relaxation, and whatever set is admitted is digest-bound, so all honest receivers apply the identical set — §6 F-2.) There is no silent divergence channel left open under the F2 + S-033 + S-038 composition.
...
- Union for `equivocation_events` + `abort_events` (T-5 lift): any honest member's true observation is *admissible* — it is in the committee-wide union, so an honest assembler includes it (a Byzantine assembler may omit it, but only as a bounded liveness delay — §6 F-2 — since the observation persists and re-lands in a later block). The Byzantine member's extra entries are in the union too, so they pass the subset check `check_eqabort_reconciliation` — but each entry is cryptographically verified at V10 / V11, blocking spurious slashing evidence, and any that slips the union check is caught at the apply layer (`EquivocationSlashing.md` T-6 V11 check).
...
The K-of-K Phase-2 signature surface is computed over the admitted lists, which `compute_block_digest` binds (per F2-SPEC §Q5). Honest signers run the subset checks (`check_inbound_receipts` / `check_eqabort_reconciliation`) before signing Phase-2, so they refuse to sign a block that admits a non-committee entry or whose digest does not cover the admitted set. The Byzantine member can sign over false inputs, but that single signature is insufficient under the BFT quorum (Q signatures needed; Byzantine = 1 < Q ≤ K).
...
### A2 — Byzantine producer inserts non-committee evidence, or strips admitted evidence after signing

**Capability.** Two distinct manipulations of the pool-fed body fields, given a valid own-`view_eq_root`:

- **Insertion.** The producer puts an entry in `B.equivocation_events` (symmetrically `B.abort_events` / `B.inbound_receipts`) that is **not** in the committee-wide `reconcile_union` / `reconcile_intersection` of the K committed views.
- **Post-signature strip.** A relayer removes an admitted entry from the body *after* the K-of-K Phase-2 signatures are gathered, hoping the two versions share a digest.

(A third manipulation — the producer carrying **fewer** entries than the maximal reconciliation at assembly time — is **not** an attack under the shipped rule; see the Non-defeat note below.)

**Defeat.** Both manipulations are defeated by the shipped accept path — a **subset admission check plus a digest set-binding**, not a byte-equality reconciliation:

- *Insertion* is rejected by `check_inbound_receipts` (`src/node/validator.cpp:1461–1497`) and `check_eqabort_reconciliation` (`:1554–1629`), routed from `BlockValidator::validate` at `validator.cpp:55–57`. Each authenticates the per-creator carried view-lists against their committed roots and requires every *admitted* eq/abort/inbound key to lie in `reconcile_union` / `reconcile_intersection` of those views (`:1490–1496`, `:1607–1610`). A non-committee entry fails at every honest receiver.
- *Post-signature strip* is defeated by `compute_block_digest`, which binds a root over the admitted inbound / eq / abort sets, each behind a non-zero-root gate (`src/node/producer.cpp:778–810`). The Phase-2 signatures attest to the exact admitted set, so a strip changes the digest and the signatures no longer verify.

**Non-defeat (by design).** The subset check is **not** exact-cardinality (`validator.cpp:1563–1568` documents the decision — the event hashes carry observer-dependent forensic fields no single assembler can fully materialize, so requiring maximal cardinality would stall). A producer that omits a witnessed event/receipt therefore produces a *valid* block. This is an accepted **union-persistence liveness relaxation**, not a defeated attack: the omitted evidence survives in honest pools and is re-proposed in a later block, so the F2 guarantee for the pool-fed fields is *eventual* inclusion, not same-block inclusion. Earlier revisions of this proof claimed omission "fails V25/V26"; that byte-equality check (`validate_view_reconciliation`, `producer.cpp:638–676`) is **not on any accept path** — see §6 F-2.

**Load-bearing gate.** The load-bearing gate for F2's *removal-resistance* is therefore the pair (subset check + digest set-binding). Without the subset check a non-committee entry would pass; without the digest binding a post-signature strip would go undetected. The Phase-1 contributors' committed view-roots are bound by their Ed25519 signatures (A1 EUF-CMA), and the subset check is over those bound view-lists, not over advisory metadata. The *omission* direction is intentionally left open (liveness).

**Cite-points.** `src/node/validator.cpp:55–57` + `:1461–1497` + `:1554–1629` (shipped subset admission); `src/node/producer.cpp:778–810` (admitted-set digest binding); `F2ViewReconciliationAnalysis.md` §5.2 (validator-pass composition); `F2-SPEC.md` §Q5 (Phase-2 signature over reconciled lists); `MakeContribCommitmentBackwardCompat.md` T-2 (commitment is binding); §6 F-2 (the dead byte-equality helper).
...
**Consequence for this composition.** The validator's `reconcile_union` / `reconcile_intersection` computation produces a byte-identical committee-wide set across all honest receivers, regardless of which committee member assembled the block. So the *bound* each honest receiver enforces on `B.equivocation_events` etc. — the admitted list must be a subset of that set — is unambiguous; the assembler chooses which admissible subset to carry, and `compute_block_digest` binds that choice.
...
### F-2 — The removal-resistance gate is the subset check + digest set-binding (not a byte-equality reconciliation)

**Observation.** F2's commitment binding (`MakeContribCommitmentBackwardCompat.md` T-2) is cryptographically sound — Phase-1 commit signatures bind each member to a specific view via the Merkle root — but the commitment alone is advisory at the apply layer. Two shipped mechanisms make it binding:

1. **Subset admission.** `BlockValidator::validate` (`src/node/validator.cpp:55–57`) routes the three pool-fed fields through `check_inbound_receipts` (`:1461–1497`) and `check_eqabort_reconciliation` (`:1554–1629`). Each authenticates the per-creator carried view-lists against their committed roots and enforces that every *admitted* eq/abort/inbound key lies in `reconcile_union` / `reconcile_intersection` over those views (`:1490–1496`, `:1607–1610`). This is a **subset** rule, explicitly *not* exact-cardinality (`validator.cpp:1563–1568`): a non-committee entry is rejected, but a producer may carry fewer than the maximal reconciled set.
2. **Digest set-binding.** `compute_block_digest` binds a root over the admitted inbound / eq / abort sets, each behind a non-zero-root gate (`src/node/producer.cpp:778–810`), so the K-of-K Phase-2 signature attests to the exact admitted set and a post-signature strip changes the digest.

**A byte-equality reconciliation exists but is dead code.** `validate_view_reconciliation` (`src/node/producer.cpp:638–676`) *does* compare the block's lists to `derive_canonical_view_lists` for byte-equality (the historical "V25 + V26"), but it is **not on any accept path** — its only caller is the in-process unit test at `src/main.cpp:19489` (confirmed by call-site search; `EqAbortViewDigestExtension.md` also records that `validate_view_reconciliation` "exists but is **not called** anywhere in the validator path"). Earlier revisions of this proof (and of §2 T-2/T-3/T-4 and §3 A2, now corrected) attributed the load-bearing role to that equality check; the shipped enforcement is the subset check + digest binding above.

**Composition impact — omission is a liveness relaxation, not a fault.** Because admission is subset-only, a producer that *omits* a witnessed event or receipt produces a valid block. This is intentional **union-persistence**: the omitted evidence survives in honest pools and is re-proposed in a later block, so the F2 guarantee for the pool-fed fields is *eventual* inclusion. What is closed is the *removal* gap (S-030 D2): a non-committee insertion is rejected, and a post-signature strip is caught by the digest. A2 in §3 hinges on this pair, not on a byte-equality check. CI-gated by `tools/test_view_root.sh` per `F2ViewReconciliationAnalysis.md` §6.

**Recommended discipline.** Any future refactoring of the validator-side accept path must preserve, as a per-block check, both (a) the subset authentication in `check_inbound_receipts` + `check_eqabort_reconciliation` and (b) the eq/abort/inbound digest binding in `compute_block_digest`. If a future mode wants *maximal*-set (exact-cardinality) admission, that is a new consensus rule — it is not what ships today. Same maintenance-contract discipline as the S-033 gate's "post-restore" placement (per `S012SnapshotStateRootGate.md` §1.3).
...
- `src/node/producer.cpp:757–899` — `compute_block_digest`; the admitted inbound/eq/abort set-binding is `:778–810` (primary cite for F-2 removal-resistance).
