...
| **D3.3b-read EXTENDED gate** | **The production sign-off** (PENDING — the only thing between the shipped pin and EXTENDED going live). A block-level acceptance test (a drifted-frozen creator's block passes `check_creator_selection`+`check_creators_registered`+`check_block_sigs`+`check_abort_certs`; the pool-only variant REJECTS) + a genesis fixture audit + a **LIVE EXTENDED cluster** (≥3 region-pinned shards, small `epoch_blocks`, mid-epoch registrant/stake/abort/key-rotation churn → single head, drifted-but-frozen honest member keeps participating) + EXTENDED-golden re-bless. Shared prerequisite with D3.5 (beacon emission). | The full §6 gate: SINGLE/CURRENT goldens byte-identical + FAST both platforms + the LIVE EXTENDED cluster + a re-bless of only the drift-exhibiting EXTENDED goldens, before EXTENDED is production-declared. |
