...
| **D3.6** ✅ SHIPPED | **MERGE_EVENT fail-closed on non-BEACON chains + the BEACON historical-witness gate.** A design pass + Fable-5 + adversarial critic found the naive "EXTENDED-only" gate is WRONG: `t:` records are BEACON-only state, but MERGE_EVENT is submitted to a SHARD (apply gates `shard_count()>1`, not role) → a shard-scoped witness can never run and a fabricated-distress MERGE_BEGIN dilutes the partner committee UNCHECKED (the reachable S-036 exploit; the beacon witness path has NO emitter). **OWNER chose fail-close (Fable-5 rec):** the validator + mempool-admission gate reject MERGE_EVENT on every non-BEACON chain (reconciles code to the long-standing block.hpp "valid only under BEACON chain role" doc; `set_chain_role` threaded into BlockValidator; the mempool reject stops a queued tx stalling the producer). On a BEACON (BEGIN-only) the witness loop reads committed `t:` over `[evidence_window_start, +merge_threshold_blocks)` — SOURCE-shard-height indexed (the old `b.index` bound was a source-vs-beacon axis bug, now removed) — accept only on contiguous sub-`2K` coverage; **uniform fail-closed on any absent in-window record** (`A_beacon_omit`), the pre-ring case, `threshold==0`, and u64 overflow. Under-quorum merge is DORMANT until a beacon merge flow exists. **S-036: the reachable shard exploit is fail-closed; the beacon witness path + trustless closure are Layer 2 (D3.5e, owner-gated) — S-036 stays STRONGLY MITIGATED, NOT closed.** | `test_under_quorum_merge.sh` rewritten: a shard MERGE_BEGIN never populates `merge_state_` + the chain does not stall (LIVE, green both platforms). FAST 226/0 both platforms. D3.7 = the beacon-path in-process falsifier (accept genuine / reject fabricated). |
