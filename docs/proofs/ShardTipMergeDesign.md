...
| **D3.3b-read STEP 3 + gate** | **Node gossip-identity pin + the EXTENDED production gate** (PENDING). STEP 3: the 5 node.cpp gossip-identity sites (`on_contrib` 2462, `on_block_sig_locked` 2627, `on_abort_claim` 1468, `on_abort_event` 1564 + `on_equivocation_evidence`) still resolve signers/claimers present-head — a frozen-but-drifted honest member is spuriously aborted per round until the epoch ends (bounded liveness, all nodes drop identically ⇒ **no fork**; the review REFUTED it as a defect). `on_shard_tip` (node.cpp:1747) beacon re-derivation DEFERS to D3.4/D3.5 (the beacon holds the wrong checkpoints — RP-3). | STEP 3 wiring + `test-committee-pin` extended to block level (a drifted-frozen creator's block passes `check_creator_selection`+`check_creators_registered`+`check_block_sigs`+`check_abort_certs`; pool-only REJECTS) + fixture audit + a **LIVE EXTENDED cluster** (≥3 region-pinned shards, mid-epoch registrant/stake/abort/key-rotation churn → single head, drifted-but-frozen honest member keeps participating) + EXTENDED-golden re-bless, before EXTENDED is production-declared. |
