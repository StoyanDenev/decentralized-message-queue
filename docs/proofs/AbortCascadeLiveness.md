...
This document is the formal analysis of the two OPEN liveness findings registered in `docs/SECURITY.md` §S-044 (K=2 committees wedge permanently under timing skew — High) and §S-045 (BFT escalation unreachable under multi-member abort exclusion — Medium). It derives the wedge conditions mathematically from the shipped code (T-1, T-2), proves the small-committee corollaries (T-3, T-4), tabulates the margins for every shipped profile (T-5), and analyzes the safety interaction of each candidate fix — (F-a) abort-claim quorum floor, (F-b) abort decay, (F-c) formation-failure counting — against the existing proof corpus (FA1 `Safety.md`, FA4 `Liveness.md`, FA5 `BFTSafety.md`, FA6 `EquivocationSlashing.md`, the FA-Apply abort pipeline `AbortEventApply.md`, and the trigger-soundness composition `S025BFTEscalationSoundness.md`).

**This is decision support, not a closure proof.** S-044 and S-045 are OPEN; none of the candidate fixes is shipped; each is consensus-critical and requires explicit authorization. The analysis here exists so that the authorization decision can be made against derived bounds rather than against the empirical reproductions alone.
...
This return is **silent and terminal for the attempt**: no timer is armed, no message is sent, no state changes. `check_if_selected` is the *only* caller of `start_contrib_phase` (`src/node/node.cpp:814` → `:822`), and it is itself invoked only on discrete events: boot grace (`:615`), abort-quorum formation (`:1309`), gossiped-abort adoption (`:1362`), block accept (`:1945`), and sync catch-up (`:2445`). There is **no retry timer**. The genesis default for the threshold is `5` (`include/determ/chain/genesis.hpp:145`, loader `src/chain/genesis.cpp:173`, node-config mirror `src/node/node.cpp:88`).
...
`tools/test_bft_escalation.sh` (M=K=3, kill 1 of 3) demonstrates the single-dead-member escalation working end-to-end: MD aborts against the dead node, escalation to BFT 2-of-3 with designated proposer, sustained BFT block production. Per T-4 this is sound **and** threshold-dependent: the test's genesis pins `bft_escalation_threshold: 1` (`tools/test_bft_escalation.sh:84`), the only value ≤ the deterministic counter floor for that topology. It validates the trigger and the BFT round; it does not validate reachability at the genesis default `θ = 5` (T-4, T-2.1).

### 5.3 Operational containment (in force while OPEN)

- **Deploy K ≥ 3.** At K ≥ 3 the claim quorum is ≥ 2 — a single straggle observation excludes nobody. K=3-of-4 and K=3-of-5 run green under identical conditions, including kill-one-creator recovery (`tools/test_hybrid_liveness.sh`).
- The two K=2-intent cluster tests carry loud `KNOWN-BUG S-044` notes instead of production bars (§5.1) — restore those bars on closure.
- Provision pools above nominal: `N ≥ θ + k_bft` puts single-member faults outside the frozen-counter regime entirely (T-2.1); independently, operators may genesis-tune `bft_escalation_threshold` down toward the T-5 table's reachable region for their topology, accepting S025's gate-2 trade (faster escalation = more FA5-conditional blocks).
...
- **S-044, S-045: OPEN** (`docs/SECURITY.md` §S-044 High, §S-045 Medium). This document is decision support for the consensus-change authorization; it ships **no** code change.
- Candidate-fix verdicts (this analysis): **F-a** — sound; provably a no-op for K ≥ 3; at K=2 trades wedge-by-cascade for the strictly narrower halt-by-single-death (§4.1); closes the S-044 mechanism only. **F-b** — unsound as wall-clock decay (consensus non-determinism, §4.2); reject in proposed form. **F-c** — sound for S-045 *only with* a verifiable formation-failure certificate carrying a `k_bft` signature floor (§4.3); without it, it re-opens S025's `A_premature` forced-escalation attack; does not address K=2.
