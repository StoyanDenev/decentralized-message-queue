...
Two structural consequences used repeatedly below: **(C1)** because each surviving committee member assembles its *own* `AbortEvent` with its *own* local timestamp, two assemblies of the same logical abort yield *distinct* `event_hash` values iff their second-granularity timestamps (or prior chains) differ — and then cross-adoption appends *both*, so one aborted round can contribute up to `k_use − 1` counter increments while excluding only **one** distinct member; **(C2)** if the survivors assemble within the same second from the same chain state, the events collide on `event_hash` and dedup to one. The counter's growth beyond `|E|` is therefore *timing-dependent* (finding F-2). **Update 2026-09-15 (S-074):** the assembler now uses the PARENT BLOCK's timestamp, not its clock, and every consumer recomputes `event_hash`, so two assemblies of the same logical abort from the same chain state always collide and dedup — C1's distinct-hash case no longer occurs and `μ_r = 1` for honest assemblers (the C2 deterministic case is the only honest case); a replay of an already-excluded member's public claims as a canonically chained second event — which would have inflated the counter and produced a tail no block can carry — is dropped at adoption because the accused must be in the current committee (the same rule V10 step 2 applies). The escalation-reachability arithmetic below is therefore Corollary T-2.1's pure-distinct climb. A further consequence: on the concurrent-quorum wedge (two survivors quorum against different members) the foreign first-form event is now dropped instead of appended, so the tail size stays immobile and the S-050 valve trips at its 5 s soft window rather than drifting toward the 30 s hard window. At the shipped default `bft_escalation_threshold = 1` (the S-045 fix) reachability is unchanged; the historical `θ = 5` readings in T-2/T-4/T-5 that leaned on the duplicate-adoption surplus no longer apply.

### 1.4 The clear site and the generation gate

- `current_aborts_.clear()` has **two** call sites, both **full-scratch resets** (never a per-entry age decay): (1) block accept, inside `post_append_bookkeeping_locked` (`src/node/node.cpp:2454`, factored verbatim out of `apply_block_locked`, immediately after the registry rebuild at `:2453`); and (2) the **S-050 round-stall valve** `maybe_stall_reset_locked` (`:1657`), added after this analysis by commit `48bc54f` (2026-07-15), which fires only on a wall-clock stall (no block applied for the soft/hard window) and re-enters selection from canonical post-head state. The valve clear is **not** the F-b decay rejected in §4.2 — it is a one-shot full reset on an already-stalled round, coupled to an explicit re-sync (STATUS_REQUEST + `stalled_resync_` + the S-047 relay), and it is proven safety-neutral in `RoundStallValveSoundness.md` (Claim C-1: a block's abort events are validated *from the block itself*, so the cleared in-memory scratch is never a validation input; the automated per-node analogue of the §5.4 coordinated-restart escape). `reset_round` (`:2353`) clears the per-round buffers and returns the phase to `IDLE` but touches neither `current_aborts_` nor `current_creator_domains_`. No per-entry decay, no expiry, no per-entry timeout — the two clears are the only removals. (The state is in-memory only: a process restart zeroes it — see §5.4.)
...
**Interaction with FA6 / suspension — the safe part.** Chain-baked consequences are untouched by construction: suspension derives from `chain.abort_records()` via the registry build (`src/node/registry.cpp:40-64`, Phase-1-only, S-032 cache; `AbortEventApply.md` T-A3), i.e. from abort events already **applied in finalized blocks**, a different data structure from the in-memory pre-finalize list. The equivocation evidence channel (FA6) keys off `EquivocationEvent`s, untouched entirely — and since D4 (2026-09-16) it carries no L1 consequence at all, so it can neither be weakened nor strengthened by anything here. Decay of the in-memory list erases neither. The one apply-side consequence: a decayed event never reaches `b.abort_events`, so its `abort_records_` increment — and hence the S-032 suspension window — never happens; the T-A1 proportional Phase-1 slash it used to also lose was RETIRED by D13 (2026-09-16) and no longer exists to lose. The deterrent that weakens by exactly the decayed entries is therefore the suspension window alone. Tolerable, but it must be stated.
...
**Boundary vs. the shipped S-050 stall valve (not F-b).** The later S-050 valve (`Node::maybe_stall_reset_locked`, `48bc54f`; `RoundStallValveSoundness.md`) *does* clear `current_aborts_` on a wall-clock window, so it is worth stating explicitly why it does **not** re-import F-b's disqualifier. Three differences are decisive. (i) It is a **full** scratch reset to the empty tail, not a per-entry age expiry — there is no "expiry boundary" for two peers to straddle; a reset node lands at `aborts_gen = 0`, the same clean state as a coordinated restart (§5.4) or a fresh node. (ii) It fires **only when the round is already stalled** (no block for the soft/hard window), so there is no in-flight block whose committee derivation could diverge from a validator's — and blocks self-certify their abort events regardless (`RoundStallValveSoundness.md` C-1), so a local scratch reset cannot change any block-validity verdict. (iii) It **carries the convergence mechanism this paragraph says F-b lacks**: on firing it broadcasts `STATUS_REQUEST`, arms `stalled_resync_`, and re-adopts a peer's events through the S-047 relay in that peer's chain order — the automated per-node form of the §5.4 escape. F-b was continuous partial decay biasing *live* consensus with no way to re-anchor; the valve is a one-shot full reset on a *dead* round with an explicit re-anchor. The two are not the same mechanism, and the §4.2 rejection of F-b stands unchanged.
...
**Safety check against FA5.** `BFTSafety.md` T-5's bound is conditional only on (A1), (A2), (B1) `f_h < |K_h|/3` *within the escalated committee*, and (B2′) honest single-sign at the height (the former (B2) "FA6 slashing enforced" was deleted in step 3c — the proof never consumed it); the proof (L-5.1 intersection `≥ 2Q − |K_h|`, L-5.2 honest-intersection) consumes the committee size `|K_h| = k_bft` and the V8 quorum `Q` — **it nowhere consumes the reason escalation was triggered.** Escalating more often therefore widens the set of blocks whose safety is conditional (the FA5-vs-FA1 trade of BFTSafety §5.1: more blocks rely on B1, fewer on FA1's unconditional clause) but does not weaken the bound on any individual BFT block: at the escalation point the committee is still `k_bft = ⌈2K/3⌉` drawn from the abort-adjusted pool and `Q = ⌈2·k_bft/3⌉` still intersects per L-5.1. **Corrected 2026-09-17 (step 3c):** T-5.1's slashing recovery does NOT backstop B1 violations any more — it was WITHDRAWN when D4 removed the consequence (`BFTSafety.md` §4, T-5.1-R). A B1 violation now yields exactly classical BFT failure with an evidence record and no recovery, so widening the set of blocks whose safety is conditional on B1 is a strictly larger exposure than this paragraph assumed. T-5 itself (the L-5.1/L-5.2 counting argument) never consumed B2 and is unaffected; F-c's effect on the trigger is unchanged. The FA4 L-4.3 trigger condition changes (gate 2's input set grows) while its finalize condition is untouched. This is the same conclusion S025BFTEscalationSoundness reaches for legitimate threshold *tuning* — F-c is, in effect, deterministic auto-tuning of gate 2's patience in the regime where the original counter is provably starved (T-2).
...
| Clear sites (on-accept + S-050 valve); post-accept reselect | `src/node/node.cpp:2454` (on-accept, `post_append_bookkeeping_locked`) + `:1657` (S-050 valve, `maybe_stall_reset_locked`; safety-neutral scratch reset, `RoundStallValveSoundness.md` C-1); reselect `:2560`; `reset_round` `:2353` |
...
- Companion TLA+ state machine: `tla/AbortEscalation.tla` (TLC-checked **green** 2026-07-02 on the shipped-fix posture — all 5 invariants + the progress property; results in `tla/CHECK-RESULTS.md`). It models §1's state machine (pool / exclusion set / counter / four gates / claim quorum / generation gate) and is expected to pin T-1–T-4 as invariants plus the wedge as a reachable deadlock state.
