...
- **Non-claims.** SHIELD is the DEPOSIT half only — no CONFIDENTIAL_TRANSFER / UNSHIELD / nullifier yet; the SHIELD **amount is PUBLIC** (SHIELD hides nothing by itself — amount privacy begins only on the not-yet-shipped confidential→confidential transfer); no sender/receiver/graph privacy; not post-quantum; single-shard / P-256 profile.
