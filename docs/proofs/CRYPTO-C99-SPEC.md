...
- **PERMANENCE**: the formula is permanent once accounts exist (no-migrations); any change is a `-v2` DST, never an in-place edit.
- **On-chain publication (SHIPPED, NC-8 inc.5a)**: so a third-party sender can *find* a recipient's `note_pk`, it is published by the `REGISTER_NOTE_KEY` (`TxType = 17`) consensus tx into a conditional `nk:` + addr state leaf (`SHA256(note_pk)`, emitted only while set → byte-neutral when unused). Owner-decided **Option (a)** (dedicated tx + leaf) — the only option that reaches anonymous/bearer payees; `REGISTER` is validator-only and address-derivation is infeasible without a new hardness assumption. Consensus-inert (the point is never decompressed/validated). Gate `test-register-note-key`. See `EncryptedNoteDeliveryDesign.md` §5.5. The trustless READ path (an `nk` `rpc_state_proof` branch + a light `verify-notekey` command) is the owner-gated follow-on increment (inc.5b).
