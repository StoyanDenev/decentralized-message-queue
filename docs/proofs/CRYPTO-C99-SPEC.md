...
- **inc.4 — the on-chain `PQ_TRANSFER` accept-rule.** Owner chose **Option B (PQ-native bearer address)**: `address = "0x"‖hex(form)‖hex(ML-DSA pubkey)`, form 0x01/02/03 = ML-DSA-44/65/87 (`determ::pq_address`; length 2628/3908/5188 — disjoint from the 66-char Ed25519 anon address, so `is_anon_address` is untouched). New `TxType::PQ_TRANSFER` + a `Transaction.pq_auth` field **excluded from `signing_bytes` and serialized only when non-empty**, so every existing tx type is byte-identical → a PQ-free chain's state root is unchanged (FAST golden vectors green). The shared accept-rule `determ::chain::verify_pq_transaction` (S-043 one-helper; called by BOTH the block validator and mempool admission) recovers the ML-DSA key from `from`, verifies the PQ-ONLY DPQ1 envelope over `signing_bytes`, and **requires `envelope.pq_pk == address key`** — the quantum-resistance binding (a quantum adversary cannot substitute its own key). Apply/build-body treat `PQ_TRANSFER` as `TRANSFER`. Gated by `test-pq-transaction` (address round-trip + accept + 6 fail-closed rejections + serialization byte-identity). **Follow-ups (liveness, not safety): client-side production of a submittable `PQ_TRANSFER` (derive the PQ `from`-address) + a mempool-policy audit for the new type.**
