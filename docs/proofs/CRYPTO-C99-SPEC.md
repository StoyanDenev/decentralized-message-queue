...
> **NOTICE 2026-06-07 — FROST removed from v1.1 chain consensus path.** Per `FROST_DEVIATION_NOTICE.md`, FROST was identified as a Claude-introduced design deviation, not part of Stoyan Denev's original Determ design. The §3.8 FROST-Ed25519 implementation under `src/crypto/frost/` was retained as a library after that decision and was subsequently **DELETED from the tree 2026-07-09** (pre-launch register B2, `PRE-LAUNCH-DECISIONS.md`; NOTICE §8 — git history + the audit/soundness docs are the design record). It was NOT part of the v1.1 consensus path, NOT part of the v1.1-locked formal-verification surface, and any re-introduction into chain consensus requires Stoyan's explicit sign-off per `FROST_DEVIATION_NOTICE.md §3`. All §3.1–§3.7 primitives (SHA-2, HMAC, HKDF, PBKDF2, ChaCha20-Poly1305, AES-256-GCM, Ed25519, X25519, BLAKE2b) remain in v1.1 scope; only §3.8 FROST is excluded from the consensus-path commitment.
...
Out of scope:
- Higher-level protocol design (handled by F2-SPEC, v2.22-PRIVACY-SPEC, etc.)
...
**Why NIST P-256 is also in the stack (for FIPS profile).** NIST P-256 IS a third curve — added because both the `tactical` and `cluster` profiles bundle FIPS-compliant cryptography (per §2.Q10), and secp256k1 is not in NIST's FIPS-validated curve list. P-256 is FIPS 186-5 validated.

> **SUPERSEDED (2026-07-07, owner — `DECISION-LOG.md` D3):** the secp256k1 selection above (this row + the "why not single-family" rationale) is the HISTORICAL plan-time deliberation. secp256k1 was **never built**; the owner rejected adding it — P-256 is the prime-order group for BOTH profiles (profile-agnostic, see the §"FIPS crypto mapping" table below), and MODERN = Ed25519 + X25519 + P-256 with no new crypto. Retained here as the design record only.
...
| Theme 9 DSSO OPRF (v2.25) | **NIST P-256 T-OPRF — profile-agnostic** (threshold-OPAQUE: t-of-n over the RFC 9497 P256-SHA256 VOPRF, §3.9b; `v2.25-DSSO-DAPP-SPEC.md`, DECISION-LOG 2026-07-15) | **NIST P-256 T-OPRF (same — profile-agnostic)** |
...
`ed25519_group.h` that FROST built on (FROST module REMOVED 2026-07-09, register B2). Also shipped:
...
> again to profile-agnostic P-256 threshold-OPAQUE 2026-07-15 — still no secp256k1
> consumer). Vendoring
...
### 3.8 FROST-Ed25519 from RFC 9591 — **REMOVED (2026-07-09)** (was FROZEN 2026-07-03; deleted from the tree per pre-launch register B2)

> **REMOVED (2026-07-09, pre-launch register B2 — authority: Stoyan Denev, `PRE-LAUNCH-DECISIONS.md`; `FROST_DEVIATION_NOTICE.md` §8).** The frozen module and its test surfaces were deleted from the tree; git history + `C99CryptoStackAudit.md` §7/§8 are the retained design record (`FrostThresholdSoundness.md` was subsequently deleted 2026-07-09, doc-consolidation inc.1 — `FROST_DEVIATION_NOTICE.md` §9). The bullets below record the shipped scope pre-removal.
...
- Proactive Secret Sharing (PSS) refresh extension — **SHIPPED**: `determ_frost_pss_commit`
  ...
  §6 (proof: `FrostThresholdSoundness.md` T-6 — doc deleted 2026-07-09, git history)
...
**Consumer (2026-07-15): DSSO threshold-OPAQUE.** The DSSO DApp's t-of-n T-OPRF
...
- `tools/vectors/<primitive>.json` — 17 files / 129 vectors for the shipped
  ...
  SHA-256 pinned in each `source` field; and formerly frost_ed25519_rfc9591.json —
  the RFC 9591 E.1 FROST(Ed25519, SHA-512) 2-of-3 signing vector, removed
  with the FROST module 2026-07-09, register B2). Mixed provenance is declared per-file in each `source` field
...
- **Increment 7 — confidential-tx balance proof (`src/crypto/pedersen/balance.c`):**
  the *amount-conservation* half of a confidential transaction (the inc.5/6 range proofs are
...
- **Increment 8 — end-to-end confidential-tx composition (`test-p256-confidential-tx-c99`,
  a structural test — NOT a new primitive):** composes the two shipped halves into one confidential
...
- **Validation:** `determ test-pedersen-c99` (14 assertions — inc.1: H KAT +
  ...
  proof + **balance proof**) — the P-256 confidential-tx primitive set (range proofs
  for no-inflation + a balance proof for amount conservation) is available under BOTH
  the FIPS and MODERN profiles. Next: chain integration
...
### 3.20 Finite-field Bulletproofs stack over Z_p* — **REMOVED (2026-07-07)**

The MODERN-profile large-prime (`Z_p*`, RFC 3526 MODP-3072) confidential-tx backend
(finite-field Pedersen / IPA / range proofs / balance proof / end-to-end composition) was
**removed 2026-07-07** by owner decision (see `DECISION-LOG.md`). Rationale: the
MODERN-profile curve objection was to **secp256k1 specifically** (a Koblitz curve), never
to prime-order groups in general — Ed25519 / X25519 / P-256 are unobjectionable — so a
separate large-prime ZK backend solved a non-problem. **MODERN now reuses the
profile-agnostic §3.22 P-256 shielded pool** for confidential transactions and adds NO
separate ZK curve. The §3.19 P-256 range-proof / balance / composition stack (available
under both profiles) is unaffected; this number slot is retained (not renumbered) so
downstream §3.21–§3.23 references stay stable.

### 3.21 DPQ1 post-quantum transaction authentication (`determ::pqauth`) + on-chain `PQ_TRANSFER` — **SHIPPED (increments 1-4)**

The owner-authorized on-chain post-quantum **signature** track (distinct from the §3.19/§3.22 confidential-tx track). Reopens the 2026-07-04 anon-address freeze (`AnonAddressDerivationMigration`) by owner authority; ships as an **additive, state-root-invariant** extension. Full accounting: [`PQSignatureEnvelopeSoundness.md`](PQSignatureEnvelopeSoundness.md) (PQE-1..5, non-claims, the §Trust-Root binding).
...
- **inc.4 — the on-chain `PQ_TRANSFER` accept-rule.** **A5 (owner GO 2026-07-09) — the PQ address is the HASH form (Option A), frozen at genesis** (this replaced the originally-shipped Option-B bearer; see the A5 note below): `address = "0x" + hex(SHA256(u8(len(DST)) || DST || u8(form) || u32_be(len(pk)) || pk))`, DST `determ-pq-anon-addr-v1`, form 0x01/02/03 = ML-DSA-44/65/87 (`determ::pq_address`). It is a 66-char commitment — the SAME shape as the Ed25519 anon address (the two families share the string space and are separated by TX TYPE at the router, never by shape). New `TxType::PQ_TRANSFER` + a `Transaction.pq_auth` field **excluded from `signing_bytes` and serialized only when non-empty**, so every existing tx type is byte-identical → a PQ-free chain's state root is unchanged (FAST golden vectors green). The shared accept-rule `determ::chain::verify_pq_transaction` (S-043 one-helper; called by BOTH the block validator and mempool admission) reads the ML-DSA key + scheme from the CARRIED DPQ1 envelope (the hash address is non-invertible), maps scheme→form, verifies the PQ-ONLY envelope over `signing_bytes`, and **requires `make_pq_anon_address(form, envelope.pq_pk) == from`** — the quantum-resistance binding recomputed as a commitment check (substituting a different key changes the commitment; matching a given address needs a 2^256 preimage). Apply/build-body treat `PQ_TRANSFER` as `TRANSFER`. Gated by `test-pq-transaction` (shape + collision-resistance + scheme→form + accept + fail-closed rejections + the DUAL-ORACLE corpus cross-check vs `tools/vectors/pq_address.json`).

  > **A5 bearer→hash swap (owner GO 2026-07-09).** register A5 directed the hash-based Option-A PQ address; the code had shipped Option-B bearer (`0x‖hex(form)‖hex(pk)`, invertible, 2628/3908/5188 chars, length-disjoint from Ed25519). The swap: `make_pq_anon_address` → hash; `parse_pq_anon_pubkey` + `pq_anon_address_form` REMOVED (a hash is non-invertible); `verify_pq_transaction` reroutes through the envelope's carried `pq_pk` + scheme→form and a recompute-and-compare. The shape-disjointness invariant is intentionally gone; the two 66-char families coexist safely because spending via the wrong mechanism needs an Ed25519 privkey for a hash (unknown) or an ML-DSA preimage of a pubkey (2^256). Full gate (FAST 207 both platforms + PQ e2e + dual-oracle) + adversarial review.
- **inc.6 — the client→consensus loop closer.** `determ-light pq-address` (derive the PQ-native HASH address from a seed — A5) + `pq-transfer` (build a canonical, SUBMITTABLE `PQ_TRANSFER`: derives `from`, signs the PQ-only DPQ1 envelope over `signing_bytes`, emits a `from_json`-compatible tx with a 64-zero-byte `sig` placeholder). `determ verify-pq-tx --file <tx.json>` applies the SHARED consensus accept-rule to a tx file (exit 0 verified / 3 invalid) — a validator would accept exactly what this accepts. Cross-binary gate `tools/test_pq_transfer_e2e.sh` (light produces → full binary's consensus rule verifies; tamper → INVALID; hybrid refused).
...
### 3.23 LSAG linkable ring signature over P-256 (`determ::ringsig`) — **REMOVED (2026-07-09)** (was SHIPPED — input-unlinkability inc.1, library primitive)

> **REMOVED (2026-07-09, pre-launch register B2, jointly A7 — `PRE-LAUNCH-DECISIONS.md`).** The whole `ringsig` library (§3.23/§3.23b/§3.23c: LSAG, CLSAG, RingCT spend composition) and its test surfaces/vectors/oracles were deleted from the tree; git history + the three soundness docs are the retained design record; re-adding is a normal reviewed feature. The §3.23 family entries below record the shipped design pre-removal.
...
### 3.23b CLSAG concise linkable ring signature over P-256 (`determ::ringsig`) — **REMOVED (2026-07-09)** (was SHIPPED — input-unlinkability inc.2, library primitive; see the §3.23 removal note)
...
### 3.23c RingCT spend-statement composition over P-256 (`determ::ringsig`) — **REMOVED (2026-07-09)** (was SHIPPED — input-unlinkability inc.3, LIBRARY composition; see the §3.23 removal note)

The **input-unlinkability inc.3** — the increment that shows the shipped privacy layers **compose** into one end-to-end confidential + unlinkable spend proof, with ZERO consensus touch. Full accounting: [`RingCTSpendCompositionSoundness.md`](RingCTSpendCompositionSoundness.md) (RCS-1..6).

- **The convention gap it reconciles**: CLSAG is **amount-on-H** (its balance `C_l − Coffset = z·G` means "same amount" only because the amount rides `H`); the §3.19/§3.22c range+balance stack is **amount-on-G** (`C = v·G + r·H`). Composing them needs a bridge, not a relabel.
- **The commitment-transposition proof** (`determ_commit_transpose_prove/_verify`, the ONLY new crypto): a Schnorr AND-proof with a **shared value response** `sv` certifying `C_H = v·H + a·G` (value-on-H) and `C_G = v·G + b·H` (value-on-G) hide the SAME amount `v`. Verify accepts iff BOTH `sv·H + sa·G == A_H + c·C_H` and `sv·G + sb·H == A_G + c·C_G`; the shared `sv` is what binds the two amounts. Wire (162 B): `A_H(33) ‖ A_G(33) ‖ sv(32) ‖ sa(32) ‖ sb(32)`. Built over the P-256 + §3.19 `pedersen_generator_h` APIs — NO new hardness assumption (soundness = ECDLP + ROM).
- **The spend verifier** `determ_ringct_spend_verify` composes: (1) §3.23b CLSAG (input membership + `Coffset_H` commits to the input amount + nullifier `I`); (2) parse the §3.22c DCT1 bundle with `n_in == 1`, extract `Coffset_G = C_in[0]`; (3) the transpose bridges `Coffset_H ↔ Coffset_G`; (4) the DCT1 bundle (range + balance `Coffset_G = Σ outputs + fee`). Returns 0 iff all four hold — a hidden ring member's amount flows through the transposed pseudo-out to hidden, in-range, conserved outputs.
- **Dual-oracle**: `test-ringct-spend-c99` (C — the transpose proof prove/verify + a wrong-amount reject + the full spend accept + any-layer-tamper/wrong-pseudo-out/wrong-message reject) + `tools/verify_ringct_spend.py` (INDEPENDENT python reproducing the transpose proof byte-for-byte into `tools/vectors/ringct_spend.json`). Deterministic.
- **NOT a consensus feature.** LIBRARY composition — no on-chain nullifier set / pool / ring-selection; a PARTIAL tx (input membership+balance + output range+balance), not a whole one; single-input demo (`n_in==1`); NOT constant-time; NOT post-quantum. The consensus wiring is the owner-gated step.

### 3.24 A1 per-epoch view-key derivation (`determ_view_key_derive`) — **SHIPPED (pre-launch register A1, Option C ratified 2026-07-09)**

The CT-layer per-epoch view key: `vk_epoch_n = HKDF-SHA256(salt="determ-view-key-v1", IKM=view_master_sk (32 B), info=len64_be(chain_id)‖chain_id‖len64_be(addr)‖addr‖epoch_u64_be, L=32)` — a thin composition over the §3.1 HKDF, no new cryptographic assumption. Files: `src/crypto/viewkey/viewkey.c` + `include/determ/crypto/viewkey/viewkey.h`.

- **Python-prove-first**: the exact RFC 5869 mapping was frozen BEFORE any C existed — `tools/verify_view_key.py` (from-scratch HKDF on stdlib `hmac`/`hashlib`, gated on RFC 5869 Appendix A.1/A.2) generated `tools/vectors/view_key.json` (14 vectors: epoch adjacency/rotation, u32-boundary + max-u64 epochs, addr/chain_id/sk distinctness pairs, the concat-ambiguity pair the length prefixes exist to kill; `info_hex` pinned SEPARATELY from `vk_hex` so an encoding bug is distinguishable from an HKDF bug).
- **Dual-oracle**: `test-view-key-c99` (C — every vector in BOTH halves: rebuild `info` + compare `info_hex`, raw `determ_hkdf_sha256(info)` vs `vk_hex`, AND end-to-end `determ_view_key_derive` vs `vk_hex`; plus the fail-closed edges and determinism) + the python side (`verify_view_key.py` no-args re-verify + the `view_key` checker in `tools/test_c99_vector_files.sh`).
- **Fail-closed**: empty `chain_id` (cross-network replay), empty `addr` (ownerless key), fields > 256 B, NULL args — all return −1 with the output untouched. The length-prefixed big-endian `info` encoding is byte-identical to the `unshield_spend_ctx_hash` convention (`include/determ/chain/shielded.hpp`), so distinct `(chain_id, addr)` pairs can never alias.
- **PERMANENCE**: the formula is permanent once accounts exist (no-migrations); any change is a `-v2` DST, never an in-place edit.
- **Non-claims / not yet built**: this is the DERIVATION primitive only — the A2 audit integration (`LOG_AUDIT_ACCESS`, `audit_view_master_pk`, `ROTATE_AUDIT_KEY`), the epoch-schedule policy (what block range an epoch spans), key distribution/disclosure tooling, and the A3 light-client verification are the next register increments (`v2.22-PRIVACY-SPEC.md` TO-BUILD sections).
...
- **PERMANENCE**: the formula is permanent once accounts exist (no-migrations); any change is a `-v2` DST, never an in-place edit.
- **On-chain publication (SHIPPED, NC-8 inc.5a)**: so a third-party sender can *find* a recipient's `note_pk`, it is published by the `REGISTER_NOTE_KEY` (`TxType = 17`) consensus tx into a conditional `nk:` + addr state leaf (`SHA256(note_pk)`, emitted only while set → byte-neutral when unused). Owner-decided **Option (a)** (dedicated tx + leaf) — the only option that reaches anonymous/bearer payees; `REGISTER` is validator-only and address-derivation is infeasible without a new hardness assumption. Consensus-inert (the point is never decompressed/validated). Gate `test-register-note-key`. See `EncryptedNoteDeliveryDesign.md` §5.5. The trustless READ path (SHIPPED, inc.5b): an `nk` `rpc_state_proof` branch + a cleartext `note_pk` on the `account` RPC + a light `verify-notekey` command binding `key_bytes == "nk:"+addr` AND `value_hash == SHA256(note_pk)` against a committee-attested `state_root`; gate `test-light-verify-notekey`. NC-8 §5.6 adds the symmetric light-client enote-inclusion scan — an `en` `rpc_state_proof` branch + a `verify-enote-inclusion` command binding `key_bytes == "en:"+hex(commitment)` AND `value_hash == SHA256(commitment‖enote)` (MODERN-only), so a wallet trustlessly confirms a scanned delivery before trial-decrypting; gate `test-light-verify-enote-inclusion`.

### 3.26 DSSO G4 OPAQUE-3DH AKE core (`determ_opaque3dh_*`) — **SHIPPED (DSSO G4 inc.1)**

The "Sign-In With Determ" login (`v2.25-DSSO-DAPP-SPEC.md` §4-5) co-generates a shared session key `sso_key` between the client wallet and the threshold IdP via an OPAQUE authenticated key exchange (spec §4 step 5, *"the OPAQUE handshake co-generates a shared session key"*). This module realizes the **AKE core** — the 3DH key schedule + the transcript-MAC mutual authentication (RFC 9807 §6.4 "3DH"), the construction the owner selected for the co-generation (DECISION-LOG 2026-07-21). The `credential_request` / `credential_response` are opaque transcript blobs here — they are produced by the already-shipped threshold-OPRF + OPAQUE envelope (§3.9b, G1/G2/G3, `test-dsso-threshold-oprf`) and threaded in at the login layer (inc.2), so this proves the AKE in isolation. Source: `src/crypto/dsso/opaque3dh.c`, header `include/determ/crypto/dsso/opaque3dh.h`.

The 3DH combines three P-256 scalar mults (each side sees the same three shared points), `ikm = compress(dh1)‖compress(dh2)‖compress(dh3)` (99 B), then an RFC-9807 / TLS-1.3 HKDF-Expand-Label schedule over HKDF-SHA256: `prk = HKDF-Extract(0³², ikm)`; `session_key = Derive-Secret(prk, "SessionKey", preamble)`; `Km2/Km3 = Expand-Label(HandshakeSecret, "ServerMAC"/"ClientMAC", "", 32)`; `server_mac = HMAC(Km2, SHA256(preamble))`, `client_mac = HMAC(Km3, SHA256(preamble‖server_mac))`. The preamble is streamed into SHA-256 (never materialized): the house `"DTM-DSSO-OPAQUEv2-"` tag, the length-prefixed context, the RFC 9807 §4.1.1 **`CleartextCredentials`** block (`"DTM-DSSO-CLEARCRED-v2-"‖compress(pk_s)‖compress(pk_c)‖lp(server_identity)‖lp(client_identity)`), `ke1 = cred_request‖client_nonce‖compress(epk_c)`, and `inner_ke2 = cred_response‖server_nonce‖compress(epk_s)`. Binding both STATIC public keys is the C2 fix (2026-09-17, `v2.25-DSSO-DAPP-SPEC.md` §0.0(2)): the v1 transcript bound neither and `pk_s`/`pk_c` were bare call arguments, so an attacker holding only the victim's PUBLIC `pk_c` impersonated the whole threshold IdP — the arguments are now gone and a static key can enter only through that one MAC-covered transcript field. `determ_opaque3dh_cleartext_credentials()` serializes the identical block for use as the credential envelope's AAD, so the envelope tag and the transcript MAC commit to the same bytes. The Expand-Label prefix is the house `"DTM-DSSO-OPAQUE3DH-v2-"` domain tag (Determ realizes the CONSTRUCTION for its own DApp, not wire-interop with other OPAQUE stacks). **No new hardness assumption / no new primitive** — a pure composition of `determ_p256_point_mul`/`_base_mul`/`_point_compress` (§3.8c) + `determ_hmac_sha256` (§3.1) + streaming `determ_sha256`. Secret scratch (prk, handshake_secret, Km2/Km3, the DH points) is `determ_secure_zero`'d on every path; fail-closed on NULL args (both static public keys are REQUIRED), an over-length transcript field, an uncompressible/off-curve static key, or an identity ephemeral.

- **Python-prove-first**: the exact 3DH + key-schedule byte layout was frozen BEFORE any C existed — `tools/verify_opaque3dh.py` (dependency-free: a from-scratch P-256 ladder + SEC1 compress reusing the `verify_notekey.py` scaffold, plus HKDF-SHA256 / HMAC over hashlib) emits the KAT (`CleartextCredentials`, `session_key`, `server_mac`, `client_mac`) and self-tests both-sides agreement, tamper rejection and the C2 arms (impersonator rejected; a server that lies about its static key rejected; substituted `client_public_key` / identities rejected).
- **Dual-oracle**: `determ test-dsso-opaque3dh` (43 assertions — both parties derive the SAME `session_key` from the 3DH + the whole transcript; the two MACs mutually authenticate; a changed `server_nonce` yields a different key AND a server MAC the honest client rejects (the transcript is bound into every output); the **C2 arms** C2-a…C2-g (an impersonator holding only `pk_c` is rejected; a server holding the real `sk_s` but claiming a different static key is rejected although all three DH values agree; a substituted `client_public_key` or identity is rejected; the MAC compare covers all 32 bytes); fail-closed NULL edges; and the KAT `session_key = 669097b2…`, `server_mac = 5a86590c…`, `client_mac = 4de72806…`, plus the serialized `CleartextCredentials`, byte-for-byte vs the python oracle). Wrapper `tools/test_dsso_opaque3dh.sh`, FAST both platforms. Falsify-on-mutant: M1 drop `pk_s` from the transcript, M2 drop `pk_c`, M3 drop the identities, M4 bind server-side only, M5 weaken the MAC compare, M6 restore the v1 unbound preamble — each RED.
- **Permanence**: the domain tags (`DTM-DSSO-OPAQUEv2-` / `DTM-DSSO-OPAQUE3DH-v2-` / `DTM-DSSO-CLEARCRED-v2-`) + the encoding are permanent once a DSSO deployment exists (login transcripts must reproduce). Any change is a `-v3` tag, never an in-place edit. The one-time v1 → v2 **replacement** (C2, 2026-09-17) was taken because NO deployment existed — no login transcript had to reproduce — and retaining v1 beside v2 would have left the impersonable construction compiled in as a downgrade target; the v1 KAT is RETIRED, not archived.

See `DssoThresholdOprfSoundness.md` (G4-AKE section) and `v2.25-DSSO-DAPP-SPEC.md` §4/§9.
