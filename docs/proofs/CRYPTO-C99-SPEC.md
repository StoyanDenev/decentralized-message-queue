...
- **NOT a chain integration.** This is a LIBRARY bundle. A confidential-tx CONSENSUS feature needs a shielded-pool STATE MODEL (a commitment set + a nullifier set for double-spend prevention + deposit/withdraw between the transparent and confidential value pools) and the owner-gated PROFILE choice (P-256 FIPS vs Z_p* MODERN) — both separate, owner-gated steps. The bundle proves a transfer is well-formed; it does NOT prevent replay/double-spend (the nullifier set's job) nor hide sender/receiver/graph/fee (only amounts).
