...
The 3DH combines three P-256 scalar mults (each side sees the same three shared points), `ikm = compress(dh1)‖compress(dh2)‖compress(dh3)` (99 B), then an RFC-9807 / TLS-1.3 HKDF-Expand-Label schedule over HKDF-SHA256: `prk = HKDF-Extract(0³², ikm)`; `session_key = Derive-Secret(prk, "SessionKey", preamble)`; `Km2/Km3 = Expand-Label(HandshakeSecret, "ServerMAC"/"ClientMAC", "", 32)`; `server_mac = HMAC(Km2, SHA256(preamble))`, `client_mac = HMAC(Km3, SHA256(preamble‖server_mac))`. The preamble is streamed into SHA-256 (never materialized): the house `"DTM-DSSO-OPAQUEv1-"` tag, then the length-prefixed context/identity fields, `ke1 = cred_request‖client_nonce‖compress(epk_c)`, and `inner_ke2 = cred_response‖server_nonce‖compress(epk_s)`. The Expand-Label prefix is the house `"DTM-DSSO-OPAQUE3DH-v1-"` domain tag (Determ realizes the CONSTRUCTION for its own DApp, not wire-interop with other OPAQUE stacks). **No new hardness assumption / no new primitive** — a pure composition of `determ_p256_point_mul`/`_base_mul`/`_point_compress` (§3.8c) + `determ_hmac_sha256` (§3.1) + streaming `determ_sha256`. Secret scratch (prk, handshake_secret, Km2/Km3, the DH points) is `determ_secure_zero`'d on every path; fail-closed on NULL args, an over-length transcript field, or an identity ephemeral.

- **Python-prove-first**: the exact 3DH + key-schedule byte layout was frozen BEFORE any C existed — `tools/verify_opaque3dh.py` (dependency-free: a from-scratch P-256 ladder + SEC1 compress reusing the `verify_notekey.py` scaffold, plus HKDF-SHA256 / HMAC over hashlib) emits the KAT (`session_key`, `server_mac`, `client_mac`) and self-tests both-sides agreement + tamper rejection.
- **Dual-oracle**: `determ test-dsso-opaque3dh` (17 assertions — both parties derive the SAME `session_key` from the 3DH + the whole transcript; the two MACs mutually authenticate; a changed `server_nonce` yields a different key AND a server MAC the honest client rejects (the transcript is bound into every output); fail-closed NULL edges; and the KAT `session_key = 6d58d64b…`, `server_mac = 9f85241f…`, `client_mac = df2fef7f…` byte-for-byte vs the python oracle). Wrapper `tools/test_dsso_opaque3dh.sh`, FAST both platforms.
- **Permanence**: the domain tags (`DTM-DSSO-OPAQUEv1-` / `DTM-DSSO-OPAQUE3DH-v1-`) + the encoding are permanent once a DSSO deployment exists (login transcripts must reproduce). Any change is a `-v2` tag, never an in-place edit.
