...
**What increment 2 does NOT do (later owner-gated increments):** the recipient-key *derivation* (§5 sub-decision 1 — MODERN dedicated note key / FIPS view-master), the **scan RPC** (a node endpoint returning enote payloads over a height range for wallet pull-and-trial-decrypt), and the **light-client scan** (verify `en:` inclusion against `state_root` before trial-decrypting). Increment 2 carries the opaque ciphertext and commits it; who it is sealed to and how it is fetched are the next increments.
...
- **STATUS: primitive SHIPPED (dual-oracle); wiring sub-decisions OWNER-DECIDED (profile-keyed, §5); wiring increments 1 (genesis `CryptoProfile` pin, §5.1) + 2 (the per-output enote region on `CONFIDENTIAL_TRANSFER` — MODERN `en:` state leaf / FIPS payload-only, consensus-inert, §5.2) SHIPPED; recipient-key derivation + scan RPC + light-client scan remain owner-gated increments.**
