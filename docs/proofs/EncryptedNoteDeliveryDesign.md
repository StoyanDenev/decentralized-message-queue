> **TIER: NEAR-TERM — 1.1.x in-flight.** The crypto PRIMITIVE + its dual-oracle gate ship here (library-only, additive, state-root-invariant); the CONSENSUS WIRING is an owner-gated follow-on (§5). Roadmap index: docs/ROADMAP.md
...
**What ships in THIS increment is the crypto primitive only** (`src/crypto/enote/enote.c` + its header), plus its dual-oracle test gate. It is **library-only, additive, and state-root-invariant**: no chain, consensus, validator, producer, wallet, or light-client code path calls it yet. The consensus wiring that would actually carry a ciphertext on a `CONFIDENTIAL_TRANSFER` output — and the scan RPC / light-client scan that would consume it — is laid out in §5 and **deferred to the owner** under the one-design-doc-per-decision directive, because it touches consensus-load-bearing surfaces (tx payload / state root) and carries genuine sub-decisions.
...
- **STATUS: primitive SHIPPED (dual-oracle); consensus wiring owner-gated.**
...
- **Owner-gated follow-on (§5).** Consensus wiring — attach the ciphertext to `CONFIDENTIAL_TRANSFER` (and, where a foreign recipient key applies, `SHIELD`) outputs as a consensus-inert additional payload region; a scan RPC; light-client scan — deferred with three sub-decisions: (1) recipient key = dedicated P-256 note key vs A2/view-key derivation; (2) ciphertext-in-tx-payload (state-root-invariant) vs a new `en:` state leaf (light-client-provable); (3) per-output vs per-tx. Nothing in §5 is decided here.
