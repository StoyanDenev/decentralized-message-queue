...
- **Profile-agnostic.** The ciphertext rides the tx payload on **both** MODERN and FIPS chains, so the scan returns hits on either; the profile only decides whether an `en:` **state** leaf *also* exists (MODERN 2b → light-client-provable inclusion; §5.4, the remaining increment).
...
**Remaining owner-gated increments:** the recipient-key *derivation* (§5 sub-decision 1 — MODERN dedicated note key / FIPS view-master; decides *whom* each enote is sealed to) and the **light-client scan** (§5.4 — verify an `en:` leaf's inclusion against a committed `state_root` before trial-decrypting; meaningful only on MODERN, which has the `en:` leaf).
...
- **STATUS: primitive SHIPPED (dual-oracle); wiring sub-decisions OWNER-DECIDED (profile-keyed, §5); wiring increments 1 (genesis `CryptoProfile` pin, §5.1) + 2 (the per-output enote region on `CONFIDENTIAL_TRANSFER` — MODERN `en:` state leaf / FIPS payload-only, consensus-inert, §5.2) + 3 (the read-only `scan_enotes` RPC, §5.3) SHIPPED; recipient-key derivation + light-client scan remain owner-gated increments.**
