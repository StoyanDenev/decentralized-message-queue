...
- **SIGS** — for every header with `index != 0`, `verify_block_sigs(header, committee, bft)` (`light/verify.cpp:334-427`), which **recomputes** the block digest via `light_compute_block_digest` and Ed25519-verifies the committee signatures over that recomputed digest. **Only `index == 0` (the genesis) is skipped.**
...
- **SIGS** — for each header in `doc` with `index != 0`, `determ::light::verify_block_sigs` (`light/verify.cpp:334-427`), which calls `light_compute_block_digest` (`light/verify.cpp:124-191`) to **recompute** the digest internally, then per-creator Ed25519-verifies (`determ::crypto::verify`) against it, enforcing `required = bft ? (2K+2)/3 : K`. The genesis skip is keyed on `idx == 0` at `light/main.cpp:962`.
...
| **SIGS** | per non-genesis header `verify_block_sigs` (`light/verify.cpp:334-427`) | Every header with `index != 0` carries a committee-signature quorum over **`light_compute_block_digest(b)` — recomputed from THAT header** | A1, **UNCONDITIONAL** per header (VCF-2 §3.2, inheriting LBV-2) |
...
- **Cross-shard / F2-reconciled digest extensions — now in scope (F-7).** `light_compute_block_digest` now binds the `compute_view_root` terms the producer binds for cross-shard / F2 blocks (plus the merged-block `partner_subset_hash` / `timestamp` tail), on the **same data-driven gates** as the node — FULL byte-parity. SIGS therefore **VERIFIES** cross-shard / F2 blocks rather than fail-closing on them. Because `rpc_headers` STRIPS the heavy F2 collections, the online `verify-chain-file`'s online sibling `verify-chain` re-fetches the FULL block when a stripped header's sig check fails (the F2 full-block fallback in `verify_chain_walk`); for the file path the operator's exported headers must carry the F2 collections (a full-block export) for SIGS to recompute their roots. Inherited from `LightBlockVerifySoundness.md` F-LBV5; surfaced here as F-VCF6.
...
For each header `h` with `idx != 0`, the composite calls `verify_block_sigs(h, committee, bft)` (`light/verify.cpp:334-427`). The core (§2.2 of `LightBlockVerifySoundness.md`, reproduced for the per-header instance):
...
with the pre-checks: every `creators[i]` must be in the supplied committee (`light/verify.cpp:367-373`, FAIL otherwise) and `creator_block_sigs.size() == creators.size()` (`light/verify.cpp:374-380`, FAIL otherwise). The digest is byte-for-byte the producer's `compute_block_digest` over the **full** field set — v1 core plus (F-7) the three F2 view roots and the merged-block `partner_subset_hash` / `timestamp` tail, each gated identically to the node (`light_compute_block_digest`, `light/verify.cpp:124-191` ≡ `src/node/producer.cpp:619-704`), per `LightClientThreatModel.md` Lemma L-2.

**The digest binds `prev_hash`.** Critically for the composite (§3.3), `light_compute_block_digest` appends `b.prev_hash` as its second field (`light/verify.cpp:127`): `h.append(b.prev_hash);`. So a tampered `prev_hash` changes the recomputed digest and breaks the committee signatures — SIGS FAILs. This is the bridge by which SIGS, an independent per-block check, also re-validates the linkage CONTINUITY walks over.
...
- *Part 1 (digest is the header's true digest).* `verify_block_sigs` computes `digest_j := light_compute_block_digest(b_j)` at `light/verify.cpp:382`, where `b_j` is the parsed header. By `LightClientThreatModel.md` Lemma L-2, `light_compute_block_digest` is byte-for-byte `producer.cpp::compute_block_digest` over the **full** field set — v1 core plus (F-7) the three F2 view roots and the merged-block `partner_subset_hash` / `timestamp` tail, each gated identically to the node — so `digest_j` equals the exact digest the committee signed when it produced `H_j`, common-case *or* cross-shard / F2-reconciled. No operator input enters; the digest is a pure function of `H_j`'s own fields. (For an F2 header this requires the header to carry the full F2 collections — a full-block export, not a stripped header; see F-VCF6, inheriting F-LBV5.)
- *Part 2 (per-signer verification is sound for that digest).* Each counted-valid creator `i` satisfies `verify(pk_i, digest_j, σ_i) = 1` with `pk_i` drawn from the resolved committee (`light/verify.cpp:398`); every creator is pre-checked for committee membership (`:367-373`) and `creator_block_sigs.size() == creators.size()` is enforced (`:374-380`). By A1 (Ed25519 EUF-CMA, `Preliminaries.md §2.2`), an adversary without `sk_i` produces such a `σ_i` for a never-signed `digest_j` with probability `≤ 2⁻¹²⁸`. The per-signer loop is a faithful `K_j`-fold replication of the single-sig verify primitive with no cross-signer state — instantiating `BatchSigningSoundness.md` BS-3's structural-isolation property for the committee-sig loop.
...
- If `creator_block_sigs` is absent/empty while `creators` is non-empty, the size check `creator_block_sigs.size() != creators.size()` FAILs (`light/verify.cpp:374-380`).
...
Moreover the composite covers **two distinct attacks that neither check covers alone**: CONTINUITY catches **reordering / linkage breaks** (VCF-1), SIGS catches **content tampering + forgery** (VCF-2), and the linkage-vs-content split is bridged because **the digest SIGS verifies binds `prev_hash`** (`light/verify.cpp:127`).
...
- *Content tampering is caught by SIGS regardless of CONTINUITY's weaker walk.* CONTINUITY (VCF-1) walks stored `block_hash` fields without recomputing them, so it cannot detect a block whose *body* was altered while its stored `block_hash` and the neighboring `prev_hash` were kept consistent. **SIGS catches exactly this.** Any change to a digest-bound field of a non-genesis header `H_j` — the v1 core `index`, `prev_hash`, `tx_root`, `delay_seed`, `consensus_mode`, `bft_proposer`, `creators`, `creator_tx_lists`, `creator_ed_sigs`, `creator_dh_inputs` (`light/verify.cpp:126-136`), or (F-7) the F2 inbound / equivocation / abort roots and the `partner_subset_hash` / `timestamp` tail (`light/verify.cpp:137-189`) — changes `digest_j = light_compute_block_digest(H_j)`. The genuine committee signatures were produced over the *original* digest; over the tampered digest they no longer verify (A1), so `verify_block_sigs` FAILs and the loop throws (`light/main.cpp:968-969`) ⇒ SIGS FAIL ⇒ `VCF = FAIL`. To make a tampered non-genesis block pass, the attacker must forge `required` committee signatures over the new digest — the VCF-2 A1 bound (`≤ K·2⁻¹²⁸` per block).
...
### F-VCF6 The self-recomputed digest is FULL byte-parity with the producer — F2 / cross-shard blocks now VERIFY (CLOSED, F-7, inherited F-LBV5)

**Previously** `light_compute_block_digest` reproduced only the v1 core and **omitted** the `compute_view_root` extensions the producer binds for cross-shard / F2-reconciled blocks, so on such a block the light digest differed, the genuine committee sigs did not verify, and SIGS **FAILed** — a fail-closed false-negative.

**F-7 closes this.** `light_compute_block_digest` (`light/verify.cpp:124-191`) is now a **byte-for-byte mirror of the whole** `producer.cpp::compute_block_digest` (`producer.cpp:619-704`): after the v1 core it binds the `compute_view_root` over the `inbound_receipts` keys (`light/verify.cpp:146-151` ≡ `producer.cpp:640-646`), the non-zero-gated eq/abort view-roots (`light/verify.cpp:157-168` ≡ `producer.cpp:661-672`), and the merged-block `partner_subset_hash` (`light/verify.cpp:175-177` ≡ `producer.cpp:685-687`) / `timestamp` (`light/verify.cpp:187-189` ≡ `producer.cpp:700-702`) tail. The four F2 hash helpers are file-local statics (`light/verify.cpp:34-95`) byte-equal to `producer.cpp:322-382`. Every append is gated on the **same data-driven condition** as the node, so the parity is exact on **all** blocks: a STRIPPED header collapses to the byte-identical v1 core (non-F2 header-only sync unchanged), and a FULL block binds the identical F2 data the committee signed — so SIGS **VERIFIES** the cross-shard / F2 block instead of fail-closing on it (under A1, exactly as VCF-2 / LBV-2). Because `rpc_headers` STRIPS the heavy F2 collections, the online `verify-chain` walk re-fetches the FULL block via the F2 full-block fallback in `verify_chain_walk` (`light/trustless_read.cpp:198-248`), pinned by the requirement that the re-fetched body's `compute_hash()` EQUAL the stripped header's already-chained `block_hash` (`trustless_read.cpp:218-234`) — the same `committee_bound_state_root` full-block trust step. For the file path, the exported headers must carry the F2 collections (a full-block export) for SIGS to recompute their roots; with a stripped-header export an F2 block's SIGS would still fail-close (no false PASS). This is `LightBlockVerifySoundness.md` **F-LBV5** inherited verbatim; the KEEP-IN-SYNC comment at `light/verify.cpp:97-123` documents the now-fully-mirrored field set, and `tools/test_block_digest_xbinary_parity.sh` pins FULL byte-parity (all 15 tokens incl. the 3 F2 roots).

### F-VCF7 Sentinel / quorum semantics inherited from `verify_block_sigs`

SIGS inherits `verify_block_sigs`'s exact handling: a 64-byte all-zero signature is a *sentinel abstention* — permitted (uncounted) only under `--bft`, a hard FAIL in MD mode (`light/verify.cpp:388-396`); a creator absent from the resolved committee is a hard FAIL (`:367-373`); `creator_block_sigs.size() != creators.size()` is a hard FAIL (`:374-380`). The MD quorum is full `K`; the `--bft` quorum is `⌈2K/3⌉ = (2K+2)/3` (`:408-410`). An operator running MD against a BFT-escalated chain gets a clean FAIL, not a false PASS — they must pass `--bft`. These are `verify_block_sigs`'s behavior, carried verbatim (the same as `LightBlockVerifySoundness.md` F-LBV6).
...
| VCF-2 | per-header `verify_block_sigs` | `light/verify.cpp:334-427` | Per-creator Ed25519 verify over the **self-recomputed** digest; quorum `K` / `⌈2K/3⌉`. |
| VCF-2 | `light_compute_block_digest` | `light/verify.cpp:124-191` | **FULL** byte-for-byte mirror of `producer.cpp::compute_block_digest` (v1 core + F2 tail `:137-189`, F-7); binds `prev_hash` (`:127`). |
| VCF-2 | F2 hash helpers (file-local statics) | `light/verify.cpp:34-95` | `hash_cross_shard_receipt` / `hash_equivocation_event` / `hash_abort_event` / `compute_view_root` — byte-equal to `producer.cpp:322-382`. |
| VCF-2 | F2 full-block fallback | `light/trustless_read.cpp:198-248` | On a stripped-header sig-check failure, re-fetch the FULL block and re-verify against the now-F2-aware digest; pinned to the chained `block_hash` (F-7). |
| VCF-2 | digest recomputed (not supplied) | `light/verify.cpp:382` | `Hash digest = light_compute_block_digest(b);` — the UNCONDITIONAL improvement over wallet BV-2. |
| VCF-2 | daemon original | `src/node/producer.cpp:619-704` (v1 core `:619-631` + F2 tail `:632-702`) | `compute_block_digest`; F-VCF6 (F-7) confirms the light copy now mirrors the F2 tail in full. |
...
| VCF-S | size-mismatch FAIL | `light/verify.cpp:374-380` | `creator_block_sigs.size() != creators.size() ⇒ FAIL` — the fail-close a stripped header hits. |
