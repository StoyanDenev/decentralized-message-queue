...
| [StateRootAnchorSoundness.md](StateRootAnchorSoundness.md) | SR-1..SR-5 | Per-height committee-verified `state_root` anchoring (`verify-state-root`). **(F6 R40 — sibling; cited at spec level, see §note below.)** |
...
MT-4 is the cryptographic core that T-L3 *is*, and that SR-1 / AH-1 Gate-3 invoke per height. Limitation **S-040** (`leaf_count` not bound into the hash) is a caller-trust item orthogonal to MT-4's soundness, mitigated by sourcing `leaf_count` from the same anchor as `state_root`.

### 3.3 StateRootAnchorSoundness.md — SR-1..SR-5 (F6 R40, spec level)

| Theorem | One-line (spec level) | Reduces to |
|---|---|---|
| **SR-1** | Committee-anchored per-height `state_root`: a daemon cannot present a `state_root` at height `h` that the committee did not sign; the root is bound into `header[h]`'s committee-signed digest/hash chain. | T-L2 + the digest binding (A1 + A2) |
...
| **SR-3..SR-5** | Supporting: root-equality / head-vs-historical anchoring / fail-closed on mismatch (the per-height analog of L-5 + L-6). | A2 + (fail-closed) |

> SR labels per F6's task description; confirm exact statements against `StateRootAnchorSoundness.md` once threaded. The lattice edge **SR-1 ← T-L2 + (A1+A2)** is the structural fact that the other light-client reads (T-L4, AH-1) already depend on: a committee-signed header pins a *specific* `state_root` at its height.
...
- **SR-1 (committee-anchored `state_root`, F6 R40) ← T-L2 + the digest binding (A1 + A2).** A committee-signed `header[h]` pins a *specific* `state_root` value at height `h` (the root is bound into the header's `block_hash`/`signing_bytes` chain, which the K-of-K signatures cover). This is the structural fact `S033StateRootNamespaceCoverage.md` + `LightClientThreatModel.md` §5.2 record and that T-L4 / AH-1 already consume. *(F6 SR-1, spec level; structurally = T-L2 + S-033 binding.)*
...
| `verify-state-proof` | pure verifier (offline file in) | **T-L3 = MT-4** (A2) | Self-anchoring against the supplied root unless `--state-root` pins an independently-trusted root; **S-040** — `leaf_count` must be sourced from the same anchor as `state_root`. |
| `verify-chain` | composite | T-L1 + T-L2 + continuity, end-to-end from block 0 to tip | Single-daemon: a truncated-tip lie (daemon under-reports head) is an *availability* lie, not caught (`LightClientThreatModel.md` F-4); O(height) sig-verifies per invocation (no persistence). |
| `balance-trustless` | composite | **T-L4** (= T-L1 + T-L2 + T-L3 + L-4 + race-window L-5) | Requires **S-033 + S-038** active (else throws `chain has not activated state_root`); head-only state-proof RPC — balance Merkle-verified at the verified head. |
...
| `verify-state-root` | per-height verifier **(F6 R40)** | **SR-1** (committee-anchored per-height `state_root`) + **SR-2** (genesis-binding) | **(F6 R40 — confirm against `StateRootAnchorSoundness.md`.)** Per-height committee-verified root; the historical-vs-head anchoring + fail-closed are SR-3..SR-5. Static `K_0`; requires S-033 + S-038 for a non-zero root (else vacuous — pre-S-033). |
...
| `verify-and-submit` | composite (read → sign → submit) | **T-L5** (= **T-L4** verified nonce + sign + submit under A1) | Composes `nonce-trustless` (T-L4) + `sign-tx` + `submit-tx`; requires S-033 + S-038 (via the nonce read). End-to-end bound `≈ 2⁻⁹²`. Static `K_0`. |

**Reading the residual column.** Four limitations recur across nearly every row — single-daemon (no multi-peer cross-check), static genesis committee `K_0` (cross-rotation fails closed), head-only state-proof RPC (balance/nonce Merkle-verified at head; `state_root` committee-verified per height), and pre-S-033 vacuity (the state-anchored reads throw on a chain without S-033+S-038). §6 consolidates these so they are stated once rather than re-derived per command.
...
- **All referenced proofs shipped (R39 + R40).** `LightClientThreatModel.md` (T-L1..T-L5), `MerkleTreeSoundness.md` (MT-1..MT-5), `TxInclusionProofSoundness.md` (TI-1..TI-4), `AccountHistorySoundness.md` (AH-1..AH-4), `LightClientArchiveSoundness.md` (AR-1..AR-4) are all in the tree. `StateRootAnchorSoundness.md` (SR-1..SR-5) is the F6 R40 sibling, cited here at the spec level and tagged **(F6 R40)**; confirm the exact SR statements against that document once threaded.
