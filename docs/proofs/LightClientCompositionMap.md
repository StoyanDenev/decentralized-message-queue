...
MT-4 is the cryptographic core that T-L3 *is*, that SR-1 invokes per height, and that AH-1b Gate-3 invokes **at the head** (the `account-history` `state_proof` RPC is head-only; per-height rows bind only the committee-signed `R_h` via AH-1a, which is not Merkle-opened). **S-040 CLOSED** — `leaf_count` is bound into the committed root via the root-wrapper hash `root = SHA256(0x02 ‖ u32_be(leaf_count) ‖ inner_root)` (`src/crypto/merkle.cpp::merkle_root_wrap`), so a forged count yields a different wrapper hash and is rejected; the former single-envelope-sourcing obligation is now enforced by the hash, not just a caller guideline. The light client inherits this via its direct `crypto::merkle_verify` call (`light/verify.cpp`).
...
| Theorem | One-line | Reduces to |
|---|---|---|
| **AH-1a** | Per-height `state_root` soundness: if a row is emitted, its `state_root` `R_h` is `D`'s chain's genuine committee-signed commitment at `h`. Bound `≤ K·2⁻¹²⁸` per height. **= T-L2 + T-L1 at a fixed historical height.** | T-L2 + T-L1 |
| **AH-1b** | Balance/nonce are Merkle-proven only at the **head** (the `state_proof` RPC is head-only); every non-head row carries the head value under an explicit `head@H` label (`balance_source`) and is never presented as `D`'s balance at `h`. Head-row bound `≤ log₂(leaf_count)·2⁻¹²⁸ + 2⁻¹²⁸`. **Head value = T-L4; non-head = a labeling obligation.** | T-L4 (at head) + labeling |
| **AH-2** | Trajectory consistency: each row binds the `state_root` from ITS OWN committee-verified `header[h]`; no shared root across rows, so `h_i`'s `state_root` cannot validate against `h_j`'s row (cross-height confusion = A1/A2 break). | A1 + A2 |
| **AH-3** | Nonce monotonicity is **necessary-not-sufficient** (and **vacuous** in the shipped head-only tool — all rows carry the identical head nonce): a forged-but-coherent history could preserve monotonicity, so it is a *cross-check*, not a soundness source. The real guarantee is AH-1a's per-height `state_root` anchoring. | (honesty theorem; rests on AH-1a + FA-Apply-3) |
| **AH-4** | Sampling-gap honesty: the trajectory proves its per-height facts AT the sampled heights only; balance can move up-and-back within a `--step` gap unobserved. A verified *sample*, not a continuous audit. | (scope statement) |

AH-1a is structurally T-L2 + T-L1 specialized to a fixed `h` (the per-height committee-signed `state_root`); AH-1b is T-L4 at the head only plus a `head@H` labeling discipline for the value copied onto non-head rows (the `state_proof` RPC is head-only). AH-2 is AH-1a's multi-point extension (independent per-row `state_root` anchoring under one genesis anchor). AH-3 / AH-4 are honesty theorems (the temporal-sampling analog of AR-3's range honesty).
...
- **AH-1a (per-height `state_root`) ← T-L2 + T-L1 at a fixed `h`; AH-1b (value) ← T-L4 at the head.** AH-1a binds `R_h = header[h].state_root` per sampled height via T-L2 (committee-signed `header[h]`, SR-1's anchor) + T-L1 (genesis anchor, once) — it Merkle-opens nothing. AH-1b runs the full T-L4 read (T-L3/MT-4 state-proof + L-4 cleartext) **once at the head** (the `state_proof` RPC is head-only) and copies that value onto every non-head row under an explicit `head@H` label. *(`AccountHistorySoundness.md` AH-1a/AH-1b; AH-1a = T-L2+T-L1 at fixed `h`, AH-1b = T-L4 at head.)*

- **AH-2 (trajectory) ← AH-1a + genesis-linkage (the `IncrementalChainWalker` single pass).** Each row binds the `state_root` from its *own* committee-verified `header[h]`; the only inter-row coupling is the one-time genesis anchor + the per-height prev_hash linkage to the genesis-rooted chain (a single monotonic pass via `IncrementalChainWalker::advance_to`). No shared root couples row `state_root`s; cross-height confusion = A1/A2 break. *(`AccountHistorySoundness.md` AH-2 + AH-L2/AH-L3.)*
...
| `account-history` | composite (trajectory) | **AH-1a** (per-row committee-signed `state_root` = T-L2+T-L1 @ fixed h) + **AH-1b** (head-only value, non-head rows labeled) + **AH-2** (independent anchoring) + **SR-1** (per-height committee `state_root`, F6 R40) | **Sampling gaps (AH-4)** — per-height facts at sampled heights only; balance can move within a `--step` gap unseen. **Head-only Merkle (AH-1b)** — `state_root` committee-verified per height (AH-1a), but balance/nonce Merkle-verified at HEAD only; every non-head row carries the head value under a `head@H` label (`balance_source`, rows annotated `balance_merkle_verified`), NOT the value at that height; a genuine per-height value read needs the RPC to gain a height param AND the loop to fetch a per-height proof. Static `K_0` (§6.2). Nonce monotonicity is a *cross-check* not a guarantee, and **vacuous** here (AH-3). |
...
| [MerkleTreeSoundness.md](MerkleTreeSoundness.md) | MT-1..MT-5 | **Inclusion substrate.** MT-4 = T-L3's core; consumed by SR-1 and by AH-1b Gate-3 (at the head; `account-history`'s value RPC is head-only). |
...
| `tools/test_light_account_history.sh` | AH-1a (per-height `state_root` match) / AH-1b(i) (head-row Merkle) / AH-2 / AH-4 (cluster-bound; the monotonic-nonce leg is vacuous, §4.3). |
| `tools/test_light_account_history_label.sh` | AH-1b(ii) — the non-head `head@H` labeling gate (FAST, offline; falsify-on-mutant). |
