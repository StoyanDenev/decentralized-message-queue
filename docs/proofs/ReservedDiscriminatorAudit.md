...
- **Zero readers in `src/`.** The planned consumer (producer/validator branch, `V210ImplementationRoadmap.md` Phase D — doc deleted 2026-07-09, inc.1) never landed. The only other code reference ever was a comment in the FROST header (`include/determ/crypto/frost.hpp`) — already GONE: the B2 purge executed in the working tree while this audit ran, so the declaration at [genesis.hpp:228](../../include/determ/chain/genesis.hpp) was the field's sole remaining code site (deleted with this drop).
...
- **MsgType 12 `BEACON_HEADER`, 13 `SHARD_TIP`, 14 `CROSS_SHARD_RECEIPT_BUNDLE` — R-9 verdict: KEEP (owner, 2026-09-16; DECISION-LOG 2026-09-16 D20a).** The 2026-08-13 sharding evaluation noted that a P1 (parallel-independent-chains) posture would flip these three uplink types and G-3/G-4 from KEEP to DROP (14/8 → 12/10). P1 was evaluated and not adopted; the launch posture is EXTENDED (D3, reaffirmed 2026-09-16 D2a), under which all three are load-bearing wire types. Tally unchanged: 14 KEEP / 8 DROP; DROP execution of verdicts 2–8 remains the last pre-genesis act (DECISION-LOG 2026-08-13, directive 3).
- **Wire versions** — reconciled to the D2 binary-only wire (2026-07-28): `kWireVersionLegacy=0` is DELETED along with the JSON (v0) envelope and the per-pair HELLO version negotiation; `kWireVersionBinary=1` ([messages.hpp:89](../../include/determ/net/messages.hpp)) is the single shipped wire version, and HELLO's `wire_version` byte survives in the fixed binary HELLO frame as an additive post-genesis upgrade ADVERTISEMENT only (nothing negotiates on it). Not a reserved slot. No action.
