...
> **Source-availability note.** Every claim below is grounded in code present in this worktree. The hold-and-wait loop is `light/trustless_read.cpp::committee_bound_state_root` (`:383-387`, the `for (waited …)` poll); the captured-once proof is `light/trustless_read.cpp::read_account_trustless` (`state_proof` fetched at `:466-468`, verified at `:477`, the held `proof_root` bound at `:498-527`); the head-fail-closed throw is `:388-401`; the load-bearing binding is `:417-432`. The forwarding call sites are `light/main.cpp` (`read_stake_trustless` decl `:1899-1906`, its embedded `committee_bound_state_root` forward `:1976-1977`; `verify_state_root_at` decl `light/verify_state_root.cpp:102-107`, its forward `:194-196`). The **nine direct `committee_bound_state_root` call sites** in `light/main.cpp` — each forwarding the command's parsed `wait_seconds` — are `:1976` (inside `read_stake_trustless`), `:3071` (`verify-receipt-inclusion`), `:3387` (`verify-merge-state`), `:3719` (`verify-param-change`), `:4022` (`verify-param-value`), `:4350` (`verify-registrant`), `:4694` (`verify-dapp-registration`), `:5062` (`verify-account`, inline), and `:5717` (`supply-trustless`); the remaining consumers reach the binding through `read_account_trustless` (`trustless_read.cpp`), `verify_state_root_at` (`verify_state_root.cpp`), and `run_account_history` (`account_history.cpp`). The flag default is `0` (every parse site initializes `uint64_t wait_seconds = 0;` and forwards it; the helper's parameter defaults to `0` per `trustless_read.hpp:224`).
...
(i) **The proof is frozen before the helper runs (§3.3).** In each binding consumer the `state_proof` is fetched once (`read_account_trustless:466-468`; `read_stake_trustless` `light/main.cpp:1928-1929`; the eight composite consumers in `light/main.cpp` each fetch their `state_proof` once *before* their `committee_bound_state_root` call — the nine direct calls being `:1976`, `:3071`, `:3387`, `:3719`, `:4022`, `:4350`, `:4694`, `:5062`, `:5717`). The extracted `proof_root` is an immutable local; no path re-issues the `state_proof` RPC after that point.
...
|---|---|---|---|
| `balance-trustless` / `nonce-trustless` (account read) | `:1823-1824` | `read_account_trustless(…, wait_seconds)` `:1849` | `committee_bound_state_root` `trustless_read.cpp:518-520` |
| `stake-trustless` | `:2043-2044` | `read_stake_trustless(…, wait_seconds)` `:2064-2066` | `committee_bound_state_root` `:1976-1977` |
| `verify-unstake-eligibility` | `:2166` | embeds `read_stake_trustless(…, wait_seconds)` `:2208-2210` | via the stake read |
| `verify-state-root` | `:2375-2376` | `verify_state_root_at(…, wait_seconds)` `:2398-2399` | `committee_bound_state_root` `verify_state_root.cpp:195-196` |
| `verify-and-submit` | `:2564` | embeds `read_account_trustless(…, wait_seconds)` `:2593-2595` | via the account read |
| `verify-receipt-inclusion` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:3071-3073` | direct |
| `verify-merge-state` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:3387-3389` | direct |
| `verify-param-change` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:3719-3721` | direct |
| `verify-param-value` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:4022-4024` | direct |
| `verify-registrant` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:4350-4352` | direct |
| `verify-dapp-registration` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:4694-4696` | direct |
| `verify-account` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:5062-5064` | direct (inline anchoring — **not** via `read_account_trustless`) |
| `supply-trustless` | its `--wait` arg-loop branch | `committee_bound_state_root(…, wait_seconds)` `:5717-5719` | direct (per supply counter) |
| `account-history` | `:2336-2337` (`opts.wait_seconds`) | `run_account_history(opts)` → `committee_bound_state_root(…, max_wait)` in `light/account_history.cpp` | direct (per sampled height) |
...
(The help text exposing `--wait` on each of these is in `light/main.cpp:183-483`.)
...
| `read_stake_trustless` | `light/main.cpp:1899-1986` (proof `:1928-1929`, helper `:1976-1977`) | Stake consumer; identical capture-once + forward. |
...
| The nine direct `committee_bound_state_root` call sites | `light/main.cpp:1976` (in `read_stake_trustless`), `:3071`, `:3387`, `:3719`, `:4022`, `:4350`, `:4694`, `:5062`, `:5717` | The `WH-6` coverage enumeration (verify-* / supply readers); the rest route via `read_account_trustless` / `verify_state_root_at` / `run_account_history`. |
