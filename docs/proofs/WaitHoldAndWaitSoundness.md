...
**Executed-mutant gate (`tools/test_light_wh2_norefetch.sh`, cluster-bound).** WH-2 is verified by an executed mutant, not inspection (`ProofClaimGateTraceability.md` §3h). A MOVING DAEMON — `tools/rpc_tamper_proxy.py` serving the honest `state_proof` on call #1 but flipping its `state_root` on call #2+ (`--serve-first 1`), and withholding the successor `headers` poll K times (`--withhold-successor K`) so the `--wait` loop is forced to iterate — leaves the CLEAN client `verified`/exit 0 while its request log shows **exactly one** `state_proof` fetch, the loop iterated (successor withheld ≥K×, polling only `headers`), and **zero** flips fired. The falsify inserts a caller-side `state_proof` re-fetch after the wait (overwriting `proof_root`): the re-fetch is call #2, the daemon flips it, and the client throws `SECURITY … does NOT match proof.state_root` (exit 1) — and, sharply, it fails **even through a pass-through proxy**, because the re-fetch binds an *advanced* root the honestly-moving chain produced during `--wait`. The fetch-once freeze is thus load-bearing against an ordinary moving chain, not only a malicious daemon.

**Note on line citations.** The `:NNN` line cites in §3–§4 index an earlier `light/trustless_read.cpp`; the current file maps the proof fetch to `:683-684`, the `--wait` loop to `:596-600`, and the held compare to `:765` (the mutant site).
...
**Scope of "every trust-minimized reader" (corrected 2026-09-18).** The statement quantifies over *readers*, not over `light/main.cpp`. It is the statement that is authoritative — `WH-SN` (§5) is stated for "any binding consumer of §4.6", so a reading confined to one file would leave the theorem silent about readers that exist. The enumeration below was therefore **incomplete, not narrower on purpose**: it listed only `light/main.cpp`'s commands and omitted the `determ-light outbox` verbs, two of which are binding readers. That omission is how `outbox enqueue`'s dropped `--wait` (`docs/SECURITY.md` S-112) went unnamed — nothing in the enumeration said it owed a forward. The `outbox` rows below close it. Nothing else in this section changes: no lemma is re-derived, and `WH-1`–`WH-5` applied to those readers already, since they reach the identical helper.
...
| `outbox enqueue` (the nonce hint) | the shared `outbox` `parse_args` `--wait` branch (`light/outbox_cli.cpp`) | `nonce_hint_trustless(…, a.wait)` → `read_account_trustless(…, wait_seconds)` (`light/outbox.cpp`) | via the account read |
| `outbox reconcile` | the same parse site, into `ReconcileOptions::wait_seconds` | `reconcile_all(…, opt)` → `read_account_trustless(…, opt.wait_seconds)` and `verify_and_bind(…, wait_seconds)` (`light/outbox.cpp`) | via the account read, and direct per located inclusion |
...
The five `outbox` verbs with no binding read — `submit`, `status`, `replace`, `prune`, `recover` — are in that class, and until 2026-09-18 they were the one place where "does not accept it" was false: the shared `parse_args` took `--wait` for all seven verbs and ignored it on those five. Since then each verb accepts only the flags it reads and refuses the rest with a named diagnostic and a non-zero exit (`docs/SECURITY.md` S-113), so the omission is now enforced rather than assumed. The same rule holds within `enqueue`, whose binding read is optional: `--wait` there is refused when no `--rpc-port` is given or when an explicit `--nonce` skips the read entirely, so an accepted `--wait` on `enqueue` always reaches a binding read — which is what makes the row above unconditional. ∎
...
| `tools/test_light_outbox_flag_surface.sh` | (offline; the `outbox` per-verb option surface) | The `WH-6` enumeration mechanized for the `outbox` verbs: every (verb, flag) pair the shared parser recognizes is measured against the running binary and must be acted on or refused, and `selftest-outbox-hint-wait` drives the `enqueue` nonce hint over a committee-signed fixture chain to show the operator's `--wait` reaches it. |
| `tools/test_light_wait_surface.sh` | (offline; static completeness) | Every head-anchored binding consumer forwards a wait expression into the helper's DERIVED wait slot — the check that covers the `cmd_enqueue` → `nonce_hint_trustless` call site. |
...
- **Implementation.** The `--wait <seconds>` flag and its `max_wait_seconds` plumbing are shipped: the hold-and-wait loop is `light/trustless_read.cpp:383-387`; the parameter is threaded through `read_account_trustless` / `read_stake_trustless` / `verify_state_root_at` and forwarded by all nineteen binding consumers of `WH-6` (seventeen in `light/main.cpp`, plus `outbox enqueue` and `outbox reconcile` in `light/outbox_cli.cpp` / `light/outbox.cpp`; the census grew from fifteen when the `b:` `verify-abort-record` and `k:` `verify-constant` readers landed, and to nineteen on 2026-09-18 when `WH-6`'s enumeration was corrected to cover the `outbox` verbs it had omitted — every one forwards `wait_seconds` into the helper's declared wait slot, so `WH-1`–`WH-5` apply verbatim). Default is `0` (fail-closed-at-head, unchanged). Tracked as the liveness follow-up to `docs/SECURITY.md §S-042`.
