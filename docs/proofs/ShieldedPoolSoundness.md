...
Together these close the **deposit/withdraw + commitment-set + nullifier** parts of [`ConfidentialTxBundleSoundness.md`](ConfidentialTxBundleSoundness.md) **NC-1** ("no shielded-pool STATE MODEL"). The remaining piece is **CONFIDENTIAL_TRANSFER** (confidential→confidential, the full DCT1 bundle consuming and producing notes) — not yet shipped.
...
- **Spec.** Complete (this document); design entries CRYPTO-C99-SPEC.md §3.22 (SHIELD) + §3.22b (UNSHIELD).
- **Consensus integration shipped and green.** SHIELD (`TxType = 12`) + UNSHIELD (`TxType = 13`): apply cases + `cn:`/`c:accumulated_shielded` state leaves + conditional snapshot serialize/restore (`chain.cpp`); `accumulated_shielded_` counter + `expected_total()` subtraction + `shielded_note_exists` (`chain.hpp`); submit-time accept-rules + relaxed anon gate (`validator.cpp`); provisional debit/credit (`producer.cpp`); accept-rules `determ_shield_verify` / `determ_unshield_verify` + the tx-bound `determ_p256_balance_prove_bound`/`_verify_bound` (`ctxbundle.c` / `balance.c`); the shared `unshield_spend_ctx_hash` (`shielded.hpp`). Gates: `test-shield` + `test-unshield` via `tools/test_shield.sh` / `tools/test_unshield.sh`. State-root invariance witnessed by the FAST golden corpus. Validated MSVC + GCC/MinGW (`ci_local`).
- **Claims.** §3.22 SHIELD: SP-1 (accept-rule binds `C` to public `A`), SP-2 (supply conserved), SP-3 (additive + state-root-invariant), SP-4 (double-mint blocked + belt-and-suspenders re-verify), SP-5 (snapshot round-trip / shield-free byte-identity). §3.22b UNSHIELD: SP-6 (tx-bound proof defeats front-run/redirect), SP-7 (withdraw conserves supply, no underflow), SP-8 (commitment is its own nullifier — no double-spend), SP-9 (enforced at both ingresses via the shared ctx helper) — at the proven-in-code / inherited-in-prose split in §4.
- **Non-claims (NC-1..NC-6).** No confidential→confidential transfer yet (the actual privacy op); the SHIELD/UNSHIELD amount is PUBLIC; no sender/receiver privacy; not post-quantum; single-shard / P-256 profile; UNSHIELD is NOT graph-private (links the note to the recipient — protects theft, not linkability).
- **Limits (L-1..L-5).** Soundness is an inherited reduction; conformance is a fixed witness + the golden invariance corpus; the accept-rules live in two sites (shared helpers); not a timing proof beyond §3.19's CT-hardening; the tx-bound proof is a new AI-designed, session-audited construction, not externally reviewed.
