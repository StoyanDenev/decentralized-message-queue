...
- **NC-8 — No on-chain output-secret delivery (recipient channel is off-chain).** A transfer's output notes are commitments whose blindings the *sender* chose; to let a recipient later spend an output, the recipient must learn its `(value, blinding)` — which §3.22c does **not** deliver on-chain (no encrypted memo / stealth-address scheme). In this increment the sender and recipient coordinate the output secret off-chain (or the sender transfers to their own future notes). An on-chain encrypted note-plaintext channel is a follow-up.
