...
**`int cmd_block_verify(int, char**)` is IMPLEMENTED and SHIPPED in `light/main.cpp:806-959`** (dispatched on `block-verify` at `:7564`; help text at `:118-122`). It is exercised offline by `tools/test_light_block_verify.sh` (boots a 3-node cluster, fetches block 1 as a FULL `Block` via `determ block-info 1 --json` plus the committee via `determ validators --json`, then runs `block-verify` locally: happy-path PASS + four fail-closed negatives — tampered `tx_root`, tampered `creator_block_sig`, wrong committee pubkey, missing field).

The composed verification cores are read directly off source:
- **TX-ROOT** — recomputed inline in `cmd_block_verify` (`light/main.cpp:882-915`): a `std::vector<Hash>` filled from `creator_tx_lists`, then `std::sort` + `std::unique`, then `SHA256(concat)`, compared against the stored `tx_root` (case-normalized). This mirrors `src/node/producer.cpp::compute_tx_root` (`:304-312`) and the light client's own `light_compute_tx_root` in `light/verify_tx_inclusion.cpp:35-43`.
- **SIGS** — delegated to `determ::light::verify_block_sigs` (`light/verify.cpp:334-427`), which calls `light_compute_block_digest` (`light/verify.cpp:124-191`) to recompute the digest **internally**, then per-creator Ed25519-verifies (`determ::crypto::verify`) against it, enforcing `required = bft ? (2K+2)/3 : K`.
- **STRUCTURE** — a pure well-formedness predicate over the parsed JSON (`light/main.cpp:846-880`): seven required fields present and correctly typed (`index`, `prev_hash`, `timestamp`, `creators[]`, `creator_tx_lists[]`, `tx_root`, `creator_block_sigs[]`), `creators[]` non-empty.
...
The `determ-light block-verify` composite. Its control flow, read off `light/main.cpp:806-959`:

1. **Argument parse + validation.** `--block` and `--committee` are required; `--bft`, `--json` are optional. An unknown arg (`:815-818`), a missing `--block`/`--committee` (`:820-823`), an unreadable / non-JSON file (`:826-832`), or a non-object block (`:837-840`) → `return 1` (args/parse error) before any check verdict is recorded. A `{block:{...}}` envelope **is** transparently unwrapped *only* when it lacks a top-level `creator_tx_lists` (`:833-836`), so both the `block-info --json` envelope shape and the raw `Block` shape are accepted.

2. **STRUCTURE check (`:846-880`).** A pure well-formedness predicate: the seven required fields are present and of the right JSON type (string / number / array), and `creators[]` is non-empty. STRUCTURE PASS ⟺ the JSON is a syntactically well-formed `Block` envelope.

3. **TX-ROOT check (`:882-915`), gated on STRUCTURE PASS.** Build the union `uni` of every 32-byte hash across `creator_tx_lists` (each entry hex-decoded to `Hash` via `from_hex_arr<32>`), `std::sort` + `std::unique` it, compute `SHA256(‖_{h∈uni} h)`, and byte-compare against the case-normalized stored `tx_root`. TX-ROOT PASS ⟺ `computed == stored`. If STRUCTURE FAILed, TX-ROOT is recorded as **SKIP** (`:914`).

4. **SIGS check (`:917-934`), gated on STRUCTURE PASS.** Call `verify_block_sigs(block_json, committee_json, bft)`; SIGS PASS ⟺ `r.ok`. The verifier **recomputes the digest internally** (`light_compute_block_digest`), confirms every `creators[i]` is in the supplied committee, requires `creator_block_sigs.size() == creators.size()`, Ed25519-verifies each non-sentinel signature against the recomputed digest, and enforces `valid ≥ required`. If STRUCTURE FAILed, SIGS is recorded as **SKIP** (`:933`).

5. **Aggregate verdict + exit (`:936-958`).** `overall = (failed == 0)`. Exit `0` iff `overall`; exit `2` iff any check FAILed; exit `1` on the args/parse faults of step 1. `--json` emits an aggregate object (`audit`, `passed`, `failed`, `checks[]`); human mode emits a per-check summary. Output is read-only over the verdicts.

### 1.2 The three components

| Component | Implementation basis | What it establishes | Backing |
|---|---|---|---|
| **STRUCTURE** | `light/main.cpp:846-880` (new predicate) | The block JSON is a syntactically well-formed `Block` envelope (seven fields present, correct types, `creators[]` non-empty) | Structural; no cryptographic assumption (LBV-3 §3.3) |
| **TX-ROOT** | `light/main.cpp:882-915` | The block's stored `tx_root` equals the canonical commitment over its `creator_tx_lists` — **byte-identical to the daemon's accept gate** | A2 (LBV-1 §3.1) |
| **SIGS** | `verify_block_sigs` (`light/verify.cpp:334-427`) | A quorum of valid Ed25519 committee signatures exist over **`light_compute_block_digest(b)` — recomputed from THIS block** | A1, **UNCONDITIONAL** (LBV-2 §3.2) |

### 1.3 Out of scope (intentional — the verifier's coverage boundary)

- **Committee provenance.** SIGS verifies signatures against the **operator-supplied** `--committee` map. `block-verify` proves the block is signed by *that supplied committee*; it does **not** prove the supplied committee is the genuine, height-correct committee. For full trust-minimization the committee must be the genesis/chain-derived set — derived trustlessly by the sibling commands `determ-light verify-chain` (`light/main.cpp:1443`, anchor + walk + per-block sig verify) and `committee-at-height` (`light/main.cpp:6572`, report committee-verified creators at block H). §5 F-LBV3.
...
- **Cross-shard / F2-reconciled digest extensions — now in scope (F-7).** `light_compute_block_digest` is byte-identical to `producer.cpp::compute_block_digest` on **ALL blocks**: the three F2 roots (inbound-receipts / equivocation-events / abort-events) plus the merged-block tail (`partner_subset_hash`, `timestamp`) are now bound on the **same data-driven gates** as the node. A stripped header (empty F2 collections / zero view roots) collapses to the byte-identical v1 core; a full block binds the F2 roots, reproducing the node's digest exactly. For SIGS to PASS on an F2 / cross-shard block the verifier must see the FULL body — the online `verify-chain` walk re-fetches it (the F2 full-block fallback in `verify_chain_walk`); the offline single-block `block-verify` PASSes when handed the full `Block` JSON (the `block-info --json` shape it already consumes). See §3.2 (the byte-equivalence domain) and §5 F-LBV5.
...
The daemon's `src/node/producer.cpp::compute_tx_root` (`:304-312`):
...
The light client's inline reimplementation in `cmd_block_verify` (`light/main.cpp:887-901`):
...
The daemon's `compute_block_digest` (`src/node/producer.cpp:619-704`; the v1 core is the `:619-631` prefix, the F2 tail is `:632-702`):
...
The light client's `light_compute_block_digest` (`light/verify.cpp:124-191`) reproduces the **whole** function **byte-for-byte** (same field set, same order, same encoders) — both the v1 core *and* the F2 tail (the three view roots + the merged-block `partner_subset_hash` / `timestamp`, `light/verify.cpp:137-189`), each bound on the **same data-driven gates** as the node (F-7; the four F2 hash helpers are re-implemented as file-local statics at `light/verify.cpp:34-95`). Critically, the light client's `verify_block_sigs` (`light/verify.cpp:382-406`) computes the digest **from the block itself** and verifies each signature against it:
...
A pure structural predicate (no crypto, `light/main.cpp:850-873`): the parsed JSON `contains` each of the seven required keys with the correct JSON type (`{"index",num}`, `{"prev_hash",str}`, `{"timestamp",num}`, `{"creators",arr}`, `{"creator_tx_lists",arr}`, `{"tx_root",str}`, `{"creator_block_sigs",arr}`), and `creators[]` is non-empty. The hex-width / byte-decoding of individual fields is enforced downstream by TX-ROOT (`from_hex_arr<32>` throws on bad width) and SIGS (`Block::from_json` + `parse_committee`); STRUCTURE supplies the presence + type + non-emptiness conjunction those two consume.
...
2. **Hashing.** Both append each set/range member to a `determ::crypto::SHA256Builder` and `finalize()` — the *identical* builder type and call sequence in both (the daemon at `producer.cpp:309-311`, the light client at `main.cpp:899-901`). `SHA256Builder::append(Hash)` appends exactly the 32 raw bytes with no per-append framing; appending the same byte sequence then finalizing yields the same digest. ⇒ Same digest.
...
*Part 1 — the digest is the block's true digest.* `verify_block_sigs` computes `digest := light_compute_block_digest(b)` at `light/verify.cpp:382`, where `b` is the parsed block under test. By Lemma L-2 of `LightClientThreatModel.md` (and §2.2 above), `light_compute_block_digest` is a byte-for-byte copy of `src/node/producer.cpp::compute_block_digest` over the **full** field set — the v1 core `index, prev_hash, tx_root, delay_seed, consensus_mode, bft_proposer`, then per-creator `(domain ‖ tx_list ‖ ed_sig ‖ dh_input)`, then (F-7) the three conditional F2 view roots (inbound-receipts / equivocation-events / abort-events) and the merged-block tail `partner_subset_hash`, `timestamp`, each appended on the **same data-driven gates** as the node (`producer.cpp:619-704` ≡ `verify.cpp:124-191`). Therefore, for **any** block the producer signed — common-case *or* cross-shard / F2-reconciled — `light_compute_block_digest(b)` equals the *exact* digest the committee signed when it produced `b`. There is **no operator input** in this step: the digest is a pure function of `b`'s own fields. (For an F2 block this requires the verifier to see the FULL body, not a stripped header — the walk re-fetches it; see §5 F-LBV5 for the stripped-header collapse and the full-block path.)

*Part 2 — the per-signer verification is sound for that digest.* For each creator `i` with a non-sentinel signature `σ_i` and committee pubkey `pk_i = pubkey_of.at(b.creators[i])`, SIGS counts `i` as valid iff `determ::crypto::verify(pk_i, digest, σ_i)` succeeds (`light/verify.cpp:398`). By A1 (Ed25519 EUF-CMA, `Preliminaries.md §2.2`), an adversary not holding `sk_i` cannot produce a `σ_i` with `Verify(pk_i, digest, σ_i) = 1` for a `digest` member `i` never signed, except with probability `≤ 2⁻¹²⁸`. SIGS PASS requires `valid ≥ required` such signers, each a distinct committee domain: the loop iterates `creators[i]` once each; every creator is checked for membership in the supplied committee before verification (`verify.cpp:367-373`, FAIL if any creator is absent), and `creator_block_sigs.size() == creators.size()` is enforced (`verify.cpp:374-380`). The per-signer loop is a faithful `N`-fold replication of the single-sig verify primitive with no cross-signer state — the only shared state is the immutable `pubkey_of` map and the immutable `digest` — instantiating `BatchSigningSoundness.md` BS-3's "loop adds no attack surface" structural-isolation property for the committee-sig loop.
...
- **Args/parse fault.** Unknown arg (`:815-818`), missing `--block`/`--committee` (`:820-823`), unreadable / non-JSON file (`:826-832`), non-object block (`:837-840`) → `return 1` *before any verdict is recorded*. Exit 1, never PASS.
- **STRUCTURE FAIL.** A missing/mistyped required field or empty `creators[]` → `struct_ok = false` → `++failed` → `overall = false` → exit 2 (`:874-879, 936, 958`). TX-ROOT and SIGS are then recorded SKIP (`:914, :933`), which cannot rescue the verdict — `failed ≥ 1` already.
...
SIGS's per-creator verification loop (`verify.cpp:386-406`) is a composition of the single-sig verify primitive, exactly as BS-3 composes the single-tx verify. LBV-2 Part 2's "no cross-signer state, sole shared state is the immutable committee map + immutable digest" argument *is* BS-3's structural-isolation argument specialized to the committee-sig loop: signer `i`'s verdict depends only on `(σ_i, digest, pk_i)`, never on signer `j ≠ i`.
...
### F-LBV5 The self-recomputed digest is FULL byte-parity with the producer — F2 / cross-shard blocks now VERIFY (CLOSED, F-7)

**Previously** `light_compute_block_digest` bound only the v1 core (`index, prev_hash, tx_root, delay_seed, consensus_mode, bft_proposer`, then per-creator `(domain ‖ tx_list ‖ ed_sig ‖ dh_input)`) and **omitted** the further roots the producer appends for cross-shard / F2-reconciled blocks. On such a block the light digest differed from the producer's, the genuine committee signatures did not verify, and SIGS **FAILed** — a fail-closed false-negative on cross-shard / F2 blocks.

**F-7 closes this (full digest parity).** `light_compute_block_digest` (`light/verify.cpp:124-191`) is now a **byte-for-byte mirror of the whole** `producer.cpp::compute_block_digest` (`producer.cpp:619-704`): after the v1 core it binds the three F2 view roots — a `compute_view_root` over the `inbound_receipts` keys (`light/verify.cpp:146-151` ≡ `producer.cpp:640-646`), and the non-zero-gated eq/abort view-roots (`light/verify.cpp:157-168` ≡ `producer.cpp:661-672`) — then the merged-block tail `partner_subset_hash` (`light/verify.cpp:175-177` ≡ `producer.cpp:685-687`) and `timestamp` (`light/verify.cpp:187-189` ≡ `producer.cpp:700-702`). The four F2 hash helpers (`hash_cross_shard_receipt`, `hash_equivocation_event`, `hash_abort_event`, `compute_view_root`) are re-implemented as file-local statics (`light/verify.cpp:34-95`) byte-equal to `producer.cpp:322-382`, since the light client deliberately does not link `producer.cpp`. **Every append is gated on the SAME data-driven condition as the node**, so the parity is exact on **all** blocks: a STRIPPED header (empty F2 collections, zero view roots) makes every F2 gate false and the digest collapses to the byte-identical v1 core — non-F2 header-only sync is unchanged; a FULL block populates the collections and the light client binds the identical inbound / eq / abort / partner / timestamp data the committee signed, so it **VERIFIES** the cross-shard / reconciled block rather than fail-closing on it.

**The full-block path.** `rpc_headers` STRIPS the heavy F2 collections, so a stripped header alone cannot reproduce an F2 block's digest. The online `verify-chain` walk handles this with the F2 full-block fallback in `verify_chain_walk` (`light/trustless_read.cpp:198-248`): when a stripped header's committee-sig check fails (MD then BFT), it re-fetches the FULL block via `rpc.call("block", {index})`, recomputes the now-F2-aware digest, and re-verifies — pinned by the requirement that the full body's recomputed `compute_hash()` EQUAL the stripped header's `block_hash` that `verify_headers` already chained (`trustless_read.cpp:218-234`), so a daemon can neither substitute a differently-positioned signed block nor forge sigs over a doctored body (the same `committee_bound_state_root` full-block trust step). The offline single-block `block-verify` needs no fallback: handed the FULL `Block` JSON (the `block-info --json` shape it already accepts) it binds the F2 roots and SIGS PASSes directly. Either way, SIGS now **PASSES soundly on F2 / cross-shard blocks** (under A1, exactly as LBV-2), not fail-closes. The KEEP-IN-SYNC comment at `light/verify.cpp:97-123` documents the full mirrored field set; `tools/test_block_digest_xbinary_parity.sh` pins FULL byte-parity (light == producer, all 15 tokens including the 3 F2 roots).

### F-LBV6 Sentinel / quorum semantics inherited from `verify_block_sigs`

SIGS inherits `verify_block_sigs`'s exact handling: a 64-byte all-zero signature is a *sentinel abstention* — permitted (and uncounted) only under `--bft`, a hard FAIL in MD mode (`verify.cpp:388-396`); a creator absent from the committee map is a hard FAIL (`verify.cpp:367-373`); `creator_block_sigs.size() != creators.size()` is a hard FAIL (`verify.cpp:374-380`). The MD quorum is full K; the `--bft` quorum is `⌈2K/3⌉ = (2*creators.size()+2)/3` (`verify.cpp:408-410`). An operator running MD (`--bft` omitted) against a BFT-escalated block (which carries sentinel-zero slots) gets a clean FAIL, not a false PASS — they must pass `--bft` to verify such blocks. These are not new surfaces; they are `verify_block_sigs`'s behavior, carried verbatim.
...
|---|---|---|---|
| — | `cmd_block_verify` (composite) | `light/main.cpp:806-959` | Arg parse, `{block:{...}}` unwrap, three checks, conjunction, exit 0/1/2. |
| LBV-1 | TX-ROOT inline recompute | `light/main.cpp:882-915` | `vector<Hash>` + `sort` + `unique` + `SHA256(concat)`; case-normalized compare vs stored `tx_root`. |
...
| LBV-1 | daemon original | `src/node/producer.cpp:304-312` | `compute_tx_root` — the byte-identical reference. |
| LBV-2 | `verify_block_sigs` | `light/verify.cpp:334-427` | Per-creator Ed25519 verify over the **self-recomputed** digest; quorum `K` / `⌈2K/3⌉`. |
| LBV-2 | `light_compute_block_digest` | `light/verify.cpp:124-191` | **FULL** byte-for-byte mirror of `producer.cpp::compute_block_digest` (v1 core `:124-136` + F2 tail `:137-189`, F-7). |
| LBV-2 | F2 hash helpers (file-local statics) | `light/verify.cpp:34-95` | `hash_cross_shard_receipt` / `hash_equivocation_event` / `hash_abort_event` / `compute_view_root` — byte-equal to `producer.cpp:322-382`. |
| LBV-2 | digest recomputed (not supplied) | `light/verify.cpp:382` | `Hash digest = light_compute_block_digest(b);` — the UNCONDITIONAL improvement over wallet BV-2. |
| LBV-2 | daemon original | `src/node/producer.cpp:619-704` (v1 core `:619-631` + F2 tail `:632-702`) | `compute_block_digest`; F-LBV5 (F-7) confirms the light copy now mirrors the F2 tail in full. |
| LBV-2 | per-signer loop isolation | `light/verify.cpp:386-406` | No cross-signer state; instantiates `BatchSigningSoundness.md` BS-3. |
| LBV-3 | STRUCTURE predicate | `light/main.cpp:846-880` | Seven-field presence + type + non-empty `creators[]`. |
| LBV-3 | conjunction + exit | `light/main.cpp:936-958` | `overall = (failed == 0)`; exit 0/1/2. |
| LBV-3 | committee-provenance escape hatch | `light/main.cpp:1443` (`verify-chain`), `:6572` (`committee-at-height`) | Trustless committee derivation (anchor + walk per T-L1/T-L2). |
...
- **Implementation.** **SHIPPED.** `int cmd_block_verify` is in `light/main.cpp:806-959` (dispatched on `block-verify` at `:7564`), exercised by `tools/test_light_block_verify.sh` (cluster-boot fixture + happy-path PASS + four fail-closed negatives). Composes STRUCTURE (`:846-880`), TX-ROOT (`:882-915`), and SIGS via `verify_block_sigs` (`light/verify.cpp:334-427`).
...
- **Known limitations (§findings).** F-LBV1 (STRUCTURE is well-formedness, not semantic validity); F-LBV2 (single block only — no genesis/continuity/chain context); **F-LBV3 (the load-bearing one: the committee is operator-supplied — `block-verify` proves the block is signed by the SUPPLIED committee, not the genuine height-correct one; pair with `verify-chain` / `committee-at-height` for trustless committee provenance)**; F-LBV4 (TX-ROOT commits over the supplied lists, not their correctness as a tx-set); F-LBV5 (CLOSED, F-7: the self-recomputed digest is now FULL byte-parity with the producer — cross-shard / F2-reconciled blocks VERIFY when handed the full body, via the walk's full-block fallback, instead of fail-closing); F-LBV6 (sentinel / quorum semantics inherited verbatim from `verify_block_sigs`). None undermines the per-invocation soundness of LBV-3.
