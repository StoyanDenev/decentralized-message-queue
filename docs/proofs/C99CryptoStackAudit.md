...
1. **OpenSSL is a complete oracle for the core.** `test-blake2b-c99` checks the
   ...
   length grid (0..1000, spanning the 128-byte block + multi-block + counter
   paths). Any error in the IV, SIGMA schedule, G mixing, 12-round structure,
   counter, or finalize flag fails this immediately — it exercises the entire
   compression machinery.
2. **An independent (non-OpenSSL, non-libsodium) oracle for the modes OpenSSL
   can't reach.** OpenSSL's EVP exposes only the unkeyed 64-byte digest, so the
   keyed (MAC) and variable-length (e.g. 256-bit) outputs are pinned against
   reference vectors generated by Python's `hashlib.blake2b` (a separate BLAKE2
   implementation) — exercising the param-block key-length + digest-length bytes
   and the key-block prepend.
3. **Self-consistency + parameter hardening.** Incremental `update` == one-shot
   across the unkeyed/keyed × 64/32-byte-output matrix; keyed ≠ unkeyed;
   output-length is in the param block (BLAKE2b-256 is not a truncated
