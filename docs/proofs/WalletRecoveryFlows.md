...
| output-file permissions (all of the above) | `tools/test_wallet_out_perms.sh` | 18 assertions (measured 2026-09-18 on Linux x86_64 — this one was already right), covering the (g) ORDERING property under `strace` against a PRE-EXISTING 0644 target — among the mode-setting calls between the creating open and the first write, at least one exists and the last sets 0600 — and, under an `LD_PRELOAD` shim that denies every chmod, that a failed narrowing is both real harm (the file is left at 0644) and reported (stderr diagnostic + `perms_narrowed: false`), with the success path unchanged. Both legs SKIP by name where strace/ptrace or LD_PRELOAD+cc are unavailable. |
