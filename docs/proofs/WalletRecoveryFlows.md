...
The flows under analysis are operator-facing thin compositions: each CLI takes file inputs and produces file outputs.

> **At-rest containers are binary since 2026-08-12 (D2 step 3).** What were "shares JSON / envelopes
> JSON / recovery-setup JSON" files are now the canonical binary containers `DSS1` (Shamir shares,
> DISTINCT-x enforced on decode), `DBE1` (share envelopes), `DRS1` (recovery setup; the `scheme` string
> dropped — the passphrase scheme is the only one shipped) and `DAK1` (plaintext keypair), each
> magic-prefixed, explicit-LE, length-prefixed, EXACT-length on decode
> (`wallet/keyfmt.hpp`, `wallet/recovery.hpp`). `keyfile-recover --out` now writes the RAW secret bytes
> rather than a `{"secret_hex": …}` JSON wrapper; hex renderings remain on stdout as non-authoritative
> views. **No theorem in this document changes**: T-1..T-5 are about the Shamir/AEAD composition, the
> threshold gate, and the CLI output sinks, none of which depend on how the files are framed. The one
> strengthening worth noting is that `DAK1` decode DERIVES the public key from the seed and requires
> equality, so a recovered account file cannot state an address its key material does not produce.
> Gate: `determ-wallet selftest-backup-binary` (19 cases, mutant-verified on the DSS1 DISTINCT-x check).
> References below to `…JSON` file inputs/outputs of the recovery flows should be read as those
> containers; the account records that remain JSON (`account-import` / `account-recover` stdout and
> `--json`) are unchanged. The proof's role is to confirm that the *composition* introduces no soundness loss relative to the underlying primitives (`shamir::split` / `shamir::combine` / `envelope::encrypt` / `envelope::decrypt` / `crypto_sign_ed25519_seed_keypair`) and that the CLI boundary doesn't introduce side-channel leakage (priv-key material on stdout, lingering ephemeral buffers, AAD-binding bypass paths).
...
**Theorem T-3 (Keyfile-decryption soundness — wrong passphrase ⇒ AEAD-tag-verify fail).** For every encrypted keyfile produced by the canonical `keyfile-create` path (since 2026-08-12: the `DNK1` binary container — magic ‖ raw 32-byte pubkey ‖ length-prefixed `DWE` envelope, with `aad := the raw 32-byte pubkey` and plaintext = the raw 32-byte seed; before that: a `DETERM-NODE-V1` header line + a dot-hex `DWE1` blob with `aad := utf8(header_pubkey_hex)` over a `{pubkey, priv_seed}` JSON plaintext), decryption with the correct passphrase yields exactly the original seed, whose derived public key the decoder additionally checks against the header; decryption with any other passphrase `P' ≠ P` returns `std::nullopt` with probability `≥ 1 - 2⁻¹²⁸` per attempt. Under AES-GCM SUF-CMA + PBKDF2-HMAC-SHA-256 strong key-derivation (RFC 8018 + NIST SP 800-132), the tag-verify failure is indistinguishable from a tampered-ciphertext failure, so the operator cannot extract per-byte information about `P` from the failure mode alone. Proof reduces to FA12 T-16 (AEAD envelope binding) via the `wallet/main.cpp:3365` decrypt call.
...
The wallet's anon-address derivation is `"0x" + lowercase_hex(pubkey)` (per `wallet/main.cpp:1728`); the canonical single-account JSON record shape is `{"address": "0x...", "privkey_hex": "..."}` byte-identical between `account-import` (lines 1730-1733) and `account-recover` (lines 4389-4392), ensuring T-4. The `keyfile-decrypt` AAD binding (`wallet/main.cpp:3520`) defends against keyfile-substitution: an attacker swapping the envelope blob from a different validator's keyfile fails AEAD verification because the AAD bytes (= the raw 32-byte header pubkey since 2026-08-12; `utf8(header_pubkey_hex)` before) differ. A second, independent layer now also applies: the decoder derives the public key from the decrypted seed and requires equality with the header, so even an attacker who controls envelope generation cannot present a container whose stated identity differs from its key material. The `keyfile-info` flow is **passive** (never invokes decrypt; no passphrase required; T-3's claim doesn't apply because no decryption is attempted).
...
**(g) Recovery transcripts (file leakage).** Output file contains priv-key in plaintext. **Mitigated operationally — on POSIX from creation, since 2026-09-17.** The ORDERING is the substance of this row and it used to be wrong: every output file was created by `std::ofstream` at the umask mode (0644 under the usual `umask 022`), the complete secret was written into it, and `chmod 0600` ran only afterwards, so the secret sat on disk world-readable for the duration of every successful write. `write_bytes_file_0600` now creates the file at 0600 and `fchmod`s the descriptor before the first write — both, because the create mode is ignored when the file already exists — and a failed narrowing is reported on stderr and as `perms_narrowed: false` in the `--json` summary instead of being discarded. On Windows the window is UNCHANGED: the mode argument only drives FILE_ATTRIBUTE_READONLY and the effective ACL is inherited from the parent directory (`S005PassphraseKeyfile.md` F-4). `account-import-many` was migrated to the same helper on the same day, per record, so every imported account's plaintext keyfile is 0600 before its seed is written — measured on records 2 and 3, not only the first. `keyfile-rotate` was the one write-and-rename shape here: it staged correctly but created the temp at the umask mode and narrowed it only after `fs::rename`, so the wide file existed under the FINAL name; the staging write now goes through the helper, so the inode the rename publishes is already narrow. S-109 in `docs/SECURITY.md` carries what is still open: the Windows half, six non-key-material `std::filesystem::permissions` sites in `wallet/main.cpp` that still discard their `error_code`, and three outputs that narrow nothing at all — `create-recovery`'s DRS1 container and the `encrypt-message` / `decrypt-message` `--out` files, the last of which is recovered plaintext.
...
|---|---|---|
| `shamir-rotate` | `tools/test_wallet_shamir_rotate.sh` | **52 assertions** (measured 2026-09-18 on Linux x86_64 by running the wrapper; it read 33 from this section's first version until then, and the wrapper additionally reports a `skip` column since 2026-09-18 — on a non-POSIX platform the 0600 check is 1 skip and 51 pass, never 52), covering polynomial-refresh round-trip, x-coordinate preservation, polynomial-distinctness across rotations, no-secret-in-summary, T-N edge cases, T=1 degeneracy, large-N (50 shares), multiple secret sizes, --json shape, --force overwrite, threshold validation, duplicate-x rejection, fresh-randomness-on-each-call. |
| `keyfile-recover` | `tools/test_wallet_keyfile_recover.sh` | **60 assertions** (measured 2026-09-18 on Linux x86_64; previously recorded as "30+"), covering T-of-N round-trip, different T-subsets yielding the same secret, --out / --json output shape, wrong-passphrase exit 2, insufficient-shares exit 2, shares/envelopes mismatch detection (cross-verification step 2 in L-2), --threshold optional gating, missing-flag exit 1 vs malformed-input exit 2 split. |
| `account-recover` | `tools/test_wallet_account_recover.sh` | **57 assertions** (measured 2026-09-18 on Linux x86_64; previously recorded as 32), covering composite round-trip (account-create-batch → backup-create → account-recover), different T-subsets yielding the same address+priv, --json / --out shape, wrong-passphrase exit 2, insufficient-shares exit 2, T=1 trivial case, share/envelope mismatch, --threshold required + bounds-check, **cross-CLI parity assertion (#31) verifying T-4 composition identity** vs `keyfile-recover` + `account-import` end-to-end. |
| `account-import` | `tools/test_wallet_account_import.sh` | **58 assertions** (measured 2026-09-18 on Linux x86_64; previously recorded as 26), covering 32-byte seed form, 64-byte seed||pubkey form, mismatched-pubkey rejection, round-trip with account-create-batch, determinism (same seed → same address), --out file shape + perms, --json shape, parent-dir-missing exit 1, --priv length validation, output-pubkey-derivation correctness. |
| `keyfile-decrypt` | `tools/test_wallet_keyfile_decrypt.sh` | **52 assertions** (measured 2026-09-18 on Linux x86_64); covers correct-passphrase decrypt, wrong-passphrase exit 2, AAD-binding detection, malformed-keyfile rejection. |
| `keyfile-info` | `tools/test_wallet_keyfile_info.sh` | **40 assertions** (measured 2026-09-18 on Linux x86_64); covers passive metadata emission, no-decrypt path, malformed-keyfile exit 2 (structural error). |
| output-file permissions (all of the above) | `tools/test_wallet_out_perms.sh` | 50 assertions in four sections (measured 2026-09-18 on Linux x86_64; the figure was 18 for the two original sections, a sibling increment extended it to 44 across three in the same wave, and the outcome section D below brought it to 50 across four), covering the (g) ORDERING property under `strace` against a PRE-EXISTING 0644 target — among the mode-setting calls between the creating open and the first write, at least one exists and the last sets 0600 — and, under an `LD_PRELOAD` shim that denies every chmod, that a failed narrowing is both real harm (the file is left at 0644) and reported (stderr diagnostic + `perms_narrowed: false`), with the success path unchanged; section D adds the OUTCOME leg, the final mode of six outputs, which needs only `stat` and so runs on macOS and the BSDs where the other three cannot. Sections A-D SKIP by name where strace/ptrace or LD_PRELOAD+cc are unavailable, and the wrapper still fails closed if that leaves nothing asserted. **On Windows the wrapper skips ENTIRELY** — there is no POSIX file mode there, so the property is absent rather than unobserved — emitting the `PLATFORM-SKIP:` marker `tools/run_all.sh` counts in its own column: not a pass, not a failure, and the at-rest permissions of wallet key outputs consequently have NO coverage on that platform. |
