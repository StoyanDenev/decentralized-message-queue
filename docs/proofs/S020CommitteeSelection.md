...
1. **Performance.** At any production profile (`single_test` M=K=3, `web` / `web_test` M=4 K=3, `cluster_test` M=K=3, `regional_test` M=5 K=4, `global_test` M=7 K=5), `K` is small enough that the expected hash cost is microseconds — performance is not actually a bottleneck. But at non-standard deployments with larger K (consortium chains might run K=20 or K=50 for stronger Byzantine tolerance), or under pool depletion (validators dropping until `N_pool` shrinks to barely-larger-than-K), the K/N ratio rises and the hash cost compounds.
