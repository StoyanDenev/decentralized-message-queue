...
- **Equivocation replay defense.** A validator that signs two distinct block digests at the same height (BlockSigMsg equivocation) or two distinct contrib commitments at the same (height, generation) (ContribMsg equivocation post-S-006) is recorded via an `EquivocationEvent`. This is a detection + evidence rule, not an apply-layer rule on `AccountState`: since D4 (2026-09-16) apply reads nothing from `b.equivocation_events`, so the event touches neither `stakes_`, nor `accounts_[equivocator].balance`, nor `accounts_[equivocator].next_nonce`. The conclusion this section needs is unchanged and now holds a fortiori — the equivocator's regular tx-flow is gated by `next_nonce` per this proof and the equivocation channel is orthogonal to it.
...
| `EquivocationSlashing.md` (FA6) | Consensus-layer signed-message replay (BlockSigMsg + ContribMsg equivocation); a different replay category, covered separately, and since D4 with no account-state effect at all. |
