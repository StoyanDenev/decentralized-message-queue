...
- [SECURITY.md](SECURITY.md) — canonical security posture. 22 findings mitigated in-session + 1 partial (S-030 D2 via S-033 apply-layer, v2.7 F2 spec'd for full consensus-layer closure) + 2 open Medium (S-016, S-018) + 5 open Low/Op. Zero open Critical, zero open High. Track A (localized fixes) 14/14 complete. Reconciles the rev.7 audit and the prior `OPEN-VULNERABILITIES.md` against current code. What a security reviewer should look at.
