...
- [SECURITY.md](SECURITY.md) — canonical security posture. 25 findings mitigated in-session + 3 partial (S-030 D2 via S-033 apply-layer with v2.7 F2 spec'd for full consensus-layer closure; S-016 inbound-receipts via Option 2 time-ordered admission with v2.7 F2 closing fully via intersection commitment; S-036 `EXTENDED`-mode-specific MERGE_BEGIN witness-window with v2.11 spec'd for full closure) + 1 open Medium (S-018 JSON schema validation, mechanical) + 1 open Low/Op (S-035 unit tests / CI — engineering culture). Zero open Critical, zero open High. Track A (localized fixes) 14/14 complete. S-037 (`dapp_registry` snapshot-restore gap) discovered + closed in-session — `tools/test_dapp_snapshot.sh` regression covers the joint DApp+snapshot surface that had been previously untested. Reconciles the rev.7 audit (preserved out-of-tree) and the prior in-tree `OPEN-VULNERABILITIES.md` (superseded and removed) against current code. What a security reviewer should look at.
