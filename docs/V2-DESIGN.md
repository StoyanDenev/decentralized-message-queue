...
| v2.25 Distributed identity provider (DSSO) | 🔄 reclassified as DApp (2026-05-24); launch scope per `PRE-LAUNCH-DECISIONS.md` §E1 (2026-07-09) | Theme 9 originally framed DSSO as chain-level substrate; reclassified as a chain-aware DApp on top of v2.18 + v2.19 + v2.26 substrate per `proofs/DECISION-LOG.md` 2026-05-24 entry and `proofs/Improvements.md §8.1`. Mechanism: **DLT-B**, `proofs/v2.25-DSSO-DAPP-SPEC.md` (2026-07-15) — the single authoritative DSSO doc. Substrate spec below preserved as historical reference. |
...
- **v2.25 DSSO OPRF** (Theme 9) uses the **DLT-B** threshold-OPRF — t-of-n on the **profile-agnostic NIST P-256 RFC 9497 VOPRF stack** (`src/crypto/p256/`, §3.9b) for BOTH profiles, user-dealt Shamir shares (no DKG) — **not** ristretto255 (never used), **not** X25519 (lacks the group ops; `proofs/v2.25-DSSO-DAPP-SPEC.md` §11), and **not** FROST (removed from the chain path, `FROST_DEVIATION_NOTICE.md`). Assertions are block-anchored via DAPP_CALL — the chain's K-of-K block signature attests inclusion — so DSSO does not depend on a v2.10 FROST DKG. See `proofs/DECISION-LOG.md` 2026-07-15.
...
> - **DSSO's actual direction is DLT-B** (`proofs/v2.25-DSSO-DAPP-SPEC.md`, `proofs/DECISION-LOG.md` 2026-07-15) = a T-OPAQUE-shaped composition without FROST and without an AKE: a **t-of-n verifiable threshold-OPRF on the shipped P-256 RFC 9497 VOPRF stack** (user-dealt Shamir shares, per-share DLEQ verification, Lagrange recombination — any t of n suffice, no evaluation order) sealing an envelope that holds the user's credential key, with assertions **block-anchored via DAPP_CALL** — the K-of-K block signature attests inclusion, and assertion truth reduces to the user's credential signature. T-OPAQUE's essential property (no single operator learns the password) is delivered by the T-OPRF. **FROST stays removed** from the chain path (2026-06-07, `FROST_DEVIATION_NOTICE.md`; module deleted 2026-07-09) — DLT-B introduces no threshold signing. (The intermediate DLT-A direction — X25519 additive n-of-n T-OPRF, 2026-06-07/2026-07-07 — was found unimplementable on the shipped clamped x-only X25519 API and proof-less; see spec §11.)
> - The DSSO OPRF is **profile-agnostic NIST P-256** (**RFC 9497 P256-SHA256**, `src/crypto/p256/`, §3.9b) for BOTH profiles — same posture as the §3.22 shielded pool. Read every "ristretto255", "T-OPAQUE over ristretto255", and "FROST-based" reference below as this DLT-B / P-256 T-OPRF direction, and the substrate narrative itself per `proofs/v2.25-DSSO-DAPP-SPEC.md`.
...
| Item | Effort | Notes |
|---|---|---|
| DSSO DApp — T-OPAQUE-style flow per `proofs/v2.25-DSSO-DAPP-SPEC.md` (DLT-B) | 1-2 weeks | Launch scope (V1.1-PLAN Bundle A / `PRE-LAUNCH-DECISIONS.md` §E1). K DApp instances run by committee members; t-of-n T-OPRF on the shipped §3.9b P-256 stack — nothing vendored, DApp level only. |
