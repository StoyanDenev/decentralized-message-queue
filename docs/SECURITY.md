...
| Partially mitigated | **1** (S-030) | — | **1** (S-016 Option 2 shipped + v2.7 F2 closes Option 1) | **3** (S-035 Option 3 path-portability shipped + Option 1 seeded with 126 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes. S-039 genesis-hash diagnostic-UX gap discovered + lock-in test shipped; fix deferred as wire-compat-break) | **5** |
...
**Severity:** Operational • **Status:** 🟠 Partially mitigated (Option 3 path portability shipped in-session; Option 1 seeded with 126 in-process `determ test-*` subcommands covering every cryptographic foundation; Option 2 outstanding) • **Sources:** Architectural Analysis §4.1, §4.2
...
**Option 1 seeded in-session (per-feature unit tests).** 126 in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof:
...
**Recommended next.** Option 2 first (DSF, ~3-4 weeks, before Phase A) — gives Phase A through D items deterministic-simulation coverage as they land, eliminates ~3 months of A10 work, satisfies Phase D's DSF prerequisite up-front. Option 1 continues incrementally in parallel (add unit tests for new code as it lands; 126 in-process subcommands already shipped). The committed-to-CI workflow file is intentionally not auto-generated (operator picks the platform + Cron schedule).
