...
- ~~S-042 Light-client trustless reads bound state_root to a daemon field~~ — closed via `light/trustless_read.cpp::committee_bound_state_root` (full-block recompute via the `"block"` RPC + committee-signed successor `prev_hash` binding; head fails closed); all trustless readers routed through it; consensus digest / signing_bytes unchanged. New finding (HIGH), fixed same session. See §3 S-042.
