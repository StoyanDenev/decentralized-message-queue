...
- ~~S-042 Light-client trustless reads bound state_root to a daemon field~~ — closed via `light/trustless_read.cpp::committee_bound_state_root` (full-block recompute via the `"block"` RPC + committee-signed successor `prev_hash` binding; head fails closed); all trustless readers routed through it; consensus digest / signing_bytes unchanged. New finding (HIGH), fixed same session. Regression-locked by `tools/test_light_state_root_binding_guard.sh` (static guard + `SELFTEST=1` liveness: asserts no `!= vc.head_state_root` field-trust anchor survives, every reader keeps routing through `committee_bound_state_root`, and the helper's full-block fetch + successor binding check stay intact). See §3 S-042.
