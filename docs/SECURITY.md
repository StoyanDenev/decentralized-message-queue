...
| `determ test-stake-accounting` | Full stake-state-machine invariants. 12 assertions: genesis sentinel; STAKE / slash / DEREGISTER / UNSTAKE state transitions; UNSTAKE pre-unlock fee refund; A1 conservation across full STAKE→UNSTAKE round-trip. Complements `test-unstake-deregister-apply` (per-tx) with lifecycle composition. | stake lifecycle / composition | 12 |
| `determ test-fee-distribution-edge` | Fee + subsidy distribution edge cases. 13 assertions: 3 creators + subsidy 100 → 33/33/34 split (dust to creator[0] alphabetically); fee + subsidy combine; zero fee + zero subsidy no-op; zero-fee tx with subsidy still mints; exact-divide no dust; subsidy < creator count (each 0 + dust); A1 under all scenarios. | E1/E3/E4 / dust | 13 |
| `determ test-equivocation-multi` | Multi-equivocation edge cases. ~14 assertions: two distinct equivocators same block; same equivocator twice (single forfeit); equivocator with no stake (DOMAIN_INCLUSION still deactivates registry); pre-deactivated equivocator (inactive_from overridden); determinism. | FA6 / dual-mechanism edges | 14 |
