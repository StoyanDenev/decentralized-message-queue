...
| S-035 | 🟠 Partially mitigated | No unit tests, no CI, no deterministic simulation framework — Option 3 path-portability shipped (all 79 tests source `tools/common.sh` + use `$PROJECT_ROOT` / `$DETERM`; portable to Linux/Mac builds); Option 1 (gtest/Catch2 unit tests) seeded with 24 in-process `determ test-*` subcommands covering Merkle, committee selection, shard routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types, transaction, merge-event-codec, consensus-msgs, tx-root, genesis, envelope, resolve-fork, shamir, random-state, snapshot-defense surfaces (428 assertions; see `docs/UNIT-TESTS.md`); Option 2 (deterministic-simulation framework) outstanding | `tools/common.sh` + `tools/test_*.sh` (path layer); `determ test-*` subcommands in `src/main.cpp` (Option 1 seeds); `docs/UNIT-TESTS.md` (coverage map); CI workflow file is operator policy | Option 1 continuing per-feature; Option 2: 3-4w |
...
**Option 1 seeded in-session (per-feature unit tests).** Twenty-four in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof:
...
| # | Option | Cost | Status |
|---|---|---|---|
| 1 | **Add gtest/Catch2** for crypto, serialization, state transitions, validator rules. CMake + Linux CI. | 1-2w to seed + ongoing per-feature. | 🟡 seeded in-session — 24 in-process `determ test-*` subcommands ship under `src/main.cpp` (covering Merkle, committee selection, shard routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types, transaction, merge-event-codec, consensus-msgs, tx-root, genesis, envelope, resolve-fork, shamir, random-state, snapshot-defense [S-018 wrong-type rejection lock-in] surfaces; 428 total assertions; FAST=1 suite 29/29 PASS in <14s). See `docs/UNIT-TESTS.md` for the comprehensive coverage map + extension guide. Pattern is extensible; each new feature pairs with a `determ test-FEATURE` subcommand. Backbone is the existing `tools/test_*.sh` infrastructure rather than a separate gtest CMake target — pragmatic for an MVP without a CI runner. |
...
- Low/Op findings: 0 fully open; 2 partially mitigated (S-035 — Option 3 path-portability shipped in-session via `tools/common.sh`; Option 1 [gtest seed] seeded in-session with 24 in-process `determ test-*` subcommands [428 assertions over Merkle / committee selection / shard routing / Ed25519 / SHA-256 / anon-address / genesis-message / state-root / block-rand / rate-limiter / block-digest / block-hash / binary-codec / wire-types / transaction / merge-event-codec / consensus-msgs / tx-root / genesis / envelope / resolve-fork / shamir / random-state / snapshot-defense; coverage map in `docs/UNIT-TESTS.md`]; Option 2 [deterministic-simulation framework] outstanding as v1.x quality work, not blocking. S-039 genesis-hash diagnostic-UX gap discovered + lock-in test shipped in-session; fix is wire-compat break, deferred to coordinated migration); 8 closed in-session; T-001..T-004 are informational `EXTENDED`-mode trade-offs, not bugs
