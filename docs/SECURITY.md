...
| Partially mitigated | **1** (S-030) | — | **1** (S-016 Option 2 shipped + v2.7 F2 closes Option 1) | **3** (S-035 Option 3 path-portability shipped + Option 1 seeded with 93 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes. S-039 genesis-hash diagnostic-UX gap discovered + lock-in test shipped; fix deferred as wire-compat-break) | **5** |
...
**Severity:** Operational • **Status:** 🟠 Partially mitigated (Option 3 path portability shipped in-session; Option 1 seeded with 93 in-process `determ test-*` subcommands covering every cryptographic foundation; Option 2 outstanding) • **Sources:** Architectural Analysis §4.1, §4.2
...
**Option 1 seeded in-session (per-feature unit tests).** Ninety-three in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof:
...
| # | Option | Cost | Status |
|---|---|---|---|
| 1 | **Add gtest/Catch2** for crypto, serialization, state transitions, validator rules. CMake + Linux CI. | 1-2w to seed + ongoing per-feature. | 🟡 seeded in-session — 93 in-process `determ test-*` subcommands ship under `src/main.cpp` (covering atomic-scope, composable-batch, dapp-register, dapp-call, s018-json-validation, Merkle, committee-selection, shard-routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types, transaction, merge-event-codec, consensus-msgs, tx-root, genesis, envelope, resolve-fork, shamir, random-state, snapshot-defense, encoding, chain-helpers, json-validate, block-roundtrip, config-roundtrip, tx-binary-codec, chain-append, state-types, validator-config, timing-profiles, params-constants, supply-invariant, enum-values, block-accessors, make-block-sig, domain-separation, tx-signing-bytes, merge-event-bytes, make-genesis-block, pending-param-changes, merge-state, chain-apply-block, snapshot-roundtrip, state-proof, abort-event-apply, equivocation-apply, unstake-deregister-apply, cross-shard-receipt-apply, param-change-apply, subsidy-distribution, merge-event-apply, cross-shard-outbound-apply, supply-lifecycle, dapp-state-transition, overflow-paths, state-root-namespaces, multi-tx-block, state-proof-namespaces, applied-receipt-restore, stake-accounting, fee-distribution-edge, equivocation-multi, cross-shard-multi-receipt, multi-block-chain, tx-edge-cases, snapshot-then-apply, genesis-with-region, anon-routing, merge-event-apply-edge, block-event-composition, nef-pool-drain, tx-payload-bounds, empty-block-apply, account-create-on-credit, randomized-delay, block-timestamp, node-registry, tx-replay-protection, chain-save-load, block-validator-basic, genesis-sharded, cross-shard-atomicity, chain-ctor-bootstrap, snapshot-version-rejection surfaces; 1756 total assertions; FAST=1 suite 93/93 PASS in <45s). See `docs/UNIT-TESTS.md` for the comprehensive coverage map + extension guide. Pattern is extensible; each new feature pairs with a `determ test-FEATURE` subcommand. Backbone is the existing `tools/test_*.sh` infrastructure rather than a separate gtest CMake target — pragmatic for an MVP without a CI runner. |
