...
| Partially mitigated | **1** (S-030 — D1 effective-closed + all three D2 pool-fed dimensions consensus-closed via the shipped v2.7 F2; residual is the non-pool `partner_subset_hash` + `timestamp` digest fields) | — | **0** | **2** (S-035 Option 3 path-portability shipped + Option 1 seeded with 136 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes.) | **3** |
...
**Severity:** Critical (consensus integrity) • **Status:** Partially mitigated (D1 effective via S-033; **D2 pool-fed dimensions — inbound / equivocation / abort — consensus-closed via the shipped v2.7 F2** view-reconciliation + digest binding, commits `850d2c3` / `c16d6c3` / `a727cb2` / `48c4b45`; residual = the non-pool `partner_subset_hash` + `timestamp` digest fields) • **Sources:** Architectural Analysis §2.3
...
| **v2.7 F2 view reconciliation** (shipped — pool-fed dimensions) | Consensus-layer prevention | K-of-K signatures cover the inbound / equivocation / abort views directly; divergent views → signatures fail to gather → second instance never finalizes. Non-pool `partner_subset_hash` + `timestamp` remain digest-uncovered. |
...
**Severity:** Operational • **Status:** 🟠 Partially mitigated (Option 3 path portability shipped in-session; Option 1 seeded with 136 in-process `determ test-*` subcommands covering every cryptographic foundation; Option 2 outstanding) • **Sources:** Architectural Analysis §4.1, §4.2
...
**Option 1 seeded in-session (per-feature unit tests).** 136 in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof, plus recent additions state-proof-composite-key, state-proof-value-hash, abort-reselection, subsidy-pool-clamp, wire-negotiation, equivocation-evidence (canonical list: tools/run_all.sh self-counts the suite; src/main.cpp `cmd == "test-..."` dispatch is the registry):
...
**Recommended next.** Option 2 first (DSF, ~3-4 weeks, before Phase A) — gives Phase A through D items deterministic-simulation coverage as they land, eliminates ~3 months of A10 work, satisfies Phase D's DSF prerequisite up-front. Option 1 continues incrementally in parallel (add unit tests for new code as it lands; 136 in-process subcommands already shipped). The committed-to-CI workflow file is intentionally not auto-generated (operator picks the platform + Cron schedule).
...
| S-030 D2 | Consensus-layer closure via v2.7 F2 view reconciliation | ✅ shipped for the three pool-fed dimensions (inbound / equivocation / abort); residual = non-pool `partner_subset_hash` + `timestamp` |
...
**Track B status: 5 of 5 closed** (v2.7 F2 shipped for the three pool-fed S-030 D2 dimensions; the only residual is the small non-pool `partner_subset_hash` + `timestamp` digest-binding item).
...
- Critical findings: 0 fully-open (1 partially mitigated — S-030: D1 effective-closed via S-033; all three D2 pool-fed dimensions [inbound / equivocation / abort] consensus-closed via the **shipped** v2.7 F2 view-reconciliation + digest binding; residual = the non-pool `partner_subset_hash` + `timestamp` digest fields)
...
- Low/Op findings: 0 fully open; 1 partially mitigated (S-035 — Option 3 path-portability shipped in-session via `tools/common.sh`; Option 1 [gtest seed] seeded in-session with 136 in-process `determ test-*` subcommands [2705 assertions; full coverage map in `docs/UNIT-TESTS.md`]; Option 2 [deterministic-simulation framework] outstanding as v1.x quality work, not blocking); 9 closed in-session (incl. S-039 — operational params now bound into the genesis hash, shipped pre-launch as a free wire-compat break); T-001..T-004 are informational `EXTENDED`-mode trade-offs, not bugs
...
- v2.7 F2: **shipped** — view-reconciliation + admitted-set digest binding closed the three pool-fed S-030 D2 dimensions (inbound / equivocation / abort) at the consensus layer (commits `850d2c3` / `c16d6c3` / `a727cb2` / `48c4b45`); residual is the non-pool `partner_subset_hash` + `timestamp` digest fields
...
2. ~~v2.7 F2 implementation per F2-SPEC.md~~ — **shipped in-session** (consensus-layer closure for the three pool-fed S-030 D2 dimensions; commits `850d2c3` / `c16d6c3` / `a727cb2` / `48c4b45`). Residual: bind the two non-pool digest fields `partner_subset_hash` + `timestamp` into `compute_block_digest` (small, separate hardening item).
