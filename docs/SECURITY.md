...
| Partially mitigated | **1** (S-030 — D1 effective-closed + all three D2 pool-fed dimensions consensus-closed via the shipped v2.7 F2; `partner_subset_hash` bound, commit `8585a50`; `timestamp` now bound via median reconciliation, commit `f99eeb8` — S-030-D2 fully digest-closed; the remaining "partial" tag tracks only the apply-layer belt-and-suspenders framing, not an uncovered digest field) | — | **0** | **2** (S-035 Option 3 path-portability shipped + Option 1 seeded with 152 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes.) | **3** |
...
**Severity:** Operational • **Status:** 🟠 Partially mitigated (Option 3 path portability shipped in-session; Option 1 seeded with 152 in-process `determ test-*` subcommands covering every cryptographic foundation; Option 2 outstanding) • **Sources:** Architectural Analysis §4.1, §4.2
...
**Option 1 seeded in-session (per-feature unit tests).** 152 in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof, plus recent additions state-proof-composite-key, state-proof-value-hash, abort-reselection, subsidy-pool-clamp, wire-negotiation, equivocation-evidence (canonical list: tools/run_all.sh self-counts the suite; src/main.cpp `cmd == "test-..."` dispatch is the registry):
...
| # | Option | Cost | Status |
|---|---|---|---|
| 1 | **Add gtest/Catch2** for crypto, serialization, state transitions, validator rules. CMake + Linux CI. | 1-2w to seed + ongoing per-feature. | 🟡 seeded in-session — 152 in-process `determ test-*` subcommands ship under `src/main.cpp` (covering atomic-scope, composable-batch, dapp-register, dapp-call, s018-json-validation, Merkle, committee-selection, shard-routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types, transaction, merge-event-codec, consensus-msgs, tx-root, genesis, envelope, resolve-fork, shamir, random-state, snapshot-defense, encoding, chain-helpers, json-validate, block-roundtrip, config-roundtrip, tx-binary-codec, chain-append, state-types, validator-config, timing-profiles, params-constants, supply-invariant, enum-values, block-accessors, make-block-sig, domain-separation, tx-signing-bytes, merge-event-bytes, make-genesis-block, pending-param-changes, merge-state, chain-apply-block, snapshot-roundtrip, state-proof, abort-event-apply, equivocation-apply, unstake-deregister-apply, cross-shard-receipt-apply, param-change-apply, subsidy-distribution, merge-event-apply, cross-shard-outbound-apply, supply-lifecycle, dapp-state-transition, overflow-paths, state-root-namespaces, multi-tx-block, state-proof-namespaces, applied-receipt-restore, stake-accounting, fee-distribution-edge, equivocation-multi, cross-shard-multi-receipt, cross-shard-supply-invariant, multi-block-chain, tx-edge-cases, snapshot-then-apply, genesis-with-region, anon-routing, merge-event-apply-edge, block-event-composition, nef-pool-drain, tx-payload-bounds, empty-block-apply, account-create-on-credit, randomized-delay, block-timestamp, node-registry, tx-replay-protection, chain-save-load, block-validator-basic, genesis-sharded, cross-shard-atomicity, chain-ctor-bootstrap, snapshot-version-rejection, config-defaults, required-block-sigs, config-load-save, block-from-json-minimal, config-permissive, chain-shard-routing-config surfaces; 2718 total assertions; FAST=1 suite 142/142 PASS in <60s). See `docs/UNIT-TESTS.md` for the comprehensive coverage map + extension guide. Pattern is extensible; each new feature pairs with a `determ test-FEATURE` subcommand. Backbone is the existing `tools/test_*.sh` infrastructure rather than a separate gtest CMake target — pragmatic for an MVP without a CI runner. |
...
**Recommended next.** Option 2 first (DSF, ~3-4 weeks, before Phase A) — gives Phase A through D items deterministic-simulation coverage as they land, eliminates ~3 months of A10 work, satisfies Phase D's DSF prerequisite up-front. Option 1 continues incrementally in parallel (add unit tests for new code as it lands; 152 in-process subcommands already shipped). The committed-to-CI workflow file is intentionally not auto-generated (operator picks the platform + Cron schedule).
...
- Low/Op findings: 0 fully open; 1 partially mitigated (S-035 — Option 3 path-portability shipped in-session via `tools/common.sh`; Option 1 [gtest seed] seeded in-session with 152 in-process `determ test-*` subcommands [2718 assertions; full coverage map in `docs/UNIT-TESTS.md`]; Option 2 [deterministic-simulation framework] outstanding as v1.x quality work, not blocking); 9 closed in-session (incl. S-039 — operational params now bound into the genesis hash, shipped pre-launch as a free wire-compat break); T-001..T-004 are informational `EXTENDED`-mode trade-offs, not bugs
