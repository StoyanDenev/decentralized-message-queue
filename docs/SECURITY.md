...
**A4.4 (SHIPPED `dcb49be`) — sync-path rejoiner.** `request_next_chunk` fetches from `height()-1` so the one-block replacement arrives as a candidate (was `prev_hash mismatch` forever), and `on_chain_response` routes a block at `height()-1` into `maybe_reorg_to_locked`. A `progressed` guard (head advanced OR head-hash changed) stops a Byzantine peer spinning the fetch loop; the pre-revert gate 3b (finding 4) rejects a competitor whose `creator_block_sigs` count mismatches its `creators` before any revert (the validator enforces the same size invariant — sound, and it caps the fail-closed revert churn a padded competitor can force). Gate: `test-node-reorg-s048` scenario 5 (winner delivered via CHAIN_RESPONSE, not gossip) + `test-fa-liveness-virtual` REJOIN 10/10 ("restarted node caught up … real sync path"); FAST 216/0 both platforms; soundness `docs/proofs/BoundedReorgSoundness.md` (REORG-1..7). **Remaining (non-blocking).** A4.5 = crash-consistency hardening for a daemon reorg-during-save (the `persisted_count_` reset already landed in A4.2).
