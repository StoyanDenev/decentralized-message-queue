...
Each runs in <5s with no network, no flakes. Wrappers in `tools/test_*.sh`. `FAST=1 bash tools/run_all.sh` runs only this subset (50/50 PASS in <23s) for fast iteration. The next mechanical extension is to add unit tests for the remaining cryptographic surface — equivocation detection logic (FA6 — needs NodeRegistry fixture), bounded mempool (S-008 — needs partial Node fixture), AbortClaimMsg verification path. Then mid-level invariants (chain.append validation paths, abort-tally semantics, fork resolution).
...
| # | Option | Cost | Status |
|---|---|---|---|
| 1 | **Add gtest/Catch2** for crypto, serialization, state transitions, validator rules. CMake + Linux CI. | 1-2w to seed + ongoing per-feature. | 🟡 seeded in-session — 45 in-process `determ test-*` subcommands ship under `src/main.cpp` (covering Merkle, committee selection, shard routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types, transaction, merge-event-codec, consensus-msgs, tx-root, genesis, envelope, resolve-fork, shamir, random-state, snapshot-defense, encoding, chain-helpers, json-validate, block-roundtrip, config-roundtrip, tx-binary-codec, chain-append, state-types, validator-config, timing-profiles, params-constants, supply-invariant, enum-values, block-accessors, make-block-sig, domain-separation, tx-signing-bytes, merge-event-bytes, make-genesis-block, pending-param-changes, merge-state surfaces; 1035 total assertions; FAST=1 suite 50/50 PASS in <23s). See `docs/UNIT-TESTS.md` for the comprehensive coverage map + extension guide. Pattern is extensible; each new feature pairs with a `determ test-FEATURE` subcommand. Backbone is the existing `tools/test_*.sh` infrastructure rather than a separate gtest CMake target — pragmatic for an MVP without a CI runner. |
