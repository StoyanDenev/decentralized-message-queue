...
| S-030 | 🟠 Partially mitigated | Block body not authenticated by `block_digest` (D1 effective via S-033 state_root; D2 partial via S-033; **inbound dimension fully closed** via v2.7 F2 + inbound-set digest binding, commit `a727cb2`; eq/abort view dimensions remain) | `node/producer.cpp::compute_block_digest` + the S-030 D2 comment block immediately below the function | v2.7 |
...
| 5 | **F2 view reconciliation (v2.7).** Phase-1 commits include hash of each member's evidence/receipt pool views; canonical reconciliation at Phase 1→2; Phase-2 sigs cover the reconciled lists via extended `compute_block_digest`. | 1-2 days implementation + design-decision time for reconciliation rule per field. | **Inbound dimension complete:** commit-bind + block-carry + validator recompute + intersection enforce + inbound-set digest binding shipped (`850d2c3` + `c16d6c3` + `a727cb2`). Remaining: extend the same pattern to the equivocation/abort view dimensions (eq/abort roots are carried + sig-bound but not yet intersection-enforced + digest-bound), and the optional contrib-time V21–V26 passes. |
