...
|---|---|---|---|---|---|
| Open (untouched) | **0** | **1** (S-044) | **1** (S-045) | **0** | **2** |
...
| S-045 | ⛔ OPEN | **Medium (liveness).** BFT escalation structurally unreachable under MULTI-member abort exclusion: needs `avail ≥ k_bft` AND `total_aborts ≥ threshold(5)`, but once `avail < k_use` `check_if_selected` silently returns — no round runs, the counter can never reach the threshold, no abort decay/retry exists. Single-dead-member escalation works (`test_bft_escalation.sh` green); the gap: e.g. M=K=3 with TWO distinct aborted members at one height = permanent halt at total_aborts=2 < 5. | `node.cpp::check_if_selected` (escalation conjunction + silent return) | open |
...
**Candidate fixes (consensus-critical — NOT shipped; need explicit authorization).** (a) Raise the abort-claim quorum floor to `max(2, K−1)` — makes K=2 require both members' claims (degenerate but safe) and leaves K ≥ 3 unchanged; (b) wall-clock decay of `current_aborts_` entries; (c) count failed committee-formation attempts toward `bft_escalation_threshold` (composes with S-045). Each changes consensus behavior and needs its own safety analysis.

---

### S-045 — BFT escalation unreachable under multi-member abort exclusion — ⛔ OPEN (Medium, liveness)

**Mechanism.** `check_if_selected` escalates to a BFT round only when `avail < k_target && bft_enabled && total_aborts >= bft_escalation_threshold && avail >= k_bft`; otherwise, with `avail < k_use`, it **silently returns**. Rounds are what generate abort claims — so once aborts against ≥ (pool − k_bft + 1) DISTINCT members accumulate at one height while `total_aborts` is still below the threshold (default 5), no round can run, the counter is frozen, and there is no abort decay or retry timer: a permanent halt in exactly the small-committee regime escalation was designed to rescue.

**What works vs. what doesn't.** The single-flaky-member case escalates correctly: repeated aborts against the SAME member keep `avail = pool − 1 ≥ k_bft`, the counter climbs to the threshold across rounds, and the BFT round proceeds (proven green by `tools/test_bft_escalation.sh`). The gap is multi-member exclusion: e.g. M=K=3 with TWO distinct aborted members at one height → avail = 1 < k_bft = 2 → permanent halt at `total_aborts = 2 < 5`. The K=2 instance of this gap is subsumed by S-044.

**Candidate fixes (consensus-critical — NOT shipped).** Count failed formation attempts toward the threshold; or relax the `avail ≥ k_bft` precondition with a grace-period fallback; or abort decay (shared with S-044). Needs explicit authorization + safety analysis (any relaxation interacts with the FA5 BFT safety bound).
