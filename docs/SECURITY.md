...
**Severity:** Critical (architectural) • **Status:** Partially mitigated in-session (shared_mutex landed + A9 Phase 1 atomicity landed; full A9 overlay/delta still v2) • **Sources:** Architectural Analysis §3.1
...
**Mitigation landed in-session (partial — atomicity layer, A9 Phase 1).** Wrapped `Chain::apply_transactions` body in a try-catch with a state snapshot taken at entry. On any exception inside the apply body (invariant assertion, malformed-tx arithmetic, supply-conservation violation, state_root mismatch) the snapshot is move-restored before the exception propagates to the caller. The chain is observably unchanged from the failed apply — every observer sees either the full block applied or nothing. Without this, a mid-apply throw left state partially mutated (some accounts debited but stakes not unlocked, abort_records bumped but block not in blocks_, etc.), and the next apply operated on inconsistent data. The snapshot is a deep-copy of all mutable state maps + scalars; cost is dominated by allocator overhead at <1ms per block at 10k accounts. The chain block-vector itself is appended only after apply_transactions returns successfully (see `Chain::append`), so the block-vector atomicity is handled at the caller level. This is the minimum-viable atomicity guarantee; the full A9 Phase 2 (overlay/delta primitive enabling concurrent readers during apply + composable txs) remains v2 work.

**Verified post-fix:** bearer (mutator path), state_root (S-033 hash + apply path), governance_param_change (mutator + activation + snapshot), equivocation_slashing (slashing apply path), snapshot_bootstrap (restore-from-snapshot path) all PASS. Hot-path overhead unmeasurable in test wall-clock.

**Remaining open under S-031** (tracked here, requires v2 work):

- Long-running write operations (apply, `chain.save()` fsync, snapshot persistence) still hold `unique_lock` and block readers during the operation. Closing this requires the full A9 Phase 2 overlay/delta state model — reads proceed against an immutable snapshot while apply mutates an overlay in the background. The Phase 1 atomicity layer is the precursor that defines the rollback semantics Phase 2's overlay-discard-on-abort relies on.
