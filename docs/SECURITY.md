...
|---|---|---|---|---|---|
| Open (untouched) | **0** | **0** | **1** (S-048 — node-local fork-recovery liveness; owner-gated head-reorg wiring) | **0** | **1** |
...
| S-048 | ⛔ Open (owner-gated) | **Medium (node-local liveness).** The abort-vs-finalize race can produce two VALIDLY-SIGNED same-height blocks (different abort generations → different committees). `Chain::resolve_fork` (S-029) defines the deterministic winner but is NOT wired into `Node`, and sync is append-only (`request_next_chunk` asks from own height; `on_chain_response` cannot replace a divergent tail) — a node that finalized the minority block is stuck on its fork permanently (`invalid block: prev_hash mismatch` spam), and a validator that died holding a minority tail cannot rejoin from its saved chain. CLUSTER liveness holds (majority kept finalizing, +134 blocks in the reproduction, while the forked node sat still); recovery today is operational (resync from snapshot / fresh data_dir). Fix = wire `resolve_fork` + a bounded head-reorg into the sync/gossip path — consensus architecture, requires owner sign-off. `test-fa-liveness-virtual` detects the mode and prints a `KNOWN-OPEN S-048` marker. | `chain.cpp::resolve_fork` (unwired), `node.cpp::on_chain_response` / `request_next_chunk` (append-only); harness marker in `test-fa-liveness-virtual` | owner |
...
### S-048 — minority same-height fork cannot reorg (resolve_fork unwired; append-only sync) — ⛔ OPEN (owner-gated)

**Mechanism.** The abort-vs-finalize race can produce two validly-signed blocks at the SAME height: one part of the committee completes phase 2 on generation g while another forms an abort quorum and re-runs the round at generation g+1 — both blocks carry legitimate committee signatures for their respective abort generations, and the validator accepts either. `Chain::resolve_fork` (S-029) defines the deterministic preference (heaviest sig set → fewer aborts → smallest hash) and is unit-tested — but NO Node path calls it: a node that already applied the minority block drops the majority block (old index), and the sync path is append-only (`request_next_chunk` from own height; incoming majority blocks fail `prev_hash` validation against the divergent tail forever). A validator that dies holding a minority tail hits the same wall on restart from its saved chain.

**Impact.** Node-local liveness only: the CLUSTER keeps finalizing (majority observed +134 blocks while the forked node sat still), safety is unaffected (the stuck node halts rather than double-applying), but the forked node serves its stale fork over RPC until an operator resyncs it (snapshot or fresh data_dir). Frequency in the failover harness: roughly 1 in 12 kill-recovery cycles.

**Reachable by ordinary link loss, not just a deliberate partition (`test-fa-partition-virtual`, `docs/proofs/AdversarialTransportHarness.md`).** The new adversarial-network harness confirmed that plain link loss — not node death, not a partition — reaches this fork: sustained loss induces the timing skew that drives the abort-vs-finalize race, and its loss diagnostic OBSERVES two holders disagreeing on a same-height block. The S-047 retry re-delivers the round messages but cannot **reorg** the resulting fork, so under loss the cluster can even fragment below quorum on every branch. This raises S-048 from a partition-edge curiosity to **the load-bearing open item for adversarial-network liveness**: a reliable loss-liveness gate is blocked on this fix. (The harness's partition-SAFETY gate stays clean because the majority's internal links are loss-free — no skew, no fork among the quorum.)

**Why open.** The fix — wiring `resolve_fork` plus a bounded one-block head-reorg (state rollback + replacement) into the gossip/sync path — changes consensus-visible node behavior and adds a rollback primitive the chain layer deliberately lacks; that is an architecture decision requiring owner sign-off, not a patch. Until then `test-fa-liveness-virtual` classifies the mode precisely and prints a `KNOWN-OPEN S-048` marker (both for a stuck survivor and for a rejoiner blocked by its saved minority tail) so the defect stays visible in every FAST run that hits it without flaking the gate.
