...
| Partially mitigated | **1** (S-030 — D1 effective-closed + all three D2 pool-fed dimensions consensus-closed via the shipped v2.7 F2; residual is the non-pool `partner_subset_hash` + `timestamp` digest fields) | — | **0** | **2** (S-035 Option 3 path-portability shipped + Option 1 seeded with 152 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes.) | **3** |
