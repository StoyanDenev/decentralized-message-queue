...
| Partially mitigated | **1** (S-030 — D1 effective-closed + all three D2 pool-fed dimensions consensus-closed via the shipped v2.7 F2; residual is the non-pool `timestamp` digest field — `partner_subset_hash` now bound, commit `8585a50`) | — | **0** | **2** (S-035 Option 3 path-portability shipped + Option 1 seeded with 152 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes.) | **3** |
...
**Residual (genuinely outstanding).** Per the authoritative per-field treatment in `docs/proofs/S030-D2-Analysis.md` (§1 field table + §summary), the remaining ✗-row fields fall in three buckets. (1) **Bound elsewhere:** `state_root` is bound via `block_hash`/signing_bytes through S-033 + S-038 (not via the digest). (2) **Already deterministically pinned:** `cross_shard_receipts` is derived from the committee tx set, which `tx_root` + `creator_tx_lists` already bind — so it is not an independent divergence vector despite not appearing in the digest directly. (3) **Intentionally digest-excluded as Phase-2-reveal material:** `delay_output` (per the rev.9 S-009 closure comment at `producer.cpp:555`), `creator_dh_secrets`, and `cumulative_rand` (which derives from `delay_output`). `partner_subset_hash` was bound into `compute_block_digest` in commit `8585a50` — it is deterministic from merge state (§3.2), so it needed no view-reconciliation, distinct from the pool-fed F2 dimensions. That leaves exactly **one genuinely-uncovered residual field — `timestamp`** — as the remaining structural step toward the literal "≤ 1 block instance per height" property. (`timestamp` specifically cannot simply be hashed in: honest members' clocks differ within the validator's `±30s` window, so binding it requires the assembler to propose a value at the Phase 1→2 transition that others verify before signing — handled together with the same view-reconciliation machinery; see `docs/proofs/S030-D2-Analysis.md` §5.) Full design in `docs/proofs/F2-SPEC.md`.
...
| `determ test-block-digest` | `compute_block_digest` (FA1 Phase-2 signature target) — INCLUSION contract (every digested field changes the digest) + EXCLUSION contract on the non-F2 path (Phase-2-reveal + the sole genuine S-030-D2 residual `timestamp` MUST NOT change the digest) + F2 POSITIVE BINDING (the shipped v2.7 F2 binds inbound / equivocation / abort sets on an F2 block — strip/add detected) + partner_subset_hash binding (bound when non-zero, `8585a50`; v1 digest when zero). Fences the digest at exactly the surface FA1 / S-030 D2 / v2.7 F2 assume | FA1 (signature target) | 26 |
...
| S-030 D2 | Consensus-layer closure via v2.7 F2 view reconciliation | ✅ shipped for the three pool-fed dimensions (inbound / equivocation / abort) + `partner_subset_hash` bound directly (`8585a50`); residual = non-pool `timestamp` (§5 non-fix) |
...
**Track B status: 5 of 5 closed** (v2.7 F2 shipped for the three pool-fed S-030 D2 dimensions + `partner_subset_hash` bound directly via `8585a50`; the only residual is the small non-pool `timestamp` digest-binding item, the §5 non-fix).
...
- Critical findings: 0 fully-open (1 partially mitigated — S-030: D1 effective-closed via S-033; all three D2 pool-fed dimensions [inbound / equivocation / abort] consensus-closed via the **shipped** v2.7 F2 view-reconciliation + digest binding, and `partner_subset_hash` bound directly via `8585a50`; residual = the non-pool `timestamp` digest field, the §5 non-fix)
...
- v2.7 F2: **shipped** — view-reconciliation + admitted-set digest binding closed the three pool-fed S-030 D2 dimensions (inbound / equivocation / abort) at the consensus layer (commits `850d2c3` / `c16d6c3` / `a727cb2` / `48c4b45`); `partner_subset_hash` bound directly via `8585a50`; residual is the non-pool `timestamp` digest field
...
2. ~~v2.7 F2 implementation per F2-SPEC.md~~ — **shipped in-session** (consensus-layer closure for the three pool-fed S-030 D2 dimensions; commits `850d2c3` / `c16d6c3` / `a727cb2` / `48c4b45`). Residual: bind the non-pool field `timestamp` into `compute_block_digest` (`partner_subset_hash` done in commit `8585a50`; small, separate hardening item — `timestamp` is the §5 non-fix needing the assembler-proposed/±30s reconciliation).
