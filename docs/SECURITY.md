...
| Partially mitigated | **1** (S-030) | — | **1** (S-016 Option 2 shipped + v2.7 F2 closes Option 1) | **3** (S-035 Option 3 path-portability shipped + Option 1 seeded with 136 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes. S-039 genesis-hash diagnostic-UX gap discovered + lock-in test shipped; fix deferred as wire-compat-break) | **5** |
...
| S-039 | 🟠 Partially mitigated (lock-in test shipped; fix deferred) | `compute_genesis_hash` does NOT bind operational params (m_creators, k_block_sigs, block_subsidy, subsidy_pool_initial, subsidy_mode, min_stake, initial_shard_count, bft_enabled, bft_escalation_threshold, epoch_blocks, shard_address_salt). Two operators running the same chain_id with different m_creators get matching genesis_hash but cryptic consensus failures (different K-committees → signature gathering never converges). Discovered while writing `tools/test_genesis.sh`. Fix is mechanical (~10 LOC in `make_genesis_block`) but is a wire-compat break — needs coordinated migration with wire-version flag + flag-day height. `tools/test_genesis.sh` locks in the CURRENT no-effect behavior so an accidental partial fix is caught. Operationally not a security regression (chain stalls loudly when configs drift; no silent state divergence), but a real operator-UX gap. | `chain/genesis.cpp::make_genesis_block` | v2.X coordinated migration |
...
### S-039 — `compute_genesis_hash` does not bind operational params — 🟠 Diagnostic-UX gap (Low/Op)

**Severity:** Operational (Low) • **Status:** 🟠 Registered, lock-in test shipped, fix deferred (wire-compat-break) • **Source:** Discovered while writing `tools/test_genesis.sh` (S-035 Option 1 seed)
...
**Why the fix is deferred (wire-compat-break).** Adding any of these to `compute_genesis_hash` would change every existing chain's `genesis_hash`, requiring coordinated migration (operators re-pinning their config's `genesis_hash` field). The fix is mechanical (~10 LOC in `src/chain/genesis.cpp::make_genesis_block`) but the migration is the hard part. Tracked as forward-dev item.

**Mitigation in place.** `tools/test_genesis.sh` (S-035 Option 1 seed) locks in the CURRENT no-effect behavior with explicit `expect == base_hash` assertions for each of these fields — if a future commit silently moves any of them into the hash, the test fails immediately. This prevents the gap from getting worse and gates the eventual fix on a coordinated commit that updates both the hashing and the test.

**Tracking.** Forward-dev candidate: bind missing fields, with a wire-version flag + flag-day height to coordinate migration. Estimated 2-3 days (small implementation, large coordination overhead).
...
- Low/Op findings: 0 fully open; 2 partially mitigated (S-035 — Option 3 path-portability shipped in-session via `tools/common.sh`; Option 1 [gtest seed] seeded in-session with 136 in-process `determ test-*` subcommands [2699 assertions; full coverage map in `docs/UNIT-TESTS.md`]; Option 2 [deterministic-simulation framework] outstanding as v1.x quality work, not blocking. S-039 genesis-hash diagnostic-UX gap discovered + lock-in test shipped in-session; fix is wire-compat break, deferred to coordinated migration); 8 closed in-session; T-001..T-004 are informational `EXTENDED`-mode trade-offs, not bugs
