...
| `determ test-genesis` | `compute_genesis_hash` + `make_genesis_block` — chain-identity origin. Locks in: chain_id / shard_id / chain_role / suspension_slash / merge_threshold_blocks / genesis_message / committee_region ARE bound; m_creators / k_block_sigs / block_subsidy / min_stake / initial_shard_count / bft_enabled ALSO bound (S-039 CLOSED — operational params are now part of chain identity). make_genesis_block invariants (index 0, prev_hash zero). JSON round-trip preserves identity hash. Oversized genesis_message rejected. | chain identity + S-039 | 22 |
...
**Tests flipped.** The lock-in assertions that pinned the old no-effect behaviour were inverted to assert the new binding: `determ test-genesis-determinism` Scenario 7 (4 assertions — `m_creators` / `block_subsidy`+`subsidy_pool_initial` / `bft_enabled`+`bft_escalation_threshold`+`epoch_blocks` / `shard_address_salt`+`initial_shard_count` now each CHANGE the hash) and `determ test-genesis-sharded` (`initial_shard_count` now bound). FAST=1 green at 142/142.
