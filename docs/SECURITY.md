...
| S-035 | 🟠 Partially mitigated | No unit tests, no CI, no deterministic simulation framework — Option 3 path-portability shipped (all 68 tests source `tools/common.sh` + use `$PROJECT_ROOT` / `$DETERM`; portable to Linux/Mac builds); Option 1 (gtest/Catch2 unit tests) seeded with 14 in-process `determ test-*` subcommands covering Merkle, committee selection, shard routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest, block-hash, binary-codec, wire-types surfaces (231 assertions; see `docs/UNIT-TESTS.md`); Option 2 (deterministic-simulation framework) outstanding | `tools/common.sh` + `tools/test_*.sh` (path layer); `determ test-*` subcommands in `src/main.cpp` (Option 1 seeds); `docs/UNIT-TESTS.md` (coverage map); CI workflow file is operator policy | Option 1 continuing per-feature; Option 2: 3-4w |
...
**Option 1 seeded in-session (per-feature unit tests).** Fourteen in-process `determ test-*` subcommands added to `src/main.cpp` cover the cryptographic foundations under every FA-track safety proof:
...
Each runs in <5s with no network, no flakes. Wrappers in `tools/test_*.sh`. `FAST=1 bash tools/run_all.sh` runs only this subset (19/19 PASS in <12s) for fast iteration. The next mechanical extension is to add unit tests for the remaining cryptographic surface — equivocation detection logic (FA6 — needs NodeRegistry fixture), bounded mempool (S-008 — needs partial Node fixture), AbortClaimMsg verification (FA3), genesis loader. Then mid-level invariants (chain.append validation paths, abort-tally semantics, fork resolution).
...
| # | Option | Cost | Status |
|---|---|---|---|
| 1 | **Add gtest/Catch2** for crypto, serialization, state transitions, validator rules. CMake + Linux CI. | 1-2w to seed + ongoing per-feature. | 🟡 seeded in-session — 14 in-process `determ test-*` subcommands ship under `src/main.cpp` (covering Merkle, committee selection, shard routing, Ed25519, SHA-256, anon-address, genesis-message, state-root, block-rand, rate-limiter, block-digest [FA1 Phase-2 sig target + S-030 D2 EXCLUSION fence], block-hash [FA1 chain-anchor identity], binary-codec [A3 / S8 wire format + S-022 cap table], wire-types [CrossShardReceipt / AbortEvent / EquivocationEvent / GenesisAlloc round-trip + S-018 strict-rejection] surfaces; 231 total assertions; FAST=1 suite 19/19 PASS in <12s). See `docs/UNIT-TESTS.md` for the comprehensive coverage map + extension guide. Pattern is extensible; each new feature pairs with a `determ test-FEATURE` subcommand. Backbone is the existing `tools/test_*.sh` infrastructure rather than a separate gtest CMake target — pragmatic for an MVP without a CI runner. |
...
- Low/Op findings: 0 fully open; 1 partially mitigated (S-035 — Option 3 path-portability shipped in-session via `tools/common.sh`; Option 1 [gtest seed] seeded in-session with 14 in-process `determ test-*` subcommands [231 assertions over Merkle / committee selection / shard routing / Ed25519 / SHA-256 / anon-address / genesis-message / state-root / block-rand / rate-limiter / block-digest / block-hash / binary-codec / wire-types; coverage map in `docs/UNIT-TESTS.md`]; Option 2 [deterministic-simulation framework] outstanding as v1.x quality work, not blocking); 8 closed in-session; T-001..T-004 are informational `EXTENDED`-mode trade-offs, not bugs
