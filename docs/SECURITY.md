...
| `determ test-genesis-sharded` | Genesis variants for sharded deployments. 14 assertions across 8 scenarios: chain_role sensitivity (SINGLE/BEACON/SHARD pairwise distinct hashes); shard_id sensitivity (shards 0/1/2 distinct); JSON round-trip preserves chain_role + shard_id + initial_shard_count; BEACON with non-zero shard_id distinct from BEACON 0; S-039 gap lock-in (initial_shard_count NOT bound — wire-compat-break to fix); SHARD make_genesis_block constructs cleanly; Chain bootstraps from SHARD genesis. Defends against chain-identity collision in multi-shard deployments. | chain identity / sharded deployments | 14 |
| `determ test-cross-shard-atomicity` | Cross-shard atomicity at chain-pair level. 10 assertions across the dual-chain flow: source shard S0 outbound debit; destination shard S1 inbound credit; cross-shard A1 atomicity (src_outbound == dst_inbound); per-shard A1 invariant; net supply across the pair conserved; determinism. Pins that the two single-Chain tests COMPOSE — defends against drift where the two sides' counters could move by different amounts (silent value loss / creation across shard boundary). | rev.9 B3 / cross-shard / FA7 | 10 |
| `determ test-chain-ctor-bootstrap` | Chain constructor bootstrap paths. 15 assertions across 7 scenarios: Chain() default is empty; head() on empty throws "Empty chain"; at(0) on empty throws out_of_range-style; Chain(genesis) bootstraps to height=1, head matches genesis; both bootstrap paths (append + ctor) produce equivalent state; both support continued append. Defends against ctor-bootstrap drift that would let two equivalent-on-paper bootstrap paths produce different chain identity. | Chain ctor / bootstrap | 15 |
