...
|---|---|---|---|---|---|
| Open (untouched) | **0** | **2** (S-044, S-045 — S-045 upgraded Medium→High per AbortCascadeLiveness.md T-4) | **0** | **0** | **2** |
...
| Mitigated in-session | **6** (S-001, S-002, S-003, S-004, S-031, S-043) | **14** (S-006, S-007, S-008, S-010, S-011, S-012, S-013, S-014, S-017, S-020, S-032, S-033, S-038, S-042) | **2** (S-016, S-018) | **11** (S-021, S-022, S-024, S-026, S-027, S-028, S-029, S-037, S-041, S-046, F-7) | **33** |
...
| S-044 | ⛔ OPEN | **High (liveness).** K=2 committees wedge permanently under ordinary timing skew: abort-claim quorum at K=2 is K−1=1 (one claim excludes a member); `aborts_gen` divergence drops contribs between peers and cascades further single-claim aborts until pool < K; aborts clear only on block accept (`node.cpp:1856`); BFT escalation unreachable (k_bft=⌈2K/3⌉=2=K). Reproduced live ×3 (K=2-of-3, K=2-of-4) at 2000 ms timers. The shipped `web` profile (init DEFAULT) is M=3/K=2 — exposed. Interim: deploy K ≥ 3. Candidate fixes are consensus-critical (quorum floor ≥ 2 / abort decay / formation-failure escalation counting) — need authorization. | `node.cpp::on_abort_claim` quorum (K−1), `check_if_selected` (escalation gates), `current_aborts_` lifecycle; live evidence in `tools/test_web_hybrid.sh` + `test_regional_shards.sh` S-044 known-bug notes | open |
| S-045 | ⛔ OPEN | **High (liveness; upgraded from Medium per FB67 T-4/T-2.1).** BFT escalation structurally unreachable at the DEFAULT threshold (5) on every shipped profile's nominal pool: rounds generate abort events, `check_if_selected` silently returns once `avail < k_use`, so the counter ceiling per stuck height = what the last runnable round produced (K−1 events for one dead member at M=K). M=K=3 + ONE dead member → frozen at ≤ 2 < 5 forever. Pure-distinct climb needs N ≥ θ + k_bft (no shipped nominal pool qualifies). `test_bft_escalation.sh` green only via genesis-pinned θ=1. Interim: pin θ ≤ K−1 or grow pool ≥ θ + k_bft. | `node.cpp::check_if_selected` (escalation conjunction + silent return); analysis `docs/proofs/AbortCascadeLiveness.md` (FB67) | open |
...
**Mechanism.** At K=2 the abort-claim quorum is K−1 = 1 (`Node::on_abort_event` validates a "K−1 claim quorum"), so a SINGLE phase-1/phase-2 straggle by either committee member produces an immediate abort quorum against it — no corroboration. Each abort event transiently desyncs `aborts_gen` across peers (contribs whose `aborts_gen` doesn't match the local `current_aborts_.size()` are dropped, `node.cpp:2113-2116`), so the next round's selected member often appears silent to its peers → another single-claim abort. `current_aborts_` clears ONLY on block accept (`node.cpp:1856`) — there is no decay, no expiry, no retry — so exclusions accumulate at the height until the eligible pool falls below K, at which point `check_if_selected` silently returns forever. BFT escalation cannot rescue K=2: k_bft = ⌈2K/3⌉ = 2 = K (no committee shrink exists), and the avail ≥ k_bft precondition fails once the pool is down to 1.

**Evidence (live, 2026-06-11).** Three independent cluster tests at 2000 ms test timers (generous; the green M=K=3 siblings mint 50+ blocks under them): `test_weak_3node` in its K=2-of-3 AND K=2-of-4 forms (logs: block #1 accepted → single-claim quorums against node4, node2, node3 in successive rounds → silence), `test_web_hybrid` (K=2-of-3, wedged at height 3-4), `test_regional_shards` (K=2-of-3, blocks 1-2 finalize with correct region-filtered committees → cascade → wedge at height 3). Hybrid selection and finalization are themselves CORRECT — every pre-cascade block carries exactly K=2 creators with valid sigs; the defect is purely sustained liveness.

**Exposure.** The shipped `web` profile — the `determ init` DEFAULT — is M=3/K=2. Any deployment on it halts permanently after the first cascade (minutes-to-hours under realistic skew). `web_test` inherits the posture.

**Interim guidance (operators).** Deploy K ≥ 3 (abort quorum becomes ≥ 2 claims — a single straggle excludes nobody; K=3-of-4 / K=3-of-5 run green under identical conditions, including kill-one-creator recovery per `test_hybrid_liveness.sh`). The remediated cluster tests pin this: postures with K=2 intent (`test_web_hybrid`, `test_regional_shards`) assert boot/role/selection and carry a loud `KNOWN-BUG S-044` note in place of the sustained-production bar — restore those bars when this closes.

**Candidate fixes (consensus-critical — NOT shipped; need explicit authorization).** (a) Raise the abort-claim quorum floor to `max(2, K−1)` — a no-op for K ≥ 3; at K=2 the 2-claim quorum is unsatisfiable by a single survivor, so it trades wedge-by-cascade for halt-by-single-death (analyzed and argued preferable in FB67 §4); (b) wall-clock decay of `current_aborts_` entries; (c) count failed committee-formation attempts toward `bft_escalation_threshold` (composes with S-045). Each changes consensus behavior and needs its own safety analysis — `docs/proofs/AbortCascadeLiveness.md` §4 carries the per-fix derivations against FA1/FA4/FA5/FA6.

**Decision-support artifacts (written 2026-06-11).** `docs/proofs/AbortCascadeLiveness.md` (FB67 — incl. **T-3**: at K=2 the escalation gates `avail < 2 ∧ avail ≥ 2` are *unsatisfiable* — the BFT arm is dead code at any threshold, sharper than the k_bft=K framing above); `docs/proofs/tla/AbortEscalation.tla` + `.cfg` (state machine with the candidate fixes toggleable; the shipped `.cfg` is the web-profile S-044 exhibit; model-check pending); `tools/operator_liveness_posture.sh` (offline exposure audit); `tools/test_s044_gate_surface.sh` (static source-shape guard: red on any escalation-gate change, forcing guard + SECURITY.md reconciliation).

---

### S-045 — BFT escalation unreachable at the default threshold (counter freeze) — ⛔ OPEN (High, liveness; upgraded from Medium)

**Mechanism.** `check_if_selected` escalates to a BFT round only when `avail < k_target && bft_enabled && total_aborts >= bft_escalation_threshold && avail >= k_bft`; otherwise, with `avail < k_use`, it **silently returns**. Rounds are what generate abort events — so the counter's ceiling at a stuck height is whatever the LAST runnable round produced, and there is no abort decay or retry timer: once that ceiling is below the threshold, escalation is frozen out permanently.

**Why High (the T-4/T-2.1 derivation, `docs/proofs/AbortCascadeLiveness.md` FB67).** Initially registered Medium on the belief that the single-dead-member case escalates correctly. The formal derivation shows it does NOT at defaults: at M=K, a single dead member's stuck round yields at most K−1 abort events (one per survivor), so with the genesis-default `bft_escalation_threshold = 5`, M=K=3 (the `cluster` and `tactical` profiles) freezes at `total_aborts ≤ 2 < 5` — **the common one-dead-node failure permanently halts M=K profiles at default settings.** Pure-distinct-member accumulation requires pool N ≥ θ + k_bft (T-2.1), which NO shipped profile's nominal pool satisfies (`global` is the closest: N=7 vs required 9). `tools/test_bft_escalation.sh` is green only because its genesis pins `θ=1` (its formerly-stale header claimed θ=2 — fixed; registered as FB67 finding F-1): it proves the escalation *machinery* works when the gates are satisfiable, not that the gates are satisfiable at defaults. The K=2 instance is subsumed by S-044 (there the gates are unsatisfiable at ANY threshold).

**Interim guidance (operators).** Deployments relying on BFT escalation must genesis-pin `bft_escalation_threshold ≤ K−1` (the per-stuck-height event ceiling) or run a registered pool ≥ θ + k_bft. `tools/operator_liveness_posture.sh` audits a genesis/config pair for this exposure offline.

**Candidate fixes (consensus-critical — NOT shipped).** Count failed formation attempts toward the threshold (restores reachability in the frozen case); or relax the `avail ≥ k_bft` precondition with a grace-period fallback; or abort decay (shared with S-044). Needs explicit authorization + safety analysis (any relaxation interacts with the FA5 BFT safety bound) — see `docs/proofs/AbortCascadeLiveness.md` §4 for the per-fix analysis against FA1/FA4/FA5/FA6.

**Decision-support artifacts (written 2026-06-11).** `docs/proofs/AbortCascadeLiveness.md` (FB67 — T-1..T-5 derivations + per-fix safety analysis); `docs/proofs/tla/AbortEscalation.tla` + `.cfg` (the round/abort/escalation state machine with all three candidate fixes toggleable via CONSTANTS; model-check pending); `tools/operator_liveness_posture.sh` (offline exposure audit; exit 2 = S-044 CRITICAL, 1 = S-045 WARNING, 0 = OK); `tools/test_s044_gate_surface.sh` (static guard pinning the OPEN-defect source shape — a consensus fix MUST turn it red, forcing guard + SECURITY.md reconciliation together).
