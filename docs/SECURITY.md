...
| Partially mitigated | **1** (S-030) | — | **1** (S-016 Option 2 shipped + v2.7 F2 closes Option 1) | **2** (S-035 Option 3 path-portability shipped + Option 1 seeded with 136 in-process unit tests; Options 1 continuing / 2 outstanding. S-036 EXTENDED-mode-only; v2.11 closes.) | **4** |
| Mitigated in-session | **5** (S-001, S-002, S-003, S-004, S-031) | **13** (S-006, S-007, S-008, S-010, S-011, S-012, S-013, S-014, S-017, S-020, S-032, S-033, S-038) | **1** (S-018) | **9** (S-021, S-022, S-024, S-026, S-027, S-028, S-029, S-037, S-041) | **28** |
...
**Status.** Option 4 (S-033) shipped — partial D2 closure with apply-layer enforcement. Option 5 (v2.7) in progress — F2 foundation (`compute_view_root` / `reconcile_union` / `reconcile_intersection`), extended `ContribMsg` with view-roots + view-lists, extended `make_contrib_commitment` with all-zero-short-circuit + `DTM-F2-v1` domain separator, and validator-side V21..V26 helpers (`validate_contrib_view_roots` / `derive_canonical_view_lists` / `validate_view_reconciliation`) all landed as pure unit-testable primitives. Remaining sub-steps: producer-side population at Phase-1 commit (sub-step 2), validator wire-in (sub-step 3 completion), 3-node integration test (sub-step 5), and doc threading (sub-step 6). Options 1-3 superseded by Option 4's broader coverage. See `docs/proofs/F2-V210-IMPLEMENTATION-PLAN.md` for status of remaining sub-steps.
