...
> 2. **Decentralized identity provider** — DSSO via DLT-B composition: t-of-n T-OPRF on the shipped P-256 RFC 9497 VOPRF stack (joint password evaluation, per-share DLEQ-verified; no operator — indeed no < t coalition — learns the password; any t of n responses suffice, in any order) + block-anchored DAPP_CALL assertion (the K-of-K block signature attests inclusion; assertion truth is anchored to the user's credential signature). Uses only primitives already shipped — Ed25519, P-256 (§3.9b), DAPP_CALL, light-client verifier. See `docs/proofs/v2.25-DSSO-DAPP-SPEC.md` + `docs/proofs/FROST_DEVIATION_NOTICE.md`.
...
**Distributed identity provider (DSSO).** The K-of-K committee is structurally a mutual-distrust operator group — a natural fit for distributed-IdP designs in the literature. v2.25 + v2.26 (V2-DESIGN.md Theme 9) specify a "Sign-In With Determ" substrate using **T-OPAQUE** (threshold-OPAQUE, replacing the original SRP in PAKE-as-black-box framework designs) over the committee, paired with on-chain key rotation. RPs register via the existing v2.18 DAPP_REGISTER channel; challenges and signed assertions ride v2.19 DAPP_CALL. As reframed (DLT-B — the t-of-n P-256 T-OPRF composition; `docs/proofs/v2.25-DSSO-DAPP-SPEC.md`, `DECISION-LOG.md 2026-07-15`, `docs/proofs/FROST_DEVIATION_NOTICE.md` §10), the ceremony uses only already-shipped primitives (Ed25519, the P-256 RFC 9497 VOPRF stack §3.9b, DAPP_CALL, plus the chain's own K-of-K block signature as the inclusion attestation — assertion truth is anchored to the user's credential signature) and no longer depends on v2.10 or FROST (v2.10's block-beacon is de-scoped; Determ has no BLS infrastructure). See `docs/proofs/v2.25-DSSO-DAPP-SPEC.md` for the full mechanism; `docs/V2-DESIGN.md` Theme 9 retains the architectural narrative.
