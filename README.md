...
- **Full anonymity / graph privacy** (mixnets, stealth addresses, sender/receiver hiding). The anonymous bearer-wallet account model is the *identity*-privacy story. *Amount*-privacy confidential transactions — Bulletproofs range proofs + the **SHIELD** transparent→confidential on-ramp (CRYPTO-C99-SPEC §3.22a; first shielded-pool consensus op, MODERN profiles) — are an owner-authorized, in-flight feature, but they hide **amounts only**, never the sender/receiver/transaction graph; no broader anonymity layer is planned.
