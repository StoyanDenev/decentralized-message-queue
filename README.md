...
> 2. **Decentralized identity provider** — DSSO via DLT-A composition: T-OPRF over X25519 threshold DH (joint password evaluation; no operator learns the password) + block-anchored DAPP_CALL assertion (the chain's K-of-K block signature IS the threshold attestation). Uses only primitives already shipped — Ed25519, X25519, DAPP_CALL, light-client verifier. See `docs/proofs/FROST_DEVIATION_NOTICE.md`.
...
**Distributed identity provider (DSSO).** The K-of-K committee is structurally a mutual-distrust operator group — a natural fit for distributed-IdP designs in the literature. v2.25 + v2.26 (V2-DESIGN.md Theme 9) specify a "Sign-In With Determ" substrate using **T-OPAQUE** (threshold-OPAQUE, replacing the original SRP in PAKE-as-black-box framework designs) over the committee, paired with on-chain key rotation. RPs register via the existing v2.18 DAPP_REGISTER channel; challenges and signed assertions ride v2.19 DAPP_CALL. As reframed (DLT-A — the X25519 T-OPRF composition; `DECISION-LOG.md 2026-07-07`, `docs/proofs/FROST_DEVIATION_NOTICE.md`), the ceremony uses only already-shipped primitives (Ed25519, X25519, DAPP_CALL, plus the chain's own K-of-K block signature as the threshold attestation) and no longer depends on v2.10 or FROST (v2.10's block-beacon is de-scoped; Determ has no BLS infrastructure). See `docs/V2-DESIGN.md` Theme 9 for the full architecture.
