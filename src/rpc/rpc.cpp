...
json RpcServer::dispatch(const json& req) {
    ...
    // NC-8 wiring inc.3: read-only enote scan over a height range (wallet
    // pull-and-trial-decrypt of CONFIDENTIAL_TRANSFER delivery ciphertexts).
    if (method == "scan_enotes")
        return node_.rpc_scan_enotes(params);
