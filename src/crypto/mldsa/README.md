...
§3.16. Status: **increments 1-6 of the owner-authorized on-chain post-quantum
...
ring operations, and the matrix/vector layer (ExpandA/ExpandS/ExpandMask +
polyvec arithmetic + the NTT-domain matrix·vector product), all shared by every
ML-DSA parameter set. Public domain, C99, seven translation units plus a
...
**Scope (increments 1-6).** The ring's reduction + NTT, the coefficient rounding
...
add/sub/reduce/caddq + pointwise-Montgomery multiply), and the matrix/vector layer
(`polyvec.c`: ExpandA/ExpandS/ExpandMask + polyvec arithmetic + the NTT-domain
matrix·vector product). **Not yet here:** the keygen/sign/verify top level (the
Fiat-Shamir rejection loop that assembles these pieces), the three parameter sets'
full assembly, and the key/signature serialization. Those are later increments.
...
- **Later PQ increments (owner-authorized):** keygen/sign/verify (the Fiat-Shamir
  rejection loop that assembles the matrix/vector layer + rounding + samplers +
  packing) + the three parameter sets' full assembly + the key/signature
  serialization, gated by FIPS 204 / ACVP KATs (the first AUTHORITATIVE end-to-end
  pin of the sampler / pack value mappings AND the seed-expansion byte layout) —
  and only then the chain-integration + anon-address-format reopening, each
  separately reviewed.
